...
module.exports = {
  ...
  settings: {
    policy_group_admins: {
      name: "Global Admins.",
      description:
        "Comma separated list of all admins with permissions to use all resources to ignore for this plugin.",
      regex: "^.{1,255}$",
  ...
  run: function (cache, settings, callback) {
        ...
        policy.statements.forEach((statement) => {
          ...
          if (!statementPasses) {
            ...
            helpers.addResult(
              results,
              2,
              `${statementObj["subjectType"]}${statementObj["subject"]} has the ability to ${statementObj["verb"]} all resources in ${statementObj["location"]}`,
              region,
              policy.id
            );
          }
    ...
    if (policyProtection && entered) {
      helpers.addResult(
        results,
        0,
        "All policies follow least access.",
        region
      );
    }
