...
const authTokens = [
  {
    ...
    compartmentId:
      "ocid1.tenancy.oc1..aaaaaaaaip3imsyctt3vhme6rnemweaxe4b6q2gcodq",
    description: "For CIS",
    timeCreated: new Date(),
    userId:
      "ocid1.user.oc1..aaaaaaaaddzaosqxqgeryy43emaptcnzzcrekfkxmha6u5cyvqjkmvo7kdxa",
...
describe("userAuthTokenRotated", function () {
  describe("run", function () {
    ...
    it("should give failing result Auth token has not been rotated within set days limit", function (done) {
      ...
      const cache = createCache([authTokens[1]]);
    ...
    it("should give passing result Auth token has been rotated within set days limit", function (done) {
      ...
      const cache = createCache([authTokens[0]]);
    ...
    it("should give unknown result if unable to query user auth tokens", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Unable to query user auth tokens"
        );
        expect(results[0].region).to.equal("global");
        done();
      };
