...
module.exports = {
  ...
  settings: {
    customer_secret_key_rotated_fail: {
      ...
      description:
        "Return a failing result when customer secret keys exceed this number of days without being rotated",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: "90",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      customer_secret_key_rotated_fail: parseInt(
        settings.customer_secret_key_rotated_fail ||
          this.settings.customer_secret_key_rotated_fail.default
      ),
    };
    ...
    if (customerSecretKeys.err || !customerSecretKeys.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query user customer secret keys: " +
          helpers.addError(customerSecretKeys),
        "global"
      );
      return callback(null, results, source);
    }
    ...
    if (!customerSecretKeys.data.length) {
      helpers.addResult(
        results,
        0,
        "No user customer secret keys found",
        "global"
      );
      return callback(null, results, source);
    }
    ...
    for (let csk of customerSecretKeys.data) {
      ...
      if (difference > config.customer_secret_key_rotated_fail) {
        helpers.addResult(
          results,
          2,
          `Customer secret key is ${difference} days old`,
          "global",
          csk.id
        );
      } else {
