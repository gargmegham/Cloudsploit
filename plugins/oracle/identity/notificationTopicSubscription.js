...
module.exports = {
  ...
  more_info:
    "Creating and subscribing to one or more notification topics allows administrators to be notified of any changes in the Oracle Cloud Infrastructure.",
  link: "https://docs.oracle.com/en-us/iaas/Content/Notification/Tasks/managingtopicsandsubscriptions.htm",
  recommended_action:
    "Create at least one notification topic with an active subscription.",
        ...
        if (helpers.checkRegionSubscription(cache, source, results, region)) {
          ...
          if (topics.err || !topics.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for topics: " + helpers.addError(topics),
              region
            );
            return rcb();
          }
          ...
          const activeTopic = topics.data.find(
            (topic) => topic.lifecycleState && topic.lifecycleState === "ACTIVE"
          );
          ...
          if (!activeTopic) {
            helpers.addResult(
              results,
              2,
              "No active topics found in the region",
              region
            );
            return rcb();
          }
          ...
          if (!subscriptions || subscriptions.err || !subscriptions.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for subscriptions: " +
                helpers.addError(subscriptions),
              region
            );
            return rcb();
          }
          if (!subscriptions.data.length) {
            helpers.addResult(
              results,
              2,
              "No subscriptions found in the region",
              region
            );
            return rcb();
          }
          ...
          if (activeSubscription) {
            helpers.addResult(
              results,
              0,
              "There is at least one notification topic with an active subscription",
              region
            );
          } else {
            helpers.addResult(
