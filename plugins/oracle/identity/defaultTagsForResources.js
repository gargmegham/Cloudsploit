...
module.exports = {
  ...
  more_info:
    'Having default tags like "CreatedBy" on resources help determine who created the resource in case of an accident.',
  link: "https://docs.oracle.com/en-us/iaas/Content/Tagging/Tasks/managingtagdefaults.htm",
  recommended_action:
    "Create default tags at the root compartment to ensure that all resources get tagged.",
  ...
  run: function (cache, settings, callback) {
    ...
    if (defaultTags.err || !defaultTags.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query for default tags: " + helpers.addError(defaultTags),
        region
      );
    ...
    const resourceTag = defaultTags.data.find(
      (tag) =>
        tag.value &&
        tag.value === "${iam.principal.name}" &&
        tag.lifecycleState &&
    ...
    if (resourceTag) {
      helpers.addResult(
        results,
        0,
        "Compartment is using default tags for resources",
        region,
        compartment
      );
    } else {
      helpers.addResult(
        results,
        2,
        "Compartment is not using default tags for resources",
        region,
        compartment
      );
    }
