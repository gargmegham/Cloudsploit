...
module.exports = {
  ...
  description:
    "Ensure that user API keys are rotated regularly in order to reduce accidental exposures.",
  more_info:
    "User API keys should be rotated frequently to avoid having them accidentally exposed.",
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      api_keys_rotated_fail: parseInt(
        settings.api_keys_rotated_fail ||
          this.settings.api_keys_rotated_fail.default
      ),
    };

    var users = helpers.addSource(cache, source, ["user", "list", region]);

    var apiKeys = helpers.addSource(cache, source, ["apiKey", "list", region]);
    ...
    if (apiKeys.err || !apiKeys.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query user API keys: " + helpers.addError(apiKeys),
        "global"
      );
      return callback(null, results, source);
    }
    ...
    for (let key of apiKeys.data) {
      ...
      if (difference > config.api_keys_rotated_fail) {
        helpers.addResult(
          results,
          2,
          `API key is ${difference} days old`,
          "global",
          key.id
        );
      } else {
