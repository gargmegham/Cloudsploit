...
module.exports = {
  ...
  more_info:
    "Having multiple user accounts that have been inactive for a period of time can increase the risk of security attacks and breaches.",
  ...
  run: function (cache, settings, callback) {
    ...
    var source = {};
    var config = {
      identity_users_password_last_used_fail: parseInt(
        settings.identity_users_password_last_used_fail ||
          this.settings.identity_users_password_last_used_fail.default
      ),
    };
    ...
    users.data.forEach((user) => {
      ...
      if (user.lastSuccessfulLoginTime) {
        daysAgo = helpers.daysBetween(
          new Date(),
          new Date(user.lastSuccessfulLoginTime)
        );
