...
module.exports = {
  ...
  description:
    "Ensure that user auth tokens are rotated regularly in order to reduce accidental exposures.",
  more_info:
    "User auth tokens should be rotated frequently to avoid having them accidentally exposed.",
  ...
  settings: {
    auth_tokens_rotated_fail: {
      ...
      description:
        "Return a failing result when auth tokens exceed this number of days without being rotated",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: "90",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      auth_tokens_rotated_fail: parseInt(
        settings.auth_tokens_rotated_fail ||
          this.settings.auth_tokens_rotated_fail.default
      ),
    };
    ...
    if (authTokens.err || !authTokens.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query user auth tokens: " + helpers.addError(authTokens),
        "global"
      );
      return callback(null, results, source);
    }
    ...
    for (let token of authTokens.data) {
      ...
      if (difference > config.auth_tokens_rotated_fail) {
        helpers.addResult(
          results,
          2,
          `Auth token is ${difference} days old`,
          "global",
          token.id
        );
      } else {
