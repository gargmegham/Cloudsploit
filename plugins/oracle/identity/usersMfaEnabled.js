...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      warn_federated_users:
        settings.warn_federated_users ||
        this.setting.warn_federated_users.default,
    };
    ...
    users.data.forEach((user) => {
      if (user.isMfaActivated) {
        ...
        if (
          ...
          user.identityProviderId.length &&
          warnFedUsers
        ) {
          helpers.addResult(
            results,
            1,
            "The federated user has MFA disabled",
            "global",
            user.id
          );
        } else
