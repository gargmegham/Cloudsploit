...
module.exports = {
  ...
  description:
    "Ensures password policy requires at least one uppercase character.",
  more_info:
  ...
  recommended_action:
    "Update the password policy to require the use of uppercase characters.",
  ...
  run: function (cache, settings, callback) {
    ...
    var defaultRegion = helpers.objectFirstKey(
      cache["regionSubscription"]["list"]
    );
    ...
    if (authenticationPolicy.err || !authenticationPolicy.data) {
      helpers.addResult(
        ...
        "Unable to query for password policy status: " +
          helpers.addError(authenticationPolicy)
      );
    ...
    authenticationPolicy.data.forEach((policy) => {
      ...
      if (passwordPolicy && passwordPolicy.isUppercaseCharactersRequired) {
        helpers.addResult(
          results,
          0,
          "Password policy requires uppercase characters",
          "global",
          authenticationPolicy.data.compartmentId
        );
      } else {
        helpers.addResult(
          results,
