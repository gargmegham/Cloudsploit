...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    var groups = helpers.addSource(cache, source, ["group", "list", region]);
    ...
    if (!users || !groups || !userGroups || !apiKeys)
    ...
    if (groups.err || !groups.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query user groups: " + helpers.addError(groups),
        "global"
      );
      return callback(null, results, source);
    }
    ...
    let adminGroup = groups.data.find(
    ...
    if (adminGroup) {
      let adminUsers =
        userGroups.data.map((userGroup) => userGroup.userId) || [];
