...
module.exports = {
  ...
  more_info:
    "To Have a valid email address associated with an OCI IAM local user account enables you to tie the account to identity in your organization " +
    ...
    users.data.forEach((user) => {
      ...
      if (user.email && user.email.length && user.emailVerified) {
        helpers.addResult(
          results,
          0,
          `Email for user ${user.name} is verified`,
          region,
          user.id
        );
        ...
        helpers.addResult(
          results,
          2,
          `Email for user ${user.name} not found`,
          region,
          user.id
        );
      } else {
        helpers.addResult(
          results,
          2,
          `Email for user ${user.name} is not verified`,
          region,
          user.id
        );
      }
