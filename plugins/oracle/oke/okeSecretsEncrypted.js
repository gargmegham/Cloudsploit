...
module.exports = {
  ...
  settings: {
    oke_encryption_level: {
      ...
      description:
        "Desired protection level for OKE Secrets. default: oracle-managed, cloudcmek: customer managed encryption keys, " +
  ...
  run: function (cache, settings, callback) {
    ...
    let desiredEncryptionLevelStr =
      settings.oke_encryption_level ||
      this.settings.oke_encryption_level.default;
              ...
              if (
                ...
                if (clusters.err || !clusters.data) {
                  helpers.addResult(
                    results,
                    3,
                    "Unable to query for OKE clusters: " +
                      helpers.addError(clusters),
                    region
                  );
                  return rcb();
                }
                ...
                clusters.data.forEach((cluster) => {
                  if (
                    ...
                    cluster.lifecycleState === "DELETED"
                  )
                    return;
                  ...
                  if (currentEncryptionLevel >= desiredEncryptionLevel) {
                    helpers.addResult(
                      results,
                      0,
                      `OKE cluster (${cluster.name}) has secret encryption level ${currentEncryptionLevelStr} which is greater than or equal to ${desiredEncryptionLevelStr}`,
                      region,
                      cluster.id
                    );
                  } else {
                    helpers.addResult(
                      results,
                      2,
                      `OKE cluster (${cluster.name}) has secret encryption level ${currentEncryptionLevelStr} which is less than ${desiredEncryptionLevelStr}`,
                      region,
                      cluster.id
                    );
                  }
