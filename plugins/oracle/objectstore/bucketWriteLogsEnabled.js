...
module.exports = {
  ...
  more_info:
    "Enabling write level logging for object store buckets will provide you more visibility into changes to objects in your buckets.",
        ...
        if (helpers.checkRegionSubscription(cache, source, results, region)) {
          ...
          if (buckets.err || !buckets.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for object store buckets: " +
                helpers.addError(buckets),
              region
            );
            return rcb();
          }
          ...
          if (!buckets.data.length) {
            helpers.addResult(
              results,
              0,
              "No object store buckets to check",
              region
            );
            return rcb();
          }

          var logs = helpers.addSource(cache, source, ["log", "list", region]);

          buckets.data.forEach((bucket) => {
            let bucketLog = null;

            if (logs && !logs.err && logs.data && logs.data.length) {
              bucketLog = logs.data.find(
                (log) =>
                  log.isEnabled &&
                  log.configuration &&
                  Object.keys(log.configuration).length &&
                  log.configuration.source &&
                  Object.keys(log.configuration.source).length &&
                  log.configuration.source.service === "objectstorage" &&
            ...
            if (bucketLog) {
              helpers.addResult(
                results,
                0,
                "The bucket has write level logging enabled",
                region,
                bucket.name
              );
            } else {
              helpers.addResult(
                results,
                2,
                "The bucket does not have write level logging enabled",
                region,
                bucket.name
              );
            }
