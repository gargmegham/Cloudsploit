...
module.exports = {
  ...
  compliance: {
    hipaa:
      "HIPAA requires all services that contain user information to be " +
      ...
      function (region, rcb) {
        if (helpers.checkRegionSubscription(cache, source, results, region)) {
          ...
          requests.data.forEach((request) => {
            ...
            if (request.accessType && request.accessType === "AnyObjectWrite") {
              helpers.addResult(
                results,
                2,
                "pre-authenticated request allows write access to all objects",
                region,
                request.id
              );
            } else if (
              ...
              helpers.addResult(
                results,
                1,
                `pre-authenticated request allows write access to ${request.objectName}`,
                region,
                request.id
              );
            } else {
              helpers.addResult(
                results,
                0,
                "Pre-authenticated requests has least access",
                region,
                request.id
              );
            }
