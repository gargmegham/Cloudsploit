...
module.exports = {
  ...
  more_info:
    "To minimize data loss in case of inadvertent or malicious deletes, make sure that all your object store buckets are configured with object versioning.",
            ...
            getBucket.data.forEach(function (bucket) {
              if (bucket.versioning && bucket.versioning === "Enabled") {
                helpers.addResult(
                  results,
                  0,
                  `Object store bucket (${bucket.name}) has versioning enabled`,
                  region,
                  bucket.id
                );
              } else {
                helpers.addResult(
                  results,
                  2,
                  `Object store bucket (${bucket.name}) does not have versioning enabled`,
                  region,
                  bucket.id
                );
              }
