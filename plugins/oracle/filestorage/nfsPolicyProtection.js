  ...
  run: function (cache, settings, callback) {
        ...
        policy.statements.forEach((statement) => {
          ...
          if (!statementPasses) {
            ...
            helpers.addResult(
              results,
              2,
              `${statementObj["subjectType"]}${statementObj["subject"]} has the ability to delete all file storage services in ${statementObj["location"]}`,
              region,
              policy.id
            );
          }
    ...
    if (policyProtection && entered) {
      helpers.addResult(
        results,
        0,
        "All policies have file storage delete protection enabled",
        region
      );
    }
