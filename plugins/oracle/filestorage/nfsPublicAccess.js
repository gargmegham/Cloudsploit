...
module.exports = {
  ...
  more_info:
    "All network file systems should be configured to only allow access from trusted sources.",
  ...
  compliance: {
    hipaa:
      "HIPAA requires strict access controls to all data. " +
      "Restricting NFS ensures all access is limited to those " +
      "with explicit approval.",
    pci:
      "PCI requires all access to be restricted and identified. Limiting NFS " +
          ...
          fileSystems.data.forEach((fileSystem) => {
            if (fileSystem.exportOptions) {
              fileSystem.exportOptions.forEach((exportOption) => {
                if (
                  exportOption.source !== "0.0.0.0/0" &&
                  exportOption.source !== "::/0"
                ) {
                  helpers.addResult(
                    results,
                    0,
          ...
          if (publicExportSets.length) {
            ...
            if (!mountTargets || mountTargets.err || !mountTargets.data) {
              helpers.addResult(
                results,
                3,
                "Unable to query for mount targets: " +
                  helpers.addError(mountTargets),
                region
              );
              return rcb();
            }
            if (!mountTargets.data.length) {
              var publicFileSystemStr =
                Object.values(publicFileSystem).join(", ");
              helpers.addResult(
                results,
                2,
                `The following NFS allow public access: ${publicFileSystemStr}`,
                region
              );
              return rcb();
            }
            mountTargets.data.forEach((mountTarget) => {
              if (publicExportSets > 1) {
                if (publicExportSets.indexOf(mountTarget.exportSetId) > -1) {
                  mountSubnets[mountTarget.subnetId] =
                    publicFileSystem[mountTarget.exportSetId];
                }
              } else {
                  ...
                  if (subnetsToCheck.indexOf(subnet.id) > -1) {
                    if (subnet.prohibitPublicIpOnVnic) {
                      helpers.addResult(
                        ...
                        region,
                        mountSubnets[subnet.id]
                      );
                    } else {
                      helpers.addResult(
                        results,
                        2,
                        "NFS allows public access",
                        region,
                        mountSubnets[subnet.id]
                      );
                    }
