  ...
  describe("run", function () {
    it("should give passing result if an error is passed or no data is present", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Unable to query for file systems"
        );
    ...
    it("should give failing result if there is public access on the File System", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "The following NFS allow public access"
        );
        expect(results[0].region).to.equal("us-ashburn-1");
        done();
      };
        ...
        [
          {
            ...
            exportSetId:
              "ocid1.exportset.oc1.iad.aaaaaa4np2snjqnanfqwillqojxwiotjmfsc2ylefuzqaaaa",
            fileSystemId:
              "ocid1.filesystem.oc1.iad.aaaaaaaaaaaal26cnfqwillqojxwiotjmfsc2ylefuzqaaaa",
    ...
    it("should give passing result if there isnt public access on the File System", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          "NFS does not allow public access"
        );
        expect(results[0].region).to.equal("us-ashburn-1");
        done();
      };
    ...
    it("should give passing result if there is public access on the File System but the subnet is private", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          "NFS is in a private subnet and does not allow public access"
        );
        expect(results[0].region).to.equal("us-ashburn-1");
        done();
      };
        ...
        [
          {
            ...
            exportSetId:
              "ocid1.exportset.oc1.iad.aaaaacvippxhznzjnfqwillqojxwiotjmfsc2ylefuzqaaaa",
            fileSystemId:
              "ocid1.filesystem.oc1.iad.aaaaaaaaaaaal26cnfqwillqojxwiotjmfsc2ylefuzqaaaa",
            id: "ocid1.export.oc1.iad.aaaaacvippxgdr7nnfqwillqojxwiotjmfsc2ylefuzqaaaa",
            lifecycleState: "ACTIVE",
        ...
        [
          {
            ...
            subnetId:
              "ocid1.subnet.oc1.iad.aaaaaaaatheunvhfdmm72yortkxqydw6s4kvhzddyoj5nq5zvitcoyglkepa",
            nsgIds: [],
          ...
          },
          {
            ...
            subnetId:
              "ocid1.subnet.oc1.iad.aaaaaaaa7gwzgnac2rav2rsapkkogspt6vsgrb6imav44jgu3h5xyef3zrgq",
            nsgIds: [],
