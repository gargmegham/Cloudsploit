...
module.exports = {
  ...
  more_info:
    "Enabling automatic backup on databases ensures that all sensitive data is protected from unwarranted deletion or loss of data.",
  link: "https://docs.cloud.oracle.com/iaas/Content/Database/Tasks/backingupOS.htm",
  recommended_action:
    "when creating a new database, under advanced settings enable Auto Backup.",
        ...
        if (helpers.checkRegionSubscription(cache, source, results, region)) {
          ...
          if (databases.err || !databases.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for databases: " + helpers.addError(databases),
              region
            );
            return rcb();
          }
          ...
          databases.data.forEach((database) => {
            if (
              database.dbBackupConfig &&
              database.dbBackupConfig.autoBackupEnabled
            ) {
              helpers.addResult(
                results,
                0,
                "The database has auto backup enabled",
                region,
                database.id
              );
            } else {
              helpers.addResult(
                results,
                2,
                "The database has auto backup disabled",
                region,
                database.id
              );
            }
