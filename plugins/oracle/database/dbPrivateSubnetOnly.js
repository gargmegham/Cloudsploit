...
module.exports = {
  ...
  more_info:
    "Database systems in private subnets ensure that access to the database can " +
  ...
  recommended_action:
    "When creating a new database, ensure that that subnet it is being " +
    "launched in is a private subnet.",
  apis: ["vcn:list", "dbSystem:list", "subnet:list"],
  compliance: {
    hipaa:
      "DB systems should only be launched in private subnets and " +
    ...
    pci:
      "PCI requires DB systems to be properly secured. Ensure " +
        ...
        if (helpers.checkRegionSubscription(cache, source, results, region)) {
          ...
          dbSystems.data.forEach((dbSystem) => {
            if (dbSystem.lifecycleState === "AVAILABLE") {
              if (
                dbSystem.subnetId &&
                privateSubnets.indexOf(dbSystem.subnetId) > -1
              ) {
                helpers.addResult(
                  results,
                  0,
