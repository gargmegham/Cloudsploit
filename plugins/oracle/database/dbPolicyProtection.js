...
module.exports = {
  ...
  domain: "Databases",
  description:
    "Ensures policy statements have deletion protection for database systems, databases, and database homes unless it is an administrator group.",
  more_info:
    "Adding deletion protection to Oracle database policies mitigates unintended deletion of database services by unauthorized users or groups.",
  link: "https://docs.cloud.oracle.com/iaas/Content/Security/Reference/dbaas_security.htm",
  recommended_action:
    "Ensure policy statements have deletion protection for Database Systems, databases, and Database Homes unless it is an administrator group.",
  apis: ["policy:list"],
  settings: {
    policy_group_admins: {
  ...
  run: function (cache, settings, callback) {
        ...
        policy.statements.forEach((statement) => {
          ...
          if (!statementPasses) {
            ...
            helpers.addResult(
              results,
              2,
              `${statementObj["subjectType"]}${statementObj["subject"]} has the ability to delete all database services in ${statementObj["location"]}`,
              region,
              policy.id
            );
          }
    ...
    if (policyProtection && entered) {
      helpers.addResult(
        results,
        0,
        "All policies have database delete protection enabled",
        region
      );
    }
