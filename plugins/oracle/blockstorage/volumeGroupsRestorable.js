...
module.exports = {
  ...
  more_info:
    "Enabling volume groups backups ensures that the volume group can be restored following in the event of data loss.",
        ...
        if (helpers.checkRegionSubscription(cache, source, results, region)) {
          ...
          if (volumeGroups.err || !volumeGroups.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for volume groups: " +
                helpers.addError(volumeGroups),
              region
            );
            return rcb();
          }
          ...
          if (
            !volumeGroupBackups ||
            volumeGroupBackups.err ||
            !volumeGroupBackups.data
          ) {
            helpers.addResult(
              results,
              3,
              "Unable to query for volume group backups: " +
                helpers.addError(volumeGroupBackups),
              region
            );
            return rcb();
          }
          ...
          volumeGroups.data.forEach((volumeGroup) => {
            if (enabledVolumeGroups.indexOf(volumeGroup.id) > -1) {
              helpers.addResult(
                results,
                0,
                "Volume group is actively restorable",
                region,
                volumeGroup.id
              );
            } else {
              helpers.addResult(
                results,
                2,
                "Volume group is not actively restorable",
                region,
                volumeGroup.id
              );
            }
