...
module.exports = {
  ...
  more_info:
    "Enabling block volume backup policies ensures that the block volume can be restored following in the event of data loss.",
    ...
    async.each(
      ...
      function (region, rcb) {
        if (helpers.checkRegionSubscription(cache, source, results, region)) {
          ...
          if (
            !blockVolumeBackupPolicies ||
            blockVolumeBackupPolicies.err ||
            !blockVolumeBackupPolicies.data
          ) {
            helpers.addResult(
              results,
              3,
              "Unable to query for block volume backups: " +
                helpers.addError(blockVolumeBackupPolicies),
              region
            );
            return rcb();
          }
          ...
          blockVolumes.data.forEach((blockVolume) => {
            if (enabledBlockVolumes.indexOf(blockVolume.id) > -1) {
              helpers.addResult(
                results,
                0,
                "Block volume has a backup policy enabled",
                region,
                blockVolume.id
              );
            } else {
              helpers.addResult(
                results,
                2,
                "Block volume has a backup policy disabled",
                region,
                blockVolume.id
              );
            }
