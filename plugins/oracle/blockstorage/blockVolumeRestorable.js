...
module.exports = {
  ...
  more_info:
    "Having recent backups on block volumes prevents data loss in the case of a catastrophe.",
  recommended_action:
    "Ensure block volumes have recent backups to prevent data loss.",
    ...
    async.each(
      ...
      function (region, rcb) {
        if (helpers.checkRegionSubscription(cache, source, results, region)) {
          ...
          if (
            !blockVolumeBackups ||
            blockVolumeBackups.err ||
            !blockVolumeBackups.data
          ) {
            helpers.addResult(
              results,
              3,
              "Unable to query for block volume backups: " +
                helpers.addError(blockVolumeBackups),
              region
            );
            return rcb();
          }
          ...
          blockVolumeBackups.data.forEach((blockVolumeBackup) => {
            if (
              blockVolumeBackup.lifecycleState &&
          ...
          blockVolumes.data.forEach((blockVolume) => {
            if (enabledBlockVolumes.indexOf(blockVolume.id) > -1) {
              helpers.addResult(
                results,
                0,
                "Block volume is actively restorable",
                region,
                blockVolume.id
              );
            } else {
              helpers.addResult(
                results,
                2,
                "Block volume is not actively restorable",
                region,
                blockVolume.id
              );
            }
