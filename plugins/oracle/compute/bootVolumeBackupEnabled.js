    ...
    async.each(
      ...
      function (region, rcb) {
        if (helpers.checkRegionSubscription(cache, source, results, region)) {
          ...
          if (
            !bootVolumeBackupPolicies ||
            bootVolumeBackupPolicies.err ||
            !bootVolumeBackupPolicies.data
          ) {
            helpers.addResult(
              results,
              3,
              "Unable to query for boot volume backups: " +
                helpers.addError(bootVolumeBackupPolicies),
              region
            );
            return rcb();
          }
          ...
          if (!bootVolumes.data.length) {
            helpers.addResult(
              results,
              2,
              "No boot volume backup policies found",
              region
            );
            return rcb();
          }
          ...
          bootVolumeBackupPolicies.data.forEach((bootVolumeBackupPolicy) => {
            enabledBootVolumes.push(
              bootVolumeBackupPolicy.bootVolumeId ||
                bootVolumeBackupPolicy.assetId
            );
          });
