  ...
  describe("run", function () {
    ...
    it("should give passing result if instance pools have multiple availability domains", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          "Instance pool is in multiple availability domains"
        );
        expect(results[0].region).to.equal("us-ashburn-1");
        done();
      };
      ...
      const cache = createCache(null, [
        {
          ...
          instanceConfigurationId:
            "ocid1.instanceconfiguration.oc1.iad.aaaaaaaahfjaolm2qxtslfce2hcvfyhvqatcrra25iqft6xo6us2msfunyda",
    ...
    it("should give failing result if instance pools have only one availability domain", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Instance pool is only in one availability domain"
        );
        expect(results[0].region).to.equal("us-ashburn-1");
        done();
      };
      ...
      const cache = createCache(null, [
        {
          ...
          lifecycleState: "RUNNING",
          availabilityDomains: ["fMgC:US-ASHBURN-AD-1"],
          size: 1,
