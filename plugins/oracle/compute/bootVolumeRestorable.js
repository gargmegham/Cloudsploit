...
module.exports = {
  ...
  more_info:
    "Having an active backup ensures that the boot volumes can be restored in the event of a compromised system or hardware failure.",
    ...
    async.each(
      ...
      function (region, rcb) {
        if (helpers.checkRegionSubscription(cache, source, results, region)) {
          ...
          if (
            !bootVolumeBackups ||
            bootVolumeBackups.err ||
            !bootVolumeBackups.data
          ) {
            helpers.addResult(
              results,
              3,
              "Unable to query for boot volume backups: " +
                helpers.addError(bootVolumeBackups),
              region
            );
            return rcb();
          }
          ...
          bootVolumes.data.forEach((bootVolume) => {
            if (enabledBootVolumes.indexOf(bootVolume.id) > -1) {
              helpers.addResult(
                results,
                0,
                "The boot volume is actively restorable",
                region,
                bootVolume.id
              );
            } else {
              helpers.addResult(
                results,
                2,
                "The boot volume is not actively restorable",
                region,
                bootVolume.id
              );
            }
