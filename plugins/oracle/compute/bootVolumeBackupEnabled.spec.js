  ...
  describe("run", function () {
    it("should give unknown result if a volume error is passed or no data is present", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Unable to query for boot volume attachments"
        );
    ...
    it("should give unknown result if a volume backup error is passed or no data is present", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].status).to.equal(3);
        expect(results[0].message).to.include(
          "Unable to query for boot volume backups"
        );
        expect(results[0].region).to.equal("us-ashburn-1");
        done();
      };

      const cache = createCache(null, ["lots of data"], undefined, ["error"]);
    ...
    it("should give failing result if there is a boot volume without a backup", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "The boot volume has backup policies disabled"
        );
        expect(results[0].region).to.equal("us-ashburn-1");
        done();
      };
        ...
        [
          {
            ...
            uniqueSizeInMbs: 1,
            bootVolumeId:
              "ocid1.volume.oc1.iad.abuwcljtquhbwu5divro64gimkrnfdaxo43cy44cbpuz42g652ol4gw6qsma",
          },
        ],
        null
      );
    ...
    it("should give passing result if all boot volumes have backups", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "The boot volume has backup policies enabled"
        );
      ...
      const cache = createCache(
        ...
        [
          ...
          {
            assetId:
              "ocid1.bootvolume.oc1.uk-london-1.abwgiljtpbow2jvc3dnuqwxg63ljrszfmmhkdffbm2lgihwxyiddzti2ja2q",
            id: "ocid1.volumebackuppolicyassign.oc1.uk-london-1.abwgiljt3we7xn7kllp2kpxzbnhy57xqvhwcukvkuk57rqd7iaiodr37xjba",
            policyId:
              "ocid1.volumebackuppolicy.oc1..aaaaaaaa7hwv7iscewqqcmyqe2zuzfce6setvckhbxduswtxf6ctew7e54ja",
