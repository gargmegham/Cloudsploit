...
module.exports = {
  ...
  more_info:
    "Enabling autoscaling increases efficiency and improves cost management for resources.",
      ...
      function (region, rcb) {
        if (helpers.checkRegionSubscription(cache, source, results, region)) {
          ...
          if (
            ...
            autoscaleConfigurations.err ||
            !autoscaleConfigurations.data
          ) {
            helpers.addResult(
              results,
              3,
              "Unable to query for autoscaling configurations: " +
                helpers.addError(autoscaleConfigurations),
              region
            );
            return rcb();
          }
          ...
          if (!autoscaleConfigurations.data.length) {
            helpers.addResult(
              results,
              2,
              "No autoscaling configurations found",
              region
            );
            return rcb();
          }
          ...
          autoscaleConfigurations.data.forEach((autoscaleConfiguration) => {
            if (
              autoscaleConfiguration.isEnabled &&
          ...
          instancePools.data.forEach((instancePool) => {
            if (enabledInstancePools.indexOf(instancePool.id) > -1) {
              helpers.addResult(
                results,
                0,
                "The instance pool has autoscaling enabled",
                region,
                instancePool.id
              );
            } else {
              helpers.addResult(
                results,
                2,
                "The instance pool has autoscaling disabled",
                region,
                instancePool.id
              );
            }
