  ...
  describe("run", function () {
    ...
    it("should give passing result if the number of instances is less than the threshold", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "instances in the region are within the regional expected coun"
        );
        expect(results[0].region).to.equal("us-ashburn-1");
        done();
      };
      ...
      const cache = createCache(null, [
        {
          ...
          freeformTags: {
            "oci:compute:instanceconfiguration":
              "ocid1.instanceconfiguration.oc1.iad.aaaaaaaacsmqbsufpjlyzip2w4pvkbkgle2bsf6wkahxyuvonuqjlyckad5q",
            "oci:compute:instancepool":
              "ocid1.instancepool.oc1.iad.aaaaaaaa3u3ku3miiz4cz7wpebecozqrvgdmymccqnmybhuc2uxgjettslfa",
          },
          id: "ocid1.instance.oc1.iad.abuwcljtfqlmtwvynntlowy2qrboocmbfvsc5oz5r3bev7uabny7ehvxhiva",
          imageId:
            "ocid1.image.oc1.iad.aaaaaaaa5m7pxvywx2isnwon3o3kixkk6gq4tmdtfgvctj7xbl3wgo56uppa",
          ipxeScript: null,
          launchMode: "NATIVE",
          launchOptions: {
        ...
        },
        {
          ...
          imageId:
            "ocid1.image.oc1.iad.aaaaaaaasorq3smbazoxvyqozz52ch5i5cexjojb7qvcefa5ubij2yjycy2a",
          ipxeScript: null,
          launchMode: "NATIVE",
          launchOptions: {
            bootVolumeType: "PARAVIRTUALIZED",
    ...
    it("should give failing result if the number of global instances is more than the threshold", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[results.length - 1].message).to.include(
    ...
    it("should give passing result if the number of global instances is less than the threshold", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[results.length - 1].message).to.include(
          "instances in the account are within the global expected count"
        );
        expect(results[results.length - 1].region).to.equal("global");
        done();
      };
