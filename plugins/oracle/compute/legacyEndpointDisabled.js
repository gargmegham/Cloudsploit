        ...
        if (helpers.checkRegionSubscription(cache, source, results, region)) {
          ...
          instances.data.forEach((instance) => {
            if (
              ...
              instance.instanceOptions.areLegacyImdsEndpointsDisabled
            ) {
              helpers.addResult(
                results,
                0,
                "Instance has Legacy MetaData service (IMDSv1) endpoints disabled",
                region,
                instance.id
              );
            } else {
              helpers.addResult(
                results,
                2,
                "Instance does not have Legacy MetaData service (IMDSv1) endpoints disabled",
                region,
                instance.id
              );
            }
