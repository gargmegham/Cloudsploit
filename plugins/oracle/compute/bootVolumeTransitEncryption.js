...
module.exports = {
  ...
  recommended_action:
    "boot volume transit encryption can only be configured when creating a new instance. Recreate the instance with in-transit encryption enabled.",
        ...
        if (helpers.checkRegionSubscription(cache, source, results, region)) {
          ...
          if (bootVolumeAttachments.err || !bootVolumeAttachments.data) {
            helpers.addResult(
              ...
              "Unable to query for boot volume attachments: " +
                helpers.addError(bootVolumeAttachments),
              region
            );
            return rcb();
          }
          ...
          if (!bootVolumeAttachments.data.length) {
            helpers.addResult(
              results,
              0,
              "No boot volume attachments found",
              region
            );
            return rcb();
          }
          ...
          bootVolumeAttachments.data.forEach((bootVolumeAttachment) => {
            if (bootVolumeAttachment.isPvEncryptionInTransitEnabled) {
              helpers.addResult(
                results,
                0,
                "boot volume transit encryption is enabled",
                region,
                bootVolumeAttachment.bootVolumeId
              );
            } else {
              helpers.addResult(
                results,
                2,
                "boot volume transit encryption is disabled",
                region,
                bootVolumeAttachment.bootVolumeId
              );
            }
