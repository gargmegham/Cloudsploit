...
module.exports = {
  ...
  recommended_action:
    "Update the rules for the default security list to deny all traffic by default",
        ...
        if (helpers.checkRegionSubscription(cache, source, results, region)) {
              ...
              if (displayNameArr[0].toLowerCase() === "default") {
                ...
                if (
                  ...
                  helpers.addResult(
                    results,
                    2,
                    "Default security list has " +
                      (securityList.egressSecurityRules.length || "0") +
                      " inbound and " +
                      (securityList.ingressSecurityRules.length || "0") +
                      " outbound rules",
                    region,
                    securityList.vcnId
                  );
          ...
          if (noDefault) {
            helpers.addResult(
              results,
              0,
              "No default security list found",
              region
            );
          }
        }
