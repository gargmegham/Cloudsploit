...
module.exports = {
  ...
  more_info:
    "Creating a regional subnet ensures a highly available system. Regional subnets span across multiple availability domains increasing the availability and durability of the resources launched within it.",
  link: "https://docs.cloud.oracle.com/iaas/Content/Network/Tasks/managingVCNs.htm",
  recommended_action:
    "when creating a new subnet, under subnet type, ensure that regional is selected.",
          ...
          if (Object.keys(badSubnetObj).length) {
            for (var bad in badSubnetObj) {
              helpers.addResult(
                results,
                2,
                `${badSubnetObj[bad]} subnets in the VCN are not regional`,
                region,
                bad
              );
            }
