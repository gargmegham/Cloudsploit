...
module.exports = {
  ...
  settings: {
    ...
    excessive_security_lists_warn: {
      ...
      description:
        "Return a warning result when the number of security lists exceeds this value",
      regex: "^[1-9]{1}[0-9]{0,5}$",
      default: 30,
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      excessive_security_lists_fail:
        settings.excessive_security_lists_fail ||
        this.settings.excessive_security_lists_fail.default,
      excessive_security_lists_warn:
        settings.excessive_security_lists_warn ||
    ...
    async.each(
      ...
      function (region, rcb) {
        if (helpers.checkRegionSubscription(cache, source, results, region)) {
          ...
          if (securityLists.err || !securityLists.data) {
            helpers.addResult(
              ...
              "Unable to query for security lists: " +
                helpers.addError(securityLists),
              region
            );
            return rcb();
          }
          ...
          var returnMsg =
            " number of security lists: " +
            securityLists.data.length +
            " groups found";

          if (
            securityLists.data.length > config.excessive_security_lists_fail
          ) {
            helpers.addResult(
              results,
              2,
            ...
            securityLists.data.length > config.excessive_security_lists_warn
          ) {
