...
module.exports = {
  ...
  more_info:
    "Stateless security rules are one-way-rules that help mitigate " +
        ...
        if (helpers.checkRegionSubscription(cache, source, results, region)) {
          ...
          securityLists.data.forEach((securityList) => {
            ...
            if (!isStateless) {
              helpers.addResult(
                results,
                2,
                `The security list has stateful security rules`,
                region,
                securityList.id
              );
          ...
          securityRules.data.forEach((securityRule) => {
            if (
              ...
              securityGroups.data.find(
                (group) => group.id === securityRule.networkSecurityGroups
              )
            ) {
              securityGroupName = securityGroups.data.find(
                (group) => group.id === securityRule.networkSecurityGroups
              ).displayName;
            }
            ...
            if (!securityRule.isStateless) {
              if (
                statefulNSGId.indexOf(securityRule.networkSecurityGroups) < 0
              ) {
                helpers.addResult(
                  results,
                  2,
          ...
          if (!statefulNSGId.length && securityListStateless) {
            helpers.addResult(
              results,
              0,
              "All network security groups and security lists have stateless security rules",
              region
            );
            ...
            helpers.addResult(
              results,
              0,
              "All network security groups have stateless security rules",
              region
            );
            ...
            helpers.addResult(
              results,
              0,
              "All security lists have stateless security rules",
              region
            );
          }
        }
