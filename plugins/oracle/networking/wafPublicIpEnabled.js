...
module.exports = {
  ...
  more_info:
    "Every Public IP address should have a firewall enabled to control access to the endpoints. Enabling a Web Application Firewall follows security best practices and helps prevent malicious attempts to access the network.",
        ...
        if (helpers.checkRegionSubscription(cache, source, results, region)) {
          ...
          if (publicIps.err || !publicIps.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for public IPs: " + helpers.addError(publicIps),
              region
            );
            return rcb();
          }
          ...
          if (waasPolicies.err || !waasPolicies.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for waas policies: " +
                helpers.addError(waasPolicies),
              region
            );
            return rcb();
          }
          ...
          waasPolicies.data.forEach((waasPolicy) => {
            if (waasPolicy.origins && waasPolicy.origins.length) {
          ...
          publicIps.data.forEach((publicIp) => {
            if (waasIps.indexOf(publicIp.ipAddress) > -1) {
              ...
              helpers.addResult(
                results,
                2,
                "The public IP has WAF disabled",
                region,
                publicIp.id
              );
            }
