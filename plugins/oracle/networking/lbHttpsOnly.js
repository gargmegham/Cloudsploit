...
module.exports = {
  ...
  compliance: {
    hipaa:
      "HIPAA requires that all patient information is " +
      "encrypted at rest and in transit.",
    pci:
      "PCI requires that all cardholder data is encrypted " +
            ...
            function (lb, cb) {
              ...
              for (let l in lb.listeners) {
                ...
                if (
                  ...
                  listener.ruleSetNames &&
                  listener.ruleSetNames.length
                ) {
                  ...
                  if (doesRedirect) {
                    helpers.addResult(
                      results,
                      0,
                      `The listener: ${listener.name} redirects to HTTPS`,
                      region,
                      lb.id
                    );
                  }
                  ...
                  helpers.addResult(
                    results,
                    0,
                    `The listener: ${listener.name} is HTTPS only`,
                    region,
                    lb.id
                  );
                }
              ...
              if (non_https_listener.length) {
                helpers.addResult(
                  results,
                  2,
                  `The following listeners are not HTTPS-only:  ${non_https_listener.join(
                    ", "
                  )}`,
                  region,
                  lb.id
