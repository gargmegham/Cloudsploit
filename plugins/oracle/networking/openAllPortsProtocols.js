...
module.exports = {
  ...
  recommended_action:
    "Modify the security list to specify a specific port and protocol to allow.",
    ...
    async.each(
      ...
      function (region, rcb) {
        if (helpers.checkRegionSubscription(cache, source, results, region)) {
          var vcn = helpers.addSource(cache, source, ["vcn", "list", region]);
          ...
          if (vcn.err || !vcn.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for VCNs: " + helpers.addError(vcn),
              region
            );
            return rcb();
          }
