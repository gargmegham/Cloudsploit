...
describe("multipleSubnets", function () {
  describe("run", function () {
    it("should give unknown result if a VCN error is passed or no data is present", function (done) {
      ...
      const cache = createCache(["error"], null, null, null);
        ...
        [
          {
            ...
            defaultDhcpOptionsId:
              "ocid1.dhcpoptions.oc1.iad.aaaaaaaaxh7iyslywylxkgniny66a4plra33abpyg5y2lpqcutgfyneouj2a",
            defaultRouteTableId:
              "ocid1.routetable.oc1.iad.aaaaaaaavfrldnlzxwbvdikxybzunzzgjgeq43upwzfa5whjtrndoqidm7lq",
            defaultSecurityListId:
              "ocid1.securitylist.oc1.iad.aaaaaaaaptqi7soqbtr6uaskgtyc2qjivca3a3ux5s5u4iyhcyx3d66wya6a",
            definedTags: {},
    ...
    it("should give warning result if there are no subnets", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "The VCN does not have any subnets"
        );
        expect(results[0].region).to.equal("us-ashburn-1");
        done();
      };
