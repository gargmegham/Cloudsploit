          ...
          if (dataset.access) {
            for (let rolePermission of dataset.access) {
              ...
              for (let property in rolePermission) {
                if (
                  ...
                  (rolePermission[property].toLowerCase() == "allusers" ||
                    rolePermission[property].toLowerCase() ==
                      "allauthenticatedusers")
                ) {
                  permissionStr.push(
                    `${rolePermission["role"]} access to ${rolePermission[property]}`
                  );
                }
              }
            }
            ...
            if (permissionStr.length) {
              helpers.addResult(
                results,
                2,
                `BigQuery dataset provides ${permissionStr.join(",")}`,
                region,
                resource
              );
            } else {
              helpers.addResult(
                results,
                0,
                "BigQuery dataset does not provide public access",
                region,
                resource
              );
            }
