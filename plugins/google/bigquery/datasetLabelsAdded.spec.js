  ...
  describe("run", function () {
    ...
    it("should give passing result if labels have been added to the dataset", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          "labels found for BigQuery dataset"
        );
        expect(results[0].region).to.equal("global");
        done();
      };

      const cache = createCache(null, [datasets[0]]);
    ...
    it("should give failing result if no labels have been added to the dataset", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "BigQuery dataset does not have any labels"
        );
        expect(results[0].region).to.equal("global");
        done();
      };

      const cache = createCache(null, [datasets[1]]);
