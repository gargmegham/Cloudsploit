...
module.exports = {
  ...
  settings: {
    api_keys_rotation_warn_interval: {
      ...
      description:
        "Return a warning result when api keys exceed this number of days without being rotated",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: "45",
    },
    api_keys_rotation_fail_interval: {
      ...
      description:
        "Return a failing result when api keys exceed this number of days without being rotated",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: "90",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var apiKeyRotationFailInterval = parseInt(
      settings.api_keys_rotation_fail_interval ||
        this.settings.api_keys_rotation_fail_interval.default
    );
    var apiKeyRotationWarnInterval = parseInt(
      settings.api_keys_rotation_warn_interval ||
        this.settings.api_keys_rotation_warn_interval.default
    );
    ...
    if (apiKeys.err || !apiKeys.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query API Keys for project",
        "global",
        null,
        null,
        apiKeys.err
    ...
    apiKeys.data.forEach((key) => {
      ...
      if (diffInDays > apiKeyRotationFailInterval) {
        helpers.addResult(
          results,
          2,
          `API Key was last rotated ${diffInDays} days ago which is greater than ${apiKeyRotationFailInterval}`,
          "global",
          key.name
        );
        ...
        helpers.addResult(
          results,
          1,
          `API Key was last rotated ${diffInDays} days ago which is greater than ${apiKeyRotationWarnInterval}`,
          "global",
          key.name
        );
      } else {
        helpers.addResult(
          results,
          0,
          `API Key was last rotated ${diffInDays} days ago`,
          "global",
          key.name
        );
      }
