...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    apiKeys.data.forEach((key) => {
      ...
      if (key.restrictions) {
        if (
          key.restrictions.browserKeyRestrictions &&
          key.restrictions.browserKeyRestrictions.allowedReferrers &&
          key.restrictions.browserKeyRestrictions.allowedReferrers.length
        ) {
          isRestricted =
            key.restrictions.browserKeyRestrictions.allowedReferrers.every(
              (referrer) => referrer.match(/^(\*\.)?([\w-]+\.)+[\w-]+$/)
            );
        }
        if (
      ...
      if (isRestricted) {
        helpers.addResult(
          results,
          0,
          "API Key usage is restricted to trusted hosts, HTTP referrers, or applications",
          "global",
          key.name
        );
      } else {
        helpers.addResult(
          results,
          2,
          "API Key usage is not restricted to trusted hosts, HTTP referrers, or applications",
          "global",
          key.name
        );
      }
