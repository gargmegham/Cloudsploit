...
module.exports = {
  ...
  remediation_description:
    "All Google Cloud Functions will be configured to require HTTPS for HTTP invocations.",
  ...
  permissions: {
    remediate: ["cloudfunctions.functions.update"],
    rollback: ["cloudfunctions.functions.create	"],
  },
  realtime_triggers: [
      ...
      (region, rcb) => {
        ...
        if (functions.err || !functions.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for Google Cloud Functions: " +
              helpers.addError(functions),
            region,
            null,
            null,
            functions.err
        ...
        if (!functions.data.length) {
          helpers.addResult(
            results,
            0,
            "No Google Cloud functions found",
            region
          );
          return rcb();
        }
        ...
        functions.data.forEach((funct) => {
          ...
          if (funct.httpsTrigger) {
            if (
              funct.httpsTrigger.securityLevel &&
              funct.httpsTrigger.securityLevel == "SECURE_ALWAYS"
            ) {
              helpers.addResult(
                results,
                0,
                "Cloud Function is configured to require HTTPS for HTTP invocations",
                region,
                funct.name
              );
            } else {
              helpers.addResult(
                results,
                2,
                "Cloud Function is not configured to require HTTPS for HTTP invocations",
                region,
                funct.name
              );
            }
            ...
            helpers.addResult(
              results,
              0,
              "Cloud Function trigger type is not HTTP",
              region,
              funct.name
            );
          }
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    var baseUrl =
      "https://cloudfunctions.googleapis.com/v1/{resource}?updateMask=httpsTrigger.securityLevel";
    var method = "PATCH";
