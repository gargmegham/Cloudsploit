    ...
    async.each(
      ...
      (region, rcb) => {
        ...
        functions.data.forEach((func) => {
          ...
          if (
            func.ingressSettings &&
            func.ingressSettings.toUpperCase() == "ALLOW_ALL"
          ) {
            helpers.addResult(
              results,
              2,
              "Cloud Function is configured to allow all traffic",
              region,
              func.name
            );
          } else {
            helpers.addResult(
              results,
              0,
              "Cloud Function is configured to allow only internal and CLB traffic",
              region,
              func.name
            );
          }
