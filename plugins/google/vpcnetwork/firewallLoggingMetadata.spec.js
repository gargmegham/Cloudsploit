...
const networks = [
  {
    ...
    selfLink:
      "https://www.googleapis.com/compute/v1/projects/test-project/global/networks/test-vpc",
  ...
  },
  {
    ...
    description: "plugin vpc",
    selfLink:
      "https://www.googleapis.com/compute/v1/projects/test-project/global/networks/test-vpc-1",
    autoCreateSubnetworks: false,
    routingConfig: {
      routingMode: "REGIONAL",
    },
  ...
  describe("run", function () {
    ...
    it("should give passing result if VPC Network does not have firewall metadata logging enabled", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "VPC Network does not have firewall metadata logging enabled"
        );
        expect(results[0].region).to.equal("global");
        done();
      };
      const cache = createCache([networks[0]], null, [firewalls[0]]);
      plugin.run(cache, {}, callback);
    });
    ...
    it("should give failing result if VPC Network has firewall metadata logging enabled", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "VPC Network has firewall metadata logging enabled"
        );
        expect(results[0].region).to.equal("global");
        done();
      };
      const cache = createCache([networks[1]], null, [firewalls[1]]);
      plugin.run(cache, {}, callback);
    });
  });
});
