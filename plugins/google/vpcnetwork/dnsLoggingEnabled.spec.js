  ...
  describe("run", function () {
    ...
    it("should give passing result if VPC Network has DNS logging enabled", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "VPC Network has DNS logging enabled"
        );
        expect(results[0].region).to.equal("global");
        done();
      };
      const cache = createCache([networks[0]], null, policies);
      plugin.run(cache, {}, callback);
    });
    ...
    it("should give failing result if VPC Network does not have DNS logging enabled", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "VPC Network does not have DNS logging enabled"
        );
        expect(results[0].region).to.equal("global");
        done();
      };
      const cache = createCache([networks[1]], null, policies);
      plugin.run(cache, {}, callback);
    });
  });
});
