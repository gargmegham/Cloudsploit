...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      regions.networks,
      function (region, rcb) {
        let networks = helpers.addSource(cache, source, [
          "networks",
          "list",
          region,
        ]);
        ...
        if (networks.err || !networks.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query networks: " + helpers.addError(networks),
            region,
            null,
            null,
            networks.err
        ...
        let policies = helpers.addSource(cache, source, [
          "policies",
          "list",
          region,
        ]);
        ...
        if (!policies || policies.err || !policies.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query DNS policies: " + helpers.addError(policies),
            region,
            null,
            null,
            policies.err
        ...
        policies.data.forEach((policy) => {
          if (
            policy.enableLogging &&
            policy.networks &&
            policy.networks.length
          ) {
