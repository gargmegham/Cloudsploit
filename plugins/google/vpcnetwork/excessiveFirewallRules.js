...
module.exports = {
  ...
  compliance: {
    pci:
      "PCI has strict requirements to segment networks using firewalls. " +
  ...
  settings: {
    excessive_firewall_rules_fail: {
      ...
      description:
        "Return a failing result when the number of Firewall Rules exceeds this value",
      regex: "^[1-9]{1}[0-9]{0,5}$",
      default: 40,
    },
    excessive_firewall_rules_warn: {
      ...
      description:
        "Return a warning result when the number of Firewall Rules exceeds this value",
      regex: "^[1-9]{1}[0-9]{0,5}$",
      default: 30,
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      excessive_firewall_rules_fail:
        settings.excessive_firewall_rules_fail ||
        this.settings.excessive_firewall_rules_fail.default,
      excessive_firewall_rules_warn:
        settings.excessive_firewall_rules_warn ||
        this.settings.excessive_firewall_rules_warn.default,
    };
    ...
    async.each(
      regions.firewalls,
      function (region, rcb) {
        let firewalls = helpers.addSource(cache, source, [
          "firewalls",
          "list",
          region,
        ]);
        ...
        if (firewalls.data.length > config.excessive_firewall_rules_fail) {
          ...
          firewalls.data.length > config.excessive_firewall_rules_warn
        ) {
          helpers.addResult(
            results,
            1,
