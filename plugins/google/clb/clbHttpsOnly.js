...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      regions.targetHttpProxies,
      function (region, rcb) {
        let urlMaps = helpers.addSource(cache, source, [
          "urlMaps",
          "list",
          region,
        ]);
        ...
        let httpProxies = helpers.addSource(cache, source, [
          "targetHttpProxies",
          "list",
          region,
        ]);
        ...
        for (let urlMap of urlMaps.data) {
          ...
          let found = httpProxies.data.find(
            (proxy) => proxy.urlMap && proxy.urlMap.includes(clbResource)
          );
          ...
          if (found) {
            helpers.addResult(
              results,
              2,
              "Load Balancer is not HTTPS-Only",
              region,
              clbResource
            );
          } else {
