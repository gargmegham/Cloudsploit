  ...
  describe("run", function () {
    it("should give unknown result if a backend service error is passed or no data is present", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Unable to query backend services"
        );
    ...
    it("should give passing result if Security Policy is Attached to a backend service", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          "The backend service has an attached security policy"
        );
        expect(results[0].region).to.equal("global");
        done();
      };
      ...
      const cache = createCache(null, [
        {
          ...
          description: "",
          selfLink:
            "https://www.googleapis.com/compute/v1/projects/rosy-booth-253119/global/backendServices/giotestlb1",
          healthChecks: [
          ...
          securityPolicy:
            "https://www.googleapis.com/compute/v1/projects/rosy-booth-253119/global/securityPolicies/giosecuritypolicy1",
    ...
    it("should give passing result if Security Policy is not Attached to a backend service", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "The backend service does not have an attached security policy"
        );
        expect(results[0].region).to.equal("global");
        done();
      };
