...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      regions.keyRings,
      function (region, rcb) {
        let keyRings = helpers.addSource(cache, source, [
          "keyRings",
          "list",
          region,
        ]);
        ...
        let cryptoKeys = helpers.addSource(cache, source, [
          "cryptoKeys",
          "list",
          region,
        ]);
        ...
        if (cryptoKeys.err || !cryptoKeys.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query cryptographic keys",
            region,
            null,
            null,
            cryptoKeys.err
        ...
        let cryptoKeysIamPolicies = helpers.addSource(cache, source, [
          "cryptoKeys",
          "getIamPolicy",
          region,
        ]);
        ...
        if (
          !cryptoKeysIamPolicies ||
          cryptoKeysIamPolicies.err ||
          !cryptoKeysIamPolicies.data
        ) {
          helpers.addResult(
            results,
            3,
            "Unable to query IAM Policies for Cryptographic Keys: " +
              helpers.addError(cryptoKeysIamPolicies),
            region
          );
          return rcb();
        }
        ...
        cryptoKeys.data.forEach((cryptoKey) => {
          ...
          let keyIamPolicy = cryptoKeysIamPolicies.find(
            (iamPolicy) =>
              iamPolicy.parent && iamPolicy.parent.name === cryptoKey.name
          );

          if (
            !keyIamPolicy ||
            !keyIamPolicy.bindings ||
            !keyIamPolicy.bindings.length
          ) {
            helpers.addResult(
              results,
              0,
              "No IAM Policies found for cryptographic key",
              region,
              cryptoKey.name
            );
          } else {
            ...
            keyIamPolicy.bindings.forEach((roleBinding) => {
              if (
                roleBinding.role &&
                roleBinding.members &&
                roleBinding.members.length &&
                (roleBinding.members.includes("allUsers") ||
                  roleBinding.members.includes("allAuthenticatedUsers"))
              ) {
            ...
            if (!allowedAllUsers) {
              helpers.addResult(
                results,
                0,
                "Cryptographic Key is not publicly accessible",
                region,
                cryptoKey.name
              );
            } else {
              helpers.addResult(
                results,
                2,
                "Cryptographic Key is publicly accessible",
                region,
                cryptoKey.name
              );
            }
