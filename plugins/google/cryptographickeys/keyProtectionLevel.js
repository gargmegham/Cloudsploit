...
module.exports = {
  ...
  settings: {
    kms_crypto_keys_protection_level: {
      ...
      description:
        "Desired protection level for cryptographic keys. cloudcmek: customer managed encryption keys, " +
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredProtectionLevel:
        settings.kms_crypto_keys_protection_level ||
        this.settings.kms_crypto_keys_protection_level.default,
    };
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        cryptoKeys.data.forEach((cryptoKey) => {
          let currentProtectionLevel = helpers.getProtectionLevel(
            cryptoKey,
            helpers.PROTECTION_LEVELS
          );
          let currentProtectionLevelStr =
            helpers.PROTECTION_LEVELS[currentProtectionLevel];
          if (
            currentProtectionLevel >=
            helpers.PROTECTION_LEVELS.indexOf(config.desiredProtectionLevel)
          ) {
            helpers.addResult(
              results,
              0,
