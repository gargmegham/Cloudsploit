...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    let images = helpers.addSource(cache, source, ["images", "list", "global"]);

    if (!images || images.err || !images.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query Disk Images: " + helpers.addError(images),
        "global"
      );
      return callback(null, results, source);
    }
    ...
    if (
      !getImagesIamPolicies ||
      getImagesIamPolicies.err ||
      !getImagesIamPolicies.data
    ) {
      helpers.addResult(
        results,
        3,
        "Unable to query IAM Policies for Disk Images: " +
          helpers.addError(getImagesIamPolicies),
        "global"
      );
      return callback(null, results, source);
    }
    ...
    images.data.forEach((image) => {
      let imageIamPolicy = getImagesIamPolicies.find(
        (iamPolicy) => iamPolicy.parent && iamPolicy.parent.id === image.id
      );
      ...
      if (
        ...
        !imageIamPolicy.bindings.length
      ) {
        helpers.addResult(
          results,
          0,
