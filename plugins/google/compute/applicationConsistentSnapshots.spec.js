...
const schedules = [
  ...
  },
  {
    ...
    selfLink:
      "https://www.googleapis.com/compute/v1/projects/my-project/regions/us-central1/resourcePolicies/schedule-2",
    region:
      "https://www.googleapis.com/compute/v1/projects/my-project/regions/us-central1",
    name: "schedule-2",
    snapshotSchedulePolicy: {
  ...
  describe("run", function () {
    ...
    it("should give passing result if snapshot schedule is configured to take application-consistent snapshots", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          "is configured to take application-consistent snapshots"
        );
        expect(results[0].region).to.equal("us-central1");
        done();
      };

      const cache = createCache([schedules[0]], null);
    ...
    it("should give failing result if snapshot schedule is not configured to take application-consistent snapshots", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "is not configured to take application-consistent snapshots"
        );
        expect(results[0].region).to.equal("us-central1");
        done();
      };

      const cache = createCache([schedules[1]], null);
