...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    var images = helpers.addSource(cache, source, ["images", "list", "global"]);

    if (!images || images.err || !images.data || !images.data.length) {
      helpers.addResult(
        results,
        3,
        "Unable to query for disk images: " + helpers.addError(images),
        "global",
        null,
        null,
        images ? images.err : null
      );
      return callback(null, results, source);
    }
          ...
          function (zone, zcb) {
            ...
            instances.data.forEach((instance) => {
              ...
              if (instance.disks && instance.disks.length) {
                ...
                if (bootDisk) {
                  let diskInformation = disks.data.find(
                    (disk) => disk.selfLink === bootDisk.source
                  );
                  if (diskInformation) {
                    let diskImage = images.data.find(
                      (image) => image.id == diskInformation.sourceImageId
                    );
                    if (diskImage) {
                      if (
                        diskImage.deprecated &&
                        diskImage.deprecated.state &&
                        diskImage.deprecated.state.toUpperCase() == "DEPRECATED"
                      ) {
          ...
          function () {
            if (noInstances.length) {
              helpers.addResult(
                results,
                0,
                `No instances found in following zones: ${noInstances.join(
                  ", "
                )}`,
                region
              );
