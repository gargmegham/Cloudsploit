...
const imagePolicies = [
  {
    ...
    parent: {
      ...
      name: "image-1",
      sourceDisk:
        "https://www.googleapis.com/compute/v1/projects/test-proj/zones/us-central1-a/disks/disk-1",
      sourceDiskId: "4476293856257965646",
      selfLink:
        "https://www.googleapis.com/compute/v1/projects/test-proj/global/images/image-1",
  ...
  },
  {
    ...
    bindings: [
      {
        role: "roles/compute.imageUser",
        members: ["myserviceaccount@gmail.com"],
      },
    ],
    parent: {
      id: 1232,
      ...
      name: "image-2",
      sourceDisk:
        "https://www.googleapis.com/compute/v1/projects/test-proj/zones/us-central1-a/disks/disk-2",
      sourceDiskId: "4476293856257965646",
      selfLink:
        "https://www.googleapis.com/compute/v1/projects/test-proj/global/images/image-2",
      kind: "compute#image",
    },
  },
];
const images = [
  ...
  },
  {
    ...
    name: "image-3",
    sourceDisk:
      "https://www.googleapis.com/compute/v1/projects/test-proj/zones/us-central1-a/disks/disk-3",
    sourceDiskId: "4476293856257965646",
    selfLink:
      "https://www.googleapis.com/compute/v1/projects/test-proj/global/images/image-3",
    kind: "compute#image",
  },
];
...
describe("publicDiskImages", function () {
  describe("run", function () {
    ...
    it("should give unknown if unable to query IAM Policies for disk images", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Unable to query IAM Policies for Disk Images"
        );
        expect(results[0].region).to.equal("global");
        done();
      };

      const cache = createCache(images, null, [], ["error"]);
    ...
    it("should pass if no IAM Policies found", function (done) {
      ...
      const cache = createCache(images, null, [], null);
    ...
    it("should fail if disk image is publicly accessible", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Disk Image is publicly accessible"
        );
        expect(results[0].region).to.equal("global");
        done();
      };

      const cache = createCache([images[0]], null, [imagePolicies[0]], null);
    ...
    it("should pass if no IAM policies are found for disk image", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          "No IAM Policies found for disk image"
        );
        expect(results[0].region).to.equal("global");
        done();
      };

      const cache = createCache([images[2]], null, [imagePolicies], null);
    ...
    it("should pass if disk image is not publicly accessible", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          "Disk Image is not publicly accessible"
        );
        expect(results[0].region).to.equal("global");
        done();
      };

      const cache = createCache([images[1]], null, [imagePolicies[1]], null);
