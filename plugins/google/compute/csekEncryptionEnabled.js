  ...
  run: function (cache, settings, callback) {
    ...
    regions.all_regions.forEach((region) => {
      ...
      if (regionData && regionData["disks"] && regionData["disks"].length) {
        disksInRegion = disks.data[`regions/${region}`].disks.map((disk) => {
          return { ...disk, locationType: "region", location: region };
        });
      }
      ...
      zones[region].forEach((zone) => {
        ...
        if (zoneData && zoneData["disks"] && zoneData["disks"].length) {
          disksInZone = disks.data[`zones/${zone}`].disks.map((disk) => {
            return { ...disk, locationType: "zone", location: zone };
          });
        }
      ...
      disksInRegion.forEach((disk) => {
        ...
        if (
          ...
          helpers.addResult(
            results,
            0,
            "CSEK Encryption is enabled for disk",
            region,
            resource
          );
          ...
          helpers.addResult(
            results,
            2,
            "CSEK Encryption is disabled for disk",
            region,
            resource
          );
        }
