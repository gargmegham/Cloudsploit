...
module.exports = {
  ...
  settings: {
    disk_encryption_level: {
      ...
      description:
        "Desired protection level for Virtual Machine Disk. default: google-managed, cloudcmek: customer managed encryption keys, " +
              ...
              async.each(
                ...
                function (zone, zcb) {
                  ...
                  disks.data.forEach((disk) => {
                    if (!disk.id || !disk.selfLink || !disk.creationTimestamp)
                      return;
                    ...
                    if (
                      ...
                      disk.diskEncryptionKey.kmsKeyName
                    ) {
                      currentEncryptionLevel = helpers.getProtectionLevel(
                        keysArr.find(
                          (key) =>
                            key.name &&
                            disk.diskEncryptionKey.kmsKeyName.includes(key.name)
                        ),
                ...
                function () {
                  if (noDisks.length) {
                    helpers.addResult(
                      results,
                      0,
                      `No compute disks found in following zones: ${noDisks.join(
                        ", "
                      )}`,
                      region
                    );
