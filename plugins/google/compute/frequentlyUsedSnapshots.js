...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    var snapshots_to_check =
      settings.snapshots_to_check || this.settings.snapshots_to_check.default;
    ...
    if (snapshots.err || !snapshots.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query for disk snapshots: " + helpers.addError(snapshots),
        "global"
      );
      return callback(null, results, source);
    }
    ...
    if (images.err || !images.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query for images: " + helpers.addError(images),
        "global"
      );
      return callback(null, results, source);
    }

    let snapshotsToCheck = snapshots.data.filter(
      (snapshot) => snapshot.name && snapshots_to_check.includes(snapshot.name)
    );
    ...
    snapshotsToCheck.forEach((snapshot) => {
      ...
      if (
        ...
        images.data.find(
          (image) =>
            image.sourceSnapshotId && image.sourceSnapshotId == snapshot.id
        )
      ) {
        helpers.addResult(
          results,
          0,
          "Disk snapshot has an image created",
          "global",
          resource
        );
      } else {
        helpers.addResult(
          results,
          2,
          "Disk snapshot does not have an image created",
          "global",
          resource
        );
      }
