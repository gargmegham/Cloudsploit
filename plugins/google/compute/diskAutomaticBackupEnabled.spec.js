  ...
  describe("run", function () {
    ...
    it("should fail if snapshot schedule is not configured for disk", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Snapshot schedule is not configured for"
        );
        expect(results[0].region).to.equal("us-east1");
        done();
      };
          ...
          disks: [
            {
              ...
              status: "READY",
              selfLink:
                "https://www.googleapis.com/compute/v1/projects/my-test-project/regions/us-east1/disks/disk-1",
              type: "https://www.googleapis.com/compute/v1/projects/my-test-project/regions/us-east1/diskTypes/pd-balanced",
              labelFingerprint: "42WmSpB8rSM=",
              region:
                "https://www.googleapis.com/compute/v1/projects/my-test-project/regions/us-east1",
    ...
    it("should pass if snapshot schedule is configured for disk", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          "Snapshot schedule is configured for"
        );
        expect(results[0].region).to.equal("us-east1");
        done();
      };
