          ...
          function (zone, zcb) {
            ...
            instanceGroupManagers.data.forEach((instanceGroupManager) => {
              ...
              if (
                ...
                instanceGroupManager.autoHealingPolicies.length
              ) {
                helpers.addResult(
                  results,
                  0,
                  "Instance Group has auto healing enabled",
                  region,
                  resource
                );
              } else {
                helpers.addResult(
                  results,
                  2,
                  "Instance Group does not have auto healing enabled",
                  region,
                  resource
                );
              }
          ...
          function () {
            if (noInstanceGroups.length) {
              helpers.addResult(
                results,
                0,
                `No instance groups found in following zones: ${noInstanceGroups.join(
                  ", "
                )}`,
                region
              );
