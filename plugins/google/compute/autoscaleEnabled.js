...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      instanceGroups,
      function (instanceGroupsInLocation, rcb) {
      ...
      function () {
        ...
        if (autoscalers.length) {
          async.each(
            autoscalers,
            function (autoscalersInLocation, lcb) {
            ...
            function () {
              if (Object.keys(instanceGroupURLObj).length) {
                for (let group in instanceGroupURLObj) {
                  let groupLocArr = instanceGroupURLObj[group].zone
                    ? instanceGroupURLObj[group].zone.split("/")
                    : instanceGroupURLObj[group].region
                    ? instanceGroupURLObj[group].region.split("/")
                    : ["global"];
                  let groupLoc = groupLocArr[groupLocArr.length - 1];
                  let resourceType = instanceGroupURLObj[group].zone
                    ? "zone"
                    : instanceGroupURLObj[group].region
                    ? "region"
                    : "global";
                  let resource = helpers.createResourceName(
                    "instanceGroups",
                    instanceGroupURLObj[group].name,
                    project,
                    resourceType,
                    groupLoc
                  );
                  ...
                  helpers.addResult(
                    results,
                    2,
                    "Instance group does not have autoscale enabled",
                    region,
                    resource
                  );
                }
                ...
                helpers.addResult(
                  results,
                  0,
                  "All instance groups have autoscale enabled",
                  "global"
                );
              }
