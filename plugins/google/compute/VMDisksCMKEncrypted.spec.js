...
const disks = [
  {
    ...
    diskEncryptionKey: {
      kmsKeyName:
        "projects/test-dev/locations/global/keyRings/test-kr/cryptoKeys/test-key-1/cryptoKeyVersions/1",
    },
...
describe("VMDisksCMKEncrypted", function () {
  describe("run", function () {
    it("should give unknown result if a disk error occurs", function (done) {
      ...
      const cache = createCache(null, ["error"], null, ["error"]);
    ...
    it("should pass if disk encryption level is equal to or greater than desired level", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "greater than or equal to desired encryption level"
        );
        expect(results[0].region).to.equal("us-central1");
        done();
      };
    ...
    it("should fail if disk encryption level is less than desired level", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "less than desired encryption level"
        );
        expect(results[0].region).to.equal("us-central1");
        done();
      };
