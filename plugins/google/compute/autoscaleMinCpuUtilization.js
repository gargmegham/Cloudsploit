...
module.exports = {
  ...
  settings: {
    minimum_cpu_utilization_target: {
      ...
      description:
        "Value between 1-100 for the Autoscale Minimum CPU Utilization Target",
      regex: "^(100|[1-9][0-9]?)$",
      default: "false",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var minimum_cpu_utilization_target =
      settings.minimum_cpu_utilization_target ||
      this.settings.minimum_cpu_utilization_target.default;

    if (minimum_cpu_utilization_target == "false")
    ...
    if (instanceGroupsObj.err || !instanceGroupsObj.data) {
      helpers.addResult(
        ...
        null,
        null,
        instanceGroupsObj.err
      );
      return callback(null, results, source);
    }

    var instanceGroups = Object.values(instanceGroupsObj.data).filter(
      (instanceGroup) => {
    ...
    if (autoscalersObj.err || !autoscalersObj.data) {
      ...
      var autoscalers = Object.values(autoscalersObj.data).filter(
        (autoscaler) => {
      ...
      async.each(instanceGroups, function (instanceGroupsInLocation, rcb) {
        instanceGroupsInLocation.instanceGroups.forEach((instanceGroup) => {
          let groupLocArr = instanceGroup.zone
            ? instanceGroup.zone.split("/")
            : instanceGroup.region
            ? instanceGroup.region.split("/")
            : ["global"];
          let groupLoc = groupLocArr[groupLocArr.length - 1];
          let resourceType = instanceGroup.zone
            ? "zone"
            : instanceGroup.region
            ? "region"
            : "global";
          ...
          let region =
            resourceType == "zone"
              ? groupLoc.substr(0, groupLoc.length - 2)
              : groupLoc;
            ...
            autoscalers.forEach((scaler) => {
              if (scaler && scaler.autoscalers && scaler.autoscalers.length) {
                if (instanceGroup && instanceGroup.name) {
                  let autoScalingData = scaler.autoscalers.find(
                    (scalerObj) => scalerObj.name == instanceGroup.name
                  );

                  if (
                    autoScalingData &&
                    autoScalingData.autoscalingPolicy &&
                    ...
                    if (
                      ...
                      helpers.addResult(
                        results,
                        0,
                        "Instance group has desired minimum cpu utilization target",
                        region,
                        resource
                      );
                    } else {
                      helpers.addResult(
                        results,
                        2,
                        "Instance group does not have desired minimum cpu utilization target",
                        region,
                        resource
                      );
                    }
                    ...
                    helpers.addResult(
                      results,
                      0,
                      "No auto scaling policies found for this instance group",
                      region,
                      resource
                    );
                  }
