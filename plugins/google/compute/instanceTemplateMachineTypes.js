...
module.exports = {
  ...
  settings: {
    instance_template_machine_types: {
      ...
      description:
        "Desired Google Cloud Virtual Machine instance template type",
      regex: "^.*$",
      default: "",
    },
  },
  run: function (cache, settings, callback) {
    ...
    var instance_template_machine_types =
      settings.instance_template_machine_types ||
      this.settings.instance_template_machine_types.default;

    if (!instance_template_machine_types.length)
    ...
    async.each(
      regions.instanceTemplates,
      function (region, rcb) {
        let instanceTemplates = helpers.addSource(cache, source, [
          "instanceTemplates",
          "list",
          region,
        ]);
        ...
        if (instanceTemplates.err || !instanceTemplates.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query instance templates",
            region,
            null,
            null,
        ...
        instanceTemplates.data.forEach((instanceTemplate) => {
          if (
            ...
            instanceTemplate.properties.machineType &&
            instance_template_machine_types.includes(
              instanceTemplate.properties.machineType
            )
          ) {
            helpers.addResult(
              results,
              0,
              "Virtual Machine instance template has desired machine type",
              region
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Virtual Machine instance template does not have desired machine type",
              region
            );
          }
        });
