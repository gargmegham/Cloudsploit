...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    var instance_desired_machine_types =
      settings.instance_desired_machine_types ||
      this.settings.instance_desired_machine_types.default;
    if (!instance_desired_machine_types.length)
          ...
          function (zone, zcb) {
            ...
            instances.data.forEach((instance) => {
              ...
              if (
                ...
                instance_desired_machine_types.includes(
                  instance.machineType.split("machineTypes/")[1]
                )
              ) {
                helpers.addResult(
                  results,
                  0,
                  "Virtual Machine instance has desired machine type",
                  region,
                  resource
                );
              } else {
                helpers.addResult(
                  results,
                  2,
                  "Virtual Machine instance does not have desired machine type",
                  region,
                  resource
                );
              }
