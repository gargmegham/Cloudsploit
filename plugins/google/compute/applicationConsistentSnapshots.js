      ...
      (region, rcb) => {
        ...
        if (resourcePolicies.err || !resourcePolicies.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for snapshot schedules: " +
              helpers.addError(resourcePolicies),
            region,
            null,
            null,
            resourcePolicies.err
          );
          return rcb();
        }
        ...
        resourcePolicies.data.forEach((policy) => {
          ...
          if (
            policy.snapshotSchedulePolicy &&
            policy.snapshotSchedulePolicy.snapshotProperties &&
            policy.snapshotSchedulePolicy.snapshotProperties.guestFlush
          ) {
            helpers.addResult(
              results,
              0,
              "Snapshot schedule is configured to take application-consistent snapshots",
              region,
              policy.name
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Snapshot schedule is not configured to take application-consistent snapshots",
              region,
              policy.name
            );
          }
