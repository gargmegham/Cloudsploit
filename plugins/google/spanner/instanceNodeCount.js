...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      spanner_allowed_instance_node_count: parseInt(
        settings.spanner_allowed_instance_node_count ||
          this.settings.spanner_allowed_instance_node_count.default
      ),
    };
      ...
      function (region, rcb) {
        ...
        if (instances.err || !instances.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query Spanner instances: " + helpers.addError(instances),
            region,
            null,
            null,
            instances.err
        ...
        instances.data.forEach((spannerInstance) => {
          ...
          let resultStatus =
            nodeCount <= config.spanner_allowed_instance_node_count ? 0 : 2;
