      ...
      function (region, rcb) {
        ...
        if (instances.err || !instances.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query BigTable instances",
            region,
            null,
            null,
            instances.err
        ...
        instances.data.forEach((instance) => {
          if (instance.labels && Object.keys(instance.labels).length) {
            helpers.addResult(
              results,
              0,
              `${
                Object.keys(instance.labels).length
              } labels found for BigTable instance`,
              region,
              instance.name
            );
          } else {
            helpers.addResult(
              results,
              2,
              "BigTable instance does not have any labels",
              region,
              instance.name
            );
          }
