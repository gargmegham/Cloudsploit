...
module.exports = {
  ...
  settings: {
    dataflow_job_encryption_level: {
      ...
      description:
        "Desired protection level for Dataflow jobs. default: google-managed, cloudcmek: customer managed encryption keys, " +
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelStr:
        settings.dataflow_job_encryption_level ||
        this.settings.dataflow_job_encryption_level.default,
    };

    var desiredEncryptionLevel = helpers.PROTECTION_LEVELS.indexOf(
      config.desiredEncryptionLevelStr
    );
    var keysObj = {};
            ...
            function (jregion, jrcb) {
              ...
              if (jobs.err || !jobs.data) {
                helpers.addResult(
                  results,
                  3,
                  "Unable to query Dataflow jobs: " + helpers.addError(jobs),
                  jregion
                );
                return jrcb();
              }
                ...
                (job, jcb) => {
                  ...
                  if (job.type && job.type.toUpperCase() != "JOB_TYPE_BATCH") {
                    helpers.addResult(
                      results,
                      0,
                      `CMEK is not supported for ${job.type}`,
                      jregion,
                      resource
                    );
                    return jcb();
                  ...
                  if (
                    ...
                    !job.environment.serviceKmsKeyName.length
                  ) {
