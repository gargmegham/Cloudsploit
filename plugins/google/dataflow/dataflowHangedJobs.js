...
module.exports = {
  ...
  more_info:
    "Cloud Dataflow jobs transit between different states and normally reach terminal state. If they stay in same state " +
  ...
  settings: {
    dataflow_job_state_time: {
      ...
      description:
        "Maximum allowed amount of time in hours for a Dataflow job state",
      regex: "^(0?[1-9]|[1-9][0-9])$",
      default: "6",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      dataflow_job_state_time: parseInt(
        settings.dataflow_job_state_time ||
          this.settings.dataflow_job_state_time.default
      ),
    };
    ...
    async.each(
      regions.jobs,
      function (region, rcb) {
        let jobs = helpers.addSource(cache, source, ["jobs", "list", region]);
        ...
        async.each(
          ...
          (job, cb) => {
            ...
            if (
              ...
              ].includes(job.currentState.toUpperCase())
            ) {
              helpers.addResult(
                results,
                0,
                "Dataflow job has completed",
                region,
                resource
              );
              return cb();
