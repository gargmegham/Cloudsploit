...
module.exports = {
  ...
  settings: {
    deployments_expiration_time: {
      ...
      description:
        "Number of days from creation of depoyment after which it should be considered expired",
      regex: "^.*$",
      default: false,
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      deployments_expiration_time:
        settings.deployments_expiration_time ||
        this.settings.deployments_expiration_time.default,
    };

    if (config.deployments_expiration_time === false)
      return callback(null, results, source);

    config.deployments_expiration_time = parseInt(
      config.deployments_expiration_time
    );
      ...
      function (region, rcb) {
        ...
        if (deployments.err || !deployments.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query Deployment Manager deployments: " +
              helpers.addError(deployments),
            region,
            null,
            null,
            deployments.err
        ...
        if (!deployments.data.length) {
          helpers.addResult(
            results,
            0,
            "No Deployment Manager deployments found",
            region
          );
          return rcb();
        }
        ...
        deployments.data.forEach((deployment) => {
          ...
          let difference = Math.round(
            (new Date(now).getTime() - new Date(insertTime).getTime()) /
              (24 * 60 * 60 * 1000)
          );
          ...
          if (difference > config.deployments_expiration_time) {
            helpers.addResult(
              results,
              2,
              `Deployment Manager deployment was created ${difference} days ago and has expired`,
              region,
              resource
            );
          } else {
            helpers.addResult(
              results,
              0,
              `Deployment Manager deployment was created ${difference} days ago`,
              region,
              resource
            );
          }
