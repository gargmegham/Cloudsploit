...
module.exports = {
  ...
  compliance: {
    ...
    hipaa:
      "HIPAA requires the logging of all activity " +
      ...
      function (region, rcb) {
        ...
        if ((metrics.err && metrics.err.length > 0) || !metrics.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for log metrics: " + helpers.addError(metrics),
            region,
            null,
            null,
            metrics.err
        ...
        if (
          (alertPolicies.err && alertPolicies.err.length > 0) ||
          !alertPolicies.data
        ) {
          helpers.addResult(
            results,
            3,
            "Unable to query for log alert policies: " +
              helpers.addError(alertPolicies),
            region,
            null,
            null,
            alertPolicies.err
        ...
        if (disabled) {
          helpers.addResult(
            results,
            2,
            "Log metric for storage permission changes is disbled",
            region
          );
          ...
          alertPolicies.data.forEach((alertPolicy) => {
            ...
            if (alertPolicy.conditions && alertPolicy.conditions.length) {
              alertPolicy.conditions.forEach((condition) => {
                ...
                if (
                  ...
                  var conditionFilter =
                    condition.conditionThreshold.filter.split('"')[1];
                  if (conditionFilter === metricName) {
                    conditionFound = true;
                    helpers.addResult(
                      results,
                      0,
                      "Log alert for storage permission changes is enabled",
                      region,
                      alertPolicy.name
                    );
                  }
          ...
          if (!conditionFound) {
            helpers.addResult(
              results,
              2,
              "Log alert for storage permission changes not found",
              region
            );
          }
        } else {
          helpers.addResult(
            results,
            2,
            "Log metric for storage permission changes not found",
            region
          );
        }
