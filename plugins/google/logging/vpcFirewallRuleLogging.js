      ...
      function (region, rcb) {
        ...
        if (disabled) {
          helpers.addResult(
            results,
            2,
            "Log metric for firewall rule changes is disbled",
            region
          );
                ...
                if (
                  ...
                  if (conditionFilter === metricName) {
                    ...
                    helpers.addResult(
                      results,
                      0,
                      "Log alert for firewall rule changes is enabled",
                      region,
                      alertPolicy.name
                    );
                  }
          ...
          if (!conditionFound) {
            helpers.addResult(
              results,
              2,
              "Log alert for firewall rule changes not found",
              region
            );
          }
        } else {
          helpers.addResult(
            results,
            2,
            "Log metric for firewall rule changes not found",
            region
          );
        }
