...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      regions.sinks,
      function (region, rcb) {
        let sinks = helpers.addSource(cache, source, ["sinks", "list", region]);
        ...
        if (sinks.err || !sinks.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query sinks: " + helpers.addError(sinks),
            region,
            null,
            null,
            sinks.err
        ...
        sinks.data.forEach((sink) => {
          if (
            (!sink.filter || (sink.filter && sink.filter === "")) &&
            sink.destination
          ) {
            ...
            noSinks = false;
            sinkName = sink.name;
            sinkResource = helpers.createResourceName(
              "sinks",
              sinkName,
              project
            );
        ...
        if (bucketName.length) {
          ...
          if (!buckets || buckets.err || !buckets.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query buckets: " + helpers.addError(buckets),
              region,
              null,
              null,
              buckets.err
            ...
            if (logBucket) {
              let bucketResource = helpers.createResourceName(
                "b",
                logBucket.name
              );
              helpers.addResult(
              ...
              if (logBucket.versioning && logBucket.versioning.enabled) {
                helpers.addResult(
                  results,
                  0,
              ...
              helpers.addResult(
                results,
                2,
                `The log bucket: ${bucketName} does not exist`,
                region,
                sinkResource
              );
            }
