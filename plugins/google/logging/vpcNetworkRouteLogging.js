      ...
      function (region, rcb) {
        ...
        if (disabled) {
          helpers.addResult(
            results,
            2,
            "Log metric for VPC network route changes is disbled",
            region
          );
                ...
                if (
                  ...
                  if (conditionFilter === metricName) {
                    ...
                    helpers.addResult(
                      results,
                      0,
                      "Log alert for VPC network route changes is enabled",
                      region,
                      alertPolicy.name
                    );
                  }
          ...
          if (!conditionFound) {
            helpers.addResult(
              results,
              2,
              "Log alert for VPC network route changes not found",
              region
            );
          }
        } else {
          helpers.addResult(
            results,
            2,
            "Log metric for VPC network route changes not found",
            region
          );
        }
