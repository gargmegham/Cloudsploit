...
module.exports = {
  ...
  settings: {
    check_org_audit_logs: {
      ...
      description:
        "If set to true, check if audit logging is enabled on organization level. If enabled on organization level, " +
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      check_org_audit_logs:
        settings.check_org_audit_logs ||
        this.settings.check_org_audit_logs.default,
    };
    ...
    if (config.check_org_audit_logs) {
      ...
      if (getIamPolicy.err || !getIamPolicy.data) {
        helpers.addResult(
          results,
          3,
          "Unable to query for IAM policies for org",
          "global",
          null,
          null,
          getIamPolicy.err
      ...
      if (iamPolicy && iamPolicy.auditConfigs) {
        let auditConfig = iamPolicy.auditConfigs.find(
          (auditConfig) =>
            auditConfig.service && auditConfig.service === "allServices"
        );
        if (
          auditConfig &&
          ...
          let auditLogConfigs = auditConfig.auditLogConfigs.filter(
            (auditLogConfig) => {
              return (
                ["ADMIN_READ", "DATA_READ", "DATA_WRITE"].indexOf(
          ...
          let exemptedMembers = auditConfig.auditLogConfigs.filter(
            (auditLogConfig) => {
              return (
                auditLogConfig.exemptedMembers &&
                auditLogConfig.exemptedMembers.length
              );
            }
          );
        ...
        if (!enabledOnOrg) {
          let loggingEnabledServices = iamPolicy.auditConfigs.filter(
            (config) => {
              return (
                config.auditLogConfigs.filter((auditLogConfig) => {
                  return (
                    ["ADMIN_READ", "DATA_READ", "DATA_WRITE"].indexOf(
                      auditLogConfig.logType
                    ) > -1
        ...
        if (enabledOnOrg) {
          helpers.addResult(
            results,
            0,
            "Audit logging is enabled on the organization",
            "global"
          );
        }
      }
      ...
      function (region, rcb) {
        ...
        if (iamPolicies.err || !iamPolicies.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for IAM policies",
            region,
            null,
            null,
            iamPolicies.err
        ...
        if (iamPolicy && iamPolicy.auditConfigs) {
          ...
          if (
            ...
            var auditLogConfigs = auditConfig.auditLogConfigs.filter(
              (auditLogConfig) => {
                return (
                  ["ADMIN_READ", "DATA_READ", "DATA_WRITE"].indexOf(
                    auditLogConfig.logType
            ...
            var exemptedMembers = auditConfig.auditLogConfigs.filter(
              (auditLogConfig) => {
                return (
                  auditLogConfig.exemptedMembers &&
                  auditLogConfig.exemptedMembers.length
            ...
            if (auditLogConfigs.length < 3) {
              status = 2;
              message =
                "Audit logging is not properly configured on the project";
          ...
          if (typeof status == "undefined" || status > 0) {
            ...
            if (iamPolicy.auditConfigs && iamPolicy.auditConfigs.length) {
              loggingEnabledServices = iamPolicy.auditConfigs.filter(
                (config) => {
                  return (
                    config.auditLogConfigs.filter((auditLogConfig) => {
                      return (
            ...
            if (
              loggingEnabledServices &&
              loggingEnabledServices.length >= 100 &&
              !projectExemptedMembers
            ) {
              status = 0;
              message = "Audit logging is enabled on the project";
            } else if (
              loggingEnabledServices &&
              loggingEnabledServices.length &&
              projectExemptedMembers
            ) {
              status = 2;
              message =
                "Audit logging has exempted members for some services in the project";
            } else if (
              loggingEnabledServices &&
              loggingEnabledServices.length
            ) {
        ...
        if (!foundLoggingConfig) {
          helpers.addResult(
            results,
            2,
            "Audit logging is not enabled on the project",
            region
          );
        }
