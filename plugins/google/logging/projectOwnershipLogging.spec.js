...
describe("projectOwnershipLogging", function () {
  describe("run", function () {
    ...
    it("should give passing result if no alert policies are found", function (done) {
      ...
      const cache = createCache(null, ["data"], []);
      ...
      const cache = createCache(
        ...
        [
          ...
          },
          {
            ...
            valueExtractor:
              "EXTRACT(protoPayload.authorizationInfo.permission)",
            bucketOptions: {
              exponentialBuckets: {
                numFiniteBuckets: 64,
                growthFactor: 2,
            ...
            conditions: [
              {
                conditionThreshold: {
                  filter:
                    'metric.type="logging.googleapis.com/user/ProjectOwnershipAssignmentsChanges" resource.type="metric"',
            ...
            conditions: [
              {
                conditionThreshold: {
                  filter:
                    'metric.type="logging.googleapis.com/user/loggingChanges" resource.type="metric"',
                  comparison: "COMPARISON_GT",
                  thresholdValue: 0.001,
                  duration: "60s",
                  trigger: {
