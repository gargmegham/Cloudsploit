      ...
      function (region, rcb) {
        ...
        sqlInstances.data.forEach((sqlInstance) => {
          ...
          if (
            ...
            !sqlInstance.databaseVersion.toLowerCase().includes("mysql")
          ) {
            helpers.addResult(
              results,
              0,
              "SQL instance database type is not of MySQL type",
              region,
              resource
            );
            return;
          ...
          if (
            sqlInstance.databaseVersion &&
            parseFloat(
              sqlInstance.databaseVersion.split("MYSQL_")[1].replace("_", ".")
            ) >= latestMySQLVersion
          ) {
            helpers.addResult(
              results,
              0,
              `SQL instance is using MySQL major version ${sqlInstance.databaseVersion} which is the latest version`,
              region,
              resource
            );
          } else {
            helpers.addResult(
              results,
              2,
              `SQL instance is using MySQL major version ${sqlInstance.databaseVersion} which is not the latest version`,
              region,
              resource
            );
          }
