  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        let backupRuns = helpers.addSource(cache, source, [
          "backupRuns",
          "list",
          region,
        ]);
        ...
        if (!backupRuns || backupRuns.err || !backupRuns.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query SQL backup runs: ${helpers.addError(backupRuns)}`,
            region
          );
          return rcb();
        }
        ...
        sqlInstances.data.forEach((sqlInstance) => {
          ...
          let found = backupRuns.data.find(
            (backup) =>
              backup.instance &&
              sqlInstance.name &&
              backup.instance == sqlInstance.name
          );
          ...
          if (found) {
            helpers.addResult(
              results,
              0,
              "SQL instance has backup available",
              region,
              resource
            );
          } else {
            helpers.addResult(
              results,
              2,
              "SQL instance does not have backups available",
              region,
              resource
            );
          }
