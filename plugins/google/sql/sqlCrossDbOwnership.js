...
module.exports = {
  ...
  more_info:
    "SQL databases of SQL Server provide cross DB ownership chaining flag. It is used to configure cross-database ownership chaining " +
      ...
      function (region, rcb) {
        ...
        sqlInstances.data.forEach((sqlInstance) => {
          ...
          if (
            ...
            !sqlInstance.databaseVersion.toLowerCase().includes("sqlserver")
          ) {
            helpers.addResult(
              results,
              0,
              "SQL instance database type is not of SQL Server type",
              region,
              resource
            );
            return;
          ...
          if (
            ...
            let found = sqlInstance.settings.databaseFlags.find(
              (flag) =>
                flag.name &&
            ...
            if (found) {
              helpers.addResult(
                results,
                0,
                "SQL instance has cross DB ownership chaining flag disabled",
                region,
                resource
              );
            } else {
              helpers.addResult(
                results,
                2,
                "SQL instance has cross DB ownership chaining flag enabled",
                region,
                resource
              );
            }
            ...
            helpers.addResult(
              results,
              0,
              "SQL instance does not have any flags",
              region,
              resource
            );
          }
