...
module.exports = {
  ...
  settings: {
    log_min_error_statement: {
      ...
      description:
        "Return a passing result if the flag value is used from the setting list.",
      ...
      function (region, rcb) {
        ...
        sqlInstances.data.forEach((sqlInstance) => {
          ...
          if (
            ...
            !sqlInstance.databaseVersion.toLowerCase().includes("postgres")
          ) {
            helpers.addResult(
              results,
              0,
              "SQL instance database type is not of PostgreSQL type",
              region,
              resource
            );
            return;
          ...
          if (
            ...
            let found = sqlInstance.settings.databaseFlags.find(
              (flag) =>
                ...
                flag.value &&
                flag.value == log_min_error_statement
            );
            ...
            if (found) {
              helpers.addResult(
                results,
                0,
                "SQL instance have log_min_error_statement flag set to Error",
                region,
                resource
              );
            } else {
              helpers.addResult(
                results,
                2,
                "SQL instance does not have log_min_error_statement flag set to Error",
                region,
                resource
              );
            }
