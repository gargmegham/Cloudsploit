...
module.exports = {
  ...
  settings: {
    server_certicate_expiration_threshold: {
      ...
      description:
        "Number of days in future before which SSL certiciates should be rotated",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: "30",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      expiryThreshold: parseInt(
        settings.server_certicate_expiration_threshold ||
          this.settings.server_certicate_expiration_threshold.default
      ),
    };
          ...
          if (
            ...
            if (difference >= config.expiryThreshold) {
              helpers.addResult(
                results,
                0,
                `SQL instance SSL certificate will expire after ${difference} days`,
                region,
                resource
              );
              ...
              helpers.addResult(
                results,
                2,
                "SQL instance SSL certificate has already expired",
                region,
                resource
              );
            } else {
            ...
            helpers.addResult(
              results,
              3,
              "Unable to find certicite info for instance",
              region,
              resource
            );
          }
