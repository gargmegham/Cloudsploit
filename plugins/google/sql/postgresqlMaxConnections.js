...
module.exports = {
  ...
  settings: {
    ...
    allow_default_max_connections_value: {
      ...
      description:
        "True or false, whether to allow PostgreSQL instances to use default value for max_connections flag. " +
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      maxConnections:
        settings.min_postgres_max_connections ||
        this.settings.min_postgres_max_connections.default,
      allow_default:
        settings.allow_default_max_connections_value ||
        this.settings.allow_default_max_connections_value.default,
    };
      ...
      function (region, rcb) {
        ...
        if (sqlInstances.err || !sqlInstances.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query SQL instances: " + helpers.addError(sqlInstances),
            region
          );
          return rcb();
        }
        ...
        sqlInstances.data.forEach((sqlInstance) => {
          if (
            sqlInstance.instanceType &&
            sqlInstance.instanceType.toUpperCase() == "READ_REPLICA_INSTANCE"
          )
            return;
          ...
          if (
            sqlInstance.databaseVersion &&
            !sqlInstance.databaseVersion.toUpperCase().startsWith("POSTGRES")
          ) {
            helpers.addResult(
              results,
              0,
              "SQL instance database version is not of PostgreSQL type",
              region,
              resource
            );
            return;
          ...
          if (
            ...
            sqlInstance.settings.databaseFlags.find(
              (flag) => flag.name && flag.name == "max_connections"
            )
          ) {
            let maxConnectionsFlag =
              sqlInstance.settings.databaseFlags.find(
                (flag) => flag.name && flag.name == "max_connections"
              ) || {};
            ...
            if (maxConnections >= config.maxConnections) {
              helpers.addResult(
                results,
                0,
                `PostgreSQL instance max_connection value is ${maxConnections} which is greater than or equal to ${config.maxConnections}`,
                region,
                resource
              );
            } else {
              helpers.addResult(
                results,
                2,
                `PostgreSQL instance max_connection value is ${maxConnections} which is les than ${config.maxConnections}`,
                region,
                resource
              );
            }
            ...
            if (config.allow_default) {
              helpers.addResult(
                results,
                0,
                "PostgreSQL instance does not have max_connections value set and is using default value",
                region,
                resource
              );
            } else {
