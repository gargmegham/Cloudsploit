...
module.exports = {
  ...
  compliance: {
    hipaa:
      "SQL instances should only be launched in VPC environments and " +
    ...
    pci:
      "PCI requires backend services to be properly firewalled. " +
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        sqlInstances.data.forEach((sqlInstance) => {
          ...
          if (sqlInstance.settings && sqlInstance.settings.ipConfiguration) {
            myIpConfig = sqlInstance.settings.ipConfiguration;
            if (myIpConfig.privateNetwork && !myIpConfig.ipv4Enabled) {
              helpers.addResult(
                results,
                0,
                "SQL Instance is not publicly accessible",
                region,
                resource
              );
            } else if (
              myIpConfig.ipv4Enabled &&
              ...
              if (openNetwork) {
                helpers.addResult(
                  results,
                  2,
                  "SQL Instance is publicly accessible by all IP addresses",
                  region,
                  resource
                );
                ...
                helpers.addResult(
                  results,
                  1,
                  "SQL Instance is publicly accessible by specific IP addresses",
                  region,
                  resource
                );
              } else {
