...
module.exports = {
  ...
  more_info:
    "When this feature is enabled, Cloud SQL checks your available storage every 30 seconds. If the available storage falls below a threshold size, " +
  ...
  settings: {
    sql_storage_auto_increase_limit: {
      ...
      description:
        "Maximum limit (GBs) of automatic storage increase for SQL instances. Should be between 100 and 30720. " +
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      maxLimit: parseInt(
        settings.sql_storage_auto_increase_limit ||
          this.settings.sql_storage_auto_increase_limit.default
      ),
    };
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        sqlInstances.data.forEach((sqlInstance) => {
          ...
          if (sqlInstance.settings && sqlInstance.settings.storageAutoResize) {
            let resizeLimit = sqlInstance.settings.storageAutoResizeLimit
              ? parseInt(sqlInstance.settings.storageAutoResizeLimit)
              : 0;
            ...
            helpers.addResult(
              results,
              2,
              "SQL instance has automatic storage increase disabled",
              region,
              resource
            );
          }
