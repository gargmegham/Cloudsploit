...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      regions.users,
      function (region, rcb) {
        let users = helpers.addSource(cache, source, ["users", "list", region]);
        ...
        if (users.err || !users.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query SQL users: " + helpers.addError(users),
            region,
            null,
            null,
            users.err
          ...
          if (user.name && user.name === "root") {
            ...
            if (
              ...
              helpers.addResult(
                results,
                2,
                "The root user has access to the instance from any host",
                region,
                resource
              );
            } else {
              helpers.addResult(
                results,
                0,
                "The root user does not have access to the instance from any host",
                region
              );
            }
          }
