        ...
        sqlInstances.data.forEach((sqlInstance) => {
          ...
          if (
            ...
            helpers.addResult(
              results,
              0,
              "SQL instance database type is not of POSTGRES type",
              region,
              resource
            );
            return;
          ...
          if (
            sqlInstance.databaseVersion &&
            parseInt(
              sqlInstance.databaseVersion.toUpperCase().split("POSTGRES_")[1]
            ) >= latestPostgreSQLVersion
          ) {
            helpers.addResult(
              results,
              0,
              `SQL instance is using Postgres major version ${sqlInstance.databaseVersion} which is the latest version`,
              region,
              resource
            );
          } else {
            helpers.addResult(
              results,
              2,
              `SQL instance is using Postgres major version ${sqlInstance.databaseVersion} which is not the latest version`,
              region,
              resource
            );
          }
