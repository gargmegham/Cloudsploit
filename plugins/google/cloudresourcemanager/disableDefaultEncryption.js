...
module.exports = {
  ...
  remediation_description:
    'The "Restrict Default Google-Managed Encryption for Cloud SQL Instances" constraint will be enforced at the organization level.',
  apis_remediate: ["organizations:list", "organizations:listOrgPolicies"],
  actions: { remediate: ["SetOrgPolicy"], rollback: ["SetOrgPolicy"] },
  permissions: {
    remediate: ["orgpolicy.policy.set"],
    rollback: ["orgpolicy.policy.set"],
  },
  ...
  run: function (cache, settings, callback) {
    ...
    if (
      ...
      helpers.addResult(
        results,
        3,
        "Unable to query for organizations: " + helpers.addError(organizations),
        "global",
        null,
        null,
        organizations ? organizations.err : null
      );
