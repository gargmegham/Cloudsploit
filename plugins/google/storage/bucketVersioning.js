...
module.exports = {
  ...
  remediation_description:
    "Bucket versioning will be enabled on storage buckets",
  apis_remediate: ["buckets:list"],
  actions: {
    remediate: ["storage.buckets.update"],
    rollback: ["storage.buckets.update"],
  },
  permissions: {
    remediate: ["storage.buckets.setIamPolicy", "storage.buckets.update"],
    rollback: ["storage.buckets.setIamPolicy", "storage.buckets.update"],
  },
        ...
        buckets.data.forEach((bucket) => {
          if (bucket.name) {
            ...
            if (bucket.versioning && bucket.versioning.enabled) {
              ...
              (bucket.versioning && !bucket.versioning.enabled) ||
              !bucket.versioning
            ) {
              helpers.addResult(
                results,
