...
module.exports = {
  ...
  settings: {
    bucket_retention_days: {
      ...
      description:
        "Return a passing result when bucket retention expiration date exceeds this number of days in the future",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: "0",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      bucket_retention_days: parseInt(
        settings.bucket_retention_days ||
          this.settings.bucket_retention_days.default
      ),
    };
      ...
      function (region, rcb) {
        ...
        if (buckets.err || !buckets.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query storage buckets: " + helpers.addError(buckets),
            region,
            null,
            null,
            buckets.err
        ...
        buckets.data.forEach((bucket) => {
          if (bucket.name) {
            ...
            if (
              ...
              bucket.retentionPolicy.effectiveTime
            ) {
              var retentionDays = Math.round(
                parseInt(bucket.retentionPolicy.retentionPeriod) /
                  (24 * 60 * 60)
              );
              var now = new Date();
              ...
              if (effectiveDifference < 0) {
                helpers.addResult(
                  results,
                  2,
                  "Storage bucket retention has already expired",
                  region,
                  resource
                );
                ...
                helpers.addResult(
                  results,
                  2,
                  `Storage bucket retention will expire in ${effectiveDifference} days`,
                  region,
                  resource
                );
                ...
                helpers.addResult(
                  results,
                  2,
                  "Storage bucket retention policy is not locked",
                  region,
                  resource
                );
              } else {
              ...
              helpers.addResult(
                results,
                2,
                "Storage bucket does not have a retention policy",
                region,
                resource
              );
            }
