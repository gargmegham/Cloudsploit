  ...
  describe("run", function () {
    it("should give unknown result if a bucket error is passed or no data is present", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Unable to query storage buckets"
        );
        expect(results[0].region).to.equal("global");
        done();
      };

      const cache = createCache(["error"], null, ["error"], null);
    ...
    it("should give passing result if no buckets have anonymous or public access", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          "Bucket does not have anonymous or public access"
        );
        expect(results[0].region).to.equal("global");
        done();
      };
        ...
        [
          {
            kind: "storage#policy",
            resourceId:
              "projects/_/buckets/us.artifacts.rosy-booth-253119.appspot.com",
            bindings: [
              {
              ...
              },
              {
                ...
                members: ["projectViewer:rosy-booth-253119"],
              },
            ],
            etag: "CAE=",
            version: 1,
          },
          {
            kind: "storage#policy",
            resourceId:
              "projects/_/buckets/staging.rosy-booth-253119.appspot.com",
            bindings: [
              {
                role: "roles/storage.legacyBucketOwner",
                members: [
            ...
            bindings: [
              {
                ...
                members: ["projectEditor:rosy-booth-253119"],
              },
              {
                role: "roles/storage.legacyBucketOwner",
                members: [
    ...
    it("should give failing result if bucket has anonymous or public access", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Bucket has anonymous or public access"
        );
        expect(results[0].region).to.equal("global");
        done();
      };
        ...
        [
          ...
          },
          {
            ...
            bindings: [
              ...
              {
                role: "roles/storage.legacyBucketReader",
                members: ["allUsers", "projectViewer:rosy-booth-253119"],
              },
            ],
            etag: "CAs=",
          },
