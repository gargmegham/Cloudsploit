      ...
      function (region, rcb) {
        ...
        if (subscriptions.err || !subscriptions.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for Pub/Sub subscriptions: " +
        ...
        if (!subscriptions.data.length) {
          helpers.addResult(
            results,
            0,
            "No Pub/Sub subscriptions found",
            region
          );
          return rcb();
        }
        ...
        subscriptions.data.forEach((subscription) => {
          ...
          if (
            ...
            helpers.addResult(
              results,
              0,
              "Pub/Sub subscription has dead lettering enabled",
              region,
              subscription.name
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Pub/Sub subscription does not have dead lettering enabled",
              region,
              subscription.name
            );
          }
