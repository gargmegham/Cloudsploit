          ...
          if (cluster.nodePools && cluster.nodePools.length) {
            cluster.nodePools.forEach((nodePool) => {
              if (
                ...
                disabledSecureBootNodes.push(nodePool.name);
            });
            if (disabledSecureBootNodes.length) {
              helpers.addResult(
                results,
                2,
                `Secure Boot is disabled for these node pools: ${disabledSecureBootNodes.join(
                  ", "
                )}`,
                region,
                resource
              ...
              helpers.addResult(
                results,
                0,
                "Secure Boot is enabled for all node pools",
                region,
                resource
              );
            }
