        ...
        clusters.data.forEach((cluster) => {
          ...
          if (cluster.nodePools && cluster.nodePools.length) {
            cluster.nodePools.forEach((nodePool) => {
              if (
                !nodePool.config ||
                !nodePool.config.shieldedInstanceConfig ||
                !nodePool.config.shieldedInstanceConfig
                  .enableIntegrityMonitoring
              )
                disbaledIntegrityMonitoringNodes.push(nodePool.name);
            });
            if (disbaledIntegrityMonitoringNodes.length) {
              helpers.addResult(
                results,
                2,
                `Integrity Monitoring is disabled for these node pools: ${disbaledIntegrityMonitoringNodes.join(
                  ", "
                )}`,
                region,
                resource
              ...
              helpers.addResult(
                results,
                0,
                "Integrity Monitoring is enabled for all node pools",
                region,
                resource
              );
            }
