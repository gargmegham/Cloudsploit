...
module.exports = {
  ...
  settings: {
    kubernetes_node_encryption_level: {
      ...
      description:
        "Desired protection level for GKE cluster nodes. default: google-managed, cloudcmek: customer managed encryption keys, " +
  ...
  run: function (cache, settings, callback) {
    ...
    let desiredEncryptionLevelStr =
      settings.kubernetes_node_encryption_level ||
      this.settings.kubernetes_node_encryption_level.default;
          ...
          async.each(
            ...
            function (region, rcb) {
              ...
              clusters.data.forEach((cluster) => {
                ...
                if (cluster.locations) {
                  location =
                    cluster.locations.length === 1
                      ? cluster.locations[0]
                      : cluster.locations[0].substring(
                          0,
                          cluster.locations[0].length - 2
                        );
                ...
                if (cluster.nodePools && cluster.nodePools.length) {
                  cluster.nodePools.forEach((nodePool) => {
                    let currentEncryptionLevel;

                    if (
                      nodePool.config &&
                      nodePool.config.bootDiskKmsKey &&
                      nodePool.config.bootDiskKmsKey.length &&
                      keysObj[nodePool.config.bootDiskKmsKey]
                    ) {
                  ...
                  if (nonEncryptedNodes.length) {
                    helpers.addResult(
                      results,
                      2,
                      `These node pools do not have the desired encryption level: ${nonEncryptedNodes.join(
                        ", "
                      )}`,
                      region,
                      resource
                    ...
                    helpers.addResult(
                      results,
                      0,
                      "All node pools have the desired encryption level",
                      region,
                      resource
                    );
                  }
