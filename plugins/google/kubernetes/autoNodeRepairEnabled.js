        ...
        clusters.data.forEach((cluster) => {
          ...
          if (cluster.nodePools && cluster.nodePools.length) {
            ...
            cluster.nodePools.forEach((nodePool) => {
              if (!nodePool.management || !nodePool.management.autoRepair)
                nonAutoRepairNodes.push(nodePool.name);
            });
          }
          ...
          if (nonAutoRepairNodes.length) {
            helpers.addResult(
              results,
              2,
              `Auto repair is disabled for these node pools: ${nonAutoRepairNodes.join(
                ", "
              )}`,
              region,
              resource
