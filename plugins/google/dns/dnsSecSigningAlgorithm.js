      ...
      function (region, rcb) {
        ...
        if (managedZones.err || !managedZones.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query DNS managed zones: " +
              helpers.addError(managedZones),
            region,
            null,
            null,
            managedZones.err
              ...
              if (keySpec.keyType === "keySigning") {
                if (keySpec.algorithm.toLowerCase() === "rsasha1") {
                  helpers.addResult(
                    results,
                    2,
                    "RSASHA1 algorithm is being used for key signing",
                    region,
                    resource
                  );
                } else {
                  helpers.addResult(
                    results,
                    0,
                    "RSASHA1 algorithm is not being for key signing",
                    region,
                    resource
                  );
                }
                ...
                if (keySpec.algorithm.toLowerCase() === "rsasha1") {
                  helpers.addResult(
                    results,
                    2,
                    "RSASHA1 algorithm is being used for zone signing",
                    region,
                    resource
                  );
                } else {
                  helpers.addResult(
                    results,
                    0,
                    "RSASHA1 algorithm is not being used for zone signing",
                    region,
                    resource
                  );
                }
