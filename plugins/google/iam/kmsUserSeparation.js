      ...
      function (region, rcb) {
        ...
        iamPolicy.bindings.forEach((roleBinding) => {
          if (roleBinding.role === "roles/cloudkms.admin") {
            serviceAccountUsers = serviceAccountUsers.concat(
              roleBinding.members
            );
          }
        });
        ...
        iamPolicy.bindings.forEach((roleBinding) => {
          if (
            roleBinding.role === "roles/cloudkms.cryptoKeyDecrypter" &&
            roleBinding.members
          ) {
            notSeparated = roleBinding.members
              .filter((member) => {
                return serviceAccountUsers.indexOf(member) > -1;
              })
              .concat(notSeparated);
          } else if (
            roleBinding.role === "roles/cloudkms.cryptoKeyEncrypter" &&
            roleBinding.members
          ) {
            notSeparated = roleBinding.members
              .filter((member) => {
            ...
            roleBinding.role === "roles/cloudkms.cryptoKeyEncrypterDecrypter" &&
            roleBinding.members
          ) {
            notSeparated = roleBinding.members
              .filter((member) => {
        ...
        if (notSeparated.length) {
          ...
          notSeparated.forEach((account) => {
            let accountName = account.includes(":")
              ? account.split(":")[1]
              : account;
            ...
            helpers.addResult(
              results,
              2,
              "Account has the KMS admin role and one or more CryptoKey roles",
              region,
              resource
            );
          });
          ...
          helpers.addResult(
            results,
            0,
            "No accounts have a KMS admin role or a CryptoKey key role",
            region
          );
        }
