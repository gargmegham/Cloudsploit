    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        iamPolicy.bindings.forEach((roleBinding) => {
          if (
            roleBinding.role &&
            ["roles/editor", "roles/viewer", "roles/owner"].includes(
              roleBinding.role
            )
          ) {
            ...
            roleBinding.members.forEach((member) => {
              let accountName = member.includes(":")
                ? member.split(":")[1]
                : member;
              let memberType = member.startsWith("serviceAccount")
                ? "serviceAccounts"
                : "users";
              ...
              helpers.addResult(
                results,
                2,
                `The account has the primitive role: ${roleBinding.role}`,
                region,
                resource
              );
            });
        ...
        if (!primitiveRoleExists) {
          helpers.addResult(
            results,
            0,
            "No accounts have primitive roles",
            region
          );
        }
