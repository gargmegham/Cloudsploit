  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        iamPolicy.bindings.forEach((roleBinding) => {
          if (
            roleBinding.role === "roles/iam.serviceAccountAdmin" &&
            roleBinding.members
          ) {
            ...
            if (notSeparated && notSeparated.length) {
              notSeparated.forEach((member) => {
                let accountName = member.includes(":")
                  ? member.split(":")[1]
                  : member;
                ...
                helpers.addResult(
                  results,
                  2,
                  "The account has both the service account user and admin role",
                  region,
                  resource
                );
              });
        ...
        if (!notSeparated.length) {
          helpers.addResult(
            results,
            0,
            "No accounts have both the service account user and admin roles",
            region
          );
        }
