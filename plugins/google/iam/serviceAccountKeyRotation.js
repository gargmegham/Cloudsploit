...
module.exports = {
  ...
  settings: {
    service_account_keys_rotated_fail: {
      ...
      description:
        "Return a failing result when service accoun keys exceed this number of days without being rotated",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: "90",
    },
  },
  compliance: {
    hipaa:
      "Rotating access keys helps to ensure that those keys have not been " +
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      service_account_keys_rotated_fail: parseFloat(
        settings.service_account_keys_rotated_fail ||
          this.settings.service_account_keys_rotated_fail.default
      ),
    };
    ...
    async.each(
      regions.keys,
      function (region, rcb) {
        let keys = helpers.addSource(cache, source, ["keys", "list", region]);
        ...
        if (keys.err || !keys.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query service account keys, check permissions.",
            region,
            null,
            null,
            keys.err
        ...
        keys.data.forEach((key) => {
          if (key.keyType && key.keyType === "USER_MANAGED") {
            var validAfterTime = key.validAfterTime.split("T")[0];

            var timeFromCreation =
              new Date().getTime() - new Date(validAfterTime).getTime();
            var daysInTime =
              config.service_account_keys_rotated_fail * 24 * 60 * 60 * 1000;
            timeFromCreation /= daysInTime;
            if (timeFromCreation > 1) {
              helpers.addResult(
                results,
                2,
                `The service account key has not been rotated in over ${config.service_account_keys_rotated_fail} days`,
                region,
                key.name
              );
            } else {
              helpers.addResult(
                results,
                0,
                `The service account key has been rotated within ${config.service_account_keys_rotated_fail} days`,
                region,
                key.name
              );
            }
