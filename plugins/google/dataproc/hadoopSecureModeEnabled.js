    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        clusters.data.forEach((cluster) => {
          ...
          if (
            cluster.config &&
            cluster.config.securityConfig &&
            cluster.config.securityConfig.kerberosConfig &&
            cluster.config.securityConfig.kerberosConfig.enableKerberos
          ) {
            helpers.addResult(
              results,
              0,
              "Hadoop Secure mode is enabled for Dataproc cluster",
              region,
              resource
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Hadoop Secure mode is not enabled for Dataproc cluster",
              region,
              resource
            );
          }
