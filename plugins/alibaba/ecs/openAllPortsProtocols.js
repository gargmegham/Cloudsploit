...
module.exports = {
  ...
  domain: "Compute",
  description:
    "Ensure that ECS security groups do not have all ports or protocols open to the public",
  more_info:
  ...
  compliance: {
    hipaa:
      "HIPAA requires strict access controls to networks and services " +
    ...
    pci:
      "PCI has explicit requirements around firewalled access to systems. " +
  ...
  run: function (cache, settings, callback) {
    ...
    var accountId = helpers.addSource(cache, { china: true }, [
      "sts",
      "GetCallerIdentity",
      defaultRegion,
      "data",
      ...
      function (region, rcb) {
        ...
        if (describeSecurityGroups.err || !describeSecurityGroups.data) {
          helpers.addResult(
            results,
            3,
            "Unable to describe security groups: " +
        ...
        async.each(
          ...
          (group, scb) => {
            ...
            var describeSecurityGroupAttribute = helpers.addSource(
              cache,
              { china: true },
              [
                "ecs",
            ...
            if (
              !describeSecurityGroupAttribute ||
              describeSecurityGroupAttribute.err ||
              !describeSecurityGroupAttribute.data
            ) {
              helpers.addResult(
                results,
                3,
                `Unable to query security group attributes: ${describeSecurityGroupAttribute}`,
                region,
                resource
              );
              return scb();
            ...
            if (
              describeSecurityGroupAttribute.data.Permissions &&
              ...
              for (var permission of describeSecurityGroupAttribute.data
                .Permissions.Permission) {
                if (permission.Direction && permission.Direction !== "ingress")
                  continue;
                if (permission.SourceCidrIp === "0.0.0.0/0") {
                  ...
                  if (
                    ...
                    permission.IpProtocol.toUpperCase() === "ALL"
                  ) {
            ...
            if (strings.length) {
              helpers.addResult(
                results,
                2,
                `Security group: ${group.SecurityGroupId} has ${strings.join(
                  " and "
                )}`,
                region,
                resource
