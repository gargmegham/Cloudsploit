...
const createCache = (
  ...
  let keyData =
    describeKeyData && describeKeyData.length ? describeKeyData[0].data : null;
  return {
    ecs: {
      DescribeDisks: {
        "cn-hangzhou": {
  ...
  describe("run", function () {
    it("should PASS if System disks are encrypted", function (done) {
      ...
      systemDisksEncrypted.run(cache, { china: true }, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "System disk is encrypted to at least cloudkms"
        );
    ...
    it("should PASS if System disks are encrypted to target encryption level", function (done) {
      ...
      systemDisksEncrypted.run(
        cache,
        { system_disks_encryption_level, china: true },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].message).to.include(
            `System disk is encrypted to at least ${system_disks_encryption_level}`
          );
          expect(results[0].status).to.equal(0);
          expect(results[0].region).to.equal("cn-hangzhou");
          done();
    ...
    it("should FAIL if disk is not encrypted", function (done) {
      ...
      systemDisksEncrypted.run(cache, { china: true }, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "System disk is not encrypted to cloudkms"
        );
        expect(results[0].region).to.equal("cn-hangzhou");
        done();
      });
    ...
    it("should FAIL if System disk is not encrypted to target encryption level", function (done) {
      ...
      systemDisksEncrypted.run(
        ...
        (err, results) => {
          ...
          expect(results[0].status).to.equal(2);
          expect(results[0].message).to.include(
            `System disk is not encrypted to ${system_disks_encryption_level}`
          );
          expect(results[0].region).to.equal("cn-hangzhou");
          done();
        }
