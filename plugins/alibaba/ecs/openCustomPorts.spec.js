...
const createCache = (
  ...
  securityGroupsErr,
  describeSecurityGroupAttributeErr
) => {
  const securityGroupId =
    securityGroups && securityGroups.length
      ? securityGroups[0].SecurityGroupId
      : null;
  return {
    ecs: {
  ...
  describe("run", function () {
    it("should PASS if no public open ports found", function (done) {
      const cache = createCache(
        describeSecurityGroups,
        describeSecurityGroupAttribute[0]
      );
      openCustomPorts.run(
        cache,
        { restricted_open_ports: "tcp:22", china: true },
        (err, results) => {
    ...
    it("should FAIL if security group has custom ports open to public", function (done) {
      const cache = createCache(
        describeSecurityGroups,
        describeSecurityGroupAttribute[1]
      );
      openCustomPorts.run(
        cache,
        { restricted_open_ports: "tcp:60,tcp:65-70", china: true },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(2);
          expect(results[0].message).to.include(
            "has custom:TCP:60, some of TCP:65-70 open to 0.0.0.0/0"
          );
          expect(results[0].region).to.equal("cn-hangzhou");
          done();
        }
    ...
    it("should UNKNWON unable to describe security groups", function (done) {
      const cache = createCache(null, {
        message: "Unable to describe security groups",
      });
      openCustomPorts.run(
        cache,
        { restricted_open_ports: "tcp:22", china: true },
