...
module.exports = {
  ...
  more_info:
    "Configuring secure transfer enhances the security of OSS bucket by allowing requests to the storage account by only a secure connection.",
  ...
  run: function (cache, settings, callback) {
    ...
    listBuckets.data.forEach((bucket) => {
      ...
      if (
        ...
        getBucketPolicy.data.status == 404
      ) {
        helpers.addResult(
          results,
          2,
          "No OSS bucket policy found",
          bucketLocation,
          resource
        );
        return;
      ...
      if (!getBucketPolicy || getBucketPolicy.err || !getBucketPolicy.data) {
        helpers.addResult(
          results,
          3,
          `Unable to query OSS bucket policy: ${helpers.addError(
            getBucketPolicy
          )}`,
          bucketLocation,
          resource
      ...
      for (let statement of statements) {
        if (
          ...
          let conditionValue =
            statement.Condition.Bool[
              Object.keys(statement.Condition.Bool).find(
                (key) => key.toLowerCase() == "acs:securetransport"
              )
            ];
          if (
            statement.Effect &&
            ...
            conditionValue.find(
              (boolValue) => boolValue.toLowerCase() == "false"
            )
          ) {
            ...
            conditionValue.find(
              (boolValue) => boolValue.toLowerCase() == "true"
            )
          ) {
            secureTransportEnabled = true;
            break;
      ...
      if (secureTransportEnabled) {
        helpers.addResult(
          results,
          0,
          "OSS bucket has secure transport enabled",
          region,
          resource
        );
      } else {
        helpers.addResult(
          results,
          2,
          "OSS bucket does not have secure transport enabled",
          region,
          resource
        );
      }
