          ...
          data: [
            {
              ...
              KeyArn:
                "acs:kms:cn-hangzhou:0000111122223333:key/ed204e08-f814-4788-8406-3dc19c8e5260",
            },
          ],
        },
      },
  ...
  describe("run", function () {
    it("should FAIL if OSS bucket is not encrypted to required encryption level", function (done) {
      ...
      bucketCmkEncrypted.run(cache, { china: true }, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "OSS bucket is server-side encrypted"
        );
    ...
    it("should UNKNOWN if unable to query for OSS buckets", function (done) {
      const cache = createCache([], null, {
        err: "Unable to query for OSS buckets",
      });
      bucketCmkEncrypted.run(cache, { china: true }, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(3);
        expect(results[0].message).to.include(
          "Unable to query for OSS buckets"
        );
        expect(results[0].region).to.equal("cn-hangzhou");
        done();
      });
