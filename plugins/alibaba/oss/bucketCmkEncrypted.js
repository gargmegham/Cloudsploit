...
module.exports = {
  ...
  more_info:
    "OSS buckets should be encrypted using customer master keys in order to gain greater control and transparency, " +
    "as well as increasing security by having full control of the encryption keys.",
  recommended_action:
    "Modify bucket's server-side encrypted setting to configure encryption method as KMS",
  ...
  settings: {
    oss_buckets_encryption_level: {
      ...
      description:
        "In order (lowest to highest) ossmanaged=OSS-Managed; cloudkms=Alibaba managed default service KMS; alibabacmk=Customer managed KMS; externalcmk=Customer imported key; cloudhsm=Customer managed CloudHSM sourced Key",
  ...
  compliance: {
    hipaa:
      ...
      "buckets data.",
    pci:
      "PCI requires proper encryption of cardholder data at rest. OSS " +
  ...
  run: function (cache, settings, callback) {
    ...
    var targetEncryptionLevelStr =
      settings.oss_buckets_encryption_level ||
      this.settings.oss_buckets_encryption_level.default;
    var targetEncryptionLevel = helpers.ENCRYPTION_LEVELS.indexOf(
      targetEncryptionLevelStr
    );
    ...
    if (listBuckets.err || !listBuckets.data) {
      helpers.addResult(
        results,
        3,
        `Unable to query for OSS buckets: ${helpers.addError(listBuckets)}`,
        region
      );
    ...
    if (!listKeys || listKeys.err || !listKeys.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query KMS keys: " + helpers.addError(listKeys),
        region
      );
      return callback(null, results, source);
    }
      ...
      (bucket, cb) => {
        ...
        if (!getBucketInfo || getBucketInfo.err || !getBucketInfo.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query OSS bucket info: ${helpers.addError(
              getBucketInfo
            )}`,
            bucketLocation,
            resource
        ...
        if (
          getBucketInfo.data.ServerSideEncryptionRule &&
          ...
          if (sseAlgo.toUpperCase() == "NONE") {
            helpers.addResult(
              results,
              2,
              "OSS bucket is not server-side encrypted",
              bucketLocation,
              resource
            );
            return cb();
          ...
          if (sseAlgo.toUpperCase() == "KMS") {
            if (sseRule.KMSMasterKeyID && sseRule.KMSMasterKeyID.length) {
              ...
              if (!describeKey || describeKey.err || !describeKey.data) {
                helpers.addResult(
                  results,
                  3,
                  `Unable to query KMS key (${kmsKey}): ${helpers.addError(
                    describeKey
                  )}`,
                  bucketLocation,
                  resource
