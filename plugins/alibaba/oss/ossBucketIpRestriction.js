...
module.exports = {
  ...
  more_info:
    "OSS buckets should limit access to selected networks. Restricting default network access provides a new layer of security.",
  recommended_action:
    "Add or modify bucket policy to create IP-based conditions",
    ...
    listBuckets.data.forEach((bucket) => {
          ...
          if (statement.Condition.IpAddress) {
            let conditionValue =
              statement.Condition.IpAddress[
                Object.keys(statement.Condition.IpAddress).find(
                  (key) => key.toLowerCase() == "acs:sourceip"
                )
              ];
            ...
            let conditionValue =
              statement.Condition.NotIpAddress[
                Object.keys(statement.Condition.NotIpAddress).find(
                  (key) => key.toLowerCase() == "acs:sourceip"
                )
              ];
            if (conditionValue.length) {
      ...
      if (ipRestrictionInPlace) {
        helpers.addResult(
          results,
          0,
          "OSS bucket has IP restrictions configured",
          region,
          resource
        );
      } else {
        helpers.addResult(
          results,
          2,
          "OSS bucket does not have IP restrictions configured",
          region,
          resource
        );
      }
