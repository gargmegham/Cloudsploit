...
module.exports = {
  ...
  domain: "Storage",
  description:
    "Ensure that OSS buckets have pay per requester feature enabled.",
  more_info:
    "Enabling pay per requester for OSS buckets ensures that requesters pay the request and traffic fees that are incurred when the requesters access objects in the bucket.",
  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      ...
      (bucket, cb) => {
        ...
        if (
          !getBucketRequestPayment ||
          getBucketRequestPayment.err ||
          !getBucketRequestPayment.data
        ) {
          helpers.addResult(
            results,
            3,
        ...
        if (
          getBucketRequestPayment.data.payer &&
