...
module.exports = {
  ...
  more_info:
    "Enabling lifecycle policies for OSS buckets enables automatic transition of data from one storage class to another.",
  ...
  run: function (cache, settings, callback) {
    ...
    async.forEach(
      ...
      (bucket, cb) => {
        ...
        if (
          getBucketLifecycle &&
          getBucketLifecycle.err &&
          getBucketLifecycle.err.code &&
          getBucketLifecycle.err.code == "NoSuchLifecycle"
        ) {
          helpers.addResult(
            results,
            2,
        ...
        if (
          !getBucketLifecycle ||
          !getBucketLifecycle.data ||
          getBucketLifecycle.err
        ) {
          helpers.addResult(
            results,
            3,
            `Unable to get OSS bucket lifecycle policy info: ${helpers.addError(
              getBucketLifecycle
            )}`,
            bucketLocation,
            resource
        ...
        if (
          getBucketLifecycle.data.rules &&
          getBucketLifecycle.data.rules.length
        ) {
          let bucketPolicyEnabled = getBucketLifecycle.data.rules.find(
            (rule) => rule.status && rule.status.toLowerCase() == "enabled"
          );
          ...
          if (bucketPolicyEnabled) {
            helpers.addResult(
              results,
              0,
              "OSS bucket has lifecycle policy enabled",
              bucketLocation,
              resource
            );
          } else {
            helpers.addResult(
              results,
              2,
              "OSS bucket does not have lifecycle policy enabled",
              bucketLocation,
              resource
            );
          }
