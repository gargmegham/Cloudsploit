...
module.exports = {
  ...
  more_info:
    "RAM policies represent permissions that can be granted to users, groups, or roles. It is recommended and considered a standard security advice to grant least privilege. " +
  ...
  recommended_action:
    "Ensure that administrator RAM policies are not attached with any RAM resource.",
  apis: ["RAM:ListPolicies", "RAM:GetPolicy", "STS:GetCallerIdentity"],
  settings: {
    ram_policies_ignore_name: {
      name: "RAM Policies Ignore Name",
      description:
        "A comma-separated list indicating policy name which should be ignored without checking",
  ...
  run: function (cache, settings, callback) {
    var config = {
      ram_policies_ignore_name:
        settings.ram_policies_ignore_name ||
        this.settings.ram_policies_ignore_name.default,
    };
    ...
    if (listPolicies.err || !listPolicies.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query RAM policies" + helpers.addError(listPolicies),
        region
      );
    ...
    for (let policy of listPolicies.data) {
      if (
        !policy.PolicyName ||
        !policy.PolicyType ||
        config.ram_policies_ignore_name.includes(policy.PolicyName)
      )
        continue;
      if (
        policy.PolicyType &&
        policy.PolicyType.toUpperCase() == "SYSTEM" &&
        policy.PolicyName.toUpperCase() != "ADMINISTRATORACCESS"
      )
        continue;
      ...
      let resource = helpers.createArn(
        "ram",
        accountId,
        `${policy.PolicyType.toLowerCase()}policy`,
        policy.PolicyName
      );
      ...
      if (
        ...
        getPolicy.err ||
        !getPolicy.data ||
        !getPolicy.data.PolicyDocument
      ) {
        helpers.addResult(
          results,
          3,
          `Unable to get RAM policy: ${getPolicy.err}`,
          region,
          resource
        );
      } else {
        ...
        let attachmentCount = policy.AttachmentCount
        ...
        if (adminPolicy && attachmentCount > 0) {
          helpers.addResult(
            results,
            2,
            `Policy provides admin (*:*) access and attachment count is ${attachmentCount}`,
            region,
            resource
          );
        } else if (adminPolicy) {
          helpers.addResult(
            results,
            0,
            `Policy provides admin (*:*) access but attachment count is ${attachmentCount}`,
            region,
            resource
          );
        } else {
          helpers.addResult(
            results,
            0,
            "Policy does not provide admin (*:*) access",
            region,
            resource
          );
        }
