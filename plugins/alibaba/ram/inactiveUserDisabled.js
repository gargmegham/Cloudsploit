...
module.exports = {
  ...
  description:
    "Ensure that RAM users inactive for 90 or more days are disabled.",
  more_info:
    "RAM User should not have the console access enabled on being inactive for 90 or more days.",
    ...
    for (var user of listUsers.data) {
      ...
      if (getUser.err || !getUser.data) {
        helpers.addResult(
          results,
          3,
          "Unable to query RAM user" + helpers.addError(getUser),
          region
        );
        continue;
      }
      ...
      let lastLoginDate =
        getUser.data.LastLoginDate && getUser.data.LastLoginDate.length
          ? getUser.data.LastLoginDate
          : getUser.data.CreateDate;
      ...
      if (diffInDays >= 90) {
        ...
        if (
          getUserProfile &&
          getUserProfile.err &&
          getUserProfile.err.Code &&
          getUserProfile.err.Code == "EntityNotExist.User.LoginProfile"
        ) {
          helpers.addResult(
            results,
            0,
            `RAM user inactive for ${diffInDays} days is not enabled`,
            region,
            resource
          );
          ...
          helpers.addResult(
            results,
            3,
            `Unable to query user login profile: ${helpers.addError(
              getUserProfile
            )}`,
            region,
            resource
          ...
          helpers.addResult(
            results,
            2,
            `RAM user inactive for ${diffInDays} days is enabled`,
            region,
            resource
          );
        }
        ...
        helpers.addResult(
          results,
          0,
          `RAM user last activity was ${diffInDays} days ago`,
          region,
          resource
        );
      }
