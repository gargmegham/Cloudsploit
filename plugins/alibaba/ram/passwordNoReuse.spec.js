  ...
  describe("run", function () {
    it("should FAIL if RAM password security policy does not requires to prevent reusing 5 previous passwords", function (done) {
      ...
      passwordNoReuse.run(cache, { china: true }, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "RAM password security policy requires to prevent reusing previous 3 passwords which is less than desired limit of 5"
        );
    ...
    it("should PASS if RAM password security policy requires to prevent reusing 5 previous passwords", function (done) {
      ...
      passwordNoReuse.run(cache, { china: true }, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "RAM password security policy requires to prevent reusing previous 5 passwords which is equal to or greater than desired limit of 5"
        );
        expect(results[0].region).to.equal("cn-hangzhou");
        done();
      });
