...
const getPolicy = [
  {
    ...
    PolicyDocument:
      '{"Version":"1","Statement":[{"Effect":"Allow","Action":["ecs:*"],"Resource":["*"],"Condition":{}}]}',
    CreateDate: "2021-09-21T09:16:47Z",
  },
  {
    VersionId: "v1",
...
const createCache = (
  ...
  let policyName =
    listPolicies && listPolicies.length ? listPolicies[0].PolicyName : null;
  return {
    ram: {
      ListPolicies: {
        "cn-hangzhou": {
  ...
  describe("run", function () {
    it("should FAIL if Policy provides admin (*:*) access and attachment count is greater than 0", function (done) {
      ...
      ramAdminPolicy.run(cache, { china: true }, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Policy provides admin (*:*) access and attachment count is"
        );
    ...
    it("should PASS if Policy provides admin (*:*) access but attachment count is 0", function (done) {
      ...
      ramAdminPolicy.run(cache, { china: true }, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Policy provides admin (*:*) access but attachment count is 0"
        );
        expect(results[0].region).to.equal("cn-hangzhou");
        done();
      });
    ...
    it("should UNKNOWN if Unable to query RAM policies", function (done) {
      const cache = createCache([], null, {
        err: "Unable to query RAM policies",
      });
      ramAdminPolicy.run(cache, { china: true }, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if Unable to get RAM policy", function (done) {
      const cache = createCache([listPolicies[1]], {}, null, {
        err: "Unable to query RAM policy",
      });
      ramAdminPolicy.run(cache, { china: true }, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(3);
