...
module.exports = {
  ...
  description:
    "Ensure that RAM password security settings require at least one number.",
  more_info:
  ...
  recommended_action:
    "Update the password security settings to require the use of numbers.",
  ...
  run: function (cache, settings, callback) {
    ...
    if (getPasswordPolicy.data.RequireNumbers) {
      helpers.addResult(
        results,
        0,
        "RAM password security policy requires numbers",
        region
      );
    } else {
      helpers.addResult(
        results,
        2,
        "RAM password security policy does not require numbers",
        region
      );
    }
