...
module.exports = {
  ...
  description:
    "Ensures a multi-factor authentication device is enabled for all RAM users within the account",
  more_info:
    "RAM User should have an MFA device setup to enable two-factor authentication.",
  ...
  compliance: {
    hipaa:
      "MFA helps provide additional assurance that the user accessing " +
    ...
    pci:
      "PCI requires MFA for all access to cardholder environments. " +
  ...
  run: function (cache, settings, callback) {
    ...
    for (var user of listUsers.data) {
      ...
      if (
        ...
        getUserMfa.err &&
        getUserMfa.err.code &&
        getUserMfa.err.code === "EntityNotExist.User.MFADevice"
      ) {
        helpers.addResult(
          results,
          2,
          "RAM user does not have MFA device configured",
          region,
          resource
        );
        ...
        helpers.addResult(
          results,
          3,
          "Unable to query RAM user MFA info",
          region,
          resource
        );
      } else {
        helpers.addResult(
          results,
          0,
          "RAM user has MFA device configured",
          region,
          resource
        );
      }
