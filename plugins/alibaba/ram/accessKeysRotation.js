...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    var accessKeyRotationInterval = parseInt(
      settings.ram_access_keys_rotation_interval ||
        this.settings.ram_access_keys_rotation_interval.default
    );
    ...
    if (listUsers.err || !listUsers.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query RAM users" + helpers.addError(listUsers),
        region
      );
    ...
    for (var user of listUsers.data) {
      ...
      if (getAccessKey.err || !getAccessKey.data) {
        helpers.addResult(
          results,
          3,
          "Unable to query user access keys" + helpers.addError(getAccessKey),
          region
        );
        continue;
      }
      ...
      if (
        getAccessKey.data.AccessKeys &&
        getAccessKey.data.AccessKeys.AccessKey &&
        getAccessKey.data.AccessKeys.AccessKey.length
      ) {
          ...
          if (accessKey.Status && accessKey.Status == "Active") {
            ...
            if (diffInDays <= accessKeyRotationInterval) {
              helpers.addResult(
                results,
                0,
                `RAM user access key was last rotated ${diffInDays} days ago which is equal to or less than ${accessKeyRotationInterval}`,
                region,
                resource
              );
            } else {
              helpers.addResult(
                results,
                2,
                `RAM user access key was last rotated ${diffInDays} days ago which is greater than ${accessKeyRotationInterval}`,
                region,
                resource
              );
            }
        ...
        if (!activeKeyFound) {
          helpers.addResult(
            results,
            0,
            "RAM user does not have any active access keys",
            region,
            resource
          );
        }
        ...
        helpers.addResult(
          results,
          0,
          "RAM user does not have any access keys",
          region,
          resource
        );
      }
