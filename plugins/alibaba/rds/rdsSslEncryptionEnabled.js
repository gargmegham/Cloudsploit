...
module.exports = {
  ...
  domain: "Databases",
  description:
    "Ensure that RDS instances enforce all incoming connections to use SSL.",
  more_info:
    "To enhance link security, you should enable Secure Sockets Layer (SSL) encryption for RDS instances. " +
        ...
        async.each(
          ...
          function (instance, cb) {
            ...
            if (
              !instanceSslInfo ||
              instanceSslInfo.err ||
              !instanceSslInfo.data
            ) {
              helpers.addResult(
                results,
                3,
                `Unable to query RDS instance SSL info: ${helpers.addError(
                  instanceSslInfo
                )}`,
                region,
                resource
            ...
            if (
              instanceSslInfo.data.RequireUpdate &&
              instanceSslInfo.data.RequireUpdate.toLowerCase() == "yes"
            ) {
              helpers.addResult(
                results,
                0,
                "RDS instance has SSL encryption enabled",
                region,
                resource
              );
            } else {
              helpers.addResult(
                results,
                2,
                "RDS instance does not have SSL encryption enabled",
                region,
                resource
              );
            }
