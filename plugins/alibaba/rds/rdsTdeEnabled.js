...
module.exports = {
  ...
  domain: "Databases",
  description:
    "Ensure that RDS instances have Transparent Data Encryption enabled.",
  more_info:
    "TDE should be enabled to protect against the threat of malicious activities. Real-time encryption and decryption of the database," +
  ...
  run: function (cache, settings, callback) {
    ...
    var supportedEngines = [
      "sqlserver 2012_ent_ag",
      "sqlserver 2016_ent_ag",
      "sqlserver 2017_ent_ag",
      "sqlserver 2019_ent_ag",
      "mysql 5.6",
    ];
          ...
          function (instance, cb) {
            ...
            let instanceEngine =
              instance.Engine && instance.EngineVersion
                ? `${instance.Engine.toLowerCase()} ${instance.EngineVersion}`
                : "";
            ...
            if (
              !describeDbInstanceTde ||
              describeDbInstanceTde.err ||
              !describeDbInstanceTde.data
            ) {
              helpers.addResult(
                results,
                3,
                `Unable to query RDS DB instance TDE: ${helpers.addError(
                  describeDbInstanceTde
                )}`,
                region,
                resource
            ...
            if (
              describeDbInstanceTde.data.TDEStatus &&
              describeDbInstanceTde.data.TDEStatus.toUpperCase() == "ENABLED"
            ) {
              helpers.addResult(
                results,
                0,
