...
module.exports = {
  ...
  domain: "Databases",
  description:
    "Ensure that log_disconnections parameter is set to ON for RDS instances.",
  more_info:
    "RDS instance provide the feature of logging details of termination of a connection to the server " +
  ...
  recommended_action:
    "Modify RDS DB instance to set value for log_disconnections parameter to ON",
  apis: [
      ...
      function (region, rcb) {
        ...
        async.each(
          ...
          function (instance, cb) {
            ...
            if (
              instance.Engine &&
              instance.Engine.toUpperCase() !== "POSTGRESQL"
            ) {
              helpers.addResult(
                results,
                0,
            ...
            if (
              !describeParameters ||
              describeParameters.err ||
              !describeParameters.data
            ) {
              helpers.addResult(
                results,
                3,
                `Unable to query DB parameters: ${helpers.addError(
                  describeParameters
                )}`,
                region,
                resource
            ...
            if (
              describeParameters.data.RunningParameters &&
              ...
              let parameters =
                describeParameters.data.RunningParameters.DBInstanceParameter;
              let found = parameters.find(
                (parameter) =>
                  parameter.ParameterName == "log_disconnections" &&
                  parameter.ParameterValue &&
                  parameter.ParameterValue.toLowerCase() == "on"
              );
