...
module.exports = {
  ...
  domain: "Databases",
  description:
    "Ensure that RDS DB instances SQL Audit Log retention period is configured to be greater than set days limit",
  more_info:
    "RDS instances provides auditing feature that can be used to log all the events and activities which can be used later in case of any suspicions or security reasons.",
  link: "https://partners-intl.aliyun.com/help/doc-detail/118678.htm",
  recommended_action:
    "Modify RDS DB instances to set SQL Audit Log retention period to be greater than set days limit",
  apis: [
  ...
  run: function (cache, settings, callback) {
    ...
    var sqlAuditRetentionPeriod = parseInt(
      settings.sqlAuditRetentionPeriod ||
        this.settings.sqlAuditRetentionPeriod.default
    );
      ...
      function (region, rcb) {
        ...
        if (describeDBInstances.err || !describeDBInstances.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query RDS DB instances: ${helpers.addError(
              describeDBInstances
            )}`,
            region
          );
        ...
        async.each(
          ...
          function (instance, cb) {
            ...
            if (
              !describeSqlAuditRetention ||
              describeSqlAuditRetention.err ||
              !describeSqlAuditRetention.data
            ) {
              helpers.addResult(
                results,
                3,
                `Unable to query DB sql audit log retention: ${helpers.addError(
                  describeSqlAuditRetention
                )}`,
                region,
                resource
            ...
            const currentRetentionPeriod = describeSqlAuditRetention.data
              .ConfigValue
              ? parseInt(describeSqlAuditRetention.data.ConfigValue)
              : 0;
