...
const createCache = (
  ...
  describeDBInstancesErr,
  describeSqlCollectorRetentionErr
) => {
  let instanceId =
    describeDBInstances && describeDBInstances.length
      ? describeDBInstances[0].DBInstanceId
      : null;
  return {
    rds: {
      DescribeDBInstances: {
  ...
  describe("run", function () {
    it("should FAIL if RDS DB instance does not have sql audit log retention greater than 180 days", function (done) {
      const cache = createCache(
        describeDBInstances,
        describeSqlCollectorRetention[0]
      );
    ...
    it(`should FAIL if RDS DB instance does not have sql audit log retention greater than set days limit`, function (done) {
      ...
      rdsSqlAuditRetentionPeriod.run(
        cache,
        { sqlAuditRetentionPeriod: "300", china: true },
        (err, results) => {
          expect(results.length).to.be.above(1);
    ...
    it("should PASS if RDS DB instance have sql audit log retention greater than 180 days", function (done) {
      const cache = createCache(
        describeDBInstances,
        describeSqlCollectorRetention[1]
      );
      rdsSqlAuditRetentionPeriod.run(cache, { china: true }, (err, results) => {
        expect(results.length).to.be.above(1);
    ...
    it(`should PASS if RDS DB instance have sql audit log retention greater than set days limit`, function (done) {
      ...
      rdsSqlAuditRetentionPeriod.run(
        cache,
        { sqlAuditRetentionPeriod: "365", china: true },
        (err, results) => {
          expect(results.length).to.be.above(1);
    ...
    it("should UNKNOWN if unable to query DB sql audit log retention", function (done) {
      const cache = createCache([describeDBInstances[0]], {}, null, {
        err: "Unable to query DB sql audit log retention",
      });
      rdsSqlAuditRetentionPeriod.run(cache, { china: true }, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].message).to.include(
          "Unable to query DB sql audit log retention"
        );
        expect(results[0].status).to.equal(3);
        expect(results[0].region).to.equal("cn-hangzhou");
        done();
