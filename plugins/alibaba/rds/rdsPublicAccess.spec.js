    ...
    Items: {
      DBInstanceIPArray: [
        ...
        },
        {
          ...
          SecurityIPType: "IPv4",
          SecurityIPList:
            "10.81.176.172,10.81.89.178,100.104.172.0/24,100.104.220.0/24,11.193.102.115,11.193.102.131,11.195.184.229,11.195.184.232,11.195.184.233,11.195.184.234,11.195.184.83,11.195.184.84,11.195.184.89,11.195.184.93",
          WhitelistNetworkType: "MIX",
  ...
  describe("run", function () {
    ...
    it("should UNKNOWN if Unable to query DB IP Array List", function (done) {
      const cache = createCache([describeDBInstances[0]], {}, null, {
        err: "Unable to query DB IP Array List",
      });
      rdsPublicAccess.run(cache, { china: true }, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(3);
        expect(results[0].message).to.include(
          "Unable to query DB IP Array List"
        );
        expect(results[0].region).to.equal("cn-hangzhou");
        done();
      });
