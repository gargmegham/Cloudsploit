        ...
        async.each(
          ...
          function (instance, cb) {
            ...
            if (
              ...
              describeInstanceWhitelist.err ||
              !describeInstanceWhitelist.data
            ) {
              helpers.addResult(
                results,
                3,
                `Unable to query DB IP Array List: ${helpers.addError(
                  describeInstanceWhitelist
                )}`,
                region,
                resource
            ...
            if (
              describeInstanceWhitelist.data.Items &&
              describeInstanceWhitelist.data.Items.DBInstanceIPArray &&
              describeInstanceWhitelist.data.Items.DBInstanceIPArray.length
            ) {
              let ipArray =
                describeInstanceWhitelist.data.Items.DBInstanceIPArray;
              let found = ipArray.find(
                (ipObject) =>
                  ipObject.SecurityIPList &&
                  ipObject.SecurityIPList.includes("0.0.0.0")
              );
