...
const describeTrails = [
  {
    ...
    SlsWriteRoleArn:
      "acs:ram::0000111122223333:role/aliyunserviceroleforactiontrail",
    OssBucketLocation: "",
    TrailRegion: "All",
    Name: "aqua-at",
  ...
  describe("run", function () {
    it("should FAIL if ActionTrail trail Bucket ACL allows public access", function (done) {
      ...
      actiontrailBucketPrivate.run(cache, { china: true }, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "ActionTrail trail Bucket ACL allows public-read-write access"
        );
    ...
    it("should PASS if ActionTrail trail Bucket ACL allows private access", function (done) {
      ...
      actiontrailBucketPrivate.run(cache, { china: true }, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "ActionTrail trail Bucket ACL allows private access"
        );
        expect(results[0].region).to.equal("cn-hangzhou");
        done();
      });
    ...
    it("should PASS if no ActionTrail trail with OSS bucket destination found", function (done) {
      ...
      actiontrailBucketPrivate.run(cache, { china: true }, (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          "No ActionTrail trail with OSS bucket destination found"
        );
        expect(results[0].region).to.equal("cn-hangzhou");
        done();
      });
    ...
    it("should UNKNOWN if unable to query ActionTrail trails", function (done) {
      const cache = createCache(listBuckets, null, [], {
        err: "Unable to query ActionTrail trails",
      });
      actiontrailBucketPrivate.run(cache, { china: true }, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if unable to query OSS bucket info", function (done) {
      const cache = createCache(listBuckets, null, describeTrails, null, null, {
        err: "Unable to query OSS bucket info",
      });
      actiontrailBucketPrivate.run(cache, { china: true }, (err, results) => {
        expect(results.length).to.equal(2);
        expect(results[0].status).to.equal(3);
        expect(results[0].message).to.include(
          "Unable to query OSS bucket info"
        );
        expect(results[0].region).to.equal("cn-hangzhou");
        done();
      });
