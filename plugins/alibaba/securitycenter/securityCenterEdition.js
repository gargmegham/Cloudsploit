...
module.exports = {
  ...
  more_info:
    "Premium Security Center editions like Advanced or Enterprise Edition provides crucial features liekthreat detection for network and endpoints, " +
  ...
  recommended_action:
    "Upgrade your Security Center edition to at least Advanced.",
  ...
  run: function (cache, settings, callback) {
    ...
    if (describeVersionConfig.err || !describeVersionConfig.data) {
      helpers.addResult(
        results,
        3,
        `Unable to query Security Center version config: ${helpers.addError(
          describeVersionConfig
        )}`,
        region
      );
    ...
    let securityVersion = describeVersionConfig.data.Version
      ? describeVersionConfig.data.Version
      : 1;
    ...
    if (securityVersion == 1 || securityVersion == 6) {
      helpers.addResult(
        results,
        2,
        `Security Center edition is ${versionIdNameMap[securityVersion]}`,
        region
      );
    } else {
      helpers.addResult(
