...
module.exports = {
  ...
  more_info:
    "Alibaba Cloud sends notification via email, SMS or internal message whenever security events happen. " +
  ...
  recommended_action:
    "Enable email, SMS or internal message notifications under Security Center settings.",
  ...
  run: function (cache, settings, callback) {
    ...
    var securityItemsMap = {
      ...
      yundun_sas_cloud_native_firewall_Defense:
        "Container firewall proactive defense notification",
      yundun_sas_cloud_native_firewall:
        "Container firewall exception alert notification",
      ...
      function (region, rcb) {
        ...
        if (describeNoticeConfig.err || !describeNoticeConfig.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query TDS notice config: ${helpers.addError(
              describeNoticeConfig
            )}`,
            region
          );
        ...
        for (let config of describeNoticeConfig.data) {
          if (
            (config.Project &&
              securityItemsMap[config.Project] &&
              !config.Route) ||
            (config.Project &&
              emailOnlyAlerts.includes(config.Project) &&
              (!config.Route || config.Route == 5))
          )
        ...
        if (disabledConfigs.length) {
          helpers.addResult(
            results,
            2,
            `Security notifications are not enabled for: ${disabledConfigs.join(
              ", "
            )}`,
            region
          );
          ...
          helpers.addResult(
            results,
            0,
            "Security notifications are enabled for all alerts",
            region
          );
        }
