...
module.exports = {
  ...
  description:
    "Ensure that all assets are condifgured to be installed with Security Agent.",
  more_info:
    "Security center provides a set of comprehensive endpoint intrusion detection and protection capabilities, " +
  ...
  recommended_action:
    "Go to Security Center console, select Settings, click Agent, on Client to be installed tab, select all items, " +
      ...
      function (region, rcb) {
        ...
        if (describeFieldStatistics.err || !describeFieldStatistics.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query TDS field statistics: ${helpers.addError(
              describeFieldStatistics
            )}`,
            region
          );
        ...
        if (describeFieldStatistics.data.UnprotectedInstanceCount) {
          let msg =
            describeFieldStatistics.data.UnprotectedInstanceCount == 1
              ? "There is 1 unprotected asset"
              : `There are ${describeFieldStatistics.data.UnprotectedInstanceCount} unprotected assets`;
          helpers.addResult(results, 2, msg, region);
        } else {
          helpers.addResult(
            results,
            0,
            "There are no unprotected assets",
            region
          );
        }
