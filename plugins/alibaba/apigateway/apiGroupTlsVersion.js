...
module.exports = {
  ...
  more_info:
    "API Gateway groups should enforce TLS version 1.2.1 to ensure encryption of data in transit with updated features.",
    ...
    for (const region of regions.apigateway) {
      ...
      if (describeApiGroups.err || !describeApiGroups.data) {
        helpers.addResult(
          results,
          3,
          "Unable to describe APIs: " + helpers.addError(describeApiGroups),
          region
        );
        continue;
      }
      ...
      for (const apiGroup of describeApiGroups.data) {
        ...
        if (apiGroup.HttpsPolicy && apiGroup.HttpsPolicy == "HTTPS2_TLS1_2")
          configEnabled = true;
        ...
        helpers.addResult(
          ...
          status,
          `API instance ${
            configEnabled ? "has" : "does not have"
          } latest TLS version`,
          region,
          resource
        );
