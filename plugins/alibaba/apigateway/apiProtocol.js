...
module.exports = {
  ...
  more_info:
    "HTTPS protocol should be implemented for APIs to ensure encryption of data in transit.",
    ...
    for (const region of regions.apigateway) {
      ...
      if (describeApis.err || !describeApis.data) {
        helpers.addResult(
          results,
          3,
          "Unable to describe APIs: " + helpers.addError(describeApis),
          region
        );
        continue;
      }
      ...
      for (var api of describeApis.data) {
        ...
        const resource = helpers.createArn(
          "apigateway",
          accountId,
          "api",
          api.ApiId
        ...
        if (!describeApi || describeApi.err || !describeApi.data) {
          helpers.addResult(
            results,
            3,
            "Unable to describe API: " + helpers.addError(describeApi),
            region,
            resource
          );
          continue;
        ...
        if (
          ...
          describeApi.data.RequestConfig.RequestProtocol.toUpperCase() ===
            "HTTPS"
        )
          secure = true;
        ...
        helpers.addResult(
          results,
          status,
          `API ${secure ? "has" : "does not have"} HTTPS protocol configured`,
          region,
          resource
        );
      }
