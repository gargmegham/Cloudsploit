...
module.exports = {
  ...
  more_info:
    "By default, kubernetes pods accept traffic from any source. But with NetworkPolicy, pods can be configured " +
  ...
  recommended_action:
    "Recreate Kubernetes clusters and select Terway for Network Plug-in option",
  ...
  run: function (cache, settings, callback) {
    ...
    if (describeClusters.err || !describeClusters.data) {
      helpers.addResult(
        results,
        3,
        `Unable to query ACK clusters: ${helpers.addError(describeClusters)}`,
        defaultRegion
      );
    ...
    describeClusters.data.forEach((cluster) => {
      ...
      if (cluster.meta_data) {
        ...
        if (clusterMeta.Addons && clusterMeta.Addons.length)
          found = clusterMeta.Addons.find(
            (addon) => addon.name == "terway-eniip" && !addon.disabled
          );
