      ...
      function (region, rcb) {
        ...
        if (describeLoadBalancers.err || !describeLoadBalancers.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for Application/Network load balancers: ${helpers.addError(
              describeLoadBalancers
            )}`,
            region
          );
        ...
        async.each(describeLoadBalancers.data, function (elb, cb) {
          ...
          if (
            !describeTargetGroups ||
            describeTargetGroups.err ||
            !describeTargetGroups.data
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to query for Application/Network load balancer target groups: ${helpers.addError(
                describeTargetGroups
              )}`,
              region,
              resource
          ...
          if (
            !describeTargetGroups.data.TargetGroups ||
            !describeTargetGroups.data.TargetGroups.length
          ) {
            helpers.addResult(
              results,
              2,
          ...
          async.each(
            ...
            function (targetGroup, tcb) {
              ...
              if (
                !describeTargetHealth ||
                describeTargetHealth.err ||
                !describeTargetHealth.data ||
                !describeTargetHealth.data.TargetHealthDescriptions ||
                !describeTargetHealth.data.TargetHealthDescriptions.length
              ) {
                return tcb();
              }

              describeTargetHealth.data.TargetHealthDescriptions.forEach(
                (healthDescription) => {
                  if (
                    healthDescription.Target &&
                    healthDescription.Target.Id &&
                    healthDescription.TargetHealth &&
                    healthDescription.TargetHealth.State === "healthy"
                  ) {
