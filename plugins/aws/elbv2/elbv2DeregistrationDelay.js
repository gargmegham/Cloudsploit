      ...
      function (region, rcb) {
        ...
        if (describeTargetGroups.err || !describeTargetGroups.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query Application/Network load balancer target groups: ${helpers.addError(
              describeTargetGroups
            )}`,
            region
          );
        ...
        async.each(
          ...
          function (targetGroup, tcb) {
            ...
            var describeTargetGroupAttributes = helpers.addSource(
              cache,
              source,
            ...
            if (
              ...
              describeTargetGroupAttributes.err ||
              !describeTargetGroupAttributes.data ||
              !describeTargetGroupAttributes.data.Attributes
            ) {
              helpers.addResult(
                results,
                3,
                `Unable to query for Application/Network load balancer target group attributes: ${helpers.addError(
                  describeTargetGroupAttributes
                )}`,
                region,
                resource
            ...
            if (describeTargetGroupAttributes.data.Attributes.length) {
              for (var attribute of describeTargetGroupAttributes.data
                .Attributes) {
                if (
                  attribute.Key &&
