        ...
        for (var alb of describeLoadBalancers.data) {
          ...
          if (
            ...
            helpers.addResult(
              results,
              3,
              `Unable to query for ELBv2 Listeners: ${helpers.addError(
                describeListeners
              )}`,
              region,
              alb.LoadBalancerArn
          ...
          describeListeners.data.Listeners.forEach(function (listener) {
            if (listener.SslPolicy) {
              SslPolicy = true;
              if (
                deprecatedALBCipherPolicies.includes(listener.SslPolicy) ||
                deprecatedNLBCipherPolicies.includes(listener.SslPolicy)
              ) {
