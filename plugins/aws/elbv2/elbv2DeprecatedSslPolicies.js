        ...
        for (var alb of describeLoadBalancers.data) {
          ...
          if (
            ...
            helpers.addResult(
              results,
              3,
              `Unable to query for Listeners: ${helpers.addError(
                describeListeners
              )}`,
              region,
              alb.LoadBalancerArn
          ...
          describeListeners.data.Listeners.forEach(function (listener) {
            if (listener.SslPolicy) {
              SslPolicy = true;
              if (
                deprecatedALBPolicies.includes(listener.SslPolicy) ||
                deprecatedNLBPolicies.includes(listener.SslPolicy)
              ) {
          ...
          if (!SslPolicy) {
            ...
            helpers.addResult(
              results,
              2,
              "Load balancer listeners are using these deprecated policies : " +
                depPolicies.join(", "),
              region,
              alb.LoadBalancerArn
            );
            ...
            helpers.addResult(
              results,
              0,
              "Load balancer listeners are using current SSL policies",
              region,
              alb.LoadBalancerArn
            );
          }
