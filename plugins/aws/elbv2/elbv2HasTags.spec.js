...
const getResources = [
  {
    ResourceARN:
      "arn:aws:elasticloadbalancing:us-east-1:111122223333:loadbalancer/test-83",
    Tags: [],
  },
  {
    ResourceARN:
  ...
  describe("run", function () {
    ...
    it("should FAIL if classic load balancer have no tags.", function (done) {
      ...
      elbHasTags.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "ElasticLoadbalancing does not have any tags"
        );
        done();
      });
    });
    ...
    it("should UNKNOWN if unable to describe load balancers", function (done) {
      const cache = createCache(null, {
        message: "Unable to describe Application/Network load balancers",
      });
      elbHasTags.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
