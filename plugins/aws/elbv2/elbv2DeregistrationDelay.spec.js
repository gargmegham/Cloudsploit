...
const describeTargetGroups = [
  {
    TargetGroupArn:
      "arn:aws:elasticloadbalancing:us-east-1:111122223333:targetgroup/temp-tg/fee5b45af37af625",
...
const createCache = (
  ...
  describeTargetGroupsErr,
  describeTargetGroupAttributesErr
) => {
  var targetGroupArn =
    describeTargetGroups && describeTargetGroups.length
      ? describeTargetGroups[0].TargetGroupArn
      : null;
...
describe("elbv2DeregistrationDelay", function () {
  describe("run", function () {
    ...
    it("should FAIL if ELBv2 target group does not have deregistration delay configured", function (done) {
      const cache = createCache(
        [describeTargetGroups[0]],
        describeTargetGroupAttributes[1],
        null,
        null
      );
      elbv2DeregistrationDelay.run(cache, {}, (err, results) => {
    ...
    it("should UNKNOWN if unable to query for ELBv2 target groups", function (done) {
      const cache = createCache(
        [describeTargetGroups[0]],
        null,
        { message: "Unable to describe target groups" },
        null
      );
      elbv2DeregistrationDelay.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if unable to query for ELBv2 target group attributes", function (done) {
      const cache = createCache(
        ...
        describeTargetGroupAttributes[0],
        null,
        { message: "Unable to describe target group attributes" }
      );
      elbv2DeregistrationDelay.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(3);
