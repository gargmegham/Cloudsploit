      ...
      function (region, rcb) {
        ...
        if (describeLoadBalancers.err || !describeLoadBalancers.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for Network Load Balancers: ${helpers.addError(
              describeLoadBalancers
            )}`,
            region
          );
          ...
          if (elb.Type && elb.Type === "network") {
            ...
            if (
              ...
              helpers.addResult(
                results,
                3,
                `Unable to query for Network Load Balancer listeners: ${helpers.addError(
                  describeListeners
                )}`,
                region,
                resource
        ...
        if (!networkElbFound) {
          helpers.addResult(
            results,
            0,
            "No Network Load Balancers found",
            region
          );
        }
