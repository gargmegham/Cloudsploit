...
module.exports = {
  ...
  more_info:
    "Logging requests to ELB endpoints is a helpful way " +
  ...
  compliance: {
    hipaa:
      "HIPAA requires access logging to be enabled for the auditing " +
      ...
      function (region, rcb) {
        ...
        async.each(
          ...
          function (lb, cb) {
            ...
            var describeLoadBalancerAttributes = helpers.addSource(
              cache,
              source,
            ...
            if (
              ...
              for (let attribute of describeLoadBalancerAttributes.data
                .Attributes) {
                if (
                  attribute.Key &&
                  ...
                  if (attribute.Value === "false") {
                    helpers.addResult(
                      results,
                      2,
                      "Logging not enabled for " + lb.DNSName,
                      region,
                      lb.LoadBalancerArn
                    );
                  } else {
                    helpers.addResult(
                      results,
                      0,
                      "Logging enabled for " + lb.DNSName,
                      region,
                      lb.LoadBalancerArn
                    );
                  }
