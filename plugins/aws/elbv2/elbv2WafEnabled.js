      ...
      function (region, rcb) {
        ...
        if (listWebACLs.err || !listWebACLs.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for WAFv2: " + helpers.addError(listWebACLs),
            region
          );
          return rcb();
        }
        if (listRegionalACLs.err || !listRegionalACLs.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for Regional WAF: " +
              helpers.addError(listRegionalACLs),
            region
          );
          return rcb();
        }
        ...
        combinedACLS.forEach((webACL) => {
          if (webACL.WebACLId) {
            ...
            if (
              listResources &&
              listResources.data &&
              listResources.data.ResourceArns &&
              listResources.data.ResourceArns.length
            ) {
              resourcesToCheck = resourcesToCheck.concat(
                listResources.data.ResourceArns
              );
            }
          } else if (webACL.ARN) {
          ...
          function (loc, lcb) {
            ...
            loadBalancers.data.forEach((loadBalancer) => {
              if (
                ...
                resourcesToCheck.indexOf(loadBalancer.LoadBalancerArn) > -1
              ) {
                resourcesToCheck.splice(
                  resourcesToCheck.indexOf(loadBalancer.LoadBalancerArn),
                  1
                );
                helpers.addResult(
                  results,
                  0,
                  "The Application Load Balancer has WAF enabled",
                  loc,
                  loadBalancer.LoadBalancerArn
                );
              } else {
                helpers.addResult(
                  results,
                  2,
                  "The Application Load Balancer does not have WAF enabled",
                  loc,
                  loadBalancer.LoadBalancerArn
                );
              }
