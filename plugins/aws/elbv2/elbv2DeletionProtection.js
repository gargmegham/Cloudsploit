...
module.exports = {
  ...
  more_info:
    "ELBv2 load balancers should be configured with deletion protection to prevent accidental" +
      ...
      function (region, rcb) {
        ...
        async.each(describeLoadBalancers.data, function (elb, cb) {
          ...
          if (
            !elbv2Attributes ||
            elbv2Attributes.err ||
            !elbv2Attributes.data
          ) {
            helpers.addResult(
              results,
              3,
              "Unable to query for Application/Network load balancer attributes: " +
                helpers.addError(elbv2Attributes),
              region,
              resource
            );
            return cb();
          ...
          if (
            !elbv2Attributes.data.Attributes ||
            !elbv2Attributes.data.Attributes.length
          ) {
            helpers.addResult(
              results,
              2,
