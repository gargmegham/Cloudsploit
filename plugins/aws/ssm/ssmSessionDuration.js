...
module.exports = {
  ...
  settings: {
    ssm_session_max_duration: {
      ...
      regex:
        "^((1440)|(14[0-3][0-9]{1})|(1[0-3][0-9]{2})|([1-9][0-9]{2})|([1-9][0-9]{1})|([1-9]))$",
      default: "5",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var sessionMaxDuration =
      settings.ssm_session_max_duration ||
      this.settings.ssm_session_max_duration.default;

    if (!sessionMaxDuration || !sessionMaxDuration.trim().length)
      ...
      function (region, rcb) {
        ...
        if (describeSessions.err || !describeSessions.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for active SSM sessions: " +
              helpers.addError(describeSessions),
            region
          );
          return rcb();
        }
        ...
        const uniqInstances = describeSessions.data.filter(
          (value, index, self) =>
            index ===
            self.findIndex(
              (t) => t.Target && value.Target && t.Target === value.Target
            )
        );
        ...
        const sessionsByInstances = uniqInstances.map((instance) => {
          return {
            ...
            sessions: describeSessions.data.filter(
              (session) => session.Target === instance.Target
            ),
          };
        });
        ...
        for (let instance of sessionsByInstances) {
          ...
          for (let session of instance.sessions) {
            let activeSessionTimeInMins = helpers.minutesBetween(
              new Date(),
              session.StartDate
            );
            ...
            if (
              sessionMaxDuration &&
              sessionMaxDuration < activeSessionTimeInMins
            ) {
          ...
          if (failingSessions.length) {
            ...
            helpers.addResult(
              results,
              0,
              "All SSM Sessions duration length is less than the max time threshold",
              region,
              resource
            );
          }
