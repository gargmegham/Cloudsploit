      ...
      function (region, rcb) {
        ...
        if (
          ...
          helpers.addResult(
            results,
            3,
            "Unable to query SSM describe instance information: " +
        ...
        if (!describeInstanceInformation.data.length) {
          helpers.addResult(results, 0, "No managed instances found", region);
          return rcb();
        }
        ...
        if (listAssociations.err || !listAssociations.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query SSM list associations: " +
              helpers.addError(listAssociations),
            region
          );
          return rcb();
        }
        ...
        if (listAssociations.data.length) {
          listAssociations.data.forEach((association) => {
            if (
              association.Name &&
              association.Name === "AWS-UpdateSSMAgent" &&
              association.Targets &&
              ...
              association.Targets.forEach(function (target) {
                if (
                  ...
                  target.Values.forEach(function (instanceId) {
                    if (
                      !associatedInstances.includes(instanceId) &&
        ...
        describeInstanceInformation.data.forEach(function (instance) {
          ...
          if (
            associatedInstances.includes(instance.InstanceId) ||
            associatedInstances.includes("*")
          ) {
            helpers.addResult(
              results,
              0,
              "Instance has SSM Agent auto update enabled",
              region,
              resource
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Instance does not have SSM Agent auto update enabled",
              region,
              resource
            );
          }
