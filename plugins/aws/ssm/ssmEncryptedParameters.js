...
module.exports = {
  ...
  compliance: {
    ...
    pci:
      "PCI requires proper encryption of cardholder data at rest. SSM " +
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      ssm_encryption_level:
        settings.ssm_encryption_level ||
        this.settings.ssm_encryption_level.default,
      allow_ssm_non_secure_strings:
        settings.allow_ssm_non_secure_strings ||
        this.settings.allow_ssm_non_secure_strings.default,
    };

    config.allow_ssm_non_secure_strings =
      config.allow_ssm_non_secure_strings == "true";
      ...
      function (region, rcb) {
        ...
        if (describeParameters.err || !describeParameters.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for Parameters: ${helpers.addError(
              describeParameters
            )}`,
            region
          );
        ...
        var aliases = helpers.addSource(cache, source, [
          "kms",
          "listAliases",
          region,
        ]);
        ...
        if (!aliases || aliases.err || !aliases.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query KMS Aliases: ${helpers.addError(aliases)}`,
            region
          );
          return rcb();
        }
        ...
        async.each(
          describeParameters.data,
          function (param, pcb) {
            var parameterName =
              param.Name.charAt(0) === "/" ? param.Name.substr(1) : param.Name;
            ...
            if (
              ...
              helpers.addResult(
                results,
                2,
                "Non-SecureString Parameters present",
                region,
                arn
              );
              return pcb();
            ...
            if (
              ...
              helpers.addResult(
                results,
                0,
                "Non-SecureString Parameters present but are allowed",
                region,
                arn
              );
              return pcb();
            ...
            if (!param.KeyId) {
              helpers.addResult(
                results,
                2,
                "SSM Parameters is not encrypted",
                region,
                arn
              );
              return pcb();
            ...
            if (param.KeyId.includes("alias")) {
              ...
              if (!alias || !alias.TargetKeyId) {
                helpers.addResult(
                  results,
                  3,
                  `Unable to locate alias: ${param.KeyId} for SSM Parameter`,
                  region,
                  arn
                );
                return pcb();
