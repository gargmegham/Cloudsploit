...
module.exports = {
  ...
  more_info:
    "Public documents can be viewed by all AWS accounts. To prevent unwanted access to your documents, turn on the block public access sharing setting.",
    ...
    for (const region of regions.ssm) {
      ...
      if (getServiceSetting.err || !getServiceSetting.data) {
        helpers.addResult(
          results,
          3,
          "Unable to query SSM service settings: " +
            helpers.addError(getServiceSetting),
          region,
          resource
        );
        continue;
      ...
      const isPublic =
        getServiceSetting.data.SettingValue &&
        getServiceSetting.data.SettingValue.toUpperCase() == "ENABLE"
          ? true
          : false;
      helpers.addResult(
        results,
        isPublic ? 2 : 0,
        `SSM service has block public sharing ${
