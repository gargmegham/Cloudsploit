...
const listKeys = [
  {
    ...
    KeyArn:
      "arn:aws:kms:us-east-1:112233445566:key/0723d7e2-8655-4553-b4e3-20084f6bddba",
  },
  {
  ...
  describe("run", function () {
    it("should PASS if parameter is encrypted to minimum desired encryption level", function (done) {
      const cache = createCache(
        [describeParameters[0]],
        listKeys,
        listAliases,
        describeKey[0]
      );
      ssmEncryptedParameters.run(
        cache,
        { ssm_encryption_level: "awskms" },
        (err, results) => {
    ...
    it("should PASS if Non-SecureString parameter present but allowed", function (done) {
      ...
      ssmEncryptedParameters.run(
        cache,
        { allow_ssm_non_secure_strings: "true" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          done();
