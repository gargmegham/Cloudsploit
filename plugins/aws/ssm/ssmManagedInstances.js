  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        let ec2Instances = describeInstances.data
        ...
        if (
          ...
          helpers.addResult(
            results,
            3,
            "Unable to query instance information: " +
        ...
        for (let ec2Instance of ec2Instances) {
          ...
          let instanceInfo = describeInstanceInformation.data.find(
            (instanceInfo) =>
              instanceInfo.InstanceId &&
              instanceInfo.InstanceId === ec2Instance.InstanceId
          );
          ...
          if (instanceInfo) {
            helpers.addResult(
              results,
              0,
              `EC2 Instance: ${ec2Instance.InstanceId} is managed by AWS Systems Manager`,
              region,
              arn
            );
          } else {
            helpers.addResult(
              results,
              2,
              `EC2 Instance: ${ec2Instance.InstanceId} is not managed by AWS Systems Manager`,
              region,
              arn
            );
          }
