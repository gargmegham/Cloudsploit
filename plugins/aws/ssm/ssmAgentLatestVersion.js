      ...
      function (region, rcb) {
        ...
        if (!instanceListPass.length && !instanceListFail.length) {
          helpers.addResult(
            results,
            0,
            "No SSM-managed online Linux systems found",
            region
          );
          ...
          instanceListPass.forEach(function (arn) {
            helpers.addResult(
              results,
              0,
              "Instance SSM agent is up to date",
              region,
              arn
            );
          });
          ...
          instanceListFail.forEach(function (arn) {
            helpers.addResult(
              results,
              2,
              "Instance SSM agent is out of date",
              region,
              arn
            );
          });
          ...
          helpers.addResult(
            ...
            "There are " +
              ...
              " instances with an out-of-date SSM agent and " +
              instanceListPass.length +
              " instances with an in-date SSM agent.",
            region
          );
        } else {
          helpers.addResult(
            ...
            "All " +
              instanceListPass.length +
              " instances have an in-date SSM agent.",
            region
          );
        }
