...
module.exports = {
  ...
  settings: {
    ssm_agent_threshold: {
      ...
      description:
        "Sets the value where EC2 instance reporting becomes aggregated once breached.",
      regex: "^[0-9]*$",
      default: 20,
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var threshold =
      settings.ssm_agent_threshold || this.settings.ssm_agent_threshold.default;
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        if (
          describeInstanceInformation.err ||
          !describeInstanceInformation.data
        ) {
          helpers.addResult(
            results,
            3,
            "Unable to query for SSM instance information: " +
              helpers.addError(describeInstanceInformation),
            region
          );
          return rcb();
        }
        ...
        instanceList.forEach(function (id) {
          var arn =
            "arn:aws:ec2:" + region + ":" + accountId + ":instance/" + id;

          if (
            ssmMap[id] &&
            ssmMap[id].PingStatus &&
            ssmMap[id].PingStatus == "Online"
          ) {
        ...
        if (instanceListFail.length + instanceListPass.length <= threshold) {
          instanceListPass.forEach(function (arn) {
            helpers.addResult(
              results,
              0,
              "Instance has SSM agent installed and online",
              region,
              arn
            );
          });
          ...
          instanceListFail.forEach(function (arn) {
            helpers.addResult(
              results,
              2,
              "Instance does not have online SSM agent installed",
              region,
              arn
            );
          });
          ...
          helpers.addResult(
            ...
            2,
            "There are " +
              instanceListFail.length +
              " instances without online SSM agents installed and " +
              instanceListPass.length +
              " instances with online SSM agents installed.",
            region
          );
        } else {
          helpers.addResult(
            ...
            "All " +
              instanceListPass.length +
              " instances have an online SSM agent installed.",
            region
          );
        }
