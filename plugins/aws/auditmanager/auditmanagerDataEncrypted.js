  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.auditmanager_data_encryption_level ||
        this.settings.auditmanager_data_encryption_level.default,
    };
    ...
    for (let region of regions.auditmanager) {
      ...
      if (
        getSettings.err &&
        getSettings.err.message &&
        getSettings.err.message.includes(
          "Please complete AWS Audit Manager setup"
        )
      ) {
        helpers.addResult(
          results,
          0,
          "Audit Manager is not setp up for this region",
          region
        );
        continue;
        ...
        helpers.addResult(
          results,
          3,
          `Unable to query Audit Manager settings: ${helpers.addError(
            getSettings
          )}`,
          region
        );
      ...
      if (getSettings.data.kmsKey && getSettings.data.kmsKey.length) {
        if (getSettings.data.kmsKey.toUpperCase() == "DEFAULT") {
          ...
          var kmsKeyId = getSettings.data.kmsKey.split("/")[1]
            ? getSettings.data.kmsKey.split("/")[1]
            : getSettings.data.kmsKey;
        ...
        helpers.addResult(
          results,
          3,
          "Unable to retrieve encryption settings for Audit Manager data",
          region
        );
      }
    }
