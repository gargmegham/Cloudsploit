  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.databrew_job_encryption_level ||
        this.settings.databrew_job_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listJobs.err || !listJobs.data) {
          helpers.addResult(
            results,
            3,
            `Unable to list DataBrew jobs: ${helpers.addError(listJobs)}`,
            region
          );
          return rcb();
        }
        ...
        if (!listJobs.data.length) {
          helpers.addResult(results, 0, "No DataBrew jobs found", region);
          return rcb();
        }
        ...
        for (let job of listJobs.data) {
          ...
          if (job.EncryptionMode) {
            if (job.EncryptionKeyArn) {
              var kmsKeyId = job.EncryptionKeyArn.split("/")[1]
                ? job.EncryptionKeyArn.split("/")[1]
