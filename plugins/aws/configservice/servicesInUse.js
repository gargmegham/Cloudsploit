...
module.exports = {
  ...
  settings: {
    permitted_services_list: {
      ...
      description:
        "Comma separated list of permitted services such as ec2,iam,s3. Choose only one setting at a time.",
      regex: "^.*$",
      default: "",
    },
    unpermitted_services_list: {
      ...
      description:
        "Comma separated list of unpermitted services such as ec2,iam,s3. Choose only one setting at a time.",
      regex: "^.*$",
      default: "",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      permitted_services_list:
        settings.permitted_services_list ||
        this.settings.permitted_services_list.default,
      unpermitted_services_list:
        settings.unpermitted_services_list ||
        this.settings.unpermitted_services_list.default,
    };

    config.permitted_services_list = config.permitted_services_list.replace(
      /\s/g,
      ""
    );
    config.unpermitted_services_list = config.unpermitted_services_list.replace(
      /\s/g,
      ""
    );

    if (
      !config.permitted_services_list.length &&
      !config.unpermitted_services_list.length
    )
      return callback(null, results, source);

    var checkPermitted = config.permitted_services_list.length > 0;

    config.permitted_services_list = config.permitted_services_list
      .toLowerCase()
      .split(",");
      ...
      function (region, rcb) {
        ...
        for (let resource of discoveredResources.data) {
          if (resource.resourceType) {
            ...
            if (
              newResource.length > 1 &&
              !usedServices.includes(newResource[1].toLowerCase())
            )
              usedServices.push(newResource[1].toLowerCase());
          }
        }
        ...
        for (let value of usedServices) {
          if (allServices[value])
            usedServicesShorthand.push(allServices[value]);
        }

        if (checkPermitted) {
          usedServicesShorthand = usedServicesShorthand.filter(
            (service) => !config.permitted_services_list.includes(service)
          );
          if (usedServicesShorthand.length) {
            helpers.addResult(
              results,
              2,
              "These unpermitted services are being used: " +
                usedServicesShorthand.join(","),
              region
            );
          } else {
            helpers.addResult(
              results,
              0,
              "Only allowed services are being used",
              region
            );
          }
        } else {
          usedServicesShorthand = usedServicesShorthand.filter((service) =>
            config.unpermitted_services_list.includes(service)
          );
          if (usedServicesShorthand.length) {
            helpers.addResult(
              results,
