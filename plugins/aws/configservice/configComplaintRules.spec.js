...
const describeConfigRules = [
  {
    ...
    ConfigRuleArn:
      "arn:aws:config:us-east-1:000011112222:config-rule/config-rule-vhwbj2",
...
const createCache = (
  ...
  describeConfigRulesErr,
  getErr
) => {
  var ruleName =
    describeConfigRules && describeConfigRules.length
      ? describeConfigRules[0].ConfigRuleName
      : null;
  return {
    configservice: {
      describeConfigRules: {
  ...
  describe("run", function () {
    it("should PASS if Amazon Config rule returns compliant evaluation results", function (done) {
      const cache = createCache(
        describeConfigRules,
        getComplianceDetailsByConfigRule[1]
      );
    ...
    it("should FAIL if Amazon Config rule returns noncompliant evaluation results", function (done) {
      const cache = createCache(
        describeConfigRules,
        getComplianceDetailsByConfigRule[0]
      );
      configComplaintRules.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if Unable to query Config Rules", function (done) {
      const cache = createCache(
        describeConfigRules,
        getComplianceDetailsByConfigRule[0],
        { message: "error to query for Config Rules" }
      );
      configComplaintRules.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
