      ...
      function (region, rcb) {
        ...
        if (describeDeliveryChannels.err || !describeDeliveryChannels.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query Config delivery channels: " +
              helpers.addError(describeDeliveryChannels),
            region
          );
          return rcb();
        }
        ...
        if (!describeDeliveryChannels.data.length) {
          helpers.addResult(
            results,
            0,
            "No Config delivery channels found",
            region
          );
          return rcb();
        }
        ...
        for (let record of describeDeliveryChannels.data) {
          ...
          if (
            ...
            helpers.addResult(
              results,
              3,
              "Unable to query S3 bucket: " + helpers.addError(headBucket),
              region,
              "arn:aws:s3:::" + record.s3BucketName
            );
            continue;
          }
        }
