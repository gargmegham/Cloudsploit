...
module.exports = {
  ...
  compliance: {
    pci:
      "PCI requires the development and maintenance of secure applications. " +
  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        var describeConfigurationRecorderStatus = helpers.addSource(
          cache,
          source,
        ...
        if (
          ...
          helpers.addResult(
            results,
            3,
            "Unable to query for Config Service status: " +
              helpers.addError(describeConfigurationRecorderStatus),
            region
          );
          return rcb();
        }
          ...
          if (crs.recording) {
            if (
              ...
              helpers.addResult(
                results,
                0,
                "Config Service is configured, recording, and delivering properly",
                region
              );
            } else {
              helpers.addResult(
            ...
            helpers.addResult(
              results,
              2,
              "Config Service is configured but not recording",
              region
            );
          }
      ...
      function () {
        if (!globalServicesMonitored) {
          helpers.addResult(
            results,
            2,
            "Config Service is not monitoring global services"
          );
        } else {
          helpers.addResult(
            results,
            0,
            "Config Service is monitoring global services"
          );
        }
