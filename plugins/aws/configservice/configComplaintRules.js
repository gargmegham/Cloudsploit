      ...
      function (region, rcb) {
        ...
        if (describeConfigRules.err || !describeConfigRules.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query Config Rules: " +
              helpers.addError(describeConfigRules),
            region
          );
          return rcb();
        }
        ...
        for (let rule of describeConfigRules.data) {
          ...
          var getComplianceDetailsByConfigRule = helpers.addSource(
            cache,
            source,
            [
              "configservice",
          ...
          if (
            !getComplianceDetailsByConfigRule ||
            getComplianceDetailsByConfigRule.err ||
            !getComplianceDetailsByConfigRule.data
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to get Evaluation Results: ${helpers.addError(
                getComplianceDetailsByConfigRule
              )}`,
              region,
              resource
          ...
          if (
            !getComplianceDetailsByConfigRule.data.EvaluationResults ||
            !getComplianceDetailsByConfigRule.data.EvaluationResults.length
          ) {
            helpers.addResult(
              results,
              0,
              "Amazon Config rule returns compliant evaluation results",
              region,
              resource
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Amazon Config rule returns noncompliant evaluation results",
              region,
              resource
            );
          }
