  ...
  describe("run", () => {
    it("should PASS if only allowed services are being usedd", (done) => {
      const cache = createCache(
        ...
        getDiscoveredResourceCounts[1]["resourceCounts"]
      );
      servicesInUse.run(
        cache,
        { permitted_services_list: "iam, ec2, cw, cfn" },
        (err, results) => {
    ...
    it("should FAIL if unpermitted services are being used", (done) => {
      const cache = createCache(
        [describeConfigurationRecorderStatus[0]],
        getDiscoveredResourceCounts[0]["resourceCounts"]
      );
      servicesInUse.run(
        cache,
        { unpermitted_services_list: "iam" },
        (err, results) => {
          expect(results.length).to.equal(1);
