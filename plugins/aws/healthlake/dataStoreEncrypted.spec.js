...
const listFHIRDatastores = [
  {
    ...
    DatastoreArn:
      "arn:aws:healthlake:us-east-1:000111222333:datastore/fhir/7ad17b6c9d48056865a8800b86cc2797",
    DatastoreName: "sadeed-ds1",
    DatastoreStatus: "ACTIVE",
    CreatedAt: "2021-11-23T15:31:55.180000+05:00",
    DatastoreTypeVersion: "R4",
    DatastoreEndpoint:
      "https://healthlake.us-east-1.amazonaws.com/datastore/7ad17b6c9d48056865a8800b86cc2797/r4/",
    SseConfiguration: {
      KmsEncryptionConfig: {
...
describe("dataStoreEncrypted", function () {
  describe("run", function () {
    it("should PASS if HealthLake Data Store is encrypted with desired encryption level", function (done) {
      ...
      dataStoreEncrypted.run(
        cache,
        { healthLake_data_store_encryption: "awscmk" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          expect(results[0].message).to.include(
            "HealthLake data store is encrypted with awscmk"
          );
          done();
        }
      );
    ...
    it("should FAIL if HealthLake Data Store is not encrypted with desired encyption level", function (done) {
      ...
      dataStoreEncrypted.run(
        ...
        (err, results) => {
          ...
          expect(results[0].message).to.include(
            "HealthLake data store is encrypted with awskms"
          );
          done();
        }
      );
    ...
    it("should PASS if no HealthLake Data Store found", function (done) {
      ...
      dataStoreEncrypted.run(cache, {}, (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          "No HealthLake data stores found"
        );
        done();
      });
    });
    ...
    it("should UNKNOWN if unable to list HealthLake Data Store", function (done) {
      const cache = createCache(null, null, null, {
        message: "Unable to list HealthLake Data Store encryption",
      });
      dataStoreEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
