  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.healthlake_datastore_desired_encryption_level ||
        this.settings.healthlake_datastore_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listFHIRDatastores.err || !listFHIRDatastores.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query HealthLake Data Store: ${helpers.addError(
              listFHIRDatastores
            )}`,
            region
          );
