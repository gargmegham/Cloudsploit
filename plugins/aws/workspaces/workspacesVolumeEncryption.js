...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    var desiredEncryptionLevelString =
      settings.workspace_encryption_level ||
      this.settings.workspace_encryption_level.default;
    ...
    const enabledUser =
      "Volume encryption enabled on User volume but not on Root volume";
    const enabledRoot =
      "Volume encryption enabled on Root volume but not on User volume";
    ...
    async.each(
      regions.workspaces,
      function (region, rcb) {
        var listWorkspaces = helpers.addSource(cache, source, [
          "workspaces",
          "describeWorkspaces",
          region,
        ]);
        ...
        if (!listWorkspaces.data || listWorkspaces.err) {
          helpers.addResult(
            results,
            3,
            "Unable to query for WorkSpaces information: " +
              helpers.addError(listWorkspaces),
            region
          );
          return rcb();
        }
        ...
        for (var workspace of listWorkspaces.data) {
          ...
          var queryKeys = listKeys.data.find(
            (key) => key.KeyArn === workspace.VolumeEncryptionKey
          );
          ...
          if (!queryKeys) {
            helpers.addResult(
              results,
              3,
              `Unable to find key with key arn: ${workspace.VolumeEncryptionKey}`,
              region,
              arn
            );
          } else {
            ...
            if (!describeKey || describeKey.err || !describeKey.data) {
              helpers.addResult(
                results,
                3,
                "Unable to query for Key information: " +
            ...
            if (
              ...
              desiredEncryptionLevel <= currentEncryptionLevel
            ) {
              ...
              helpers.addResult(
                results,
                2,
                `Volume encryption is enabled at level ${currentEncryptionLevel}, which is lower than the desired level ${desiredEncryptionLevel}`,
                region,
                arn
              );
            } else {
