...
describe("workspacesInstanceCount", function () {
  describe("run", function () {
    it("should PASS if all Workspaces count is upto the desired threshold i.e. 10", function (done) {
      ...
      workspacesInstanceCount.run(
        cache,
        { workspace_instance_limit: 10 },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          expect(results[0].message).to.include(
            "WorkSpaces Instance count is 1 of 10 desired threshold"
          );
          done();
        }
      );
    ...
    it("should FAIL if Workspaces count is greater than the desired threshold", function (done) {
      ...
      workspacesInstanceCount.run(
        cache,
        { workspace_instance_limit: 2 },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(2);
          expect(results[0].message).to.include(
            "WorkSpaces Instance count is 3 of 2 desired threshold"
          );
          done();
        }
      );
