...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    var workspace_desired_bundle_type =
      settings.workspace_desired_bundle_type ||
      this.settings.workspace_desired_bundle_type.default;

    if (!workspace_desired_bundle_type.length)
        ...
        listWorkspaces.data.forEach((workspace) => {
          ...
          if (
            ...
            workspace_desired_bundle_type
              .toUpperCase()
              .includes(
                workspace.WorkspaceProperties.ComputeTypeName.toUpperCase()
              )
          ) {
            helpers.addResult(
              results,
              0,
              "WorkSpaces instance is using the desired bundle type",
              region,
              resource
            );
          } else {
            helpers.addResult(
              results,
              2,
              "WorkSpaces instance is not using the desired bundle type",
              region,
              resource
            );
          }
