...
module.exports = {
  ...
  settings: {
    workspace_instance_limit: {
      ...
      description:
        "Desired threshold for the number of WorkSpace instances in AWS account.",
      regex: "/[0-9]+/",
      default: "50",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var workspace_instance_limit = parseInt(
      settings.workspace_instance_limit ||
        this.settings.workspace_instance_limit.default
    );
    var instanceCount = 0;
      ...
      function (region, rcb) {
        ...
        if (!listWorkspaces || listWorkspaces.err || !listWorkspaces.data) {
          helpers.addResult(
            results,
            3,
            "Unable to list Workspaces: " + helpers.addError(listWorkspaces),
            region
          );
          return rcb();
        }
      ...
      function () {
        if (instanceCount > workspace_instance_limit) {
          helpers.addResult(
            results,
            2,
            `WorkSpaces Instance count is ${instanceCount} of ${workspace_instance_limit} desired threshold`,
            "global"
          );
        } else {
          helpers.addResult(
