...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      regions.workspaces,
      function (region, rcb) {
        var describeWorkspacesConnectionStatus = helpers.addSource(
          cache,
          source,
        ...
        if (
          describeWorkspacesConnectionStatus.err ||
          !describeWorkspacesConnectionStatus.data
        ) {
          helpers.addResult(
            results,
            3,
            "Unable to query for WorkSpaces instance connection status: " +
              helpers.addError(describeWorkspacesConnectionStatus),
            region
          );
          return rcb();
        }
        ...
        if (!describeWorkspacesConnectionStatus.data.length) {
          helpers.addResult(
            results,
            0,
            "No WorkSpaces instance connection status found",
            region
          );
          return rcb();
        }
        ...
        describeWorkspacesConnectionStatus.data.forEach((workspace) => {
          ...
          if (!workspace.LastKnownUserConnectionTimestamp) {
            helpers.addResult(
              results,
              2,
              "WorkSpace does not have any known user connection",
              region,
              resource
            );
          } else if (
            ...
            helpers.addResult(
              results,
              2,
              `WorkSpace is not in use for last ${helpers.daysBetween(
                new Date(),
                workspace.LastKnownUserConnectionTimestamp
              )}`,
              region,
