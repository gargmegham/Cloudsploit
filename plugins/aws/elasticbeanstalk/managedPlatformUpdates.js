      ...
      function (region, rcb) {
        ...
        if (describeEnvironments.err || !describeEnvironments.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for application environments",
            region
          );
          return rcb();
        }
        ...
        async.each(describeEnvironments.data, function (environment, ecb) {
          ...
          var describeConfigurationSettings = helpers.addSource(cache, source, [
            "elasticbeanstalk",
          ...
          if (
            ...
            helpers.addResult(
              results,
              2,
              "No environment configuration settings found",
              region,
              resource
            );
            return ecb();
          ...
          for (var p in describeConfigurationSettings.data
            .ConfigurationSettings) {
            var param =
              describeConfigurationSettings.data.ConfigurationSettings[p];
              ...
              if (
                ...
                if (setting.Value && setting.Value === "true") {
                  helpers.addResult(
                    results,
                    0,
                    "Managed platform updates for environment: " +
