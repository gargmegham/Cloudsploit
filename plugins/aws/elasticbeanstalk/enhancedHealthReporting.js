      ...
      function (region, rcb) {
        ...
        if (describeEnvironments.err || !describeEnvironments.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for Elastic Beanstalk environments",
            region
          );
          return rcb();
        }
        ...
        for (let environment of describeEnvironments.data) {
          ...
          if (environment.Health && environment.HealthStatus) {
            helpers.addResult(
              results,
              0,
              `Enhanced Health Reporting feature is enabled for environment ${environment.EnvironmentName}.`,
              region,
              resource
            );
          } else {
            helpers.addResult(
              results,
              2,
              `Enhanced Health Reporting feature is not enabled for environment: ${environment.EnvironmentName}`,
              region,
              resource
            );
          }
