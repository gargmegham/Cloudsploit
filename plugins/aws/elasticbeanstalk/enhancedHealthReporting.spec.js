...
const environments = [
  {
    ...
    PlatformArn:
      "arn:aws:elasticbeanstalk:us-east-1::platform/Python 3.7 running on 64bit Amazon Linux 2/3.1.0",
    ...
    EnvironmentLinks: [],
    EnvironmentArn:
      "arn:aws:elasticbeanstalk:us-east-1:123456654321:environment/akhtar-681/Akhtar681-env-2",
  },
  {
    ...
    EnvironmentLinks: [],
    EnvironmentArn:
      "arn:aws:elasticbeanstalk:us-east-1:123456654321:environment/akhtar-681/Akhtar681-env-1",
  },
];
  ...
  describe("run", function () {
    ...
    it("should FAIL if enhanced health reporting is not enabled for application environment", function (done) {
      ...
      enhancedHealthReporting.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).includes(
          "Enhanced Health Reporting feature is not enabled for environment"
        );
        done();
      });
    });
    ...
    it("should PASS if enhanced health reporting is enabled for application environment", function (done) {
      ...
      enhancedHealthReporting.run(cache, {}, (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).includes(
          "Enhanced Health Reporting feature is enabled for environment"
        );
        done();
      });
    });
