...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      ...
      function (region, rcb) {
        var describeEnvironments = helpers.addSource(cache, source, [
          "elasticbeanstalk",
        ...
        if (describeEnvironments.err || !describeEnvironments.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for ElasticBeanstalk environments: " +
              helpers.addError(describeEnvironments),
            region
          );
          return rcb();
        }
        ...
        if (!describeEnvironments.data.length) {
          helpers.addResult(
            results,
            0,
            "No ElasticBeanstalk environments found",
            region
          );
          return rcb();
        }
        ...
        async.each(
          describeEnvironments.data,
          function (environment, ecb) {
            var resource = environment.EnvironmentArn;
            var describeConfigurationSettings = helpers.addSource(
              cache,
              source,
              [
                "elasticbeanstalk",
            ...
            if (
              ...
              helpers.addResult(
                ...
                "Unable to query for environment configuration settings: " +
                  helpers.addError(describeConfigurationSettings),
                region,
                resource
              );
              return ecb();
            ...
            if (
              !describeConfigurationSettings.data.ConfigurationSettings.length
            ) {
              helpers.addResult(
                results,
                2,
                "Environment does not have any log configuration",
                region,
                resource
              );
              return ecb();
            ...
            let OptionSettings =
              describeConfigurationSettings.data.ConfigurationSettings.map(
                ({ OptionSettings }) => OptionSettings
              );
            let persistentLogs = OptionSettings.flat().filter(
              (option) =>
                option.OptionName === "LogPublicationControl" ||
                option.OptionName === "StreamLogs"
            );
            ...
            if (!persistentLogs || !persistentLogs.length) {
              helpers.addResult(
                results,
                2,
                "Environment Persistent Logs for environment: " +
              ...
              persistentLogs[0] &&
              persistentLogs[0].Value === "false" &&
              persistentLogs[1] &&
              persistentLogs[1].Value === "false"
            ) {
              helpers.addResult(
                results,
                2,
