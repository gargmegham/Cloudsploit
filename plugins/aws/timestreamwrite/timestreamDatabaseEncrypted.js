  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.timestream_databases_desired_encryption_level ||
        this.settings.timestream_databases_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listDatabases.err || !listDatabases.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query Timestream databases: ${helpers.addError(
              listDatabases
            )}`,
            region
          );
        ...
        for (let database of listDatabases.data) {
          ...
          if (database.KmsKeyId) {
            var kmsKeyId = database.KmsKeyId.split("/")[1]
              ? database.KmsKeyId.split("/")[1]
              : database.KmsKeyId;
