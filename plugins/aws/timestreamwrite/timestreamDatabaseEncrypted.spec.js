...
const listDatabases = [
  {
    ...
    TableCount: 0,
    KmsKeyId:
      "arn:aws:kms:us-east-1:000011112222:key/b6d7913b-5604-47c5-8291-8270a1abab58",
...
const createCache = (
  ...
  var keyId =
    databases && databases.length && databases[0].KmsKeyId
      ? databases[0].KmsKeyId.split("/")[1]
      : null;
  return {
    timestreamwrite: {
      listDatabases: {
...
describe("timestreamDatabaseEncrypted", function () {
  describe("run", function () {
    it("should PASS if Timestream database is encrypted with desired encryption level", function (done) {
      ...
      timestreamDatabaseEncrypted.run(
        cache,
        { timestream_databases_desired_encryption_level: "awscmk" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          expect(results[0].message).to.include(
            "Timestream database is encrypted with awscmk"
          );
          done();
        }
      );
    ...
    it("should FAIL if Timestream database is not encrypted with desired encyption level", function (done) {
      ...
      timestreamDatabaseEncrypted.run(
        ...
        (err, results) => {
          ...
          expect(results[0].message).to.include(
            "Timestream database is encrypted with awskms"
          );
          done();
        }
      );
    ...
    it("should UNKNOWN if unable to list Timestream databases", function (done) {
      const cache = createCache(null, null, null, {
        message: "Unable to list Timestream databases encryption",
      });
      timestreamDatabaseEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
