...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    var redshift_nodes_count = parseInt(
      settings.redshift_nodes_count ||
        this.settings.redshift_nodes_count.default
    );
      ...
      function (region, rcb) {
        ...
        if (nodesCount <= redshift_nodes_count) {
          helpers.addResult(
            results,
            0,
            `Region contains "${nodesCount}" provisioned Redshift nodes of "${redshift_nodes_count}" limit`,
            region
          );
        } else {
          helpers.addResult(
