...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    var redshift_cluster_node_type =
      settings.redshift_cluster_node_type ||
      this.settings.redshift_cluster_node_type.default;
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        describeClusters.data.forEach((cluster) => {
          ...
          if (
            cluster.NodeType &&
            cluster.NodeType === redshift_cluster_node_type
          ) {
            helpers.addResult(
              results,
              0,
              "Redshift cluster is using the desired node type",
              region,
              resource
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Redshift cluster is not using the desired node type",
              region,
              resource
            );
          }
