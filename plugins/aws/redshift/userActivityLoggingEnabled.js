      ...
      function (region, rcb) {
        ...
        if (describeClusters.err || !describeClusters.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for Redshift clusters: ${helpers.addError(
              describeClusters
            )}`,
            region
          );
        ...
        if (
          !describeClusterParameterGroups ||
          describeClusterParameterGroups.err ||
          !describeClusterParameterGroups.data
        ) {
          helpers.addResult(
            results,
            3,
            `Unable to query for Redshift cluster parameter groups: ${helpers.addError(
              describeClusterParameterGroups
            )}`,
            region
          );
          ...
          if (cluster.ClusterParameterGroups.length) {
            for (var cg in cluster.ClusterParameterGroups) {
              ...
              if (!groupName.startsWith("default.redshift")) {
                var describeClusterParameters = helpers.addSource(
                  cache,
                  source,
                ...
                if (
                  ...
                  helpers.addResult(
                    results,
                    3,
                    `Unable to query parameter group "${groupName}": ${helpers.addError(
                      describeClusterParameters
                    )}`,
                    region,
                    resource
