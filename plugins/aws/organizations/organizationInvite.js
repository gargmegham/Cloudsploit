...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    var listHandshakesForAccount = helpers.addSource(cache, source, [
      "organizations",
    ...
    if (!listHandshakesForAccount.data || listHandshakesForAccount.err) {
      helpers.addResult(
        results,
        3,
        "Cannot list organization handshakes",
        "global"
      );
      return callback(null, results, source);
    }

    var invalidHandshakes = listHandshakesForAccount.data.filter(
      (handshake) => handshake.State === "OPEN" && handshake.Action === "INVITE"
    );
    ...
    if (!invalidHandshakes.length) {
      ...
      for (let invalidHandshake of invalidHandshakes) {
        helpers.addResult(
          results,
          2,
          "Unaccepted pending organization invitations",
          "global",
          invalidHandshake.Arn
        );
      }
    }
