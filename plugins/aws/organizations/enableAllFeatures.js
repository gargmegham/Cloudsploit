...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    var describeOrganization = helpers.addSource(cache, source, [
      "organizations",
    ...
    if (!describeOrganization.data || describeOrganization.err) {
      if (
        !describeOrganization.err ||
        describeOrganization.err.code !== "AWSOrganizationsNotInUseException"
      ) {
        helpers.addResult(
          results,
          3,
          "Cannot describe the organization",
          "global"
        );
      }
    ...
    if (describeOrganization.data.FeatureSet !== "ALL") {
      helpers.addResult(
        results,
        2,
        "Not all Organization features are enabled",
        "global",
        describeOrganization.data.MasterAccountArn
      );
    } else {
      helpers.addResult(
        results,
        0,
        "All Organization features are enabled",
        "global",
        describeOrganization.data.MasterAccountArn
      );
    }
