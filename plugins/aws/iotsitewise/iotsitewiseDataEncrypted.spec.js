...
const createCache = (
  ...
  keys,
  describeKey,
  describeDefaultEncryptionConfigurationErr,
  keysErr,
  describeKeyErr
) => {
  ...
  describe("run", function () {
    it("should FAIL if current encryption level is less than desired encryption level", function (done) {
      const cache = createCache(
        describeDefaultEncryptionConfiguration[1],
        listKeys,
        describeKey[1]
      );
      iotsitewiseDataEncrypted.run(
        cache,
        { iot_sitewise_data_desired_encryption_level: "awscmk" },
        (err, results) => {
    ...
    it("should PASS if current encryption level is greater than or equal to desired encryption level", function (done) {
      const cache = createCache(
        describeDefaultEncryptionConfiguration[0],
        listKeys,
        describeKey[0]
      );
      iotsitewiseDataEncrypted.run(
