  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.iot_sitewise_data_desired_encryption_level ||
        this.settings.iot_sitewise_data_desired_encryption_level.default,
    };
    ...
    async.each(
      regions.iotsitewise,
      function (region, rcb) {
        var describeDefaultEncryptionConfiguration = helpers.addSource(
          cache,
          source,
        ...
        if (
          describeDefaultEncryptionConfiguration.err ||
          !describeDefaultEncryptionConfiguration.data
        ) {
          helpers.addResult(
            results,
            3,
            `Unable to query IoT SiteWise encryption configuration: ${helpers.addError(
        ...
        if (
          describeDefaultEncryptionConfiguration.data.encryptionType ==
