  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.finspace_environment_desired_encryption_level ||
        this.settings.finspace_environment_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listEnvironments.err || !listEnvironments.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query FinSpace Environment: ${helpers.addError(
              listEnvironments
            )}`,
            region
          );
        ...
        for (let environment of listEnvironments.data) {
          ...
          if (environment.kmsKeyId) {
            var keyId = environment.kmsKeyId.split("/")[1]
              ? environment.kmsKeyId.split("/")[1]
            ...
            helpers.addResult(
              results,
              3,
              "Unable to find encryption key for environment",
              region,
              resource
            );
            continue;
