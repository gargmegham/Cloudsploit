...
const listEnvironments = [
  {
    ...
    environmentArn:
      "arn:aws:finspace:us-east-1:000011112222:environment/yk7kg7l6ab3yr42gse4jv3",
    sageMakerStudioDomainUrl:
      "https://d-7mzarbqaxxay.studio.us-east-1.sagemaker.aws",
    kmsKeyId:
...
const createCache = (
  ...
  var keyId =
    environments && environments.length && environments[0].kmsKeyId
      ? environments[0].kmsKeyId.split("/")[1]
      : null;
  return {
    finspace: {
      listEnvironments: {
...
describe("finspaceEnvironmentEncrypted", function () {
  describe("run", function () {
    it("should PASS if FinSpace Environment is encrypted with desired encryption level", function (done) {
      ...
      finspaceEnvironmentEncrypted.run(
        cache,
        { finspace_environment_encryption: "awscmk" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          expect(results[0].message).to.include(
            "FinSpace environment is encrypted with awscmk"
          );
          done();
        }
      );
    ...
    it("should FAIL if FinSpace Environment is not encrypted with desired encyption level", function (done) {
      ...
      finspaceEnvironmentEncrypted.run(
        ...
        (err, results) => {
          ...
          expect(results[0].message).to.include(
            "FinSpace environment is encrypted with awskms"
          );
          done();
        }
      );
    ...
    it("should UNKNOWN if unable to list FinSpace Environment", function (done) {
      const cache = createCache(null, null, null, {
        message: "Unable to list FinSpace Environment encryption",
      });
      finspaceEnvironmentEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
