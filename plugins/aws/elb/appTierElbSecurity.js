...
module.exports = {
  ...
  settings: {
    ...
    latest_security_policies: {
      ...
      description:
        "A comma-delimited list of security policies that indicates the latest predefined security policy versions",
      regex: "[a-zA-Z0-9-,]",
      default:
        "ELBSecurityPolicy-2016-08,ELBSecurityPolicy-TLS-1-2-2017-01,ELBSecurityPolicy-TLS-1-1-2017-01",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      elb_app_tier_tag_key:
        settings.elb_app_tier_tag_key ||
        this.settings.elb_app_tier_tag_key.default,
      latest_security_policies:
        settings.latest_security_policies ||
        this.settings.latest_security_policies.default,
    };

    if (!config.elb_app_tier_tag_key.length)
      return callback(null, results, source);

    config.latest_security_policies =
      config.latest_security_policies.split(",");
      ...
      function (region, rcb) {
        ...
        if (describeLoadBalancers.err || !describeLoadBalancers.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for load balancers: ${helpers.addError(
              describeLoadBalancers
            )}`,
            region
          );
        ...
        async.each(
          ...
          function (lb, cb) {
            ...
            if (
              ...
              helpers.addResult(
                results,
                3,
                `Unable to query load balancer tags for ELB "${
                  lb.LoadBalancerName
            ...
            if (appTierTag) {
              ...
              var describeLoadBalancerPolicies = helpers.addSource(
                cache,
                source,
              ...
              if (
                ...
                helpers.addResult(
                  results,
                  3,
                  `Unable to query policies for ELB "${
                    lb.LoadBalancerName
                  }": ${helpers.addError(describeLoadBalancerPolicies)}`,
                  region,
                  resource
              ...
              describeLoadBalancerPolicies.data.PolicyDescriptions.forEach(
                function (policyDesc) {
                  if (policyDesc.PolicyAttributeDescriptions) {
                    for (var policyAttrDesc of policyDesc.PolicyAttributeDescriptions) {
                      if (
                        policyAttrDesc.AttributeName &&
                        policyAttrDesc.AttributeName ===
                          "Reference-Security-Policy"
                      ) {
                        securityPolicyFound = true;
                        if (
                          !config.latest_security_policies.includes(
                            policyAttrDesc.AttributeValue
                          )
                        ) {
                          insecurePolicy = true;
                          break;
          ...
          function () {
            if (!appTierElbFound) {
              helpers.addResult(results, 0, "No App-Tier ELB found", region);
            }
            rcb();
          }
        );
