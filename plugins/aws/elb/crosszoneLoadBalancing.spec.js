...
const createCache = (
  ...
  describeLoadBalancersErr,
  describeLoadBalancerAttributesErr
) => {
  var dnsName =
    describeLoadBalancers && describeLoadBalancers.length
      ? describeLoadBalancers[0].DNSName
      : null;
...
describe("crosszoneLoadBalancing", function () {
  describe("run", function () {
    ...
    it("should FAIL if AWS ELB does not have cross zone load balancing enabled", function (done) {
      const cache = createCache(
        [describeLoadBalancers[0]],
        describeLoadBalancerAttributes[1]
      );
      crosszoneLoadBalancing.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if unable to query for load balancer attributes", function (done) {
      const cache = createCache([describeLoadBalancers[0]], null, null, {
        message: "Unable to query for load balancers attributes",
      });
      crosszoneLoadBalancing.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(3);
