...
const createCache = (
  ...
  var keyId =
    logGroups && logGroups.length && logGroups[0].kmsKeyId
      ? logGroups[0].kmsKeyId.split("/")[1]
      : null;
  return {
    cloudwatchlogs: {
      describeLogGroups: {
...
describe("logGroupsEncrypted", function () {
  describe("run", function () {
    it("should PASS if CloudWatch Logs log group is encrypted with desired encryption level", function (done) {
      ...
      logGroupsEncrypted.run(
        cache,
        { cloudwatchlog_groups_desired_encryption_level: "awscmk" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          expect(results[0].message).to.include(
            "CloudWatch log group is encrypted with awscmk"
          );
    ...
    it("should FAIL if CloudWatch Logs log groups is not encrypted with desired encyption level", function (done) {
      ...
      logGroupsEncrypted.run(
        ...
        (err, results) => {
          ...
          expect(results[0].message).to.include(
            "CloudWatch log group is encrypted with awskms"
          );
          expect(results[0].region).to.equal("us-east-1");
          done();
        }
    ...
    it("should UNKNOWN if unable to list CloudWatch Logs log groups", function (done) {
      const cache = createCache(null, null, null, {
        message: "Unable to list CloudWatch Logs log groups encryption",
      });
      logGroupsEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
