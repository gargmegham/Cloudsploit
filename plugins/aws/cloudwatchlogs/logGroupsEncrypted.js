...
module.exports = {
  ...
  settings: {
    ...
    cloudwatchlog_whitelist: {
      ...
      description:
        "A comma-delimited list of known lambda function Function Names that should be whitelisted",
      regex: "^.{1,255}$",
      default:
        "Aqua-CSPM-Token-Rotator-Function,-CreateCSPMKeyFunction-,-TriggerDiscoveryFunction-,-GenerateVolumeScanningEx-,-GenerateCSPMExternalIdFu-",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.cloudwatchlog_groups_desired_encryption_level ||
        this.settings.cloudwatchlog_groups_desired_encryption_level.default,
      cloudwatchlog_whitelist:
        settings.cloudwatchlog_whitelist ||
        this.settings.cloudwatchlog_whitelist.default,
    };
    ...
    if (
      ...
      config.cloudwatchlog_whitelist =
        config.cloudwatchlog_whitelist.split(",");
    } else {
      config.cloudwatchlog_whitelist = [];
    }
      ...
      function (region, rcb) {
        ...
        if (describeLogGroups.err || !describeLogGroups.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query CloudWatch log groups: ${helpers.addError(
              describeLogGroups
            )}`,
            region
          );
        ...
        for (let logGroup of describeLogGroups.data) {
          ...
          if (!logGroup.kmsKeyId) {
            ...
            var kmsKeyId = logGroup.kmsKeyId.split("/")[1]
              ? logGroup.kmsKeyId.split("/")[1]
              : logGroup.kmsKeyId;
