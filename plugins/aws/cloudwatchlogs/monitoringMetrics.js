      ...
      function (region, rcb) {
        ...
        if (describeTrails.err || !describeTrails.data) {
          helpers.addResult(
            results,
            3,
            `Unable to describe CloudTrail trails: ${helpers.addError(
              describeTrails
            )}`,
            region
          );
        ...
        if (!trailsInRegion.length) {
          helpers.addResult(
            results,
            0,
            "No CloudTrail trails found in current home region",
            region
          );
          return rcb();
        }
        ...
        if (
          !describeMetricFilters ||
          describeMetricFilters.err ||
          !describeMetricFilters.data
        ) {
          helpers.addResult(
            results,
            3,
            "Unable to query for CloudWatchLogs metric filters: " +
              helpers.addError(describeMetricFilters),
            region
          );
          return rcb();
        }
        ...
        if (!describeMetricFilters.data.length) {
          helpers.addResult(
            results,
            2,
            "There are no CloudWatch metric filters in this region",
            region
          );
          return rcb();
        }
        ...
        for (var f in describeMetricFilters.data) {
          ...
          if (filter.logGroupName && filter.filterPattern) {
            if (!filters[filter.logGroupName])
              filters[filter.logGroupName] = [];
            filters[filter.logGroupName].push(
        ...
        async.each(
          ...
          function (trail, tcb) {
            ...
            var startPos =
              trail.CloudWatchLogsLogGroupArn.indexOf("log-group:") + 10;
            var endPos = trail.CloudWatchLogsLogGroupArn.lastIndexOf(":");
            var logGroupName = trail.CloudWatchLogsLogGroupArn.substring(
              startPos,
              endPos
            );
            ...
            if (!filters[logGroupName]) {
              helpers.addResult(
                results,
                2,
                "There are no CloudWatch metric filters for this trail",
                region,
                trail.TrailARN
              );
            ...
            for (var p in filterPatterns) {
              ...
              var patternSearch = pattern.pattern
                .replace(/\s+/g, "")
                .toLowerCase();
            ...
            if (missing.length) {
              helpers.addResult(
                results,
                2,
                "Trail logs are missing filters for: " + missing.join(", "),
                region,
                trail.TrailARN
              );
            } else {
              helpers.addResult(
                results,
                0,
                "Trail logs have filter patterns for all required metrics",
                region,
                trail.TrailARN
              );
            }
