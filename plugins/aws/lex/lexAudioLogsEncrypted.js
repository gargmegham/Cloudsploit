...
module.exports = {
  ...
  settings: {
    audio_logs_desired_encryption_level: {
      ...
      description:
        "In order (lowest to highest) sse=S3-SSE; awscmk=Customer managed KMS; externalcmk=Customer managed externally sourced KMS; cloudhsm=Customer managed CloudHSM sourced KMS",
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.audio_logs_desired_encryption_level ||
        this.settings.audio_logs_desired_encryption_level.default,
    };
        ...
        for (let bot of listBots.data) {
          ...
          if (!listBotAliases || listBotAliases.err || !listBotAliases.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for Lex bot aliases: " +
                bot.botId +
                ": " +
                helpers.addError(listBotAliases),
              region,
              resource
            );
            continue;
          ...
          if (
            !listBotAliases.data.botAliasSummaries ||
            !listBotAliases.data.botAliasSummaries.length
          ) {
            helpers.addResult(
              results,
              0,
              "No bot aliases found",
              region,
              resource
            );
            continue;
          ...
          for (let alias of listBotAliases.data.botAliasSummaries) {
            ...
            if (
              ...
              helpers.addResult(
                results,
                3,
                "Unable to query Lex bot alias: " +
                  helpers.addError(describeBotAlias),
                region,
                aliasArn
              );
              continue;
            ...
            if (
              !describeBotAlias.data.conversationLogSettings ||
              !describeBotAlias.data.conversationLogSettings.audioLogSettings
            ) {
              helpers.addResult(
                results,
                0,
                "Bot alias does not have any audio logs configured",
                region,
                aliasArn
              );
              continue;
            ...
            for (let audioLog of describeBotAlias.data.conversationLogSettings
              .audioLogSettings) {
            ...
            if (!found) {
              helpers.addResult(
                results,
                2,
                "Bot alias is not saving audio logs on S3",
                region,
                aliasArn
              );
              continue;
