  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.codeartifact_domain_encryption_level ||
        this.settings.codeartifact_domain_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listDomains.err || !listDomains.data) {
          helpers.addResult(
            results,
            3,
            `Unable to list CodeArtifact domains: ${helpers.addError(
              listDomains
            )}`,
            region
          );
        ...
        for (let domain of listDomains.data) {
          ...
          if (domain.encryptionKey) {
            var kmsKeyId = domain.encryptionKey.split("/")[1]
              ? domain.encryptionKey.split("/")[1]
