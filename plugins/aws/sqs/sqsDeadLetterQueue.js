      ...
      function (region, rcb) {
        ...
        if (listQueues.err) {
          helpers.addResult(
            results,
            3,
            `Unable to query for Amazon SQS queues: ${helpers.addError(
              listQueues
            )}`,
            region
          );
          ...
          function (queueUrl, cb) {
            ...
            if (
              ...
              helpers.addResult(
                results,
                3,
                `Unable to query queue attributes for Amazon SQS queue: ${helpers.addError(
                  getQueueAttributes
                )}`,
                region,
                resource
            ...
            if (
              getQueueAttributes.data.Attributes.RedrivePolicy &&
