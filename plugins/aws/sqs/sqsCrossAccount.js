...
module.exports = {
  ...
  compliance: {
    pci:
      "PCI requires that cardholder data can only be accessed by those with " +
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      sqs_whitelisted_aws_account_principals:
        settings.sqs_whitelisted_aws_account_principals ||
        this.settings.sqs_whitelisted_aws_account_principals.default,
      sqs_whitelist_aws_organization_accounts:
        settings.sqs_whitelist_aws_organization_accounts ||
        this.settings.sqs_whitelist_aws_organization_accounts.default,
      sqs_queue_policy_condition_keys:
        settings.sqs_queue_policy_condition_keys ||
        this.settings.sqs_queue_policy_condition_keys.default,
    };

    var allowedConditionKeys =
      config.sqs_queue_policy_condition_keys.split(",");
    var whitelistOrganization =
      config.sqs_whitelist_aws_organization_accounts == "true";
      ...
      function (region, rcb) {
        ...
        if (listQueues.err) {
          helpers.addResult(
            results,
            3,
            "Unable to query for SQS queues: " + helpers.addError(listQueues),
            region
          );
          return rcb();
        }
        ...
        listQueues.data.forEach((queue) => {
            ...
            if (crossAccountAccess) {
              if (
                ...
                for (let principal of crossAccountPrincipals) {
                  if (
                    config.sqs_whitelisted_aws_account_principals.includes(
                      principal
                    )
                  )
                    continue;
                  if (
                    whitelistOrganization &&
          ...
          if (globalActions.length) {
            helpers.addResult(
              results,
              2,
              "The SQS queue policy allows global access to the action(s): " +
                globalActions,
              region,
              queueArn
            );
            ...
            helpers.addResult(
              results,
              2,
              "The SQS queue policy allows cross-account access to the action(s): " +
                crossAccountActions,
              region,
              queueArn
            );
