...
module.exports = {
  ...
  settings: {
    sqs_queues_desired_encryption_level: {
      ...
      description:
        "In order (lowest to highest) sse=SSE-SQS; awskms=AWS-managed KMS; awscmk=Customer managed KMS; externalcmk=Customer managed externally sourced KMS; cloudhsm=Customer managed CloudHSM sourced KMS",
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.sqs_queues_desired_encryption_level ||
        this.settings.sqs_queues_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listQueues.err) {
          helpers.addResult(
            results,
            3,
            "Unable to query SQS queues: " + helpers.addError(listQueues),
            region
          );
          return rcb();
        }
        ...
        for (let queue of listQueues.data) {
          ...
          if (
            ...
            helpers.addResult(
              results,
              3,
              `Unable to get SQS queues description: ${helpers.addError(
                getQueueAttributes
              )}`,
              region,
              resource
          ...
          if (
            !getQueueAttributes.data.Attributes.KmsMasterKeyId &&
