...
const getQueueAttributes = [
  ...
  },
  {
    ...
    Attributes: {
      ...
      DelaySeconds: "0",
      Policy:
        '{"Version":"2008-10-17","Id":"__default_policy_ID","Statement":[{"Sid":"__owner_statement","Effect":"Allow","Principal":{"AWS":"arn:aws:iam::000011112222:root"},"Action":"SQS:*","Resource":"arn:aws:sqs:us-east-1:000011112222:mine1"}]}',
      ReceiveMessageWaitTimeSeconds: "0",
  ...
  describe("run", function () {
    it("should PASS if SQS queue is encrypted with desired encryption level", function (done) {
      ...
      sqsEncryptionEnabled.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "SQS queue is encrypted with awscmk"
        );
    ...
    it("should FAIL if SQS queue not encrypted with desired encryption level", function (done) {
      ...
      sqsEncryptionEnabled.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "SQS queue is encrypted with sse"
        );
        expect(results[0].region).to.equal("us-east-1");
        done();
      });
    ...
    it("should UNKNOWN if unable to query SQS queues", function (done) {
      const cache = createCache(
        ...
        null,
        null,
        { message: "Unable to query SQS queues" }
      );
      sqsEncryptionEnabled.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
