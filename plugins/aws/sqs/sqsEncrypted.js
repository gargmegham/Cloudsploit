...
module.exports = {
      ...
      function (region, rcb) {
        ...
        async.each(
          ...
          function (queue, cb) {
            ...
            if (
              !getQueueAttributes ||
              (!getQueueAttributes.err && !getQueueAttributes.data)
            )
              return cb();
            ...
            if (getQueueAttributes.data.Attributes.KmsMasterKeyId) {
              if (
                ...
                helpers.addResult(
                  results,
                  1,
                  "The SQS queue uses the default KMS key (" +
                    defaultKmsKey +
                    ") for SSE",
                  region,
                  queueArn
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    if (
      !queAttributes ||
      queAttributes.err ||
      !Object.keys(queAttributes).length
    ) {
    ...
    for (var qUrl in queAttributes[queueLocation]) {
      if (
        queAttributes[queueLocation][qUrl].data.Attributes.QueueArn === resource
      ) {
        queueUrl = qUrl;
        break;
      }
    ...
    if (settings.input && settings.input.kmsKeyIdforSqs) {
      params = {
        Attributes: {
      ...
      if (!defaultKmsKeyId)
        return callback(`No default SQS key for the region ${config.region}`);
      params = {
        Attributes: {
