      ...
      Shards: [
        {
          ...
          SequenceNumberRange: {
            StartingSequenceNumber:
              "49625307919684448350212376424208934206295946525447028738",
          },
        },
      ],
      ...
      Shards: [
        {
          ...
          SequenceNumberRange: {
            StartingSequenceNumber:
              "49625308470958869657889380482971889906163549124874469378",
          },
        },
      ],
      ...
      Shards: [
        {
          ...
          SequenceNumberRange: {
            StartingSequenceNumber:
              "49625308561945910067894322900437620458568881592741134338",
          },
        },
      ],
  ...
  describe("run", function () {
    it("should PASS if Kinesis stream is encrypted with desired encryption level", function (done) {
      ...
      kinesisDataStreamsEncrypted.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Kinesis stream is encrypted with awscmk"
        );
    ...
    it("should FAIL if Kinesis stream not encrypted with desired encryption level", function (done) {
      ...
      kinesisDataStreamsEncrypted.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Kinesis stream is encrypted with awskms"
        );
        expect(results[0].region).to.equal("us-east-1");
        done();
      });
    ...
    it("should UNKNOWN if unable to to query for Kinesis streams", function (done) {
      const cache = createCache(
        ...
        null,
        null,
        { message: "Unable to to query for Kinesis streams" }
      );
      kinesisDataStreamsEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
