...
module.exports = {
  ...
  more_info:
    "Data sent to Kinesis Streams can be encrypted using KMS server-side encryption. Existing streams can be modified to add encryption with minimal overhead.",
  ...
  compliance: {
    hipaa:
      "Kinesis encryption must be used when processing any HIPAA-related data. " +
  ...
  remediation_description:
    "Encryption for the affected Kinesis streams will be enabled.",
        ...
        async.each(
          ...
          function (stream, cb) {
            ...
            if (describeStream.data.StreamDescription.KeyId) {
              if (
                describeStream.data.StreamDescription.KeyId === defaultKmsKey
              ) {
                helpers.addResult(
                  results,
                  1,
                  "The Kinesis stream uses the default KMS key (" +
                    defaultKmsKey +
                    ") for SSE",
                  region,
                  streamArn
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    var defaultKeyDesc =
      "Default master key that protects my Kinesis data when no other key is defined";
    ...
    if (settings.input && settings.input.kmsKeyIdforKinesis) {
      params = {
      ...
      var defaultKmsKeyId = helpers.getDefaultKeyId(
        cache,
        config.region,
        defaultKeyDesc
      );
      if (!defaultKmsKeyId)
        return callback(
          `No default Kinesis key for the region ${config.region}`
        );
      params = {
        EncryptionType: "KMS" /* required */,
