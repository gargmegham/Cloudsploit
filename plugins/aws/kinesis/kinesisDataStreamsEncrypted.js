...
module.exports = {
  ...
  more_info:
    "Data sent to Kinesis data streams can be encrypted using KMS server-side encryption. Existing streams can be modified to add encryption with minimal overhead. " +
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.data_streams_desired_encryption_level ||
        this.settings.data_streams_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listStreams.err) {
          helpers.addResult(
            results,
            3,
            "Unable to query for Kinesis streams: " +
              helpers.addError(listStreams),
            region
          );
          return rcb();
        }
        ...
        for (let stream of listStreams.data) {
          ...
          if (
            ...
            describeStream.err ||
            !describeStream.data ||
            !describeStream.data.StreamDescription
          ) {
            helpers.addResult(
              results,
              3,
              "Unable to query Kinesis for stream: " +
                stream +
                ": " +
                helpers.addError(describeStream),
              region,
              resource
            );
            continue;
