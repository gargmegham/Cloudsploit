...
module.exports = {
  ...
  more_info:
    "Server-side encryption is always enabled on Kinesis video streams data. If a user-provided key is not specified when the stream is created, the default key (provided by Kinesis Video Streams) is used. " +
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.video_stream_data_desired_encryption_level ||
        this.settings.video_stream_data_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listStreams.err || !listStreams.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query Kinesis Video Streams: ${helpers.addError(
              listStreams
            )}`,
            region
          );
        ...
        if (!listStreams.data.length) {
          helpers.addResult(
            results,
            0,
            "No Kinesis Video Streams found",
            region
          );
          return rcb();
        }
        ...
        for (let streamData of listStreams.data) {
          ...
          if (streamData.KmsKeyId) {
            var kmsKeyId = streamData.KmsKeyId.split("/")[1]
              ? streamData.KmsKeyId.split("/")[1]
              : streamData.KmsKeyId;
