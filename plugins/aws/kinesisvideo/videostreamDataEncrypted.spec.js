...
const listStreams = [
  {
    StreamName: "test1",
    StreamARN:
      "arn:aws:kinesisvideo:us-east-1:000111222333:stream/test1/1639473904764",
    KmsKeyId:
...
describe("videostreamDataEncrypted", function () {
  describe("run", function () {
    it("should PASS if Kinesis Video Streams data is using desired encryption level", function (done) {
      ...
      videostreamDataEncrypted.run(
        cache,
        { video_stream_data_desired_encryption_level: "awscmk" },
        (err, results) => {
          console.log(results);
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          expect(results[0].message).to.include(
            "Kinesis Video Streams data is using awscmk"
          );
          done();
        }
      );
    ...
    it("should FAIL if Kinesis Video Streams data is using desired encyption level", function (done) {
      ...
      videostreamDataEncrypted.run(
        ...
        (err, results) => {
          ...
          expect(results[0].message).to.include(
            "Kinesis Video Streams data is using awskms"
          );
          done();
        }
      );
    ...
    it("should UNKNOWN if unable to list Kinesis Video Streams", function (done) {
      const cache = createCache(null, null, null, {
        message: "Unable to list Kinesis Video Streams encryption",
      });
      videostreamDataEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
