  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.findings_desired_encryption_level ||
        this.settings.findings_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listDetectors.err || !listDetectors.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for GuardDuty detectors: " +
              helpers.addError(listDetectors),
            region
          );
          return rcb();
        }
        ...
        for (let detectorId of listDetectors.data) {
          ...
          if (
            ...
            helpers.addResult(
              results,
              3,
              "Unable to query for GuardDuty publishing destinations: " +
                helpers.addError(listPublishingDestinations),
              region,
              resource
            );
            continue;
          ...
          for (let destination of listPublishingDestinations.data
            ...
            if (
              ...
              helpers.addResult(
                results,
                3,
                "Unable to query for GuardDuty publishing destination: " +
                  helpers.addError(describePublishingDestination),
                region,
                resource
              );
              continue;
            ...
            if (
              describePublishingDestination.data.DestinationProperties &&
