...
module.exports = {
  ...
  settings: {
    guardduty_master_account: {
      ...
      description:
        "Return a failing result when the GuardDuty master account is not this account. Leave blank to allow all.",
      regex: "^([0-9]{12}|[0-9]{0})$",
      default: "",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var guarddutyMasterAccount =
      settings.guardduty_master_account ||
      this.settings.guardduty_master_account.default;
      ...
      function (region, rcb) {
        ...
        if (listDetectors.err || !listDetectors.data) {
          ...
          for (let detectorId of listDetectors.data) {
            var getMasterAccount = helpers.addSource(cache, source, [
              "guardduty",
              "getMasterAccount",
              region,
              detectorId,
            ...
            if (!getMasterAccount || !getMasterAccount.data.Master) {
              helpers.addResult(
                results,
                2,
                "GuardDuty master account is not configured",
                region,
                arn
              );
            } else {
              if (
                getMasterAccount.data.Master.RelationshipStatus !== "Enabled"
              ) {
                helpers.addResult(
                  results,
                  2,
                  "GuardDuty master account not enabled",
                  region,
                  arn
                );
              } else {
                if (guarddutyMasterAccount === "") {
                  helpers.addResult(
                    results,
                    0,
                    "GuardDuty has master account configured",
                    region,
                    arn
                  );
                } else if (
                  getMasterAccount.data.Master.AccountId ===
                  guarddutyMasterAccount
                ) {
                  helpers.addResult(
                    results,
                    0,
                    `GuardDuty master account is account ${guarddutyMasterAccount}`,
                    region,
                    arn
                  );
                } else {
                  helpers.addResult(
                    results,
                    2,
                    `GuardDuty master account is not account ${guarddutyMasterAccount}`,
                    region,
                    arn
                  );
                }
