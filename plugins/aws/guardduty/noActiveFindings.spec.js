...
const getDetector = [
  {
    ...
    ServiceRole:
      "arn:aws:iam::000011112222:role/aws-service-role/guardduty.amazonaws.com/AWSServiceRoleForAmazonGuardDuty",
    Status: "DISABLED",
    ...
    Findings: [
      {
        ...
        CreatedAt: failDate,
        Description:
          "AWS CloudTrail trail codepipeline-source-trail was disabled by sadeed calling PutEventSelectors under unusual circumstances. This can be attackers attempt to cover their tracks by eliminating any trace of activity performed while they accessed your account.",
    ...
    Findings: [
      {
        ...
        CreatedAt: new Date(),
        Description:
          "API ListFindings was invoked using root credentials from IP address 103.127.36.50.",
...
describe("noActiveFindings", function () {
  describe("run", function () {
    ...
    it("should PASS if no findings available", function (done) {
      const cache = createCache(listDetectors, getDetector, listFindings, {
        Findings: [],
      });
      noActiveFindings.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
