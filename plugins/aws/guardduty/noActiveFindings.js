...
module.exports = {
  ...
  settings: {
    guardduty_findings_fail: {
      ...
      description:
        "Return a failing result if a finding has not been archived after these many hours",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: "48",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      guardduty_findings_fail: parseInt(
        settings.guardduty_findings_fail ||
          this.settings.guardduty_findings_fail.default
      ),
    };

    regions.guardduty.forEach((region) => {
      const listDetectors = helpers.addSource(cache, source, [
        "guardduty",
        "listDetectors",
        region,
      ]);
      ...
      if (listDetectors.err || !listDetectors.data) {
        helpers.addResult(
          results,
          3,
          "Unable to list GuardDuty detectors: " +
      ...
      for (let detectorId of listDetectors.data) {
        ...
        const getDetector = helpers.addSource(cache, source, [
          "guardduty",
          "getDetector",
          region,
          detectorId,
        ...
        if (!getDetector || getDetector.err || !getDetector.data) {
          helpers.addResult(
            results,
            3,
            `Unable to get GuardDuty detector: ${helpers.addError(
              listDetectors
            )}`,
            region,
            resource
        ...
        const getFindings = helpers.addSource(cache, source, [
          "guardduty",
          "getFindings",
          region,
          detectorId,
        ...
        if (
          getFindings.err ||
          !getFindings.data ||
          !getFindings.data.Findings
        ) {
          helpers.addResult(
            results,
            3,
            `Unable to get GuardDuty findings: ${helpers.addError(
              getFindings
            )}`,
            region,
            resource
        ...
        let activeFindings = getFindings.data.Findings.filter(
          (finding) =>
            finding.CreatedAt &&
            helpers.hoursBetween(new Date(), finding.CreatedAt) >
              config.guardduty_findings_fail
        );
        ...
        helpers.addResult(
          ...
          status,
          `GuardDuty has ${
            status == 0 ? 0 : activeFindings.length
          } active finding(s)`,
          region,
          resource
        );
