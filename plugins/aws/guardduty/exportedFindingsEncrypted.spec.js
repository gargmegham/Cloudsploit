...
const listDetectors = ["febe94ba60bad6400e7ea861564c3e23"];
...
const createCache = (
  ...
  listDetectorsErr,
  keysErr,
  listPublishingDestinationsErr,
  describeKeyErr,
  describePublishingDestinationErr
) => {
  var keyId = keys && keys.length ? keys[0].KeyId : null;
  var detectorsId =
    listDetectors && listDetectors.length ? listDetectors[0] : null;
  var destinationId =
    listPublishingDestinations && listPublishingDestinations.length
  ...
  describe("run", function () {
    it("should PASS if GuardDuty Export Findings is encrypted with desired level", function (done) {
      const cache = createCache(
        [listDetectors[0]],
        [listPublishingDestinations[0]],
        describePublishingDestination[0],
        listKeys,
        describeKey[0]
      );
      exportedFindingsEncrypted.run(
        cache,
        { exported_findings_desired_encryption_level: "awscmk" },
        (err, results) => {
    ...
    it("should FAIL if GuardDuty Export Findings is not encrypted with desired level ", function (done) {
      const cache = createCache(
        [listDetectors[0]],
        [listPublishingDestinations[0]],
        describePublishingDestination[1],
        listKeys,
        describeKey[1]
      );
      exportedFindingsEncrypted.run(
        cache,
        { exported_findings_desired_encryption_level: "externalcmk" },
        (err, results) => {
          expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if unable to list GuardDuty detectors", function (done) {
      const cache = createCache(null, null, null, null, null, {
        message: "Unable to list GuardDuty detectors",
      });
      exportedFindingsEncrypted.run(
        cache,
        { exported_findings_desired_encryption_level: "awscmk" },
    ...
    it("should UNKNOWN if unable to list GuardDuty publishing destinations", function (done) {
      const cache = createCache(
        ...
        null,
        null,
        { message: "Unable to query GuardDuty publishing destinations" }
      );
      exportedFindingsEncrypted.run(
        cache,
        { exported_findings_desired_encryption_level: "awscmk" },
    ...
    it("should UNKNOWN if unable to list KMS keys", function (done) {
      const cache = createCache(
        ...
        null,
        null,
        { message: "Unable to list KMS keys" }
      );
