  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        const detectors = listDetectors.data.map((detectorId) => {
        ...
        if (!detectors.length) {
          ...
          const enabledDetectors = detectors.filter(
            (detector) => detector.Status === "ENABLED"
          );
          const badDetectors = detectors.filter(
            (detector) => detector.Status !== "ENABLED"
          );
          if (enabledDetectors.length >= 1) {
            ...
            badDetectors.forEach((detector) => {
              ...
              helpers.addResult(
                results,
                2,
                `GuardDuty detector is ${detector.Status}`,
                region,
                arn
              );
            });
