...
module.exports = {
  ...
  settings: {
    mq_broker_desired_encryption_level: {
      ...
      description:
        "In order (lowest to highest) sse=AWS-owned CMK awskms=AWS-managed KMS; awscmk=Customer managed KMS; externalcmk=Customer managed externally sourced KMS; cloudhsm=Customer managed CloudHSM sourced KMS",
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.mq_broker_desired_encryption_level ||
        this.settings.mq_broker_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listBrokers.err || !listBrokers.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query MQ brokers: " + helpers.addError(listBrokers),
            region
          );
          return rcb();
        }
        ...
        for (let broker of listBrokers.data) {
          ...
          if (
            broker.EngineType &&
            broker.EngineType.toUpperCase() == "RABBITMQ"
          ) {
            helpers.addResult(
              results,
              0,
              `AWS itself controls encryption for ${broker.EngineType.toUpperCase()} broker type`,
              region
            );
            continue;
          }
          ...
          if (!describeBroker || describeBroker.err || !describeBroker.data) {
            helpers.addResult(
              results,
              3,
              `Unable to describe MQ broker: ${helpers.addError(
                describeBroker
              )}`,
              region,
              resource
