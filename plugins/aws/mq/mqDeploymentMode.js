      ...
      function (region, rcb) {
        ...
        for (let broker of listBrokers.data) {
          ...
          if (
            ...
            broker.DeploymentMode.toUpperCase() === "ACTIVE_STANDBY_MULTI_AZ"
          ) {
            helpers.addResult(
              results,
              0,
              "Broker has active/standby deployment mode enabled",
              region,
              resource
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Broker does not have active/standby deployment mode enabled",
              region,
              resource
            );
          }
