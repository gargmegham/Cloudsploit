...
const listBrokers = [
  {
    BrokerArn:
      "arn:aws:mq:us-east-1:000011112222:broker:Mybr1:b-64f1b066-9604-46c0-ad14-b6fe482b29a0",
  ...
  {
    BrokerArn:
      "arn:aws:mq:us-east-1:000011112222:broker:MyBr2:b-ec84403c-f14b-438b-889b-e931706aaae6",
...
describe("mqDesiredInstanceType", function () {
  describe("run", function () {
    it("should PASS if brokers have the desired instance type", function (done) {
      ...
      mqDesiredInstanceType.run(
        cache,
        { mq_desired_instance_type: "mq.t3.micro" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          expect(results[0].message).to.include(
            "Broker has desired instance type"
          );
          done();
        }
      );
    ...
    it("should FAIL if broker does not have desired instance type", function (done) {
      ...
      mqDesiredInstanceType.run(
        ...
        (err, results) => {
          ...
          expect(results[0].message).to.include(
            "Broker does not have desired instance type"
          );
          done();
        }
      );
    ...
    it("should PASS if no MQ brokers found", function (done) {
      ...
      mqDesiredInstanceType.run(
        cache,
        { mq_desired_instance_type: "mq.m5.large" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
