        ...
        for (let broker of listBrokers.data) {
          ...
          if (!describeBroker || describeBroker.err || !describeBroker.data) {
            helpers.addResult(
              results,
              3,
              `Unable to get brokers description: ${helpers.addError(
                describeBroker
              )}`,
              region,
              resource
            ...
            if (describeBroker.data.AutoMinorVersionUpgrade) {
              helpers.addResult(
                results,
                0,
                "Broker has auto minor version upgrade enabled",
                region,
                resource
              );
            } else {
              helpers.addResult(
                results,
                2,
                "Broker does not have auto minor version upgrade enabled",
                region,
                resource
              );
            }
