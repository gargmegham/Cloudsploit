...
const describeGeofenceCollection = [
  {
    CollectionArn:
      "arn:aws:geo:us-east-1:000111222333:geofence-collection/sadeed1",
    CollectionName: "sadeed1",
    CreateTime: "2021-12-15T12:39:55.016000+00:00",
    Description: "",
    KmsKeyId:
      "arn:aws:kms:us-east-1:000111222333:key/ad013a33-b01d-4d88-ac97-127399c18b3e",
    PricingPlan: "MobileAssetTracking",
    PricingPlanDataSource: "Here",
    Tags: {},
    UpdateTime: "2021-12-15T12:39:55.016000+00:00",
  ...
  {
    CollectionArn:
      "arn:aws:geo:us-east-1:000111222333:geofence-collection/explore.geofence-collection",
    CollectionName: "explore.geofence-collection",
    CreateTime: "2021-12-14T14:15:06.967000+00:00",
    Description: "Created by Amazon Location Service explore",
    PricingPlan: "RequestBasedUsage",
...
const createCache = (
  ...
  keysErr,
  describeKeyErr,
  describeGeofenceCollectionErr
) => {
  var keyId = keys && keys.length ? keys[0].KeyId : null;
  var collectionName =
    collection && collection.length ? collection[0].CollectionName : null;
  return {
    location: {
  ...
  describe("run", function () {
    it("should PASS if Location geoference collection data is encrypted with desired encryption level", function (done) {
      const cache = createCache(
        [listGeofenceCollections[0]],
        listKeys,
        describeGeofenceCollection[0],
        describeKey[0]
      );
      geoCollectionDataEncrypted.run(
        cache,
        { geoference_collectiondata_desired_encryption_level: "awscmk" },
        (err, results) => {
    ...
    it("should FAIL if Location geoference collection data is not encrypted with desired encryption level", function (done) {
      const cache = createCache(
        [listGeofenceCollections[1]],
        listKeys,
        describeGeofenceCollection[1],
        describeKey[1]
      );
      geoCollectionDataEncrypted.run(
        cache,
    ...
    it("should UNKNOWN if unable to list Location geoference collections", function (done) {
      const cache = createCache(null, null, null, null, {
        message: "Unable to list Location geoference collections",
      });
      geoCollectionDataEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
