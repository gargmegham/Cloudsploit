  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.location_trackerdata_desired_encryption_level ||
        this.settings.location_trackerdata_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listTrackers.err || !listTrackers.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query Location trackers: " +
              helpers.addError(listTrackers),
            region
          );
          return rcb();
        }
        ...
        for (let tracker of listTrackers.data) {
          ...
          if (
            !describeTracker ||
            describeTracker.err ||
            !describeTracker.data
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to get Location Tracker: ${helpers.addError(
                describeTracker
              )}`,
              region,
              resource
