  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.geoference_collectiondata_desired_encryption_level ||
      ...
      function (region, rcb) {
        ...
        if (listGeofenceCollections.err || !listGeofenceCollections.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query Location geoference collection: " +
              helpers.addError(listGeofenceCollections),
            region
          );
          return rcb();
        }
        ...
        if (!listGeofenceCollections.data.length) {
          helpers.addResult(
            results,
            0,
            "No Location geoference collections found",
            region
          );
          return rcb();
        }
        ...
        for (let collection of listGeofenceCollections.data) {
          ...
          if (
            !describeGeofenceCollection ||
            describeGeofenceCollection.err ||
            !describeGeofenceCollection.data
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to get Location geoference collection: ${helpers.addError(
                describeGeofenceCollection
              )}`,
              region,
              resource
