...
module.exports = {
  ...
  compliance: {
    hipaa:
      "An MFA delete policy helps ensure that individuals attempting to " +
  ...
  run: function (cache, settings, callback) {
    var config = {
      whitelist_ct_deleted_buckets:
        settings.whitelist_ct_deleted_buckets ||
        this.settings.whitelist_ct_deleted_buckets.default,
    };
    var regBucket;
    if (config.whitelist_ct_deleted_buckets.length)
      regBucket = new RegExp(config.whitelist_ct_deleted_buckets);
    var results = [];
    var source = {};
      ...
      function (region, rcb) {
        ...
        async.each(
          ...
          function (trail, cb) {
            ...
            if (
              !getBucketVersioning ||
              getBucketVersioning.err ||
              !getBucketVersioning.data
            ) {
              helpers.addResult(
                results,
                3,
