...
module.exports = {
  ...
  compliance: {
    hipaa:
      "HIPAA has clearly defined audit requirements for environments " +
    ...
    pci:
      "CloudTrail logs satisfy the PCI requirement to log all account activity " +
      ...
      function (region, rcb) {
        ...
        if (describeTrails.err || !describeTrails.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for CloudTrail policy: " +
            ...
            if (
              ...
              if (trail.IncludeGlobalServiceEvents) {
                ...
                helpers.addResult(
                  results,
                  0,
                  "CloudTrail is enabled and monitoring regional and global services",
                  region
                );
              } else {
                helpers.addResult(
                  results,
                  0,
                  "CloudTrail is enabled and monitoring regional services",
                  region
                );
              }
          ...
          if (!found) {
            if (globalEnabled) {
              helpers.addResult(
                results,
                2,
                "CloudTrail is configured for global monitoring in the " +
                  region +
                  " region but is not logging API calls",
                region
              );
            } else {
              helpers.addResult(
                results,
                2,
                "CloudTrail is configured for regional monitoring but is not logging API calls",
                region
              );
            }
          }
      ...
      function () {
        if (!globalServicesMonitored) {
          helpers.addResult(
            results,
            2,
            "CloudTrail is not configured to monitor global services"
          );
        } else {
          if (globalEnabled) {
            helpers.addResult(
              results,
              0,
              "CloudTrail is configured and enabled to monitor global services"
            );
          } else {
            helpers.addResult(
              results,
              2,
              "CloudTrail is configured but is not logging API calls for global services"
            );
          }
        }
