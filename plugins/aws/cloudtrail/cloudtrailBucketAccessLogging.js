...
module.exports = {
  ...
  compliance: {
    hipaa:
      "Access logging for CloudTrail helps ensure strict integrity controls, " +
      "verifying that the audit logs for the AWS environment are not modified.",
    pci:
      "PCI requires tracking and monitoring of all access to environments " +
  ...
  run: function (cache, settings, callback) {
    var config = {
      whitelist_ct_access_logging_buckets:
        settings.whitelist_ct_access_logging_buckets ||
        this.settings.whitelist_ct_access_logging_buckets.default,
    };
    var regBucket;
    if (config.whitelist_ct_access_logging_buckets.length)
      regBucket = new RegExp(config.whitelist_ct_access_logging_buckets);
    var results = [];
    var source = {};
      ...
      function (region, rcb) {
        ...
        async.each(
          ...
          function (trail, cb) {
            ...
            if (trail.S3BucketName == helpers.CLOUDSPLOIT_EVENTS_BUCKET)
              return cb();
            ...
            if (regBucket && regBucket.test(trail.S3BucketName)) {
              helpers.addResult(
                ...
                region,
                "arn:aws:s3:::" + trail.S3BucketName
              );
              return cb();
            }

            if (
              !listBuckets.data.find(
                (bucket) => bucket.Name == trail.S3BucketName
              )
            ) {
              helpers.addResult(
                results,
