      ...
      function (region, rcb) {
        ...
        if (describeTrails.err || !describeTrails.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for CloudTrail trails: ${helpers.addError(
              describeTrails
            )}`,
            region
          );
        ...
        if (listTopics.err || !listTopics.data) {
          helpers.addResult(
            results,
            3,
            `Unable to list SNS topics: ${helpers.addError(listTopics)}`,
            region
          );
          return rcb();
        }
        ...
        for (let trail of describeTrails.data) {
          ...
          if (!getTopicAttributes) {
            helpers.addResult(
              results,
              2,
              "CloudTrail trail SNS topic not found",
              region,
              resource
            );
            continue;
          ...
          if (getTopicAttributes.err || !getTopicAttributes.data) {
            helpers.addResult(
              results,
              3,
              `Unable to query for SNS topic attributes: ${helpers.addError(
                getTopicAttributes
              )}`,
              region,
              resource
