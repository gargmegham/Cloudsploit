...
const createCache = (
  ...
  getEventSelectors,
  describeTrailsErr,
  getEventSelectorsErr
) => {
  var trailARN =
    describeTrails && describeTrails.length ? describeTrails[0].TrailARN : null;
  return {
    cloudtrail: {
      describeTrails: {
        "us-east-1": {
  ...
  describe("run", function () {
    ...
    it("should UNKNOWN if unable to query for trails", function (done) {
      const cache = createCache(
        null,
        null,
        { message: "Unable to describe trails" },
        null
      );
      cloudtrailManagementEvents.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if unable to query for event selectors", function (done) {
      const cache = createCache(
        ...
        getEventSelectors[0],
        null,
        { message: "Unable to get event selectors" }
      );
      cloudtrailManagementEvents.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(3);
