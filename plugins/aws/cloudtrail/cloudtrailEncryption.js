...
module.exports = {
  ...
  remediation_description:
    "Encryption for the affected Cloud trails will be enabled.",
      ...
      function (region, rcb) {
        ...
        if (describeTrails.err || !describeTrails.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for CloudTrail encryption status: " +
        ...
        if (!describeTrails.data.length) {
          ...
          for (var t in describeTrails.data) {
            if (
              describeTrails.data[t].S3BucketName ==
              helpers.CLOUDSPLOIT_EVENTS_BUCKET
            )
              continue;
            if (!describeTrails.data[t].KmsKeyId) {
              helpers.addResult(
                results,
                2,
                "CloudTrail encryption is not enabled",
                region,
                describeTrails.data[t].TrailARN
              );
            } else {
              helpers.addResult(
                results,
                0,
                "CloudTrail encryption is enabled",
                region,
                describeTrails.data[t].TrailARN
              );
            }
          ...
          helpers.addResult(
            results,
            2,
            "CloudTrail is enabled but is not properly configured",
            region
          );
        }
        rcb();
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    if (settings.input && settings.input.kmsKeyIdforCt) {
      params = {
        Name: resource,
