          ...
          function (trail, cb) {
            ...
            if (!getObjectLockConfiguration) {
              helpers.addResult(
                results,
                3,
                "Error querying for object lock configuration for bucket: " +
                  trail.S3BucketName +
                  ": " +
                  helpers.addError(getObjectLockConfiguration),
                region,
                resource
              );
            ...
            if (
              getObjectLockConfiguration.err &&
            ...
            if (
              getObjectLockConfiguration.err ||
              !getObjectLockConfiguration.data
            ) {
              helpers.addResult(
                results,
                3,
                "Unable to query for object lock configuration for bucket: " +
                  trail.S3BucketName,
                region,
                resource
              );
            ...
            if (
              getObjectLockConfiguration.data.ObjectLockConfiguration &&
              getObjectLockConfiguration.data.ObjectLockConfiguration
                .ObjectLockEnabled &&
              getObjectLockConfiguration.data.ObjectLockConfiguration.ObjectLockEnabled.toLowerCase() ===
                "enabled"
            ) {
              helpers.addResult(
                results,
