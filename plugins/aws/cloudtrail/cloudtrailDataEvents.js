          ...
          function (trail, cb) {
            ...
            if (
              ...
              helpers.addResult(
                results,
                3,
                `Unable to query event selectors: ${helpers.addError(
                  getEventSelectors
                )}`,
                region,
                resource
            ...
            for (var e in getEventSelectors.data.EventSelectors) {
              ...
              if (
                ...
                eventSelector.DataResources.length
              ) {
            ...
            if (dataResourceFound) {
              helpers.addResult(
                results,
                0,
                `CloudTrail trail "${trail.Name}" has Data Events configured`,
                region,
                resource
              );
            } else {
              helpers.addResult(
                results,
                2,
                `CloudTrail trail "${trail.Name}" does not have Data Events configured`,
                region,
                resource
              );
            }
