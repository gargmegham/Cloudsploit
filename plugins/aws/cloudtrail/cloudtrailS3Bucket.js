...
module.exports = {
  ...
  settings: {
    ...
    trails_to_check: {
      ...
      description:
        "Comma separated string of CloudTrail trail names. If provided, only check provided trails and pass all other without checking",
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      trail_s3_bucket_name:
        settings.trail_s3_bucket_name ||
        this.settings.trail_s3_bucket_name.default,
      trails_to_check:
        settings.trails_to_check || this.settings.trails_to_check.default,
    };

    if (!config.trail_s3_bucket_name.length)
      ...
      function (region, rcb) {
        ...
        if (describeTrails.err || !describeTrails.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for trails: ${helpers.addError(describeTrails)}`,
            region
          );
          return rcb();
        }
        ...
        async.each(
          ...
          function (trail, cb) {
            if (
              !trail.TrailARN ||
              (trail.S3BucketName &&
                trail.S3BucketName == helpers.CLOUDSPLOIT_EVENTS_BUCKET)
            )
              return cb();
            ...
            if (
              checkProvidedOnly &&
              trail.Name &&
              !config.trails_to_check.includes(trail.Name)
            ) {
              helpers.addResult(
                results,
                0,
            ...
            if (
              trail.S3BucketName &&
              trail.S3BucketName === config.trail_s3_bucket_name
            ) {
              helpers.addResult(
                results,
                0,
