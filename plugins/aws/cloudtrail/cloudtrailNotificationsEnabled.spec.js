...
const describeTrails = [
  {
    ...
    TrailARN:
      "arn:aws:cloudtrail:us-east-1:000011112222:trail/codepipeline-source-trail",
  ...
  },
  {
    ...
    S3KeyPrefix: "trail1",
    SnsTopicName:
      "arn:aws:sns:us-east-1:000011112222:aws-cloudtrail-logs-000011112222-8260eca1",
    SnsTopicARN:
      "arn:aws:sns:us-east-1:000011112222:aws-cloudtrail-logs-000011112222-8260eca1",
    IncludeGlobalServiceEvents: true,
...
const listTopics = [
  {
    TopicArn:
      "arn:aws:sns:us-east-1:000011112222:Default_CloudWatch_Alarms_Topic",
  },
  {
...
const createCache = (
  ...
  describeTrailsErr,
  listTopicsErr,
  getTopicAttributesErr
) => {
  let arn =
    describeTrails && describeTrails.length
      ? describeTrails[0].SnsTopicARN
      : null;
  return {
    cloudtrail: {
      describeTrails: {
  ...
  describe("run", function () {
    ...
    it("should FAIL if CloudTrail trail has no SNS topic attached", function (done) {
      const cache = createCache(
        ...
        null,
        null,
        { message: "CloudTrail trail has no SNS topic attached" }
      );
      cloudtrailNotificationsEnabled.run(cache, {}, (err, results) => {
        console.log(results);
        expect(results.length).to.equal(1);
        ...
        expect(results[0].region).to.equal("us-east-1");
        expect(results[0].message).to.include(
          "CloudTrail trail has no SNS topic attached"
        );
        done();
      });
    });
    ...
    it("should UNKNOWN if unable to query for CloudTrail trails", function (done) {
      const cache = createCache([], [], null, {
        message: "Unable to query for CloudTrail trails",
      });
      cloudtrailNotificationsEnabled.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(3);
        expect(results[0].message).to.include(
          "Unable to query for CloudTrail trails"
        );
        done();
      });
    });
    ...
    it("should UNKNOWN if unable to list SNS topics", function (done) {
      const cache = createCache([describeTrails[1]], [], null, null, {
        message: "Unable to list SNS topics",
      });
      cloudtrailNotificationsEnabled.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(3);
    ...
    it("should UNKNOWN if unable to query for SNS topic attributes", function (done) {
      const cache = createCache(
        ...
        null,
        null,
        { message: "Unable to query for SNS topic attributes" }
      );
      cloudtrailNotificationsEnabled.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(3);
        expect(results[0].message).to.include(
          "Unable to query for SNS topic attributes"
        );
        done();
      });
    });
