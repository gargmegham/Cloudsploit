  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.translate_job_encryption_level ||
        this.settings.translate_job_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listTextTranslationJobs.err || !listTextTranslationJobs.data) {
          helpers.addResult(
            results,
            3,
            `Unable to list Translate text jobs: ${helpers.addError(
              listTextTranslationJobs
            )}`,
            region
          );
        ...
        if (!listTextTranslationJobs.data.length) {
          helpers.addResult(results, 0, "No Translate text jobs found", region);
          return rcb();
        }
        ...
        for (let job of listTextTranslationJobs.data) {
          ...
          if (job.OutputDataConfig) {
            if (
              ...
              job.OutputDataConfig.EncryptionKey &&
              job.OutputDataConfig.EncryptionKey.Id
            ) {
              var kmsKeyId = job.OutputDataConfig.EncryptionKey.Id.split("/")[1]
                ? job.OutputDataConfig.EncryptionKey.Id.split("/")[1]
                : job.OutputDataConfig.EncryptionKey.Id;
