...
const describeRepositories = [
  ...
  },
  {
    ...
    encryptionConfiguration: {
      encryptionType: "KMS",
      kmsKey:
        "arn:aws:kms:us-east-1:000011112222:key/92e3e4cf-dfc3-4ea7-a225-22542c8e1528",
    },
  },
  {
...
const createCache = (
  ...
  var keyId =
    repository &&
    repository.length &&
    repository[0].encryptionConfiguration.kmsKey
      ? repository[0].encryptionConfiguration.kmsKey.split("/")[1]
      : null;
  return {
    ecr: {
  ...
  describe("run", function () {
    it("should PASS if ECR Repository is encrypted with desired encryption level", function (done) {
      const cache = createCache(
        [describeRepositories[2]],
        listKeys,
        describeKey[0]
      );
      ecrRepositoryEncrypted.run(
        cache,
        { ecr_repository_desired_encryption_level: "awscmk" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          expect(results[0].message).to.include(
            "ECR repository is encrypted with awscmk"
          );
          done();
        }
      );
    ...
    it("should FAIL if ECR Repository is not encrypted with desired encyption level", function (done) {
      ...
      ecrRepositoryEncrypted.run(
        ...
        (err, results) => {
          ...
          expect(results[0].message).to.include(
            "ECR repository encrypted with awskms"
          );
          done();
        }
      );
    ...
    it("should UNKNOWN if unable to list ECR Repository", function (done) {
      const cache = createCache(null, null, null, {
        message: "Unable to list ECR repositories",
      });
      ecrRepositoryEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        ...
        expect(results[0].message).to.include(
          "Unable to list ECR repositories"
        );
        done();
      });
    });
