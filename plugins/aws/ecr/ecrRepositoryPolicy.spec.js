      ...
      getRepositoryPolicy: {
        "us-east-1": {
          test: {
            err: {
              message:
                "Repository policy does not exist for the repository with name 'test' in the registry with id '111111111111'",
  ...
  describe("run", () => {
    ...
    it("should PASS if custom policy doe snot exist", () => {
      const cache = createCache(
        [describeRepositories[0]],
        getRepositoryPolicy[2]
      );
      ecrRepositoryPolicy.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(0);
    ...
    it("should PASS if overly permissive statement does not exist", () => {
      const cache = createCache(
        [describeRepositories[0]],
        getRepositoryPolicy[1]
      );
      ecrRepositoryPolicy.run(
        cache,
        {
          ecr_check_cross_account_principal: "false",
          ecr_check_global_principal: "false",
        },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
    ...
    it("should FAIL if global access allowed", () => {
      const cache = createCache(
        [describeRepositories[0]],
        getRepositoryPolicy[0]
      );
      ecrRepositoryPolicy.run(
        cache,
        { ecr_check_global_principal: "true" },
        (err, results) => {
          expect(results.length).to.equal(1);
    ...
    it("should FAIL if cross account access allowed", () => {
      ...
      ecrRepositoryPolicy.run(
        cache,
        { ecr_check_cross_account_principal: "true" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(2);
        }
