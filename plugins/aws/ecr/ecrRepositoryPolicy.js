...
module.exports = {
  ...
  settings: {
    ecr_check_global_principal: {
      ...
      description:
        "When set to true ECR repositories with a global principal will be marked as failing",
      regex: "^(true|false)$",
      default: "true",
    },
    ecr_check_cross_account_principal: {
      ...
      description:
        "When set to true ECR repositories with a cross account principal will be marked as failing",
      regex: "^(true|false)$",
      default: "true",
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      ecr_check_global_principal:
        settings.ecr_check_global_principal ||
        this.settings.ecr_check_global_principal.default,
      ecr_check_cross_account_principal:
        settings.ecr_check_cross_account_principal ||
        this.settings.ecr_check_cross_account_principal.default,
    };

    config.ecr_check_global_principal =
      config.ecr_check_global_principal == "true";
    config.ecr_check_cross_account_principal =
      config.ecr_check_cross_account_principal == "true";
      ...
      function (region, rcb) {
        ...
        if (describeRepositories.err || !describeRepositories.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for ECR repositories: " +
              helpers.addError(describeRepositories),
            region
          );
          return rcb();
        }
        ...
        for (var r in describeRepositories.data) {
          ...
          if (
            ...
            helpers.addResult(
              results,
              0,
              "ECR registry does not have a custom policy",
              region,
              arn
            );
            continue;
          ...
          if (
            !getRepositoryPolicy ||
            getRepositoryPolicy.err ||
            !getRepositoryPolicy.data ||
            !getRepositoryPolicy.data.policyText
          ) {
            helpers.addResult(
              results,
              3,
              "Unable to get ECR registry policy: " +
                helpers.addError(getRepositoryPolicy),
              region,
              arn
            );
            continue;
          ...
          var policy = helpers.normalizePolicyDocument(
            getRepositoryPolicy.data.policyText
          );
          ...
          for (var s in policy) {
            ...
            if (statement.Effect == "Allow") {
              ...
              if (
                ...
                var srcAcct =
                  statement.Condition.StringEquals["aws:SourceAccount"];
                if (typeof srcAcct === "string" && srcAcct == registryId)
                  continue;
                if (
                  Array.isArray(srcAcct) &&
                  srcAcct.length == 1 &&
                  srcAcct[0] == registryId
                )
                  continue;
              }
              ...
              if (
                ...
                found.push(
                  "Repository allows global access for actions: " +
                ...
                found.push(
                  "Repository allows cross-account access for actions: " +
                    statement.Action.join(", ") +
                    "."
                );
                if (result < 2) result = 2;
          ...
          if (found.length) {
            helpers.addResult(results, result, found.join(" "), region, arn);
          } else {
            helpers.addResult(
              results,
              0,
