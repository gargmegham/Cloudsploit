...
module.exports = {
  ...
  settings: {
    ecr_repository_desired_encryption_level: {
      ...
      description:
        "In order (lowest to highest) sse=AES-256; awskms=AWS-managed KMS; awscmk=Customer managed KMS; externalcmk=Customer managed externally sourced KMS; cloudhsm=Customer managed CloudHSM sourced KMS",
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.ecr_repository_desired_encryption_level ||
        this.settings.ecr_repository_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (describeRepositories.err || !describeRepositories.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query ECR repositories: ${helpers.addError(
              describeRepositories
            )}`,
            region
          );
