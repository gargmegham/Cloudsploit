  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.delivery_stream_desired_encryption_level ||
        this.settings.delivery_stream_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listDeliveryStreams.err || !listDeliveryStreams.data) {
          helpers.addResult(
            results,
            3,
            "Unable to list Firehose delivery streams: " +
              helpers.addError(listDeliveryStreams),
            region
          );
          return rcb();
        }
        ...
        if (!listDeliveryStreams.data.length) {
          helpers.addResult(
            results,
            0,
            "No Firehose delivery streams found",
            region
          );
          return rcb();
        }
        ...
        for (let stream of listDeliveryStreams.data) {
          ...
          let deliveryStreamDesc =
            describeDeliveryStream.data.DeliveryStreamDescription;
