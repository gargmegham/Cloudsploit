...
const describeDeliveryStream = [
  {
    DeliveryStreamDescription: {
      ...
      DeliveryStreamARN:
        "arn:aws:firehose:us-east-1:000011112222:deliverystream/PUT-S3-YuZ2N",
      ...
      Destinations: [
        {
          ...
          S3DestinationDescription: {
            RoleARN:
              "arn:aws:iam::000011112222:role/service-role/KinesisFirehoseServiceRole-PUT-S3-YuZ2N-us-east-1-1640786994734",
  ...
  describe("run", function () {
    it("should PASS if Firehose Delivery Stream is encrypted with desired encryption level", function (done) {
      const cache = createCache(
        [listDeliveryStreams[0]],
        listKeys,
        describeDeliveryStream[0],
        describeKey[0]
      );
      deliveryStreamEncrypted.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Firehose delivery stream is encrypted with awscmk"
        );
    ...
    it("should UNKNOWN if unable to list Firehose Delivery Streams", function (done) {
      const cache = createCache(null, null, null, null, {
        message: "Unable to list Firehose Delivery Streams",
      });
      deliveryStreamEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if Unable to query Firehose for delivery streams", function (done) {
      const cache = createCache(
        ...
        null,
        null,
        { message: "query Firehose for delivery streams" }
      );
      deliveryStreamEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(3);
    ...
    it("should UNKNOWN if unable to list KMS keys", function (done) {
      const cache = createCache(listDeliveryStreams, null, null, null, null, {
