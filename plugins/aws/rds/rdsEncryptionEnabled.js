...
module.exports = {
  ...
  settings: {
    rds_encryption_kms_alias: {
      ...
      description:
        "If set, RDS encryption must be configured using the KMS key alias specified. Be sure to include the alias/ prefix.",
    ...
    rds_encryption_level: {
      ...
      description:
        "In order (lowest to highest) awskms=AWS-managed KMS; awscmk=Customer managed KMS; externalcmk=Customer managed externally sourced KMS; cloudhsm=Customer managed CloudHSM sourced KMS",
  ...
  run: function (cache, settings, callback) {
    var config = {
      rds_encryption_kms_alias:
        settings.rds_encryption_kms_alias ||
        this.settings.rds_encryption_kms_alias.default,
      desiredEncryptionLevelString:
        settings.rds_encryption_level ||
        this.settings.rds_encryption_level.default,
    };

    var desiredEncryptionLevel = helpers.ENCRYPTION_LEVELS.indexOf(
      config.desiredEncryptionLevelString
    );
      ...
      function (region, rcb) {
        ...
        if (describeDBInstances.err || !describeDBInstances.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for RDS instances: " +
              helpers.addError(describeDBInstances),
            region
          );
          return rcb();
        }
        ...
        if (config.rds_encryption_kms_alias) {
          if (!listAliases || listAliases.err || !listAliases.data) {
            helpers.addResult(
              results,
              3,
              `RDS KMS alias setting is configured but KMS aliases could not be obtained: ${helpers.addError(
                listAliases
              )}`,
              region,
              null,
          ...
          if (!listAliases.data.length) {
            helpers.addResult(
              results,
              2,
              "RDS KMS alias setting is configured but there are no KMS aliases.",
              region,
              null,
              custom
            );
          ...
          listAliases.data.forEach(function (alias) {
            if (alias.AliasName == config.rds_encryption_kms_alias) {
              aliasId = alias.AliasArn.replace(
                /:alias\/.*/,
          ...
          if (!aliasId) {
            helpers.addResult(
              results,
              2,
              `RDS KMS alias setting is configured but the specified alias (${config.rds_encryption_kms_alias}) was not found.`,
              region,
              null,
              custom
            );
        ...
        for (var i in describeDBInstances.data) {
          ...
          if (db.StorageEncrypted) {
            ...
            currentEncryptionLevel = helpers.getEncryptionLevel(
              describeKey.data.KeyMetadata,
              helpers.ENCRYPTION_LEVELS
            );
            var currentEncryptionLevelString =
              helpers.ENCRYPTION_LEVELS[currentEncryptionLevel];
            if (config.rds_encryption_kms_alias) {
              if (aliasId == kmsKey) {
                if (desiredEncryptionLevel <= currentEncryptionLevel) {
                  helpers.addResult(
                    results,
                    0,
                    `Encryption at rest is enabled via expected KMS key: ${
                      kmsKey || "Unknown"
                    } at level ${currentEncryptionLevelString} which is ` +
                ...
                helpers.addResult(
                  results,
                  2,
                  `Encryption at rest is enabled, but is not using expected KMS key: ${aliasId}. Using key: ${
                    kmsKey || "Unknown"
                  }`,
                  region,
                  dbResource,
              ...
              if (desiredEncryptionLevel <= currentEncryptionLevel) {
                helpers.addResult(
                  results,
                  0,
                  `Encryption at rest is enabled via KMS key: ${
                    kmsKey || "Unknown"
                  } at level ${currentEncryptionLevelString} which is ` +
                    `greater than or equal to the desired level ${config.desiredEncryptionLevelString}`,
                  region,
