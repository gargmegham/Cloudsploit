      ...
      function (region, rcb) {
        ...
        if (describeDBEngineVersions.err || !describeDBEngineVersions.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for RDS engine versions: " +
              helpers.addError(describeDBEngineVersions),
            region
          );
          return rcb();
        }

        var eligibleDBEngineVersions = describeDBEngineVersions.data.filter(
          function (dbEngine) {
        ...
        for (var i in describeDBInstances.data) {
          ...
          if (
            ...
            db.EnabledCloudwatchLogsExports.length
          ) {
            helpers.addResult(
              results,
              0,
            ...
            var matchingDBEngineVersions = eligibleDBEngineVersions.filter(
              function (dbEngine) {
                return (
                  dbEngine.Engine === db.Engine &&
                  dbEngine.EngineVersion === db.EngineVersion
                );
              }
            );
            if (matchingDBEngineVersions.length) {
              ...
              helpers.addResult(
                results,
                0,
                "Logging is not enabled, but cannot be enabled",
                region,
                dbResource
              );
            }
