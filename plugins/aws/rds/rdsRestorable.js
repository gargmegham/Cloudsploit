...
module.exports = {
  ...
  more_info:
    "AWS will maintain a point to which the database can be restored. This point should not drift too far into the past, or else the risk of irrecoverable data loss may occur.",
  link: "http://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_PIT.html",
  recommended_action:
    "Ensure the instance is running and configured properly. If the time drifts too far, consider opening a support ticket with AWS.",
  ...
  settings: {
    rds_restorable_fail: {
      ...
      description:
        "Return a failing result when RDS restorable time exceeds this number of hours",
      regex: "^[1-9]{1}[0-9]{0,2}$",
      default: 24,
    },
    rds_restorable_warn: {
      ...
      description:
        "Return a warning result when RDS restorable time exceeds this number of hours",
      regex: "^[1-9]{1}[0-9]{0,2}$",
      default: 6,
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      rds_restorable_fail:
        settings.rds_restorable_fail ||
        this.settings.rds_restorable_fail.default,
      rds_restorable_warn:
        settings.rds_restorable_warn ||
        this.settings.rds_restorable_warn.default,
    };
      ...
      function (region, rcb) {
        ...
        for (var i in describeDBInstances.data) {
          ...
          if (db.LatestRestorableTime) {
            var difference = helpers.hoursBetween(
              new Date().toISOString(),
              db.LatestRestorableTime
            );
            var returnMsg =
              "RDS instance restorable time is " + difference + " hours old";
            ...
            helpers.addResult(
              results,
              0,
              "DocumentDB engine uses incremental backups, backups can be restored at any point in the backup retention period.",
              region,
              dbResource
            );
            ...
            helpers.addResult(
              results,
              2,
              "RDS instance does not have a restorable time",
              region,
              dbResource
            );
          }
        ...
        if (describeDBClusters.err || !describeDBClusters.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for RDS clusters: " +
              helpers.addError(describeDBClusters),
            region
          );
          return rcb();
        }
        ...
        for (var j in describeDBClusters.data) {
          ...
          if (dbCluster.LatestRestorableTime) {
            var differenceCluster = helpers.hoursBetween(
              new Date().toISOString(),
              dbCluster.LatestRestorableTime
            );
            var returnMsgCluster =
              "RDS cluster restorable time is " +
              differenceCluster +
              " hours old";
            ...
            helpers.addResult(
              results,
              2,
              "RDS cluster does not have a restorable time",
              region,
              dbResourceCluster
            );
          }
