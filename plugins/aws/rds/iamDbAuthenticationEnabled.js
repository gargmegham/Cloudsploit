      ...
      function (region, rcb) {
        ...
        if (describeDBInstances.err || !describeDBInstances.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for RDS instances: ${helpers.addError(
              describeDBInstances
            )}`,
            region
          );
          ...
          if (["postgres", "mysql"].includes(instance.Engine)) {
            if (instance.IAMDatabaseAuthenticationEnabled) {
              ...
              helpers.addResult(
                results,
                2,
                "RDS instance does not have IAM Database Authentication enabled",
                region,
                instance.DBInstanceArn
              );
            }
            ...
            helpers.addResult(
              results,
              0,
              `RDS instance engine type ${instance.Engine} does not support IAM database authentication`,
              region,
              instance.DBInstanceArn
            );
          }
