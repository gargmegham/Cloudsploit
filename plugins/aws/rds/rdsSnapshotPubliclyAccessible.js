      ...
      function (region, rcb) {
        ...
        if (describeDBSnapshots.err || !describeDBSnapshots.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for RDS snapshots: " +
              helpers.addError(describeDBSnapshots),
            region
          );
          return rcb();
        }
        ...
        describeDBSnapshots.data.forEach((snapshot) => {
          ...
          var describeDBSnapshotAttributes = helpers.addSource(
            cache,
            settings,
          ...
          if (
            ...
            helpers.addResult(
              results,
              3,
              `Unable to describe Snapshot attributes "${snapshotIdentifier}": ${helpers.addError(
                describeDBSnapshotAttributes
              )}`,
              region,
              resource
          ...
          if (
            ...
            publicSnapshot =
              describeDBSnapshotAttributes.data.DBSnapshotAttributesResult.DBSnapshotAttributes.find(
                (attribute) =>
                  attribute.AttributeValues &&
                  attribute.AttributeValues.includes("all")
              );
          }
