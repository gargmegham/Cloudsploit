...
module.exports = {
  ...
  more_info:
    "AWS provides a simple method of backing up RDS instances at a regular interval. This should be enabled to provide an option for restoring data in the event of a database compromise or hardware failure.",
  ...
  settings: {
    rds_backup_period: {
      ...
      description:
        "Return a passing result when RDS backup retention period is higher than this number of days",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: 6,
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      rds_backup_period:
        settings.rds_backup_period || this.settings.rds_backup_period.default,
    };
      ...
      function (region, rcb) {
        ...
        for (var i in describeDBInstances.data) {
          ...
          if (
            ...
            db.BackupRetentionPeriod > config.rds_backup_period
          ) {
            helpers.addResult(
              results,
              0,
              "Automated backups are enabled with sufficient retention (" +
                db.BackupRetentionPeriod +
                " days)",
              region,
              dbResource,
            ...
            helpers.addResult(
              results,
              1,
              "Automated backups are enabled but do not have sufficient retention (" +
                db.BackupRetentionPeriod +
                " days)",
              region,
              dbResource,
