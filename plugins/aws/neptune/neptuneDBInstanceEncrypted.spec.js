...
const describeDBClusters = [
  {
    ...
    ReaderEndpoint:
      "database-2.cluster-ro-cscif9l5pu36.us-east-1.neptune.amazonaws.com",
    MultiAZ: false,
    Engine: "neptune",
...
describe("neptuneDBInstanceEncrypted", function () {
  describe("run", function () {
    it("should PASS if Neptune database instance is encrypted with desired encryption level", function (done) {
      ...
      neptuneDBInstanceEncrypted.run(
        cache,
        { neptunedb_cluster_desired_encryption_level: "awscmk" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          expect(results[0].message).to.include(
            "Neptune database instance is encrypted with awscmk"
          );
    ...
    it("should FAIL if Neptune database instance is not encrypted with desired encyption level", function (done) {
      ...
      neptuneDBInstanceEncrypted.run(
        ...
        (err, results) => {
          ...
          expect(results[0].message).to.include(
            "Neptune database instance is encrypted with awskms"
          );
          expect(results[0].region).to.equal("us-east-1");
          done();
        }
    ...
    it("should UNKNOWN if unable to list Neptune Database instances", function (done) {
      const cache = createCache(null, null, null, {
        message: "Unable to list Neptune database instances",
      });
      neptuneDBInstanceEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
