  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.neptune_db_desired_encryption_level ||
        this.settings.neptune_db_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (describeDBClusters.err || !describeDBClusters.data) {
          helpers.addResult(
            results,
            3,
            `Unable to list Neptune database instances: ${helpers.addError(
              describeDBClusters
            )}`,
            region
          );
        ...
        if (!describeDBClusters.data.length) {
          helpers.addResult(
            results,
            0,
            "No Neptune database instances found",
            region
          );
          return rcb();
        }
