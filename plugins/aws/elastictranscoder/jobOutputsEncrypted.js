...
module.exports = {
  ...
  more_info:
    "Amazon Elastic Transcoder jobs saves th result output on S3. If you don't configure encryption parameters, these job will save the file unencrypted. " +
        ...
        for (let pipeline of listPipelines.data) {
          ...
          if (
            ...
            helpers.addResult(
              results,
              3,
              `Unable to list Elastic Transcoder jobs for pipeline: ${helpers.addError(
                pipelineJobs
              )}`,
              region,
              pipeline.Arn
          ...
          if (!pipelineJobs.data.Jobs.length) {
            helpers.addResult(
              results,
              0,
              "No Elastic Transcoder jobs found for pipeline",
              region,
              pipeline.Arn
            );
            continue;
          ...
          for (let job of pipelineJobs.data.Jobs) {
            if (job.Status && job.Status.toUpperCase() == "ERROR") {
              ...
              if (job.Outputs && job.Outputs.length)
                var unencryptedOutputs = job.Outputs.find(
              ...
              if (unencryptedOutputs) {
                helpers.addResult(
                  results,
                  2,
                  "Job does not encryption enabled for one or more outputs",
                  region,
                  job.Arn
                );
              } else {
                helpers.addResult(
                  results,
                  0,
                  "Job has encryption enabled for outputs",
                  region,
                  job.Arn
                );
              }
