  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.transcoder_pipeline_encryption_level ||
        this.settings.transcoder_pipeline_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listPipelines.err || !listPipelines.data) {
          helpers.addResult(
            results,
            3,
            `Unable to list Elastic Transcoder pipelines: ${helpers.addError(
              listPipelines
            )}`,
            region
          );
        ...
        if (!listPipelines.data.length) {
          helpers.addResult(
            results,
            0,
            "No Elastic Transcoder pipelines found",
            region
          );
          return rcb();
        }
        ...
        for (let pipeline of listPipelines.data) {
          ...
          if (pipeline.AwsKmsKeyArn) {
            var kmsKeyId = pipeline.AwsKmsKeyArn.split("/")[1]
              ? pipeline.AwsKmsKeyArn.split("/")[1]
