...
const listFunctions = [
  {
    ...
    Environment: {
      Variables: {
        password:
          "AQICAHgYvhTIHqe+Awrx6K5feBosORFD4FbhQ/XEyM9ERVB+yAF+yJdS1QYRvl9adezS+RnyAAAAaDBmBgkqhkiG9w0BBwagWTBXAgEAMFIGCSqGSIb3DQEHATAeBglghkgBZQMEAS4wEQQMw+VYDIrQVRJZeqO7AgEQgCWCe+TPNAr1VjxRifoBTXwu0YPZZJKLC7yMvnbpC7IqPQemWlTD",
...
describe("envVarsClientSideEncryption", function () {
  describe("run", function () {
    it("should PASS if sensitive environment variable values are encrypted", function (done) {
      ...
      envVarsClientSideEncryption.run(
        cache,
        { lambda_sensitive_env_vars: "password,key" },
        (err, results) => {
    ...
    it("should FAIL if sensitive environment variable values are not encrypted", function (done) {
      ...
      envVarsClientSideEncryption.run(
        cache,
        { lambda_sensitive_env_vars: "password" },
        (err, results) => {
          expect(results.length).to.equal(1);
    ...
    it("should PASS if no sensitive environment variable found", function (done) {
      ...
      envVarsClientSideEncryption.run(
        cache,
        { lambda_sensitive_env_vars: "access_keys" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          done();
