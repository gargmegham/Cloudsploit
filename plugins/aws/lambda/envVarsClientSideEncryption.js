...
module.exports = {
  ...
  more_info:
    "AWS Lambda lets you encrypt environment variable values prior to sending them to Lambda. " +
  ...
  settings: {
    lambda_sensitive_env_vars: {
      ...
      description:
        "A comma-delimited list of known lambda function Environment Variables that should be encrypted",
      regex: "^.{1,255}$",
      default: "",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      lambda_sensitive_env_vars:
        settings.lambda_sensitive_env_vars ||
        this.settings.lambda_sensitive_env_vars.default,
    };

    if (!config.lambda_sensitive_env_vars.length)
      return callback(null, results, source);

    config.lambda_sensitive_env_vars =
      config.lambda_sensitive_env_vars.split(",");
      ...
      function (region, rcb) {
        ...
        if (listFunctions.err || !listFunctions.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for Lambda functions: ${helpers.addError(
              listFunctions
            )}`,
            region
          );
        ...
        for (var lambdaFunc of listFunctions.data) {
          ...
          for (var envVar of config.lambda_sensitive_env_vars) {
            if (lambdaFunc.Environment.Variables[envVar]) {
              if (lambdaFunc.Environment.Variables[envVar].length !== 216)
                unencryptedVars.push(envVar);
