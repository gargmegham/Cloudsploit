      ...
      function (region, rcb) {
        ...
        for (var f in listFunctions.data) {
          ...
          if (!policy) {
            ...
            if (
              policy.err.code &&
              policy.err.code == "ResourceNotFoundException"
            ) {
              result = [0, "Function does not have an access policy"];
            } else {
              result = [
                3,
                "Error querying for Lambda function policy: " +
                  helpers.addError(policy),
              ];
            }
          } else if (policy.data) {
            var normalized = helpers.normalizePolicyDocument(
              policy.data.Policy
            );
            ...
            if (found.length) {
              result = [
                2,
                "Function policy allows global access to actions: " +
                  found.join(", "),
              ];
            } else {
