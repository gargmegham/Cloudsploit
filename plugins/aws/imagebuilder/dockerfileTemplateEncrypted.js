  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.docker_file_desired_encryption_level ||
        this.settings.docker_file_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listContainerRecipes.err) {
          helpers.addResult(
            results,
            3,
            "Unable to query container recipe: " +
              helpers.addError(listContainerRecipes),
            region
          );
          return rcb();
        }
        ...
        for (let recipe of listContainerRecipes.data) {
          ...
          if (
            !getContainerRecipe ||
            getContainerRecipe.err ||
            !getContainerRecipe.data ||
            !getContainerRecipe.data.containerRecipe
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to get container recipe description: ${helpers.addError(
                getContainerRecipe
              )}`,
              region,
              resource
          ...
          if (getContainerRecipe.data.containerRecipe.kmsKeyId) {
            var encryptionKey =
              getContainerRecipe.data.containerRecipe.kmsKeyId;
