  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.image_recipe_ebs_volumes_desired_encryption_level ||
        this.settings.image_recipe_ebs_volumes_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listImageRecipes.err || !listImageRecipes.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for image recipe summary list: ${helpers.addError(
              listImageRecipes
            )}`,
            region
          );
        ...
        if (!listImageRecipes.data.length) {
          helpers.addResult(
            results,
            0,
            "No Image Builder image recipes found",
            region
          );
          return rcb();
        }
        ...
        for (let recipe of listImageRecipes.data) {
          ...
          if (
            ...
            getImageRecipe.err ||
            !getImageRecipe.data ||
            !getImageRecipe.data.imageRecipe
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to get image Recipe description: ${helpers.addError(
                getImageRecipe
              )}`,
              region,
              resource
          ...
          let poorlyEncrypted = [];
          for (let mapping of getImageRecipe.data.imageRecipe
            .blockDeviceMappings) {
            ...
            if (
              mapping.ebs &&
              mapping.ebs.kmsKeyId &&
              mapping.ebs.kmsKeyId.includes("alias/aws/ebs")
            ) {
              ...
              let kmsKeyArn = encryptionKey.includes("alias/")
                ? kmsAliasArnMap[encryptionKey]
                  ? kmsAliasArnMap[encryptionKey]
                  : encryptionKey
                : encryptionKey;

              var keyId = kmsKeyArn.split("/")[1]
                ? kmsKeyArn.split("/")[1]
                : kmsKeyArn;
          ...
          if (poorlyEncrypted.length) {
            helpers.addResult(
              ...
              2,
              "Image recipe : " +
                poorlyEncrypted.length +
                " ebs volumes does not have encryption enabled",
              region,
              resource
            );
            continue;
