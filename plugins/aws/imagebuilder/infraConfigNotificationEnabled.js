...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      ...
      function (region, rcb) {
        var listInfrastructureConfigurations = helpers.addSource(
          cache,
          source,
        ...
        if (
          listInfrastructureConfigurations.err ||
          !listInfrastructureConfigurations.data
        ) {
          helpers.addResult(
            results,
            3,
            `Unable to query for infrastructure configuration summary List: ${helpers.addError(
        ...
        if (!listInfrastructureConfigurations.data.length) {
          helpers.addResult(
            results,
            0,
            "No list infrastructure configuration found",
            region
          );
          return rcb();
        }
        ...
        for (let recipe of listInfrastructureConfigurations.data) {
          ...
          var getInfrastructureConfiguration = helpers.addSource(
            cache,
            source,
            [
              "imagebuilder",
          ...
          if (
            !getInfrastructureConfiguration ||
            getInfrastructureConfiguration.err ||
            !getInfrastructureConfiguration.data
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to get infrastructure configuration description: ${helpers.addError(
                getInfrastructureConfiguration
              )}`,
              region,
              resource
          ...
          if (
            getInfrastructureConfiguration.data.infrastructureConfiguration &&
            getInfrastructureConfiguration.data.infrastructureConfiguration
              .snsTopicArn
          ) {
            helpers.addResult(
