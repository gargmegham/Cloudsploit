  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.image_component_desired_encryption_level ||
        this.settings.image_component_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listComponents.err) {
          helpers.addResult(
            results,
            3,
            "Unable to query component version list: " +
              helpers.addError(listComponents),
            region
          );
          return rcb();
        }
        ...
        if (!listComponents.data || !listComponents.data.length) {
          helpers.addResult(
            results,
            0,
            "No component version list found",
            region
          );
          return rcb();
        }
        ...
        for (let recipe of listComponents.data) {
          ...
          if (
            ...
            helpers.addResult(
              results,
              3,
              `Unable to query for component description: ${helpers.addError(
                getComponent
              )}`,
              region,
              resource
