...
const listImageRecipes = [
  {
    ...
    owner: "000011112222",
    parentImage:
      "arn:aws:imagebuilder:us-east-1:aws:image/amazon-linux-2-arm64/x.x.x",
  ...
  },
  {
    ...
    imageRecipe: {
      ...
      components: [
        ...
        {
          componentArn:
            "arn:aws:imagebuilder:us-east-1:aws:component/chrony-time-configuration-test/x.x.x",
        },
      ],
      parentImage:
        "arn:aws:imagebuilder:us-east-1:aws:image/amazon-linux-2-arm64/x.x.x",
...
var describeKey = [
  {
    KeyMetadata: {
      ...
      Enabled: true,
      Description:
        "Default master key that protects my kinesis video data when no other key is defined",
...
describe("imageRecipeVolumeEncrypted", function () {
  describe("run", function () {
    it("should PASS if Image recipe ebs volumes are encrypted with awscmk", function (done) {
      ...
      imageRecipeVolumeEncrypted.run(
        cache,
        { image_recipe_ebs_volumes_desired_encryption_level: "awscmk" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          expect(results[0].region).to.equal("us-east-1");
          expect(results[0].message).to.include(
            "Image recipe ebs volumes are encrypted with awscmk"
          );
          done();
        }
      );
    ...
    it("should FAIL if Image recipe ebs volumes are encrypted with awskms", function (done) {
      ...
      imageRecipeVolumeEncrypted.run(
        ...
        (err, results) => {
          ...
          expect(results[0].region).to.equal("us-east-1");
          expect(results[0].message).to.include(
            "Image recipe ebs volumes are encrypted with awskms"
          );
          done();
        }
      );
    ...
    it("should UNKNOWN if Unable to query for image recipe summary list", function (done) {
      const cache = createCache(null, null, {
        message: "Unable to query for image recipe summary list",
      });
      imageRecipeVolumeEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        ...
        expect(results[0].region).to.equal("us-east-1");
        expect(results[0].message).to.include(
          "Unable to query for image recipe summary list"
        );
        done();
      });
    });
    ...
    it("should UNKNOWN if unable to list KMS keys", function (done) {
      const cache = createCache([listImageRecipes[0]], null, null, null, {
