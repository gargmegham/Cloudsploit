...
const getContainerRecipe = [
  {
    ...
    containerRecipe: {
      ...
      kmsKeyId:
        "arn:aws:kms:us-east-1:000011112222:key/1425f881-3b10-486a-aca8-14d2649881e3",
      encrypted: true,
      parentImage: "amazonlinux:latest",
      dateCreated: "2022-03-29T10:23:16.837Z",
      tags: {},
  ...
  describe("run", function () {
    it("should PASS if Dockerfile Template is encrypted with desired encryption level", function (done) {
      const cache = createCache(
        [listContainerRecipes[0]],
        listKeys,
        listAliases,
        getContainerRecipe[0],
        describeKey[0]
      );
      dockerfileTemplateEncrypted.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Dockerfile Template is encrypted with awscmk"
        );
    ...
    it("should FAIL if Dockerfile Template is encrypted with awskms", function (done) {
      const cache = createCache(
        [listContainerRecipes[1]],
        listKeys,
        listAliases,
        getContainerRecipe[1],
        describeKey[1]
      );
      dockerfileTemplateEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        ...
        expect(results[0].message).to.include(
          "Dockerfile Template is encrypted with"
        );
        expect(results[0].region).to.equal("us-east-1");
        done();
      });
    ...
    it("should UNKNOWN if unable to get container recipe description", function (done) {
      const cache = createCache(
        ...
        null,
        null,
        { message: "Unable to get container recipe description" }
      );
      dockerfileTemplateEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(3);
        expect(results[0].message).to.include(
          "Unable to get container recipe description"
        );
        expect(results[0].region).to.equal("us-east-1");
        done();
      });
