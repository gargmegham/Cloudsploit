...
const domains = [
  {
    DomainStatus: {
      ...
      Deleted: false,
      AccessPolicies:
        '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"AWS":"arn:aws:iam::211111111111:user/y"},"Action":"es:*","Resource":"arn:aws:events:us-east-1:111111111111:domain/test-domain-1/*"}]}',
    },
  },
  {
    DomainStatus: {
      ...
      Deleted: false,
      AccessPolicies:
        '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"AWS":"arn:aws:iam::111111111111:user/x"},"Action":"es:*","Resource":"arn:aws:events:us-east-1:111111111111:domain/test-domain-1/*"}]}',
    },
  },
  {
    DomainStatus: {
      ...
      Deleted: false,
      AccessPolicies:
        '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"AWS":"*"},"Action":"es:*","Resource":"arn:aws:es:us-east-1:111111111111:domain/test-domain-1/*"}]}',
    },
  },
  {
    DomainStatus: {
...
describe("esCrossAccountAccess", function () {
  describe("run", function () {
    ...
    it("should PASS if cross-account role contains organization account ID and setting to allow organization account is true", function (done) {
      ...
      esCrossAccountAccess.run(
        cache,
        { es_whitelist_aws_organization_accounts: "true" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          done();
    ...
    it("should UNKNOWN if unable to describe ES domain", function (done) {
      const cache = createCache([], [], [], {
        message: "Unable to query ES domains",
      });
      esCrossAccountAccess.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
