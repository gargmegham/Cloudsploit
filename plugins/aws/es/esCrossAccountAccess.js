...
module.exports = {
  ...
  settings: {
    ...
    es_whitelisted_aws_account_principals_regex: {
      ...
      description:
        "If set, plugin will compare cross account principals against this regex instead of otherwise given comma-separated list" +
    ...
    es_policy_condition_keys: {
      ...
      description:
        "Comma separated list of AWS IAM condition keys that should be allowed i.e. aws:SourceAccount,aws:PrincipalArn",
      regex: "^.*$",
      default:
        "aws:PrincipalArn,aws:PrincipalAccount,aws:PrincipalOrgID,aws:SourceAccount,aws:SourceArn,aws:SourceOwner",
    },
  },
  run: function (cache, settings, callback) {
    var config = {
      es_whitelisted_aws_account_principals:
        settings.es_whitelisted_aws_account_principals ||
        this.settings.es_whitelisted_aws_account_principals.default,
      es_whitelisted_aws_account_principals_regex:
        settings.es_whitelisted_aws_account_principals_regex ||
        this.settings.es_whitelisted_aws_account_principals_regex.default,
      es_whitelist_aws_organization_accounts:
        settings.es_whitelist_aws_organization_accounts ||
        this.settings.es_whitelist_aws_organization_accounts.default,
      es_policy_condition_keys:
        settings.es_policy_condition_keys ||
        this.settings.es_policy_condition_keys.default,
    };
    var allowedConditionKeys = config.es_policy_condition_keys.split(",");
    var makeRegexBased = config.es_whitelisted_aws_account_principals_regex
      .length
      ? true
      : false;
    var whitelistOrganization =
      config.es_whitelist_aws_organization_accounts == "true";
    config.es_whitelisted_aws_account_principals_regex = new RegExp(
      config.es_whitelisted_aws_account_principals_regex
    );
        ...
        if (listDomainNames.err || !listDomainNames.data) {
          helpers.addResult(
            ...
            "Unable to query for ES domains: " +
              helpers.addError(listDomainNames),
            region
          );
          return rcb();
        }
        ...
        listDomainNames.data.forEach((domain) => {
          ...
          if (
            ...
            helpers.addResult(
              results,
              3,
              "Unable to query for ES domain config: " +
                helpers.addError(describeElasticsearchDomain),
              region,
              resource
            );
          } else {
            ...
            if (!localDomain.AccessPolicies) {
              helpers.addResult(
                results,
                0,
                "ES domain does not have access policy defined",
                region,
                resource
              );
              return;
            ...
            if (!statements) {
              helpers.addResult(
                results,
                0,
                "No statement exists for the policy",
                region,
                resource
              );
              return;
            ...
            statements.forEach((statement) => {
              ...
              let conditionalPrincipals = helpers.isValidCondition(
                statement,
              ...
              if (
                helpers.crossAccountPrincipal(statement.Principal, accountId) ||
                (conditionalPrincipals && conditionalPrincipals.length)
              ) {
                let crossAccountPrincipals = helpers.crossAccountPrincipal(
                  statement.Principal,
                  accountId,
                  true
                );
                ...
                if (conditionalPrincipals && conditionalPrincipals.length) {
                  conditionalPrincipals.forEach((conPrincipal) => {
                    if (!conPrincipal.includes(accountId))
                      crossAccountPrincipals.push(conPrincipal);
                  });
                }
                ...
                crossAccountPrincipals.forEach((principal) => {
                  if (whitelistOrganization) {
                    if (
                      organizationAccounts.find((account) =>
                        principal.includes(account)
                      )
                    )
                      return;
                  }
                  if (makeRegexBased) {
                    if (
                      !config.es_whitelisted_aws_account_principals_regex.test(
                        principal
                      ) &&
                      !restrictedAccountPrincipals.includes(principal)
                    )
                      restrictedAccountPrincipals.push(principal);
                  } else if (
                    !config.es_whitelisted_aws_account_principals.includes(
                      principal
                    ) &&
                    !restrictedAccountPrincipals.includes(principal)
                  )
            ...
            if (crossAccountEs && !restrictedAccountPrincipals.length) {
              helpers.addResult(
                results,
                0,
                "ES domain contains trusted account principals only",
                region,
                resource
              );
              ...
              helpers.addResult(
                results,
                2,
                `ES domain contains these untrusted account principals: ${restrictedAccountPrincipals.join(
                  ", "
                )}`,
                region,
                resource
              ...
              helpers.addResult(
                results,
                0,
                "ES domain does not contain cross-account policy statement",
                region,
                resource
              );
            }
