      ...
      function (region, rcb) {
        ...
        listDomainNames.data.forEach(function (domain) {
          ...
          if (
            ...
            if (
              localDomain.LogPublishingOptions &&
              Object.keys(localDomain.LogPublishingOptions).length
            ) {
              for (let LogPublishingOptions in localDomain.LogPublishingOptions) {
                let logGroups =
                  localDomain.LogPublishingOptions[LogPublishingOptions];
                if (!logGroups.CloudWatchLogsLogGroupArn && logGroups.Enabled) {
                  cloudWatchDisabled.push(LogPublishingOptions);
                } else if (
                  logGroups.CloudWatchLogsLogGroupArn &&
                  logGroups.Enabled
                ) {
                  if (logSelectionArr.indexOf(LogPublishingOptions) > -1) {
                    logSelectionArr.splice(
                      logSelectionArr.indexOf(LogPublishingOptions),
                      1
                    );
                  }
                }
              ...
              if (!logSelectionArr.length) {
                helpers.addResult(
                  results,
                  0,
                  "ES domain logging is enabled and sending logs to CloudWatch",
                  region,
                  localDomain.ARN
                );
                ...
                helpers.addResult(
                  results,
                  2,
                  `ES domain logging is enabled but logs are not configured to be sent to CloudWatch for: ${logStr}`,
                  region,
                  localDomain.ARN
                );
              } else {
                ...
                helpers.addResult(
                  results,
                  2,
                  `The following logs are not configured for the ES domain: ${logStr}`,
                  region,
                  localDomain.ARN
                );
              }
              ...
              helpers.addResult(
                results,
                2,
                "ES domain logging is not enabled",
                region,
                localDomain.ARN
              );
            }
