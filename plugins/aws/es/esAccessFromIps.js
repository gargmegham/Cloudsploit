...
module.exports = {
  ...
  settings: {
    whitelisted_ip_addresses: {
      ...
      description:
        "A comma-separated list of trusted IP addresses allowed to access ES domains",
      regex:
        "/^(?=.*[^.]$)((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?).?){4}/([1-9]|1[0-9]){1}$/",
      default: "",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var whitelisted_ip_addresses =
      settings.whitelisted_ip_addresses ||
      this.settings.whitelisted_ip_addresses.default;

    if (!whitelisted_ip_addresses.length)
      ...
      function (region, rcb) {
        ...
        if (listDomainNames.err || !listDomainNames.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for ES domains: ${helpers.addError(
              listDomainNames
            )}`,
            region
          );
          ...
          function (domain, dcb) {
            ...
            if (
              ...
              helpers.addResult(
                results,
                3,
                `Unable to query for ES domain config: ${helpers.addError(
                  describeElasticsearchDomain
                )}`,
                region
              );
            ...
            var statements = helpers.normalizePolicyDocument(
              describeElasticsearchDomain.data.DomainStatus.AccessPolicies
            );
            ...
            for (var s in statements) {
              ...
              if (
                ...
                statement.Condition.IpAddress["aws:SourceIp"].length
              ) {
                statement.Condition.IpAddress["aws:SourceIp"].forEach((ip) => {
                  if (whitelisted_ip_addresses.indexOf(ip) < 0)
                    intruderIps.push(ip);
                });
              }
            }
            ...
            if (globalAccess) {
              helpers.addResult(
                results,
                2,
                `ES domain "${domain.DomainName}" is publicly exposed`,
                region,
                resource
              );
              return dcb();
            ...
            if (!intruderIps.length) {
              helpers.addResult(
                results,
                0,
                `ES domain "${domain.DomainName}" is not accessible from any unknown IP address`,
                region,
                resource
              );
            } else {
              helpers.addResult(
                ...
                `ES domain "${
                  domain.DomainName
                }" is accessible from these unknown IP addresses: ${intruderIps.join(
                  ", "
                )}`,
                region,
                resource
