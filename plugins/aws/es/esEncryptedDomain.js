...
module.exports = {
  ...
  remediation_inputs: {
    kmsKeyIdForES: {
      ...
      description:
        "KMS Key Id that will be used to encrypt ElasticSearch domain",
      regex:
      ...
      function (region, rcb) {
        ...
        listDomainNames.data.forEach(function (domain) {
          ...
          if (
            ...
            if (
              localDomain.EncryptionAtRestOptions &&
              localDomain.EncryptionAtRestOptions.Enabled
            ) {
              helpers.addResult(
                results,
                0,
                "ES domain is configured to use encryption at rest",
                region,
                localDomain.ARN
              );
            } else {
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    let defaultKeyDesc =
      "Default master key that protects my Elasticsearch data when no other key is defined";
    ...
    if (settings.input && settings.input.kmsKeyIdForES) {
      params = {
        DomainName: domain,
      ...
      if (!defaultKmsKeyId)
        return callback(
          `No default ElasticSearch key for the region ${config.region}`
        );
      params = {
        DomainName: domain,
        EncryptionAtRestOptions: {
