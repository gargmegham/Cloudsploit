...
module.exports = {
  ...
  settings: {
    allow_es_public_endpoint_if_ip_condition_policy: {
      ...
      description:
        "Allows public ElasticSearch endpoints if set to true and if there is an IP Condition policy and/or a restricted non-star principal.",
      regex: "^(true|false)$",
      default: "false",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      allow_es_public_endpoint_if_ip_condition_policy:
        settings.allow_es_public_endpoint_if_ip_condition_policy ||
        this.settings.allow_es_public_endpoint_if_ip_condition_policy.default,
    };

    config.allow_es_public_endpoint_if_ip_condition_policy =
      config.allow_es_public_endpoint_if_ip_condition_policy === "true" ||
      config.allow_es_public_endpoint_if_ip_condition_policy === true;
        ...
        listDomainNames.data.forEach(function (domain) {
          ...
          if (
            ...
            if (
              ...
              // evaluate policies if the setting is enabled.
              var policies = helpers.normalizePolicyDocument(
                localDomain.AccessPolicies
              );
              ...
              for (var p in policies) {
                ...
                var containsIpPolicy =
                  policy.Condition && policy.Condition.IpAddress;

                if (
                  !containsIpPolicy &&
                  helpers.globalPrincipal(policy.Principal)
                ) {
                  validPolicy = false;
                }
              }
            ...
            if (
              ...
              helpers.addResult(
                results,
                0,
                "ES domain is configured to use a VPC endpoint",
                region,
                localDomain.ARN
              );
            } else {
              if (config.allow_es_public_endpoint_if_ip_condition_policy) {
                if (validPolicy) {
                  helpers.addResult(
                    results,
                    0,
                    "ES domain is configured to use a public endpoint, but is allowed since there are no public access policies.",
                    region,
                    localDomain.ARN
                  );
                } else {
                  helpers.addResult(
                    results,
                    2,
                    "ES domain is configured to use a public endpoint and has disallowed public access policies.",
                    region,
                    localDomain.ARN
                  );
                }
                ...
                helpers.addResult(
                  results,
                  2,
                  "ES domain is configured to use a public endpoint.",
                  region,
                  localDomain.ARN
                );
              }
