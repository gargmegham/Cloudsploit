...
module.exports = {
  ...
  remediation_inputs: {
    tlsSecurityPolicyforEs: {
      ...
      description:
        "The TLS Security Policy that needs to be applied to the HTTPS endpoint",
      regex: "^.*$",
      required: false,
    },
  },
      ...
      function (region, rcb) {
        ...
        listDomainNames.data.forEach(function (domain) {
          ...
          if (
            ...
            if (
              localDomain.DomainEndpointOptions &&
              localDomain.DomainEndpointOptions.EnforceHTTPS
            ) {
              helpers.addResult(
                results,
                0,
                "ES domain is configured to enforce HTTPS",
                region,
                localDomain.ARN
              );
            } else {
              helpers.addResult(
                results,
                2,
                "ES domain is not configured to enforce HTTPS",
                region,
                localDomain.ARN
              );
            }
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    if (settings.input && settings.input.tlsSecurityPolicyforEs) {
      params = {
        DomainName: domain,
