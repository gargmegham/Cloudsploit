...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    const es_desired_data_instance_type =
      settings.es_desired_data_instance_type ||
      this.settings.es_desired_data_instance_type.default;
    const es_desired_master_instance_type =
      settings.es_desired_master_instance_type ||
      this.settings.es_desired_master_instance_type.default;

    if (
      !es_desired_data_instance_type.length &&
      !es_desired_master_instance_type.length
    )
      ...
      function (region, rcb) {
        ...
        for (const domain of listDomainNames.data) {
          ...
          if (
            ...
            if (
              config.InstanceType &&
              !es_desired_data_instance_type.includes(config.InstanceType)
            )
              disallowedDataInstanceTypes.push(config.InstanceType);
            if (
              config.DedicatedMasterType &&
              !es_desired_master_instance_type.includes(
                config.DedicatedMasterType
              )
            )
              disallowedDedicatedInstanceTypes.push(config.DedicatedMasterType);
          }

          if (
            disallowedDedicatedInstanceTypes.length &&
            disallowedDataInstanceTypes.length
          ) {
            helpers.addResult(
              results,
              2,
              `ES domain is using ${disallowedDedicatedInstanceTypes.join(
                ", "
              )} master instance(s) and ${disallowedDataInstanceTypes.join(
                ", "
              )} data instance(s)`,
              region,
              resource
            ...
            helpers.addResult(
              results,
              2,
              `ES domain is using ${disallowedDataInstanceTypes.join(
                ", "
              )} data instance(s)`,
              region,
              resource
