            ...
            if (describeElasticsearchDomain.data.DomainStatus.AccessPolicies) {
              ...
              if (statements && statements.length) {
                for (let statement of statements) {
                  var statementPrincipals =
                    helpers.extractStatementPrincipals(statement);
                  exposed = statementPrincipals.find(
                    (principal) => principal == "*"
                  );
                  if (exposed) break;
                }
                ...
                if (exposed) {
                  helpers.addResult(
                    ...
                    "Domain :" +
                      domain.DomainName +
                      ": is exposed to all AWS accounts",
                    region,
                    resource
                  );
                } else {
                  helpers.addResult(
                    ...
                    "Domain :" +
                      domain.DomainName +
                      ": is not exposed to all AWS accounts",
                    region,
                    resource
                  );
                }
                ...
                helpers.addResult(
                  results,
                  2,
                  "No statement found for access policies",
                  region,
                  resource
                );
              }
