...
const domains = [
  {
    DomainStatus: {
      ...
      Deleted: false,
      AccessPolicies:
        '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"AWS":"*"},"Action":"es:*","Resource":"arn:aws:es:us-east-1:560213429563:domain/es-domain-1/*","Condition":{"IpAddress":{"aws:SourceIp":["18.208.0.0/13","52.95.245.0/24"]}}}]}',
    },
  },
  {
    DomainStatus: {
      ...
      Deleted: false,
      AccessPolicies:
        '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"AWS":"*"},"Action":"es:*","Resource":"arn:aws:es:us-east-1:560213429563:domain/es-domain-1/*"}]}',
    },
  },
  {
    DomainStatus: {
...
describe("esAccessFromIps", function () {
  describe("run", function () {
    it("should FAIL if domain is publicy exposed", function (done) {
      ...
      esAccessFromIps.run(
        cache,
        { whitelisted_ip_addresses: "18.208.0.0/13" },
        (err, results) => {
    ...
    it("should PASS if domain is not exposed to any unknown IP addresses", function (done) {
      ...
      esAccessFromIps.run(
        cache,
        { whitelisted_ip_addresses: "18.208.0.0/13,52.95.245.0/24" },
        (err, results) => {
          expect(results.length).to.equal(1);
    ...
    it("should FAIL if domain is not exposed to unknown IP addresses", function (done) {
      ...
      esAccessFromIps.run(
        cache,
        { whitelisted_ip_addresses: "18.208.0.0/16" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(2);
          done();
