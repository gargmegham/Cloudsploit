...
module.exports = {
  ...
  apis: [
    "ES:listDomainNames",
    "CloudWatch:getEsMetricStatistics",
        ...
        listDomainNames.data.forEach((domain) => {
          ...
          if (!getMetricStats || getMetricStats.err || !getMetricStats.data) {
            helpers.addResult(
              results,
              3,
              `Unable to query for ES domain metric stat: ${helpers.addError(
                getMetricStats
              )}`,
              region,
              resource
          ...
          const data = getMetricStats.data.Datapoints.find(
            (datapoint) => datapoint.Maximum && datapoint.Maximum >= 1
          );
          const status = data ? 2 : 0;
          helpers.addResult(
            results,
            status,
            `ES Domain is ${data ? "unhealthy" : "healthy"}`,
            region,
            resource
          );
        });
