...
const domains = [
  {
    DomainStatus: {
      ...
      EncryptionAtRestOptions: {
        Enabled: true,
        KmsKeyId:
          "arn:aws:kms:us-east-1:111122223333:key/34e9wedw-ae6b-4c36-9405-06e67bccswwd",
      },
    },
  },
  {
...
const createCache = (listData, descData, describeKey) => {
  var keyId =
    descData &&
    descData.DomainStatus &&
    descData.DomainStatus.EncryptionAtRestOptions &&
    descData.DomainStatus.EncryptionAtRestOptions.KmsKeyId
      ? descData.DomainStatus.EncryptionAtRestOptions.KmsKeyId.split("/")[1]
      : null;
...
describe("esDomainEncryptionEnabled", function () {
  describe("run", function () {
    ...
    it("should FAIL if ES domain has encryption at rest at an encryption level greater than target encryption level", function (done) {
      ...
      esDomainEncryptionEnabled.run(
        cache,
        { es_encryption_level: "awscmk" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(2);
          done();
