      ...
      function (region, rcb) {
        ...
        listDomainNames.data.forEach(function (domain) {
          ...
          if (
            ...
            var currentVersion =
              localDomain.ServiceSoftwareOptions.CurrentVersion;
            var upgradeVersion = localDomain.ServiceSoftwareOptions.NewVersion;
            var upgradeAvailable =
              localDomain.ServiceSoftwareOptions.UpdateAvailable;
            ...
            if (upgradeAvailable && upgradeStatus !== "NOT_ELIGIBLE") {
              helpers.addResult(
                results,
                2,
                "ES domain service software version: " +
                  currentVersion +
                  " is eligible for an upgrade to version: " +
                  upgradeVersion,
                region,
                localDomain.ARN
              );
              ...
              helpers.addResult(
                ...
                "ES domain service software version: " +
                  currentVersion +
                  " is the latest eligible upgraded version",
                region,
                localDomain.ARN
              );
            }
