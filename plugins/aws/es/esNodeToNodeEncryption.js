...
module.exports = {
  ...
  remediation_description:
    "ES domain will be configured to use node-to-node encryption.",
      ...
      function (region, rcb) {
        ...
        listDomainNames.data.forEach(function (domain) {
          ...
          if (
            ...
            if (
              localDomain.NodeToNodeEncryptionOptions &&
              localDomain.NodeToNodeEncryptionOptions.Enabled
            ) {
              helpers.addResult(
                results,
                0,
                "ES domain is configured to use node-to-node encryption",
                region,
                localDomain.ARN
              );
            } else {
              helpers.addResult(
                results,
                2,
                "ES domain is not configured to use node-to-node encryption",
                region,
                localDomain.ARN
              );
            }
