      ...
      function (region, rcb) {
        ...
        if (listDomainNames.err || !listDomainNames.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for ElasticSearch domains: " +
        ...
        if (!listDomainNames.data.length) {
          helpers.addResult(
            results,
            0,
            "No ElasticSearch domains found",
            region
          );
          return rcb();
        }
        ...
        listDomainNames.data.forEach(function (domain) {
          ...
          if (
            ...
            helpers.addResult(
              results,
              3,
              "Unable to query for ElasticSearch domain config: " +
            ...
            if (!describeElasticsearchDomain.data.DomainStatus) {
              helpers.addResult(
                results,
                0,
                "ElasticSearch domain has no access policies",
                region,
                localDomain.ARN
              );
            } else {
              ...
              if (!policies || !policies.length) {
                ...
                if (found.length > 0) {
                  helpers.addResult(
                    results,
                    2,
                    "ElasticSearch domain has policy that does not require IAM authentication",
                    region,
                    localDomain.ARN
                  );
                } else {
                  helpers.addResult(
                    results,
                    0,
                    "ElasticSearch domain access policies require IAM authentication",
                    region,
                    localDomain.ARN
                  );
                }
