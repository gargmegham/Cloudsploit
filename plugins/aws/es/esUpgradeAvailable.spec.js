  ...
  describe("run", function () {
    ...
    it("should give error result if ES domain upgrade is available", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "ES domain service software version: 1 is eligible for an upgrade to version: 2"
        );
        done();
      };
    ...
    it("should give passing result if ES domain is not upgrade eligible", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          "ES domain service software version: 1 is the latest eligible upgraded version"
        );
        done();
      };
