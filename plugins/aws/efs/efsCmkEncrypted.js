...
module.exports = {
  ...
  settings: {
    cmk_unencrypted_threshold: {
      ...
      description:
        "Plugin results will become aggregated once this value is breached",
      regex: "^[0-9]*$",
      default: 20,
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var cmk_unencrypted_threshold = parseInt(
      settings.cmk_unencrypted_threshold ||
        this.settings.cmk_unencrypted_threshold.default
    );
      ...
      function (region, rcb) {
        ...
        if (describeFileSystems.err || !describeFileSystems.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for EFS file systems: ${helpers.addError(
              describeFileSystems
            )}`,
            region
          );
        ...
        describeFileSystems.data.forEach(function (efs) {
          if (efs.Encrypted && efs.KmsKeyId) {
            if (
              kmsAliases[efs.KmsKeyId] &&
              kmsAliases[efs.KmsKeyId] === "alias/aws/elasticfilesystem"
            ) {
        ...
        if (cmkUnencryptedEFS.length > cmk_unencrypted_threshold) {
          helpers.addResult(
            results,
            2,
            `More than ${cmk_unencrypted_threshold} EFS systems are not using CMK for encryption`,
            region
          );
          ...
          for (let u in cmkUnencryptedEFS) {
            ...
            helpers.addResult(
              results,
              2,
              `EFS "${cmkUnencryptedEFS[u].FileSystemId}" is not CMK encrypted`,
              region,
              resource
            );
          }
          ...
          helpers.addResult(
            results,
            0,
            "No AWS managed key encrypted file systems found",
            region
          );
        }
        ...
        if (danglingKeys.length > cmk_unencrypted_threshold) {
          helpers.addResult(
            results,
            2,
            `More than ${cmk_unencrypted_threshold} EFS systems are referencing deleted KMS keys`,
            region
          );
          ...
          for (let u in danglingKeys) {
            ...
            helpers.addResult(
              results,
              2,
              `EFS "${danglingKeys[u].FileSystemId}" is referencing deleted KMS key`,
              region,
              resource
            );
          }
