...
const describeDBClusters = [
  {
    ...
    Endpoint:
      "docdb-2021-11-10-10-16-10.cluster-csumzsa0neyf.us-east-1.docdb.amazonaws.com",
    ReaderEndpoint:
      "docdb-2021-11-10-10-16-10.cluster-ro-csumzsa0neyf.us-east-1.docdb.amazonaws.com",
    MultiAZ: false,
    Engine: "docdb",
    EngineVersion: "4.0.0",
    ...
    KmsKeyId:
      "arn:aws:kms:us-east-1:000011112222:key/2cff2321-73c6-4bac-95eb-bc9633d3e8a9",
    DbClusterResourceId: "cluster-TWDPR3PSXGUPMCESNBK6W55SH4",
    DBClusterArn:
      "arn:aws:rds:us-east-1:000011112222:cluster:docdb-2021-11-10-10-16-10",
    AssociatedRoles: [],
...
const createCache = (
  ...
  var keyId =
    clusters && clusters.length && clusters[0].KmsKeyId
      ? clusters[0].KmsKeyId.split("/")[1]
      : null;
  return {
    docdb: {
      describeDBClusters: {
...
describe("docdbClusterEncrypted", function () {
  describe("run", function () {
    it("should PASS if DocumentDb Cluster is encrypted with desired encryption level", function (done) {
      ...
      docdbClusterEncrypted.run(
        cache,
        { documentdb_cluster_desired_encryption_level: "awscmk" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          expect(results[0].message).to.include(
            "DocumentDB cluster is encrypted with awscmk"
          );
    ...
    it("should FAIL if DocumentDB Clusters is not encrypted with desired encyption level", function (done) {
      ...
      docdbClusterEncrypted.run(
        ...
        (err, results) => {
          ...
          expect(results[0].message).to.include(
            "DocumentDB cluster is encrypted with awskms "
          );
          expect(results[0].region).to.equal("us-east-1");
          done();
        }
    ...
    it("should UNKNOWN if unable to list DocumentDB Clusters", function (done) {
      const cache = createCache(null, null, null, {
        message: "Unable to list DocumentDB Clusters encryption",
      });
      docdbClusterEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
