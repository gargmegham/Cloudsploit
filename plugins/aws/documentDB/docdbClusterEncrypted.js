  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.documentdb_cluster_desired_encryption_level ||
        this.settings.documentdb_cluster_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (describeDBClusters.err || !describeDBClusters.data) {
          helpers.addResult(
            results,
            3,
            `Unable to list DocumentDB clusters: ${helpers.addError(
              describeDBClusters
            )}`,
            region
          );
        ...
        if (!describeDBClusters.data.length) {
          helpers.addResult(results, 0, "No DocumentDB clusters found", region);
          return rcb();
        }
        ...
        for (let cluster of describeDBClusters.data) {
          ...
          if (cluster.KmsKeyId) {
            var kmsKeyId = cluster.KmsKeyId.split("/")[1]
              ? cluster.KmsKeyId.split("/")[1]
              : cluster.KmsKeyId;
