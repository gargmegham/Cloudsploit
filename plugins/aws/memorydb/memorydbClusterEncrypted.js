  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.memorydb_cluster_target_encryption_level ||
        this.settings.memorydb_cluster_target_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (describeClusters.err || !describeClusters.data) {
          helpers.addResult(
            results,
            3,
            `Unable to list MemoryDB clusters: ${helpers.addError(
              describeClusters
            )}`,
            region
          );
        ...
        if (!describeClusters.data.length) {
          helpers.addResult(results, 0, "No MemoryDB clusters found", region);
          return rcb();
        }
