...
describe("dmsEncryptionEnabled", function () {
  describe("run", function () {
    ...
    it("should PASS if replication instance matches the regex pattern", function (done) {
        ...
        [
          {
            ...
            ReplicationSubnetGroup: {
              ...
              ReplicationSubnetGroupDescription:
                "default group created by console for vpc id vpc-8a7c9cf0",
              VpcId: "vpc-8a7c9cf0",
            ...
            KmsKeyId:
              "arn:aws:kms:us-east-1:1156999144843:key/b4022674-ba90-4abc-9087-543cad09879b",
            ReplicationInstanceArn:
              "arn:aws:dms:us-east-1:1156999144843:rep:IP5IC7QARKU4GWDJDPQMGHKG5I",
            ReplicationInstancePrivateIpAddress: "172.31.0.71",
            ReplicationInstancePublicIpAddresses: [null],
            ReplicationInstancePrivateIpAddresses: ["172.31.0.71"],
      ...
      dmsEncryptionEnabled.run(
        cache,
        { dms_encryption_allow_pattern: "^gio-test-2$" },
        (err, results) => {
          expect(results.length).to.equal(1);
    ...
    it("should FAIL if replication instance has encryption but matching alias could not be found ", function (done) {
      ...
      dmsEncryptionEnabled.run(
        cache,
        { dms_encryption_kms_alias: "alias/my-other-alias" },
        (err, results) => {
          expect(results.length).to.equal(1);
    ...
    it("should PASS if replication instance has encryption with matching alias", function (done) {
      ...
      dmsEncryptionEnabled.run(
        cache,
        { dms_encryption_kms_alias: "alias/my-alias" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          expect(results[0].message).to.include(
          ...
          "b4022674-ba90-4abc-9087-543cad09879b": {
            data: {
              KeyMetadata: {
                ...
                Enabled: true,
                Description:
                  "Default master key that protects my DMS replication instance volumes when no other key is defined",
                KeyUsage: "ENCRYPT_DECRYPT",
                KeyState: "Enabled",
                Origin: "AWS_KMS",
                KeyManager: "AWS",
