...
module.exports = {
  ...
  compliance: {
    hipaa:
      "DMS encryption must be used when processing any HIPAA-related data. " +
  ...
  settings: {
    dms_encryption_allow_pattern: {
      ...
      description:
        "When set, whitelists DMS instances matching the given pattern. Useful for overriding buckets outside the account control.",
      regex: "^.{1,255}$",
      default: false,
    },
    dms_encryption_kms_alias: {
      ...
      description:
        "If set, DMS encryption must be configured using the KMS key alias specified. Be sure to include the alias/ prefix. Comma-delimited.",
  ...
  run: function (cache, settings, callback) {
    var config = {
      dms_encryption_allow_pattern:
        settings.dms_encryption_allow_pattern ||
        this.settings.dms_encryption_allow_pattern.default,
      dms_encryption_kms_alias:
        settings.dms_encryption_kms_alias ||
        this.settings.dms_encryption_kms_alias.default,
    };
      ...
      [
        ...
        function (cb) {
          async.each(
            ...
            function (region, rcb) {
              // List the KMS Keys
              var describeReplicationInstances = helpers.addSource(
                cache,
                source,
              ...
              if (
                describeReplicationInstances.err ||
                !describeReplicationInstances.data
              ) {
                helpers.addResult(
                  results,
                  3,
                  "Unable to query for replication instances: " +
                    helpers.addError(describeReplicationInstances),
                  region
                );
                return rcb();
              }
              ...
              if (!describeReplicationInstances.data.length) {
                helpers.addResult(
                  results,
                  0,
                  "No replication instances found",
                  region
                );
                return rcb();
              }
              ...
              var allowRegex =
                config.dms_encryption_allow_pattern &&
                config.dms_encryption_allow_pattern.length
                  ? new RegExp(config.dms_encryption_allow_pattern)
                  : false;

              describeReplicationInstances.data.forEach(function (instance) {
                if (
                  allowRegex &&
                  allowRegex.test(instance.ReplicationInstanceIdentifier)
                ) {
                  helpers.addResult(
                    results,
                    0,
                  ...
                  if (instance.KmsKeyId) {
                    ...
                    if (config.dms_encryption_kms_alias) {
                      if (!aliasKeyIds.length) {
                        helpers.addResult(
                          ...
                          "The replication instance: " +
                            ...
                            config.dms_encryption_kms_alias +
                        ...
                        helpers.addResult(
                          ...
                          "The replication instance: " +
                            instance.ReplicationInstanceIdentifier +
                            " has encryption enabled using required KMS key: " +
                            keyArn,
                          region,
                          instance.ReplicationInstanceArn,
                          custom
                      ...
                      defaultKeyIds.length &&
                      defaultKeyIds.indexOf(keyArn) > -1
                    ) {
                      helpers.addResult(
                        results,
                        2,
                        "Replication instance: " +
                          instance.ReplicationInstanceIdentifier +
                          " has default kms/dms encryption enabled",
                        region,
                        instance.ReplicationInstanceArn
                      );
                    } else {
                    ...
                    helpers.addResult(
                      ...
                      "Replication instance: " +
                        instance.ReplicationInstanceIdentifier +
                        " has default kms/dms encryption",
                      region,
                      instance.ReplicationInstanceArn
                    );
                  }
