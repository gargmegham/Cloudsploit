      ...
      function (region, rcb) {
        ...
        if (listAnalyzers.err || !listAnalyzers.data) {
          helpers.addResult(
            results,
            3,
            "Unable to list Access Analyzers: " +
              helpers.addError(listAnalyzers),
            region
          );
          return rcb();
        }
        ...
        if (!listAnalyzers.data.length) {
          helpers.addResult(
            results,
            2,
            "Access Analyzer is not configured",
            region
          );
          return rcb();
        }

        var found = listAnalyzers.data.find(
          (analyzer) => analyzer.status.toLowerCase() == "active"
        );
        if (found) {
          helpers.addResult(
            results,
