      ...
      function (region, rcb) {
        ...
        if (listAnalyzers.err || !listAnalyzers.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for IAM Access Analyzer analyzers: ${helpers.addError(
              listAnalyzers
            )}`,
            region
          );
        ...
        for (let analyzer of listAnalyzers.data) {
          ...
          if (!listFindings || listFindings.err || !listFindings.data) {
            helpers.addResult(
              results,
              3,
              `Unable to IAM Access Analyzer findings: ${helpers.addError(
                listFindings
              )}`,
              region,
              resource
          ...
          let filtered = listFindings.data.findings.filter(
