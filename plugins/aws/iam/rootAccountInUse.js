...
module.exports = {
  ...
  more_info:
    "The root account should not be used for day-to-day account management. IAM users, roles, and groups should be used instead.",
  link: "http://docs.aws.amazon.com/general/latest/gr/root-vs-iam.html",
  recommended_action:
    "Create IAM users with appropriate group-level permissions for account access. Create an MFA token for the root account, and store its password and token generation QR codes in a secure place.",
  apis: ["IAM:generateCredentialReport"],
  compliance: {
    hipaa:
      "HIPAA requires strong auditing controls surrounding actions " +
  ...
  settings: {
    root_account_in_use_days: {
      ...
      description:
        "Return a failing result when the root account has been used within this many days",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: 15,
    },
  },
  ...
  _run: function (cache, settings, callback, now) {
    var config = {
      root_account_in_use_days:
        settings.root_account_in_use_days ||
        this.settings.root_account_in_use_days.default,
    };
    ...
    for (var r in generateCredentialReport.data) {
      ...
      if (obj && obj.user === "<root_account>") {
        ...
        if (
          obj.access_key_1_last_used_date &&
          obj.access_key_1_last_used_date !== "N/A"
        ) {
        ...
        if (
          ...
          obj.access_key_2_last_used_date !== "N/A"
        ) {
        ...
        if (!accessDates.length) {
          helpers.addResult(
            results,
            0,
            "Root account has not been used",
            "global",
            obj.arn
          );
        } else {
          ...
          var resultCode =
            helpers.daysBetween(dateToCompare, now) <
            config.root_account_in_use_days
              ? 2
              : 0;
