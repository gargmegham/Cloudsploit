...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    var config = {
      certificate_expiry_pass:
        settings.certificate_expiry_pass ||
        this.settings.certificate_expiry_pass.default,
      certificate_expiry_warn:
        settings.certificate_expiry_warn ||
        this.settings.certificate_expiry_warn.default,
    };
    ...
    if (listServerCertificates.err || !listServerCertificates.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query for certificates: " +
          helpers.addError(listServerCertificates)
      );
    ...
    for (var i in listServerCertificates.data) {
      if (
        listServerCertificates.data[i].ServerCertificateName &&
        listServerCertificates.data[i].Expiration
      ) {
