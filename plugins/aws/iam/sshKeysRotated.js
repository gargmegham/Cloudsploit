...
module.exports = {
  ...
  settings: {
    ssh_keys_rotated_fail: {
      ...
      description:
        "Return a failing result when SSH keys have not been rotated for this many days",
      regex: "^[1-9]{1}[0-9]{0,2}$",
      default: 360,
    },
    ssh_keys_rotated_warn: {
      ...
      description:
        "Return a warning result when SSH keys have not been rotated for this many days",
      regex: "^[1-9]{1}[0-9]{0,2}$",
      default: 180,
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      ssh_keys_rotated_fail:
        settings.ssh_keys_rotated_fail ||
        this.settings.ssh_keys_rotated_fail.default,
      ssh_keys_rotated_warn:
        settings.ssh_keys_rotated_warn ||
        this.settings.ssh_keys_rotated_warn.default,
    };
    ...
    function addSSHKeyResults(
      ...
      if (
        helpers.daysAgo(userCreationTime) > 180 &&
        (!lastRotated ||
          lastRotated === "N/A" ||
          helpers.daysAgo(lastRotated) > config.ssh_keys_rotated_fail)
      ) {
        helpers.addResult(results, 2, returnMsg, "global", arn, custom);
      } else if (
        helpers.daysAgo(userCreationTime) > 90 &&
        (!lastRotated ||
          lastRotated === "N/A" ||
          helpers.daysAgo(lastRotated) > config.ssh_keys_rotated_warn)
      ) {
