...
module.exports = {
  ...
  settings: {
    iam_user_account_in_use_days: {
      ...
      description:
        "Return a failing result when an IAM user account has been used within this many days",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: "15",
    },
  },
  ...
  run: function (cache, settings, callback) {
    const config = {
      iam_user_account_in_use_days: parseInt(
        settings.iam_user_account_in_use_days ||
          this.settings.iam_user_account_in_use_days.default
      ),
    };
    ...
    generateCredentialReport.data.forEach((user) => {
      if (user && user.user !== "<root_account>") {
        ...
        if (!accessDates.length) {
          ...
          const resultCode =
            difference < config.iam_user_account_in_use_days ? 2 : 0;
