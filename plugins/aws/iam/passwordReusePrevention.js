...
module.exports = {
  ...
  remediation_description:
    "The password policy for password reuse prevention will be set to the value set by the user. Otherwise, it will default to 24.",
  ...
  remediation_inputs: {
    maxPreviousPasswords: {
      ...
      description:
        "The minimum number of previous passwords to allow in the account policy 1-24",
  ...
  compliance: {
    pci:
      "PCI requires that the previous 4 passwords not be reused. " +
  ...
  settings: {
    password_reuse_fail: {
      ...
      description:
        "Return a failing result when password reuse policy remembers fewer than this many past passwords",
      regex: "^[1-9]{1}[0-9]{0,2}$",
      default: 5,
    },
    password_reuse_warn: {
      ...
      description:
        "Return a warning result when password reuse policy remembers fewer than this many past passwords",
      regex: "^[1-9]{1}[0-9]{0,2}$",
      default: 24,
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      password_reuse_fail:
        settings.password_reuse_fail ||
        this.settings.password_reuse_fail.default,
      password_reuse_warn:
        settings.password_reuse_warn ||
        this.settings.password_reuse_warn.default,
    };
    ...
    if (!passwordPolicy.PasswordReusePrevention) {
      helpers.addResult(
        results,
        2,
        "Password policy does not prevent reusing previous passwords"
      );
    } else if (
      passwordPolicy.PasswordReusePrevention < config.password_reuse_fail
    ) {
      helpers.addResult(
        results,
        2,
        "Maximum password reuse of: " +
          passwordPolicy.PasswordReusePrevention +
          " passwords is less than " +
          config.password_reuse_fail,
        "global",
        null,
        custom
      );
    } else if (
      passwordPolicy.PasswordReusePrevention < config.password_reuse_warn
    ) {
      helpers.addResult(
        results,
        1,
