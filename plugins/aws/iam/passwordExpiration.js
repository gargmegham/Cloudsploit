...
module.exports = {
  ...
  remediation_description:
    "The password policy for password expiration will be set to true.",
  ...
  compliance: {
    pci:
      "PCI requires that user passwords are rotated every 90 days. Forcing " +
  ...
  run: function (cache, settings, callback) {
    ...
    if (!passwordPolicy.ExpirePasswords) {
      helpers.addResult(
        results,
        2,
        "Password expiration policy is not set to expire passwords"
      );
      return callback(null, results, source);
    }

    var returnMsg =
      "Password expiration of: " + passwordPolicy.MaxPasswordAge + " days is ";
