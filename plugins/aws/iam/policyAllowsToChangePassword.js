...
module.exports = {
  ...
  remediation_description:
    "The password policy for password to be changed by users will be set to true.",
  ...
  compliance: {
    pci:
      "PCI requires a strong password policy. Setting IAM password " +
  ...
  run: function (cache, settings, callback) {
    ...
    if (!passwordPolicy.AllowUsersToChangePassword) {
      helpers.addResult(
        results,
        2,
        "Password policy does not allow users to change their password"
      );
    } else {
      helpers.addResult(
        results,
        0,
        "Password policy allow users to change their password"
      );
    }
