...
module.exports = {
  ...
  more_info:
    "The root account should avoid using access keys. Since the root account has full permissions across the entire account, creating access keys for it only increases the chance that they are compromised. Instead, create IAM users with predefined roles.",
      ...
      if (obj && obj.user === "<root_account>") {
        ...
        if (!obj.access_key_1_active && !obj.access_key_2_active) {
          helpers.addResult(
            results,
            0,
            "Access keys were not found for the root account",
            "global",
            obj.arn
          );
        } else {
          helpers.addResult(
            results,
            2,
            "Access keys were found for the root account",
            "global",
            obj.arn
          );
        }
