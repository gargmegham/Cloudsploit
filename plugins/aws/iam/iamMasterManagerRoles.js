    ...
    async.each(
      ...
      function (role, cb) {
        ...
        if (!getRole || getRole.err || !getRole.data || !getRole.data.Role) {
          helpers.addResult(
            results,
            3,
            "Unable to query for IAM roles details: " +
              role.RoleName +
              ": " +
        ...
        var assumeRolePolicy = helpers.normalizePolicyDocument(
              ...
              if (
                ...
                if (
                  ...
                  statement.Action.forEach((perm) => {
                    if (!rolePermissions.allow.includes(perm))
                      rolePermissions.allow.push(perm);
                  });
                  continue;
                }
                ...
                if (
                  ...
                  statement.Action.forEach((perm) => {
                    if (!rolePermissions.deny.includes(perm))
                      rolePermissions.deny.push(perm);
                  });
                }
              }
            }
        ...
        if (
          masterRoleActions.allow.every((permission) =>
            rolePermissions.allow.includes(permission)
          ) &&
          masterRoleActions.deny.every((permission) =>
            rolePermissions.deny.includes(permission)
          )
        ) {
        ...
        if (
          managerRoleActions.allow.every((permission) =>
            rolePermissions.allow.includes(permission)
          ) &&
          managerRoleActions.deny.every((permission) =>
            rolePermissions.deny.includes(permission)
          )
        ) {
      ...
      function () {
        if (managerRoleFound && masterRoleFound) {
          ...
          helpers.addResult(results, 2, "IAM Manager Role not found", "global");
        } else {
          helpers.addResult(results, 2, "IAM Master Role not found", "global");
        }
