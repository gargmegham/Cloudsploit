...
module.exports = {
  ...
  settings: {
    canary_user: {
      name: "Canary User",
      description:
        "Provide a single IAM user name (not an ARN) to monitor for this account.",
  ...
  run: function (cache, settings, callback) {
    ...
    if (!config.canary_user || !config.canary_user.length)
      ...
      function (obj, cb) {
        ...
        if (
          ...
          helpers.addResult(
            results,
            2,
            "The canary user: " + config.canary_user + " has been used."
          );
        } else {
          helpers.addResult(
            results,
            0,
            "The canary user: " + config.canary_user + " has not been used."
          );
        }
      ...
      function () {
        if (!found) {
          helpers.addResult(
            results,
            3,
            "The canary user specified in the config could not be found."
          );
        }
