...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    var config = {
      whitelisted_aws_account_principals:
        settings.whitelisted_aws_account_principals ||
        this.settings.whitelisted_aws_account_principals.default,
      whitelisted_aws_account_principals_regex:
        settings.whitelisted_aws_account_principals_regex ||
        this.settings.whitelisted_aws_account_principals_regex.default,
      iam_whitelist_aws_organization_accounts:
        settings.iam_whitelist_aws_organization_accounts ||
        this.settings.iam_whitelist_aws_organization_accounts.default,
    };
    var makeRegexBased = config.whitelisted_aws_account_principals_regex.length
      ? true
      : false;
    var whitelistOrganization =
      config.iam_whitelist_aws_organization_accounts == "true";
    config.whitelisted_aws_account_principals_regex = new RegExp(
      config.whitelisted_aws_account_principals_regex
    );
    var results = [];
    var source = {};
    ...
    listRoles.data.forEach((role) => {
        ...
        if (
          ...
          if (principals.length) {
            principals.forEach((principal) => {
              ...
              if (makeRegexBased) {
                if (
                  !config.whitelisted_aws_account_principals_regex.test(
                    principal
                  ) &&
                ...
                !config.whitelisted_aws_account_principals.includes(
                  principal
                ) &&
                !restrictedAccountPrincipals.includes(principal)
              )
      ...
      if (crossAccountRole && !restrictedAccountPrincipals.length) {
        ...
        helpers.addResult(
          ...
          `Cross-account role "${
            role.RoleName
          }" contains these untrusted account principals: ${restrictedAccountPrincipals.join(
            ", "
          )}`,
          "global",
          role.Arn
