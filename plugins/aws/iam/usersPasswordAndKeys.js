...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    try {
      var machineUsernameRegex = RegExp(
        settings.iam_machine_username_regex ||
          this.settings.iam_machine_username_regex.default
      );
    } catch (err) {
      helpers.addResult(
        results,
        3,
        "Invalid regex for machine username: " + machineUsernameRegex,
        "global"
      );
    }
      ...
      function (obj, cb) {
        ...
        if (obj.access_key_1_active || obj.access_key_2_active) {
          helpers.addResult(
            results,
            2,
            "User has console access and access key",
            "global",
            obj.arn
          );
        } else {
          helpers.addResult(
            results,
            0,
            "User has console access and no access keys",
            "global",
            obj.arn
          );
        }
      ...
      function () {
        if (!found) {
          helpers.addResult(
            results,
            0,
            "No users with console access and access keys found"
          );
        }
