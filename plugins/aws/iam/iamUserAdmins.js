...
module.exports = {
  ...
  settings: {
    iam_admin_count_minimum: {
      ...
      description:
        "The minimum number of IAM user admins to require in the account",
      regex: "^[0-9]{0,4}$",
      default: 2,
    },
    iam_admin_count_maximum: {
      ...
      description:
        "The maximum number of IAM user admins to require in the account",
      regex: "^[0-9]{0,4}$",
      default: 2,
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    if (listUsers.err || !listUsers.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query for user IAM policy status: " +
          helpers.addError(listUsers)
      );
      return callback(null, results, source);
    }
      ...
      function () {
        ...
        if (userAdmins.length < config.iam_admin_count_minimum) {
          helpers.addResult(
            results,
            1,
            "There are fewer than the minimum " +
              config.iam_admin_count_minimum +
              " IAM user administrators",
            "global",
            null,
            custom
          );
        } else if (
          userAdmins.length >= config.iam_admin_count_minimum &&
          userAdmins.length <= config.iam_admin_count_maximum
        ) {
          helpers.addResult(
            results,
            0,
          ...
          for (var u in userAdmins) {
            helpers.addResult(
              ...
              "User: " +
                ...
                " is one of " +
                userAdmins.length +
                " IAM user administrators, which exceeds the expected value of: " +
                config.iam_admin_count_maximum,
              "global",
              userAdmins[u].arn,
              custom
            );
