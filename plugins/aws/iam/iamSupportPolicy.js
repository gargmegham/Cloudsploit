...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    if (!listPolicies.data.length) {
      helpers.addResult(results, 0, "No IAM policies found");
      return callback(null, results, source);
    }

    var found = listPolicies.data.find(
      (policy) => policy.PolicyName == "AWSSupportAccess"
    );
    ...
    if (found) {
      helpers.addResult(
        results,
        0,
        "AWSSupportAccess policy is attached to a user, role or group",
        "global",
        found.Arn
      );
    } else {
      helpers.addResult(
        results,
        2,
        "No role, user or group attached to the AWSSupportAccess policy",
        "global"
      );
    }
