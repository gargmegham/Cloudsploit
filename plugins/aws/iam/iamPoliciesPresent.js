...
module.exports = {
  ...
  settings: {
    iam_required_policy_names: {
      ...
      description:
        "A comma separated list of IAM policy names that all IAM roles should have",
      regex: "^.*$",
      default: "",
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      iam_required_policy_names:
        settings.iam_required_policy_names ||
        this.settings.iam_required_policy_names.default,
    };

    if (!config.iam_required_policy_names.length)
      return callback(null, results, source);

    config.iam_required_policy_names =
      config.iam_required_policy_names.split(",");
    ...
    async.each(
      ...
      function (role, cb) {
        ...
        if (listRolePolicies.data && listRolePolicies.data.PolicyNames)
          attachedPolicies = attachedPolicies.concat(
        ...
        if (difference.length) {
          helpers.addResult(
            results,
            2,
            `IAM role does not have these required policies attached: ${difference.join(
              ", "
            )}`,
            region,
            role.Arn
          ...
          helpers.addResult(
            results,
            0,
            "IAM role has all required policies attached",
            region,
            role.Arn
          );
        }
