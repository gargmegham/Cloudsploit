...
module.exports = {
  ...
  settings: {
    iam_authorized_user_arns: {
      ...
      description:
        "A comma delimited list of user ARNs authorized to contain edit IAM access policies permission",
  ...
  run: function (cache, settings, callback) {
    var whitelisted_users =
      settings.iam_authorized_user_arns ||
      this.settings.iam_authorized_user_arns.default;
    ...
    if (listUsers.err || !listUsers.data) {
      helpers.addResult(
        results,
        3,
        `Unable to query for user IAM policy status: ${helpers.addError(
          listUsers
        )}`
      );
      ...
      function (user, cb) {
        ...
        if (!listAttachedUserPolicies || listAttachedUserPolicies.err) {
          helpers.addResult(
            results,
            3,
            `Unable to query for IAM attached policy for user: ${
              user.UserName
            }: ${helpers.addError(listAttachedUserPolicies)}`,
            "global",
            user.Arn
          );
          return cb();
        ...
        if (!listUserPolicies || listUserPolicies.err) {
          helpers.addResult(
            results,
            3,
            `Unable to query for IAM user policy for user: ${
              user.UserName
            }: ${helpers.addError(listUserPolicies)}`,
            "global",
            user.Arn
          );
          return cb();
        ...
        if (!listGroupsForUser || listGroupsForUser.err) {
          helpers.addResult(
            results,
            3,
            `Unable to query for IAM user groups for user: ${
              user.UserName
            }: ${helpers.addError(listGroupsForUser)}`,
            "global",
            user.Arn
          );
          return cb();
        ...
        if (
          ...
          for (var p in listAttachedUserPolicies.data.AttachedPolicies) {
            ...
            if (
              policy.PolicyArn === adminAccessArn ||
          ...
          for (var up in listUserPolicies.data.PolicyNames) {
            ...
            if (
              ...
              for (var s in statements) {
                ...
                if (
                  helpers.userGlobalAccess(statement, iamEditAccessPermissions)
                ) {
          ...
          for (var g in listGroupsForUser.data.Groups) {
            ...
            if (
              ...
              for (var a in listAttachedGroupPolicies.data.AttachedPolicies) {
                let policyAttached =
                  listAttachedGroupPolicies.data.AttachedPolicies[a];
                if (
                  policyAttached.PolicyArn === adminAccessArn ||
                  policyAttached.PolicyArn === iamFullAccessArn
                ) {
                  addPolicyToUserObj(
                    restrictedUser,
                    user,
                    policyAttached.PolicyName
                  );
                }
              }
            }
              ...
              for (var q in listGroupPolicies.data.PolicyNames) {
                ...
                if (
                  ...
                  for (s in statementsGroup) {
                    ...
                    if (
                      helpers.userGlobalAccess(
                        statementGroup,
                        iamEditAccessPermissions
                      )
                    ) {
        ...
        if (!restrictedUser[user.Arn]) {
          ...
          helpers.addResult(
            ...
            `IAM user "${
              user.UserName
            }" is not authorized to have these policies attached: ${restrictedUser[
              user.Arn
            ].policyNames.join(", ")}`,
            "global",
            user.Arn
...
function addPolicyToUserObj(userObj, user, policy) {
  if (userObj[user.Arn]) {
    if (!userObj[user.Arn].policyNames.includes(policy))
      userObj[user.Arn].policyNames.push(policy);
