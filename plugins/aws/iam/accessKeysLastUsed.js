...
module.exports = {
  ...
  compliance: {
    pci:
      "PCI requires that all users be removed if they are inactive for 90 days. " +
  ...
  settings: {
    access_keys_last_used_fail: {
      ...
      description:
        "Return a failing result when access keys exceed this number of days without being used",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: 180,
    },
    access_keys_last_used_warn: {
      ...
      description:
        "Return a warning result when access keys exceed this number of days without being used",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: 90,
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      access_keys_last_used_fail:
        settings.access_keys_last_used_fail ||
        this.settings.access_keys_last_used_fail.default,
      access_keys_last_used_warn:
        settings.access_keys_last_used_warn ||
        this.settings.access_keys_last_used_warn.default,
    };
    ...
    function addAccessKeyResults(lastUsed, keyNum, arn) {
      if (!lastUsed) {
        helpers.addResult(
          results,
          0,
          "User access key " + keyNum + " has never been used",
          "global",
          arn
        );
      } else {
