...
module.exports = {
  ...
  settings: {
    ...
    ignore_service_specific_wildcards: {
      ...
      description:
        "This allows enables you to allow attached policies (inline and managed) to use service specific wildcards in Action. " +
    ...
    ignore_identity_federation_roles: {
      ...
      description:
        "This setting allows you to skip IdP/SAML based roles " +
    ...
    ignore_aws_managed_iam_policies: {
      ...
      description:
        "If set to true, skip AWS-managed policies attached to the role with the exception of AWS-managed AdministratorAccess policy",
      regex: "^(true|false)$",
      default: "false",
    },
    ignore_customer_managed_iam_policies: {
      ...
      description:
        "If set to true, skip customer-managed policies attached to the role",
      regex: "^(true|false)$",
      default: "false",
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      iam_role_policies_ignore_path:
        settings.iam_role_policies_ignore_path ||
        this.settings.iam_role_policies_ignore_path.default,
      ignore_service_specific_wildcards:
        settings.ignore_service_specific_wildcards ||
        this.settings.ignore_service_specific_wildcards.default,
      ignore_identity_federation_roles:
        settings.ignore_identity_federation_roles ||
        this.settings.ignore_identity_federation_roles.default,
      ignore_aws_managed_iam_policies:
        settings.ignore_aws_managed_iam_policies ||
        this.settings.ignore_aws_managed_iam_policies.default,
      ignore_customer_managed_iam_policies:
    ...
    config.ignore_service_specific_wildcards =
      config.ignore_service_specific_wildcards === "true";
    config.ignore_identity_federation_roles =
      config.ignore_identity_federation_roles === "true";
    config.ignore_aws_managed_iam_policies =
      config.ignore_aws_managed_iam_policies === "true";
    config.ignore_customer_managed_iam_policies =
      config.ignore_customer_managed_iam_policies === "true";
      ...
      function (region, rcb) {
        ...
        if (configRecorderStatus.err || !configRecorderStatus.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query config service: " +
              helpers.addError(configRecorderStatus),
            region
          );
          return rcb();
        }
        ...
        if (
          !configRecorderStatus.data[0].lastStatus ||
          (configRecorderStatus.data[0].lastStatus.toUpperCase() !==
            "SUCCESS" &&
        ...
        if (discoveredResources.err || !discoveredResources.data) {
          helpers.addResult(
            ...
            "Unable to query for Discovered Resources: " +
              helpers.addError(discoveredResources)
          );
          return rcb();
        }
      ...
      function () {
        ...
        allResources = allResources.reduce((result, resource) => {
          ...
          if (arr.length && arr.length >= 5) {
            ...
            if (
              resource.count > 0 &&
              allServices[service] &&
              allServices[service].includes(subService)
            ) {
        ...
        async.each(
          ...
          function (role, cb) {
            ...
            if (
              config.iam_role_policies_ignore_path &&
            ...
            if (!listAttachedRolePolicies || listAttachedRolePolicies.err) {
              helpers.addResult(
                ...
                "Unable to query for IAM attached policy for role: " +
                  role.RoleName +
                  ": " +
                  helpers.addError(listAttachedRolePolicies),
                "global",
                role.Arn
              );
              return cb();
            ...
            if (!listRolePolicies || listRolePolicies.err) {
              helpers.addResult(
                ...
                "Unable to query for IAM role policy for role: " +
                  role.RoleName +
                  ": " +
                  helpers.addError(listRolePolicies),
                "global",
                role.Arn
              );
              return cb();
            ...
            if (
              ...
              for (var policy of listAttachedRolePolicies.data
                .AttachedPolicies) {
                if (policy.PolicyArn === managedAdminPolicy) {
                  roleFailures.push(
                    "Role has managed AdministratorAccess policy"
                  );
                  break;
                }
                ...
                if (
                  config.ignore_aws_managed_iam_policies &&
                  /^arn:aws:iam::aws:.*/.test(policy.PolicyArn)
                )
                  continue;

                if (
                  config.ignore_customer_managed_iam_policies &&
                  /^arn:aws:iam::[0-9]{12}:.*/.test(policy.PolicyArn)
                )
                  continue;
                      ...
                      if (statement.Action && statement.Action.length) {
                        for (let action of statement.Action) {
                          let service = action.split(":")[0]
                            ...
                            : "";
                          let resourceAction = action.split(":")[1]
                            ? action.split(":")[1].toLowerCase()
                            : "";

                          if (allServices[service]) {
                            for (let supportedResource of allServices[
                              service
                            ]) {
                              if (resourceAction.includes(supportedResource)) {
                                if (
                                  !allResources[service] ||
                                  !allResources[service].includes(
                                    supportedResource
                                  )
                                ) {
                                  if (policyFailures.indexOf(action) === -1)
                                    policyFailures.push(action);
                                }
                              }
                            }
                          }
            ...
            if (listRolePolicies.data && listRolePolicies.data.PolicyNames) {
                ...
                if (
                  ...
                  for (let statement of statements) {
                    ...
                    if (
                      ...
                      let service = statement.Resource[0].includes("arn")
                        ? statement.Resource[0].split(":")[2].toLowerCase()
            ...
            if (policyFailures.length || roleFailures.length) {
              let failureMsg = policyFailures.length
                ? "Role policies contain actions for resource types which are not in use: " +
                  "[ " +
                  policyFailures.join(", ") +
                  " ]" +
                  "\r\n" +
...
function addRoleFailures(
  roleFailures,
  statements,
  policyType,
  ignoreServiceSpecific
) {
  for (var statement of statements) {
    if (statement.Effect === "Allow" && !statement.Condition) {
      ...
      if (
        ...
        if (wildcards.length)
          failMsg = `Role ${policyType} policy allows wildcard actions: ${wildcards.join(
            ", "
          )}`;
      }

      if (failMsg && roleFailures.indexOf(failMsg) === -1)
        roleFailures.push(failMsg);
    }
  }
}
