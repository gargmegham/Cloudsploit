...
const roles = [
  {
    ...
    AssumeRolePolicyDocument:
      "%7B%22Version%22%3A%222012-10-17%22%2C%22Statement%22%3A%5B%7B%22Effect%22%3A%22Allow%22%2C%22Principal%22%3A%7B%22Service%22%3A%22trustedadvisor.amazonaws.com%22%7D%2C%22Action%22%3A%22sts%3AAssumeRole%22%7D%5D%7D",
    Description:
  ...
  },
  {
    ...
    AssumeRolePolicyDocument:
      "%7B%22Version%22%3A%222012-10-17%22%2C%22Statement%22%3A%5B%7B%22Effect%22%3A%22Allow%22%2C%22Principal%22%3A%7B%22AWS%22%3A%22arn%3Aaws%3Aiam%3A%3A123456654322%3Aroot%22%7D%2C%22Action%22%3A%22sts%3AAssumeRole%22%2C%22Condition%22%3A%7B%7D%7D%5D%7D",
    MaxSessionDuration: 3600,
  },
];
...
describe("trustedCrossAccountRoles", function () {
  describe("run", function () {
    ...
    it("should PASS if cross-account role contains organization account ID and setting to allow organization account is true", function (done) {
      const cache = createCache([roles[4]]);
      trustedCrossAccountRoles.run(
        cache,
        { iam_whitelist_aws_organization_accounts: "true" },
        (err, results) => {
          expect(results.length).to.equal(1);
    ...
    it("should PASS if cross-account role contains trusted account IDs", function (done) {
      ...
      trustedCrossAccountRoles.run(
        cache,
        {
          whitelisted_aws_account_principals: "arn:aws:iam::123456654321:root",
        },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
