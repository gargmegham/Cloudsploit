...
module.exports = {
  ...
  compliance: {
    pci:
      "PCI requires that all user credentials are rotated every 90 days. " +
  ...
  settings: {
    users_password_last_used_fail: {
      ...
      description:
        "Return a failing result when users with password logins exceed this number of days without being used",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: 180,
    },
    users_password_last_used_warn: {
      ...
      description:
        "Return a warning result when users with password logins exceed this number of days without being used",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: 90,
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      users_password_last_used_fail:
        settings.users_password_last_used_fail ||
        this.settings.users_password_last_used_fail.default,
      users_password_last_used_warn:
        settings.users_password_last_used_warn ||
        this.settings.users_password_last_used_warn.default,
    };
    ...
    if (generateCredentialReport.err || !generateCredentialReport.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query for user password status: " +
    ...
    for (var r in generateCredentialReport.data) {
      ...
      if (
        ...
        obj.password_last_used !== "no_information"
      ) {
        daysAgo = helpers.daysAgo(obj.password_last_used);

        returnMsg =
          "User password login was last used " + daysAgo + " days ago";
        ...
        returnMsg =
          "User was created " +
          daysAgo +
          " days ago but password login was never used";
      } else {
