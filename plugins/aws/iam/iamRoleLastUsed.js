...
module.exports = {
  ...
  settings: {
    iam_role_last_used_fail: {
      ...
      description:
        "Return a failing result when IAM roles exceed this number of days without being used",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: 180,
    },
    iam_role_last_used_warn: {
      ...
      description:
        "Return a warning result when IAM roles exceed this number of days without being used",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: 90,
    },
  ...
  run: function (cache, settings, callback) {
    var config = {
      iam_role_last_used_fail:
        settings.iam_role_last_used_fail ||
        this.settings.iam_role_last_used_fail.default,
      iam_role_last_used_warn:
        settings.iam_role_last_used_warn ||
        this.settings.iam_role_last_used_warn.default,
      iam_role_ignore_path:
        settings.iam_role_ignore_path ||
        this.settings.iam_role_ignore_path.default,
      skip_aws_service_roles:
      ...
      function (role, cb) {
        ...
        if (
          ...
          helpers.addResult(
            results,
            2,
            "IAM role: " + role.RoleName + " has not been used",
            "global",
            role.Arn
          );
          return cb();
        ...
        var daysAgo = helpers.daysAgo(
          getRole.data.Role.RoleLastUsed.LastUsedDate
        );

        var returnCode = 0;
        var returnMsg = `IAM role was last used ${daysAgo} days ago in the ${
          getRole.data.Role.RoleLastUsed.Region || "unknown"
        } region`;
