...
module.exports = {
  ...
  more_info:
    "To reduce management complexity, IAM permissions should only be assigned to roles and groups. Users can then be added to those groups. Policies should not be applied directly to a user.",
      ...
      function (user, cb) {
        ...
        if (!listAttachedUserPolicies.data || !listUserPolicies.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query policies for user: " +
              user.UserName +
              ": no data returned",
            "global",
            user.Arn
        ...
        if (
          ...
          helpers.addResult(
            results,
            1,
            "User is using attached or inline policies",
            "global",
            user.Arn
          );
        } else {
          helpers.addResult(
            results,
            0,
            "User is not using attached or inline policies",
            "global",
            user.Arn
          );
        }
