...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    if (
      ...
      helpers.addResult(results, 2, "Root account is not using an MFA device");
    ...
    if (
      ...
      helpers.addResult(
        results,
        3,
        "Unable to query for MFA devices: " +
          helpers.addError(listVirtualMFADevices)
      );
      return callback(null, results, source);
    }
    ...
    for (var r in listVirtualMFADevices.data) {
      ...
      if (
        obj.SerialNumber &&
        obj.SerialNumber.indexOf(":mfa/root-account-mfa-device") > -1
      ) {
    ...
    if (arn) {
      helpers.addResult(
        results,
        2,
        "A virtual MFA device was found for the root account",
        "global",
        arn
      );
    } else {
      helpers.addResult(
        results,
        0,
        "Root account is using a hardware MFA device"
      );
    }
