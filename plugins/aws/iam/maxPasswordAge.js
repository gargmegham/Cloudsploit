...
module.exports = {
  ...
  remediation_description:
    "The password policy for maximum password age will be set to the value set by the user. Otherwise, it will default to 179.",
  ...
  remediation_inputs: {
    maxPasswordAge: {
      ...
      description:
        "The maximum age of passwords to allow in the account policy 1-1095",
  ...
  compliance: {
    pci:
      "PCI requires that all user credentials are rotated every 90 days. Setting " +
  ...
  settings: {
    max_password_age_fail: {
      ...
      description:
        "Return a failing result when max password age exceeds this number of days",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: 365,
    },
    max_password_age_warn: {
      ...
      description:
        "Return a warning result when max password age exceeds this number of days",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: 180,
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      max_password_age_fail:
        settings.max_password_age_fail ||
        this.settings.max_password_age_fail.default,
      max_password_age_warn:
        settings.max_password_age_warn ||
        this.settings.max_password_age_warn.default,
    };
    ...
    if (!passwordPolicy.MaxPasswordAge) {
      helpers.addResult(
        results,
        2,
        "Password policy does not specify a maximum password age"
      );
    } else if (passwordPolicy.MaxPasswordAge > config.max_password_age_fail) {
      helpers.addResult(
        results,
        2,
        `Maximum password age of: ${passwordPolicy.MaxPasswordAge} days is more than ${config.max_password_age_fail}`,
        "global",
        null,
        custom
      );
      ...
      helpers.addResult(
        results,
        1,
        `Maximum password age of: ${passwordPolicy.MaxPasswordAge} days is more than ${config.max_password_age_warn}`,
        "global",
        null,
        custom
      );
      ...
      helpers.addResult(
        results,
        0,
        `Maximum password age of: ${passwordPolicy.MaxPasswordAge} days is suitable`,
        "global",
        null,
        custom
      );
