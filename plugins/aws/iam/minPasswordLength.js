...
module.exports = {
  ...
  remediation_description:
    "The password policy for minimum password length will be set to the value set by the user. Otherwise, it will default to 14.",
  ...
  remediation_inputs: {
    minPasswordLength: {
      ...
      description:
        "The minimum password length to use in the account policy 6-128",
    ...
    minPasswordLengthCreatePolicy: {
      ...
      description:
        "Whether to create a new password policy if one does not already exist.",
      regex: "^(true|false)$",
      required: false,
    },
  },
  actions: {
    remediate: ["IAM:updateAccountPasswordPolicy"],
    rollback: ["IAM:updateAccountPasswordPolicy"],
  },
  permissions: {
    remediate: ["iam:UpdateAccountPasswordPolicy"],
    rollback: ["iam:UpdateAccountPasswordPolicy"],
  ...
  compliance: {
    pci:
      "PCI requires that passwords have a minimum length of at least 7 characters. " +
  ...
  settings: {
    min_password_length_fail: {
      ...
      description:
        "Return a failing result when min password length is fewer than this number of characters",
      regex: "^[1-9]{1}[0-9]{0,2}$",
      default: 10,
    },
    min_password_length_warn: {
      ...
      description:
        "Return a warning result when min password length is fewer than this number of characters",
      regex: "^[1-9]{1}[0-9]{0,2}$",
      default: 14,
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      min_password_length_fail:
        settings.min_password_length_fail ||
        this.settings.min_password_length_fail.default,
      min_password_length_warn:
        settings.min_password_length_warn ||
        this.settings.min_password_length_warn.default,
    };
    ...
    if (getAccountPasswordPolicy.err || !getAccountPasswordPolicy.data) {
      helpers.addResult(
        ...
        "Unable to query for password policy status: " +
          helpers.addError(getAccountPasswordPolicy)
      );
      return callback(null, results, source);
    }
    ...
    if (!passwordPolicy.MinimumPasswordLength) {
      helpers.addResult(
        results,
        2,
        "Password policy does not specify a minimum password length"
      );
    } else if (
      passwordPolicy.MinimumPasswordLength < config.min_password_length_fail
    ) {
      helpers.addResult(
        results,
        2,
        `Minimum password length of: ${passwordPolicy.MinimumPasswordLength} is less than ${config.min_password_length_fail} characters`,
        "global",
        null,
        custom
      );
    } else if (
      passwordPolicy.MinimumPasswordLength < config.min_password_length_warn
    ) {
      helpers.addResult(
        results,
        1,
        `Minimum password length of: ${passwordPolicy.MinimumPasswordLength} is less than ${config.min_password_length_warn} characters`,
        "global",
        null,
        custom
      );
      ...
      helpers.addResult(
        results,
        0,
        `Minimum password length of: ${passwordPolicy.MinimumPasswordLength} is suitable`,
        "global",
        null,
        custom
      );
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    helpers.remediatePasswordPolicy(
      putCall,
      pluginName,
      remediation_file,
      passwordKey,
