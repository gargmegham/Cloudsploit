      ...
      function (region, rcb) {
        ...
        if (listBackupVaults.err || !listBackupVaults.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for Backup vault list: ${helpers.addError(
              listBackupVaults
            )}`,
            region
          );
        ...
        for (let vault of listBackupVaults.data) {
          ...
          if (
            getBackupVaultNotifications &&
            getBackupVaultNotifications.err &&
            getBackupVaultNotifications.err.message &&
            getBackupVaultNotifications.err.message
              .toUpperCase()
              .includes("FAILED READING NOTIFICATIONS FROM DATABASE")
          ) {
            helpers.addResult(
              results,
              2,
          ...
          if (
            !getBackupVaultNotifications ||
            getBackupVaultNotifications.err ||
            !getBackupVaultNotifications.data ||
            !getBackupVaultNotifications.data.BackupVaultEvents
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to get event notifications for Backup vault: ${helpers.addError(
                getBackupVaultNotifications
              )}`,
              region,
              resource
          ...
          if (
            getBackupVaultNotifications.data.BackupVaultEvents.find(
              (notification) =>
                notification &&
                notification.toUpperCase() == "BACKUP_JOB_FAILED"
            )
          ) {
            helpers.addResult(
              results,
