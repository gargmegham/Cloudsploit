  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.backup_vault_desired_encryption_level ||
        this.settings.backup_vault_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listBackupVaults.err || !listBackupVaults.data) {
          helpers.addResult(
            results,
            3,
            `Unable to list Backup vaults: ${helpers.addError(
              listBackupVaults
            )}`,
            region
          );
        ...
        for (let backupVault of listBackupVaults.data) {
          ...
          if (backupVault.EncryptionKeyArn) {
            var kmsKeyId = backupVault.EncryptionKeyArn.split("/")[1]
              ? backupVault.EncryptionKeyArn.split("/")[1]
              : backupVault.EncryptionKeyArn;
