...
const listBackupVaults = [
  {
    ...
    BackupVaultArn:
      "arn:aws:backup:us-east-1:000011112222:backup-vault:sadeed-vault",
  ...
  },
  {
    ...
    BackupVaultArn:
      "arn:aws:backup:us-east-1:000011112222:backup-vault:aws/efs/automatic-backup-vault",
    CreationDate: "2020-10-18T10:53:45.887000-07:00",
    EncryptionKeyArn:
      "arn:aws:kms:us-east-1:000011112222:key/f4942dd6-bce5-4213-bdd3-cc8ccd87dd89",
  },
];
...
const listKeys = [
  ...
  },
  {
    KeyId: "0604091b-8c1b-4a55-a844-8cc8ab1834d9",
    KeyArn:
      "arn:aws:kms:us-east-1:000011112222:key/f4942dd6-bce5-4213-bdd3-cc8ccd87dd890",
  },
];
...
const createCache = (
  ...
  var keyId =
    backupVault && backupVault.length
      ? backupVault[0].EncryptionKeyArn.split("/")[1]
      : null;
  return {
    backup: {
      listBackupVaults: {
...
describe("backupVaultEncrypted", function () {
  describe("run", function () {
    it("should PASS if Backup Vault is encrypted with desired encryption level", function (done) {
      ...
      backupVaultEncrypted.run(
        cache,
        { backup_vault_desired_encryption_level: "awscmk" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          expect(results[0].message).to.include(
            "Backup vault is encrypted with awscmk"
          );
    ...
    it("should FAIL if Backup Vault is not encrypted with desired encyption level", function (done) {
      ...
      backupVaultEncrypted.run(
        ...
        (err, results) => {
          ...
          expect(results[0].message).to.include(
            "Backup vault is encrypted with awskms"
          );
          expect(results[0].region).to.equal("us-east-1");
          done();
        }
    ...
    it("should UNKNOWN if unable to list Backup vault", function (done) {
      const cache = createCache(null, null, null, {
        message: "Unable to list Backup vault encryption",
      });
      backupVaultEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
