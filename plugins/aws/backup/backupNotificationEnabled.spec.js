...
const createCache = (
  ...
  listBackupVaultsErr,
  getBackupVaultNotificationsErr
) => {
  let name =
  ...
  describe("run", function () {
    ...
    it("should FAIL if Backup vault does not have any notifications configured", function (done) {
      const cache = createCache([listBackupVaults[0]], null, null, {
        message: "Failed reading notifications from database",
      });
      backupNotificationEnabled.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(2);
        expect(results[0].region).to.equal("us-east-1");
        expect(results[0].message).to.include(
          "Backup vault does not have any notifications configured"
        );
        done();
      });
    });
    ...
    it("should UNKNOWN if Unable to get event notifications for selected Amazon Backup vault", function (done) {
      const cache = createCache([listBackupVaults[0]], null, null, {
        message:
          "Unable to get event notifications for selected Amazon Backup vault",
      });
      backupNotificationEnabled.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(3);
        expect(results[0].message).to.include(
          "Unable to get event notifications for Backup vault"
        );
        done();
      });
    });
