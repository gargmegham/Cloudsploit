...
module.exports = {
  ...
  settings: {
    backup_resource_type: {
      ...
      description:
        "Comma separated list of resource types that should be backup protected i.e. rds,efs",
      regex: "^.*$",
      default: "",
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      backup_resource_type:
        settings.backup_resource_type ||
        this.settings.backup_resource_type.default,
    };

    config.backup_resource_type = config.backup_resource_type.replace(
      /\s/g,
      ""
    );

    if (!config.backup_resource_type.length)
      return callback(null, results, source);

    config.backup_resource_type = config.backup_resource_type
      .toLowerCase()
      .split(",");
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        if (
          !describeRegionSettings ||
          describeRegionSettings.err ||
          !describeRegionSettings.data
        ) {
          helpers.addResult(
            results,
            3,
            `Unable to query for Backup resource type opt in preference: ${helpers.addError(
              describeRegionSettings
            )}`,
            region
          );
        ...
        if (!describeRegionSettings.data) {
          helpers.addResult(
            results,
            0,
            "No Backup region settings found",
            region
          );
          return rcb();
        }

        let loweredResourceTypes = Object.keys(
          describeRegionSettings.data
        ).reduce((acc, key) => {
          acc[key.toLowerCase().replace(/\s/g, "")] =
            describeRegionSettings.data[key];
          return acc;
        }, {});
        ...
        if (!missingResourceTypes.length) {
          helpers.addResult(
            results,
            0,
            "All desired resource types are protected by Backup service",
            region
          );
        } else {
          helpers.addResult(
            results,
            2,
            "These desired resource types are not protected by Backup service: " +
              missingResourceTypes.join(", "),
            region
          );
        }
