    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        for (let vault of listBackupVaults.data) {
          ...
          if (
            getBackupVaultAccessPolicy &&
            getBackupVaultAccessPolicy.err &&
            getBackupVaultAccessPolicy.err.code &&
          ...
          if (
            !getBackupVaultAccessPolicy ||
            getBackupVaultAccessPolicy.err ||
            !getBackupVaultAccessPolicy.data
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to get Backup vault access policy: ${helpers.addError(
                getBackupVaultAccessPolicy
              )}`,
              region,
              resource
          ...
          let statements = helpers.normalizePolicyDocument(
            getBackupVaultAccessPolicy.data.Policy
          );
          ...
          for (let statement of statements) {
            if (
              ...
              statement.Action &&
              statement.Action.find((action) =>
                action.toUpperCase().includes("BACKUP:DELETERECOVERYPOINT")
              )
            ) {
