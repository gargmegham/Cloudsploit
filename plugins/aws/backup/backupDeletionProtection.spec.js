...
const listBackupVaults = [
  {
    ...
    BackupVaultArn:
      "arn:aws:backup:us-east-1:000011112222:backup-vault:Default",
    CreationDate: "2021-11-26T17:05:36.477000+05:00",
    EncryptionKeyArn:
      "arn:aws:kms:us-east-1:000011112222:key/228d6374-d201-428d-b084-842fc7b2d148",
  ...
  },
  {
    ...
    BackupVaultArn:
      "arn:aws:backup:us-east-1:000011112222:backup-vault:sadeed1",
    CreationDate: "2022-01-21T23:05:24.095000+05:00",
    EncryptionKeyArn:
      "arn:aws:kms:us-east-1:000011112222:key/ad013a33-b01d-4d88-ac97-127399c18b3e",
...
const getBackupVaultAccessPolicy = [
  {
    ...
    BackupVaultArn:
      "arn:aws:backup:us-east-1:000011112222:backup-vault:sadeed2",
    Policy: {
      Version: "2012-10-17",
      Statement: [
        {
      ...
      Statement: [
        {
          ...
          Condition: {
            "ForAnyValue:StringLike": {
              "aws:PrincipalOrgPaths": "o-lcjto3x5wd/r-z1be/ou-[OU]/*",
            },
          },
        },
      ],
...
const createCache = (
  ...
  listBackupVaultsErr,
  getBackupVaultAccessPolicyErr
) => {
  let name =
    listBackupVaults && listBackupVaults.length
      ? listBackupVaults[0].BackupVaultName
      : null;
  return {
    backup: {
      listBackupVaults: {
  ...
  describe("run", function () {
    it("should PASS if Backup vault has deletion protection enabled", function (done) {
      ...
      backupDeletionProtection.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Backup vault has deletion protection enabled"
        );
        done();
      });
    });
    ...
    it("should FAIL if Backup vault does not have deletion protection enabled", function (done) {
      ...
      backupDeletionProtection.run(cache, {}, (err, results) => {
        ...
        expect(results[0].region).to.equal("us-east-1");
        expect(results[0].message).to.include(
          "Backup vault does not have deletion protection enabled"
        );
        done();
      });
    });
    ...
    it("should FAIL if no access policy found for Backup vault", function (done) {
      const cache = createCache([listBackupVaults[0]], null, null, {
        message:
          "An error occurred (ResourceNotFoundException) when calling the GetBackupVaultAccessPolicy operation",
      ...
      backupDeletionProtection.run(cache, {}, (err, results) => {
        ...
        expect(results[0].region).to.equal("us-east-1");
        expect(results[0].message).to.include(
          "No access policy found for Backup vault"
        );
        done();
      });
    });
    ...
    it("should UNKNOWN if Unable to query for Backup vault list", function (done) {
      const cache = createCache(null, {
        message: "Unable to query for Backup vault list",
      });
      backupDeletionProtection.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(3);
        expect(results[0].message).to.include(
          "Unable to query for Backup vault list"
        );
        done();
      });
    });
    ...
    it("should UNKNOWN if Unable to get Backup vault policy", function (done) {
      const cache = createCache([listBackupVaults[0]], null, null, {
        message: "Unable to get Backup vault policy",
      });
      backupDeletionProtection.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(3);
        expect(results[0].message).to.include(
          "Unable to get Backup vault policy"
        );
        done();
      });
    });
