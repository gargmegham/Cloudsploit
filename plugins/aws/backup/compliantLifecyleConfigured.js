      ...
      function (region, rcb) {
        ...
        if (listBackupPlans.err || !listBackupPlans.data) {
          helpers.addResult(
            results,
            3,
            "Unable to list Backup plans: " + helpers.addError(listBackupPlans),
            region
          );
          return rcb();
        }
        ...
        for (let plan of listBackupPlans.data) {
          ...
          if (!getBackupPlan || getBackupPlan.err || !getBackupPlan.data) {
            helpers.addResult(
              results,
              3,
              `Unable to get Backup plan description: ${helpers.addError(
                getBackupPlan
              )}`,
              region,
              resource
          ...
          if (
            !getBackupPlan.data.BackupPlan ||
            !getBackupPlan.data.BackupPlan.Rules
          ) {
            helpers.addResult(
              results,
              2,
              "No lifecycle configuration rules found for Backup plan",
              region,
              resource
            );
            continue;
          ...
          let found = getBackupPlan.data.BackupPlan.Rules.find(
            (rule) =>
              rule.Lifecycle &&
              rule.Lifecycle.DeleteAfterDays &&
          ...
          if (found) {
            helpers.addResult(
              results,
              0,
              "Backup plan has lifecycle configuration enabled",
              region,
              resource
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Backup plan does not have lifecycle configuration enabled",
              region,
              resource
            );
          }
