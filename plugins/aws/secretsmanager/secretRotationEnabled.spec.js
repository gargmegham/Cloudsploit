...
const listSecrets = [
  {
    ...
    KmsKeyId:
      "arn:aws:kms:us-east-1:111122223333:key/995d1b05-5f9c-4610-aae7-f8505f4458f5",
    RotationEnabled: true,
    RotationLambdaARN:
      "arn:aws:lambda:us-east-1:111122223333:function:test-lambda",
...
describe("secretRotationEnabled", function () {
  describe("run", function () {
    it("should PASS if rotation is enabled for Secrets Manager secret", function (done) {
      ...
      secretRotationEnabled.run(
        cache,
        { secretsmanager_secret_rotation_interval: 25 },
        (err, results) => {
    ...
    it("should FAIL if rotation is enabled for Secrets Manager secret but set rotation interval is less than desired interval", function (done) {
      ...
      secretRotationEnabled.run(
        cache,
        { secretsmanager_secret_rotation_interval: 40 },
        (err, results) => {
          expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if Unable to query for secrets", function (done) {
      const cache = createCache([], null, {
        message: "Unable to query secrets",
      });
      secretRotationEnabled.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if Unable to query Secrets Manager secret", function (done) {
      const cache = createCache([listSecrets[0]], {}, null, {
        message: "Unable to query Secrets Manager secret",
      });
      secretRotationEnabled.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(3);
