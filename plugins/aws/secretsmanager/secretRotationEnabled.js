...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      secretsmanager_secret_rotation_interval: parseInt(
        settings.secretsmanager_secret_rotation_interval ||
          this.settings.secretsmanager_secret_rotation_interval.default
      ),
    };
      ...
      (region, rcb) => {
        ...
        if (!listSecrets.data || listSecrets.err) {
          helpers.addResult(
            results,
            3,
            `Unable to query for Secrets Manager secrets: ${helpers.addError(
              listSecrets
            )}`,
            region
          );
          ...
          (secret, scb) => {
            ...
            if (!describeSecret || describeSecret.err || !describeSecret.data) {
              helpers.addResult(
                results,
                3,
                `Unable to query Secrets Manager secret: ${helpers.addError(
                  describeSecret
                )}`,
                region,
                resource
            ...
            if (
              describeSecret.data.RotationEnabled &&
              ...
              if (
                rotationInterval >=
                config.secretsmanager_secret_rotation_interval
              ) {
                helpers.addResult(
                  results,
                  0,
              ...
              helpers.addResult(
                results,
                2,
                "Rotation is not enabled for Secrets Manager secret",
                region,
                resource
              );
            }
