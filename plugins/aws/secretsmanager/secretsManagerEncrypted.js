...
module.exports = {
  ...
  compliance: {
    ...
    pci:
      "PCI requires proper encryption of cardholder data at rest. Secrets Manager " +
  ...
  run: function (cache, settings, callback) {
    ...
    var desiredEncryptionLevelString =
      settings.secretsmanager_minimum_encryption_level ||
      this.settings.secretsmanager_minimum_encryption_level.default;
    var desiredEncryptionLevel = helpers.ENCRYPTION_LEVELS.indexOf(
      desiredEncryptionLevelString
    );

    async.each(
      regions.secretsmanager,
      (region, rcb) => {
        var listSecrets = helpers.addSource(cache, source, [
          "secretsmanager",
          "listSecrets",
          region,
        ]);
        ...
        if (!listSecrets.data || listSecrets.err) {
          helpers.addResult(
            results,
            3,
            `Unable to query for secrets: ${helpers.addError(listSecrets)}`,
            region
          );
          return rcb();
        }
        ...
        for (let secret of listSecrets.data) {
          ...
          if (encryptionLevel < desiredEncryptionLevel) {
            helpers.addResult(
              results,
              2,
              `Secret configured to use ${encryptionLevelString} instead of ${desiredEncryptionLevelString}`,
              region,
              secret.ARN
            );
          } else {
            helpers.addResult(
              results,
              0,
              `Secret configured to use desired encryption ${encryptionLevelString}`,
              region,
              secret.ARN
            );
          }
