    ...
    item: [
      {
        ...
        webAclArn:
          "arn:aws:wafv2:us-east-1:111122223333:regional/webacl/test-waf/ca44237b-b1d8-46b2-abad-ada48c7f0894",
      },
    ],
  },
  {
...
const createCache = (apis, stages, cert) => {
  ...
  var clientCertificateId =
    stages &&
    stages.item &&
    stages.item.length &&
    stages.item[0].clientCertificateId
      ? stages.item[0].clientCertificateId
      : null;
    ...
    apigateway: {
      ...
      getClientCertificate: {
        "us-east-1": {
          err: {
            message:
              "error getting API Gateway Rest API Stage client certificate",
          },
        },
      },
    },
...
describe("apigatewayCertificateRotation", function () {
  describe("run", function () {
    it("should PASS if API Gateway API stages do not need client certificate rotation", function (done) {
      ...
      apigatewayCertificateRotation.run(
        cache,
        { api_certificate_rotation_limit: "20" },
        (err, results) => {
    ...
    it("should FAIL if API Gateway API stage needs client certificate rotation", function (done) {
      ...
      apigatewayCertificateRotation.run(
        cache,
        { api_certificate_rotation_limit: "40" },
        (err, results) => {
          expect(results.length).to.equal(1);
