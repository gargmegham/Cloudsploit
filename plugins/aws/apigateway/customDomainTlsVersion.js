      ...
      function (region, rcb) {
        ...
        if (getDomainNames.err || !getDomainNames.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for API Gateway Custom Domain: ${helpers.addError(
              getDomainNames
            )}`,
            region
          );
        ...
        if (!getDomainNames.data.length) {
          helpers.addResult(
            results,
            0,
            "No API Gateway Custom Domains found",
            region
          );
          return rcb();
        }
        for (let domain of getDomainNames.data) {
          ...
          if (domain.securityPolicy && domain.securityPolicy === "TLS_1_2") {
            helpers.addResult(
              results,
              0,
              `API Gateway Custom Domain is using current minimum TLS version ${domain.securityPolicy}`,
              region,
              domainArn
            );
          } else {
            helpers.addResult(
              results,
              2,
              `API Gateway Custom Domain is using deprecated TLS version ${domain.securityPolicy}`,
              region,
              domainArn
            );
          }
