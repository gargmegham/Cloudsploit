...
const getRestApis = [
  {
    ...
    tags: {
      ...
      "aws:cloudformation:stack-id":
        "arn:aws:cloudformation:eu-west-1:232489870748:stack/FoodLeaStack/0cffbdd0-9636-11ec-be95-062821a5a16f",
  ...
  describe("run", function () {
    ...
    it("should PASS if Response caching is not enabled for the API stage", function (done) {
      ...
      apiStageLevelCacheEncryption.run(cache, {}, (err, results) => {
        ...
        expect(results[0].region).to.equal("us-east-1");
        expect(results[0].message).to.include(
          "Response caching is not enabled for the API stage"
        );
        done();
      });
    });
    ...
    it("should UNKNOWN if unable to get API Gateway Rest APIs", function (done) {
      ...
      apiStageLevelCacheEncryption.run(cache, {}, (err, results) => {
        ...
        expect(results[0].region).to.equal("us-east-1");
        expect(results[0].message).to.include(
          "Unable to query for API Gateway rest APIs"
        );
        done();
      });
    });
    ...
    it("should UNKNOWN if unable to get API Gateway Stages", function (done) {
      ...
      apiStageLevelCacheEncryption.run(cache, {}, (err, results) => {
        ...
        expect(results[0].region).to.equal("us-east-1");
        expect(results[0].message).to.include(
          "Unable to query API Gateway stages"
        );
        done();
      });
    });
