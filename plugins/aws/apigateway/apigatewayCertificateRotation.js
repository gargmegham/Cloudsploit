...
module.exports = {
  ...
  settings: {
    api_certificate_rotation_limit: {
      ...
      description:
        "Number of days before expiration date when certificate should be rotated",
      regex: "^[0-9]*$",
      default: "30",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var api_certificate_rotation_limit = parseInt(
      settings.api_certificate_rotation_limit ||
        this.settings.api_certificate_rotation_limit.default
    );
      ...
      function (region, rcb) {
        ...
        if (getRestApis.err || !getRestApis.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for API Gateway Rest APIs: ${helpers.addError(
              getRestApis
            )}`,
            region
          );
          ...
          function (api, acb) {
            ...
            if (
              ...
              helpers.addResult(
                results,
                3,
                `Unable to query for API Gateway Rest API Stages: ${helpers.addError(
                  getStages
                )}`,
                region,
                apiArn
            ...
            getStages.data.item.forEach((stage) => {
              ...
              if (
                !getClientCertificate ||
                getClientCertificate.err ||
                !getClientCertificate.data
              ) {
                helpers.addResult(
                  results,
                  3,
                  `Unable to query for API Gateway Rest API Stage Client Certificate: ${helpers.addError(
                    getClientCertificate
                  )}`,
                  region,
                  stageArn
              ...
              var difference = Math.round(
                (new Date(then).getTime() - new Date().getTime()) /
                  (24 * 60 * 60 * 1000)
              );
              ...
              if (difference > api_certificate_rotation_limit) {
                ...
                helpers.addResult(
                  results,
                  2,
                  `API Gateway API stage client certificate needs rotation as it expired ${Math.abs(
                    difference
                  )} days ago`,
                  region,
                  stageArn
