  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        if (
          describeReservedCacheNodes.err ||
          !describeReservedCacheNodes.data
        ) {
          helpers.addResult(
            results,
            3,
            "Unable to query for ElastiCache Reserved Cache Node: " +
              helpers.addError(describeReservedCacheNodes),
            region
          );
          return rcb();
        }
        ...
        if (!describeReservedCacheNodes.data.length) {
          helpers.addResult(
            results,
            0,
            "No ElastiCache reserved cache nodes found",
            region
          );
          return rcb();
        }
        ...
        for (var cluster of describeReservedCacheNodes.data) {
          ...
          if (expirationDays >= 30) {
            helpers.addResult(
              results,
              0,
              "ElastiCache reserved cache node lease expires in " +
                expirationDays +
                " days",
              region,
              resource
            ...
            helpers.addResult(
              results,
              2,
              "ElastiCache reserved cache node lease has expired",
              region,
              resource
            );
          }
