...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      elasticache_nodes_count_per_region: parseInt(
        settings.elasticache_nodes_count_per_region ||
          this.settings.elasticache_nodes_count_per_region.default
      ),
      elasticache_nodes_count_global: parseInt(
        settings.elasticache_nodes_count_global ||
          this.settings.elasticache_nodes_count_global.default
      ),
    };
      ...
      function (region, rcb) {
        ...
        if (describeCacheClusters.err || !describeCacheClusters.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for ElastiCache clusters: " +
              helpers.addError(describeCacheClusters),
            region
          );
          return rcb();
        }
        ...
        if (nodesCount <= config.elasticache_nodes_count_per_region) {
          helpers.addResult(
            results,
            0,
            `Region contains "${nodesCount}" provisioned ElastiCache nodes of "${config.elasticache_nodes_count_per_region}" limit`,
            region
          );
        } else {
          helpers.addResult(
      ...
      function () {
        if (globalCount <= config.elasticache_nodes_count_global) {
          helpers.addResult(
            results,
            0,
            `Region contains "${globalCount}" provisioned ElastiCache nodes of "${config.elasticache_nodes_count_global}" limit`,
            "global"
          );
        } else {
          helpers.addResult(
