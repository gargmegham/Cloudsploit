      ...
      function (region, rcb) {
        ...
        if (describeCacheClusters.err || !describeCacheClusters.data) {
          helpers.addResult(
            results,
            3,
            "Unable to describe cache clusters: " +
        ...
        for (var cluster of describeCacheClusters.data) {
          ...
          var defaultPort = defaultPorts.filter((d) => {
            return (
              d.engine == cluster.Engine &&
              d.port == cluster.ConfigurationEndpoint.Port
            );
          });
          ...
          if (defaultPort && defaultPort.length) {
            helpers.addResult(
              ...
              "The " +
                cluster.Engine +
                " cluster is configured with default port " +
                cluster.ConfigurationEndpoint.Port,
              region,
              cluster.ARN
            );
            ...
            helpers.addResult(
              ...
              "The " +
                cluster.Engine +
                " cluster is configured with a non default port " +
                cluster.ConfigurationEndpoint.Port,
              region,
              cluster.ARN
            );
