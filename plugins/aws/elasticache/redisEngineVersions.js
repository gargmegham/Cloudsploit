...
module.exports = {
  ...
  more_info:
    "ElastiCache clusters with the latest version of Redis cache engine, You will benefit from new features and enhancements, " +
      ...
      function (region, rcb) {
        ...
        for (var cluster of describeCacheClusters.data) {
          ...
          if (cluster.EngineVersion) {
            ...
            let versionDeprecationType = deprecatedVersions[version]
              ? deprecatedVersions[version]
              : null;
            let versionSpecifiedType = nonRecommendedVersions[version]
              ? nonRecommendedVersions[version]
              : null;
            ...
            if (versionDeprecationType) {
              helpers.addResult(
                results,
                2,
                "ElastiCache redis cluster is using " +
                  version +
                  " engine version which is deprecated",
                region,
                resource
              );
              ...
              helpers.addResult(
                ...
                "ElastiCache redis cluster is using " +
                  version +
                  " engine version which is not recommended",
                region,
                resource
              );
            } else {
            ...
            helpers.addResult(
              results,
              2,
              "ElastiCache redis cluster is using unknown engine version",
              region,
              resource
            );
          }
