      ...
      function (region, rcb) {
        ...
        if (describeCacheClusters.err || !describeCacheClusters.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query elasticache clusters: " +
        ...
        for (let cluster of describeCacheClusters.data) {
          ...
          if (
            ...
            helpers.addResult(
              results,
              3,
              `Unable to get clusters description: ${helpers.addError(
                describeReplicationGroups
              )}`,
              region,
              resource
            ...
            if (
              ...
              describeReplicationGroups.data.ReplicationGroups.some(
                (group) =>
                  group.MultiAZ && group.MultiAZ.toLowerCase() === "enabled"
              )
            ) {
              helpers.addResult(
                results,
                0,
                "Cluster has Multi-AZ feature enabled",
                region,
                resource
              );
            } else {
              helpers.addResult(
                results,
                2,
                "Cluster does not have Multi-AZ feature enabled",
                region,
                resource
              );
            }
