...
module.exports = {
  ...
  apis: [
    ...
    "CloudWatch:getEcMetricStatistics",
  ],
  settings: {
    elasticache_idle_node_percentage: {
      name: "ElastiCache Idle Node Average Percentage",
      description:
        "A percentage value for cluster CPU utilization under which cluster is considered idle i.e. 2.50",
  ...
  run: function (cache, settings, callback) {
    ...
    var elasticache_idle_node_percentage =
      settings.elasticache_idle_node_percentage ||
      this.settings.elasticache_idle_node_percentage.default;

    if (!elasticache_idle_node_percentage.length)
      return callback(null, results, source);

    elasticache_idle_node_percentage = parseFloat(
      elasticache_idle_node_percentage
    );
      ...
      function (region, rcb) {
        ...
        if (describeCacheClusters.err || !describeCacheClusters.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for ElastiCache cluster: ${helpers.addError(
              describeCacheClusters
            )}`,
            region
          );
        ...
        describeCacheClusters.data.forEach((cluster) => {
          ...
          if (
            !getEcMetricStatistics ||
            getEcMetricStatistics.err ||
            !getEcMetricStatistics.data ||
            !getEcMetricStatistics.data.Datapoints
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to query for ElastiCache cluster metric stats: ${helpers.addError(
                getEcMetricStatistics
              )}`,
              region,
              resource
          ...
          if (!getEcMetricStatistics.data.Datapoints.length) {
            helpers.addResult(
              results,
              0,
              "ElastiCache cluster metric statistics are not configured",
              region,
              resource
            );
          } else {
            const isIdle = getEcMetricStatistics.data.Datapoints.every(
              (datapoint) =>
                datapoint.Average < elasticache_idle_node_percentage
            );
            ...
            helpers.addResult(
              results,
              status,
              `ElastiCache cluster is ${
                isIdle ? "idle since " + idleHours + " hours" : "not idle"
              }`,
              region,
              resource
            );
