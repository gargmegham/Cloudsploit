      ...
      function (region, rcb) {
        ...
        if (!describeReservedCacheNodes.data.length) {
          helpers.addResult(
            results,
            0,
            "No ElastiCache reserved cache node found",
            region
          );
          return rcb();
        }
        ...
        for (var cluster of describeReservedCacheNodes.data) {
          ...
          if (cluster.State === "payment-pending") {
            helpers.addResult(
              results,
              2,
              "ElastiCache reserved cache node have pending payment",
              region,
              resource
            );
          } else {
            helpers.addResult(
              results,
              0,
              "ElastiCache reserved cache node does not have pending payment",
              region,
              resource
            );
          }
