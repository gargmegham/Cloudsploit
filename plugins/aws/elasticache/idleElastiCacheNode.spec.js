  ...
  describe("run", function () {
    it("should PASS if metric count is greater than 5", function (done) {
      const cache = createCache(
        ...
        ecMetricStatistics[0]
      );
      idleElastiCacheNode.run(
        cache,
        { elasticache_idle_node_percentage: "5.00" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          expect(results[0].message).to.include(
            "ElastiCache cluster is not idle"
          );
          done();
        }
      );
    ...
    it("should UNKNOWN if unable to describe cache clusters", function (done) {
      ...
      idleElastiCacheNode.run(
        ...
        (err, results) => {
          ...
          expect(results[0].message).to.include(
            "Unable to query for ElastiCache cluster"
          );
          done();
        }
      );
