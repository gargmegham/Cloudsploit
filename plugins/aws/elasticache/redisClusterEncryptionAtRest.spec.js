...
const createCache = (
  ...
  var keyId =
    clusters && clusters.length && clusters[0].KmsKeyId
      ...
      : null;
  var ReplicationGroupId =
    clusters && clusters.length ? clusters[0].ReplicationGroupId : null;
  return {
    elasticache: {
...
describe("redisClusterEncryptionAtRest", function () {
  describe("run", function () {
    it("should PASS if Redis Cluster at-rest is encrypted with desired encryption level", function (done) {
      ...
      redisClusterEncryptionAtRest.run(
        cache,
        { ec_atrest_desired_encryption_level: "awscmk" },
        (err, results) => {
    ...
    it("should UNKNOWN if unable to list  Redis Cluster at-rest", function (done) {
      const cache = createCache(null, null, null, {
        message: "Unable to list AppFlow flows",
      });
      redisClusterEncryptionAtRest.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if unable to list KMS keys", function (done) {
      const cache = createCache(describeCacheClusters, null, null, null, {
