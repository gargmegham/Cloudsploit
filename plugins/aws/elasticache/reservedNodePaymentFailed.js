      ...
      function (region, rcb) {
        ...
        if (
          ...
          helpers.addResult(
            results,
            3,
            "Unable to query for ElastiCache reserved cache nodes: " +
        ...
        for (var cluster of describeReservedCacheNodes.data) {
          ...
          if (cluster.State === "payment-failed") {
            helpers.addResult(
              results,
              2,
              "ElastiCache reserved cache node have payment failure",
              region,
              resource
            );
          } else {
            helpers.addResult(
              results,
              0,
              "ElastiCache reserved cache node does not have payment failure",
              region,
              resource
            );
          }
