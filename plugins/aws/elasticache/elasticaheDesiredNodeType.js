...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      elasticache_desired_node_type:
        settings.elasticache_desired_node_type ||
        this.settings.elasticache_desired_node_type.default,
    };

    if (!config.elasticache_desired_node_type.length)
      ...
      function (region, rcb) {
        ...
        if (describeCacheClusters.err || !describeCacheClusters.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query ElastiCache cluster: " +
