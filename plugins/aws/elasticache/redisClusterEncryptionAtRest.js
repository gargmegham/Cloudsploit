  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.ec_cluster_target_encryption_level ||
        this.settings.ec_cluster_target_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (describeCacheClusters.err || !describeCacheClusters.data) {
          helpers.addResult(
            results,
            3,
            `Unable to list ElastiCache clusters : ${helpers.addError(
              describeCacheClusters
            )}`,
            region
          );
        ...
        for (let cluster of describeCacheClusters.data) {
          ...
          if (cluster.Engine !== "redis") {
            helpers.addResult(
              results,
              0,
              `Encryption is not supported for ${cluster.Engine}`,
              region,
              resource
            );
            continue;
          ...
          if (cluster.AtRestEncryptionEnabled) {
            ...
            if (
              !describeReplicationGroups ||
              describeReplicationGroups.err ||
              !describeReplicationGroups.data ||
              !describeReplicationGroups.data.ReplicationGroups ||
              !describeReplicationGroups.data.ReplicationGroups.length
            ) {
              helpers.addResult(
                results,
                3,
                `Unable to describe replication groups for cluster: ${helpers.addError(
                  describeReplicationGroups
                )}`,
                region,
                resource
            ...
            if (describeReplicationGroups.data.ReplicationGroups[0].KmsKeyId) {
              var kmsKeyId =
                describeReplicationGroups.data.ReplicationGroups[0].KmsKeyId.split(
                  "/"
                )[1]
                  ? describeReplicationGroups.data.ReplicationGroups[0].KmsKeyId.split(
                      "/"
                    )[1]
                  : describeReplicationGroups.data.ReplicationGroups[0]
                      .KmsKeyId;
            ...
            helpers.addResult(
              results,
              2,
              "Cluster does not have at-rest encryption enabled",
              region,
              resource
            );
          }
