...
module.exports = {
      ...
      function (region, rcb) {
        ...
        if (listTables.err || !listTables.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for DynamoDB tables: " +
              helpers.addError(listTables),
            region
          );
          return rcb();
        }
        ...
        for (var i in listTables.data) {
          ...
          if (
            ...
            helpers.addResult(
              ...
              "Unable to describe DynamoDB table: " +
                helpers.addError(describeTable),
              region,
              resource
            );
            return rcb();
          ...
          if (
            describeTable.data.Table.SSEDescription &&
            describeTable.data.Table.SSEDescription.Status &&
            describeTable.data.Table.SSEDescription.Status.toUpperCase() ===
              "ENABLED"
          ) {
            helpers.addResult(
              results,
              0,
              "Table encryption is enabled with a KMS master key",
              region,
              resource
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Table is using default encryption with AWS-owned key",
              region,
              resource
            );
          }
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    let defaultKeyDesc =
      "Default master key that protects my DynamoDB data when no other key is defined";
    ...
    if (settings.input && settings.input.kmsKeyIdforDynamo) {
      ...
      if (!defaultKmsKeyId)
        return callback(
          `No default DynamoDB key for the region ${config.region}`
        );
      params = {
        TableName: tableName,
        SSESpecification: {
