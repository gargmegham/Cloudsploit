...
const listTables = ["mine1", "sadeedTable"];
    ...
    BackupSummaries: [
      {
        ...
        BackupArn:
          "arn:aws:dynamodb:us-east-1:000011112222:table/sadeedTable/backup/01646839091311-17886cd9",
        BackupName: "backup1",
  ...
  describe("run", function () {
    it("should FAIL if no backup exists for DynamoDB table", function (done) {
      ...
      dynamoTableBackupExists.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "No backup exists for DynamoDB table"
        );
        done();
      });
    });
    ...
    it("should PASS if backup exists for DynamoDB table", function (done) {
      ...
      dynamoTableBackupExists.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Backup exists for DynamoDB table"
        );
        done();
      });
    });
