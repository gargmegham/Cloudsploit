    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        let findings = getRecommendationSummaries.data.find(
          (resourceType) =>
            resourceType.recommendationResourceType === "EbsVolume"
        );
        if (findings) {
          ...
          if (!notOptimized.value && !optimized.value) {
            helpers.addResult(
              results,
              0,
              "No recommendations found for EBS volumes",
              region
            );
            ...
            helpers.addResult(
              results,
              2,
              `Found ${notOptimized.value} unoptimized EBS volumes`,
              region
            );
          } else {
            helpers.addResult(
          ...
          helpers.addResult(
            results,
            2,
            "Recommendation summaries are not configured for EBS Volumes",
            region
          );
        }
