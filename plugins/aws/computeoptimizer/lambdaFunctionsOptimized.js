      ...
      function (region, rcb) {
        ...
        if (findings) {
          let notOptimized = findings.summaries.find(
            (notOpt) => notOpt.name === "NotOptimized"
          );
          ...
          if (!notOptimized.value && !Optimized.value) {
            helpers.addResult(
              results,
              0,
              "No recommendations found for Lambda functions",
              region
            );
            ...
            helpers.addResult(
              results,
              2,
              `Found ${notOptimized.value} unoptimized Lambda functions`,
              region
            );
          } else {
            helpers.addResult(
              results,
              0,
              "All Lambda Functions are optimized",
              region
            );
          }
        } else {
          helpers.addResult(
            results,
            2,
            "Recommendation summaries are not configured for Lambda functions",
            region
          );
        }
