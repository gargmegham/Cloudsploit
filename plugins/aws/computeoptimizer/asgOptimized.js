  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        if (findings) {
          let notOptimized = findings.summaries.find(
            (notOpt) =>
              notOpt.name && notOpt.name.toUpperCase() === "NOT_OPTIMIZED"
          );
          let Optimized = findings.summaries.find(
            (opt) => opt.name && opt.name.toUpperCase() === "OPTIMIZED"
          );
          ...
          if (!notOptimized.value && !Optimized.value) {
            helpers.addResult(
              results,
              0,
              "No recommendations found for Auto Scaling groups",
              region
            );
            ...
            helpers.addResult(
              results,
              2,
              `Found ${notOptimized.value} unoptimized Auto Scaling groups`,
              region
            );
          } else {
            helpers.addResult(
              results,
              0,
              "All Auto Scaling groups are optimized",
              region
            );
          }
        } else {
          helpers.addResult(
            results,
            2,
            "Recommendation summaries are not configured for Auto Scaling groups",
            region
          );
        }
