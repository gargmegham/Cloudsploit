  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        if (
          getRecommendationSummaries &&
          getRecommendationSummaries.err &&
          getRecommendationSummaries.err.code &&
          getRecommendationSummaries.err.code.toUpperCase() ===
        ...
        if (
          getRecommendationSummaries.err ||
          !getRecommendationSummaries.data
        ) {
          helpers.addResult(
            results,
            3,
            "Unable to get recommendation summaries: " +
              helpers.addError(getRecommendationSummaries),
            region
          );
          return rcb();
        }
        ...
        if (!getRecommendationSummaries.data.length) {
          helpers.addResult(
            results,
            0,
            "No Compute Optimizer recommendation summaries found",
            region
          );
          return rcb();
        }

        let findings = getRecommendationSummaries.data.find(
          (resourceType) =>
            resourceType.recommendationResourceType &&
            resourceType.recommendationResourceType.toUpperCase() ===
              "EC2INSTANCE"
        );
        if (findings) {
          let underProvisioned = findings.summaries.find(
            (underProv) => underProv.name === "UNDER_PROVISIONED"
          );
          let optimized = findings.summaries.find(
            (opt) => opt.name === "OPTIMIZED"
          );
          let overProvisioned = findings.summaries.find(
            (overProv) => overProv.name === "OVER_PROVISIONED"
          );

          if (
            !underProvisioned.value &&
            ...
            helpers.addResult(
              results,
              0,
              "EC2 instances have no recommendations enabled",
              region
            );
            ...
            helpers.addResult(
              results,
              2,
              `Found ${underProvisioned.value} under-provisioned and ${overProvisioned.value} over-provisioned EC2 instances`,
              region
            );
          } else {
            helpers.addResult(
              results,
              0,
              "All EC2 instances are optimized",
              region
            );
          }
        } else {
          helpers.addResult(
            results,
            2,
            "Recommendation summaries are not configured for EC2 instances",
            region
          );
        }
