...
const listClusters = [
  {
    ClusterArn:
      "arn:aws:kafka:us-east-1:000011112222:cluster/sadeedcluster/b08122a8-7104-476a-b6ee-c59444fb04d5-25",
    ...
    EncryptionInfo: {
      EncryptionAtRest: {
        DataVolumeKMSKeyId:
          "arn:aws:kms:us-east-1:000011112222:key/39009d78-b364-4a0b-937f-c89a2c2b473f",
      },
      EncryptionInTransit: {
        ClientBroker: "TLS",
        InCluster: true,
  ...
  describe("run", function () {
    it("should FAIL if Encryption between the client and broker is not only TLS encrypted", function (done) {
      ...
      mskClusterCBEncryption.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Encryption between the client and broker is not only TLS encrypted"
        );
        done();
      });
    });
    ...
    it("should PASS if Encryption between the client and broker is only TLS encrypted", function (done) {
      ...
      mskClusterCBEncryption.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Encryption between the client and broker is only TLS encrypted"
        );
        done();
      });
    });
    ...
    it("should UNKNOWN if Unable to query for MSK clusters", function (done) {
      ...
      mskClusterCBEncryption.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Unable to query for MSK clusters"
        );
        done();
      });
    });
