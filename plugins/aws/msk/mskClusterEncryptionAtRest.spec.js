...
const listClusters = [
  {
    BrokerNodeGroupInfo: {
      ...
      ClientSubnets: ["subnet-02ed4181800d4658b", "subnet-06629b4200870c740"],
      InstanceType: "kafka.m5.large",
      SecurityGroups: ["sg-0cb6c99daaa6b73c5"],
      StorageInfo: {
        EbsStorageInfo: {
          VolumeSize: 100,
        },
    ...
    ClusterArn:
      "arn:aws:kafka:us-east-1:000111222333:cluster/sadeed-cl1/444e81bf-14ab-4839-923e-ac424325e2df-20",
...
const createCache = (
  ...
  var keyId =
    clusters &&
    clusters.length &&
    clusters[0].EncryptionInfo.EncryptionAtRest.DataVolumeKMSKeyId
      ? clusters[0].EncryptionInfo.EncryptionAtRest.DataVolumeKMSKeyId.split(
          "/"
        )[1]
      : null;
  return {
  ...
  describe("run", function () {
    it("should PASS if MSK Cluster At-Rest is encrypted with desired encryption level", function (done) {
      ...
      mskClusterEncryptionAtRest.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "MSK cluster is encrypted with awscmk"
        );
    ...
    it("should FAIL if MSK Cluster At-Rest is not encrypted with desired encyption level", function (done) {
      ...
      mskClusterEncryptionAtRest.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "MSK cluster is encrypted with awskms"
        );
        expect(results[0].region).to.equal("us-east-1");
        done();
      });
    ...
    it("should UNKNOWN if unable to list MSK Clusters", function (done) {
      const cache = createCache(null, null, null, {
        message: "Unable to list  MSK clusters",
      });
      mskClusterEncryptionAtRest.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
