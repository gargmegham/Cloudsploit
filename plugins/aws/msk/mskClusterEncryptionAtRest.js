  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.msk_cluster_desired_encryption_level ||
        this.settings.msk_cluster_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listClusters.err || !listClusters.data) {
          helpers.addResult(
            results,
            3,
            `Unable to list MSK clusters : ${helpers.addError(listClusters)}`,
            region
          );
          return rcb();
        }
        ...
        for (let cluster of listClusters.data) {
          ...
          if (
            ...
            let dataVolumeKMSKeyId =
              cluster.EncryptionInfo.EncryptionAtRest.DataVolumeKMSKeyId;
            var keyId = dataVolumeKMSKeyId.split("/")[1]
              ? dataVolumeKMSKeyId.split("/")[1]
              : dataVolumeKMSKeyId;
