...
const listClusters = [
  {
    ...
    ClusterArn:
      "arn:aws:kafka:us-east-1:000011112222:cluster/myCluster/794ab280-627c-4705-aaab-9f5b944fb9e3-25",
  ...
  describe("run", function () {
    it("should FAIL if MSK cluster has public access enabled", function (done) {
      ...
      mskClusterPublicAccess.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "MSK cluster is publicly accessible"
        );
        done();
      });
    });
    ...
    it("should PASS if MSK cluster does not have public access enabled", function (done) {
      ...
      mskClusterPublicAccess.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "MSK cluster is not publicly accessible"
        );
        done();
      });
    });
