  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.fraud_detector_data_encryption_level ||
        this.settings.fraud_detector_data_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listDetectors.err || !listDetectors.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query Fraud Detectors: ${helpers.addError(
              listDetectors
            )}`,
            region
          );
        ...
        if (
          fraudDetectorsEncryptionKey.err ||
          !fraudDetectorsEncryptionKey.data
        ) {
          helpers.addResult(
            results,
            3,
            `Unable to query Fraud Detectors Key: ${helpers.addError(
              listDetectors
            )}`,
            region
          );
        ...
        if (
          fraudDetectorsEncryptionKey.data &&
          fraudDetectorsEncryptionKey.data.kmsEncryptionKeyArn &&
          fraudDetectorsEncryptionKey.data.kmsEncryptionKeyArn.toUpperCase() !==
            "DEFAULT"
        ) {
          let encryptionKey =
            fraudDetectorsEncryptionKey.data.kmsEncryptionKeyArn;
          var keyId = encryptionKey.split("/")[1]
            ? encryptionKey.split("/")[1]
            : encryptionKey;
