...
module.exports = {
  ...
  settings: {
    stack_failed_state_hours_limit: {
      ...
      description:
        "A value to be used as a maximum limit in hours in which a CloudFormation stack can stay in failed state",
      regex: "^[0-9]{1,4}$",
      default: 0,
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var stack_failed_state_hours_limit =
      "stack_failed_state_hours_limit" in settings
        ? parseInt(settings.stack_failed_state_hours_limit)
        : this.settings.stack_failed_state_hours_limit.default;

    async.each(
      regions.cloudformation,
      function (region, rcb) {
        ...
        if (listStacks.err || !listStacks.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for CloudFormation stacks: ${helpers.addError(
              listStacks
            )}`,
            region
          );
        ...
        async.each(
          ...
          function (stack, cb) {
            ...
            if (
              ...
              if (
                !describeStackEvents ||
                describeStackEvents.err ||
                !describeStackEvents.data
              ) {
                helpers.addResult(
                  results,
                  3,
                  `Unable to query for CloudFormation stack events: ${helpers.addError(
                    describeStackEvents
                  )}`,
                  region
                );
              ...
              if (
                !describeStackEvents.data.StackEvents ||
                !describeStackEvents.data.StackEvents.length
              ) {
                helpers.addResult(
                  results,
                  0,
                  "No CloudFormation stack events found",
                  region
                );
                return cb();
              }
