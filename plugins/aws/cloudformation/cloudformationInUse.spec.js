...
const describeStacks = [
  {
    ...
    StackName: "sam-app",
    ChangeSetId:
      "arn:aws:cloudformation:us-east-1:000011112222:changeSet/samcli-deploy1641479723/70df5351-0c36-4f77-bf41-fe5fe49c4a3a",
  ...
  describe("run", function () {
    ...
    it("should UNKNOWN if Unable to query CloudFormation stacks", function (done) {
      const cache = createCache(null, {
        message: "Unable to query CloudFormation stacks",
      });
      cloudformationInUse.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        ...
        expect(results[0].region).to.equal("us-east-1");
        expect(results[0].message).to.include(
          "Unable to query CloudFormation stacks"
        );
        done();
      });
    });
