...
module.exports = {
  ...
  settings: {
    plain_text_parameters: {
      ...
      description:
        "A comma-delimited list of parameter strings that indicate a sensitive value",
      regex: "[a-zA-Z0-9,]",
        ...
        async.each(
          ...
          function (stack, cb) {
            ...
            if (
              ...
              !describeStacks.data.Stacks.length
            ) {
              helpers.addResult(
                results,
                3,
                `Unable to query for CloudFormation stack detils: ${helpers.addError(
                  describeStacks
                )}`,
                region,
                stack.StackId
            ...
            for (var stackDetails of describeStacks.data.Stacks) {
              ...
              if (!stackDetails.Parameters || !stackDetails.Parameters.length) {
                helpers.addResult(
                  results,
                  0,
                  "Template does not contain any parameters",
                  region,
                  resource
                );
                continue;
              ...
              for (var parameter of stackDetails.Parameters) {
                if (
                  ...
                  secretWords.includes(parameter.ParameterKey.toLowerCase()) &&
                  !parameter.ParameterValue.match("^[*]+$")
                ) {
