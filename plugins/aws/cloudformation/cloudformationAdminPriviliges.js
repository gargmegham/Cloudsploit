      ...
      function (region, rcb) {
        ...
        if (listStacks.err || !listStacks.data) {
          helpers.addResult(
            results,
            3,
            `Unable to list CloudFormation stacks: ${helpers.addError(
              listStacks
            )}`,
            region
          );
        ...
        async.each(
          ...
          function (stack, cb) {
            ...
            if (
              ...
              describeStacks.err ||
              !describeStacks.data ||
              !describeStacks.data.Stacks
            ) {
              helpers.addResult(
                results,
                3,
                `Unable to query for CloudFormation stack: ${helpers.addError(
                  describeStacks
                )}`,
                region,
                resource
            ...
            if (!describeStacks.data.Stacks[0].RoleARN) {
              helpers.addResult(
                results,
                0,
                "CloudFormation stack does not have a role attached",
                region,
                resource
              );
              return cb();
            ...
            var roleName = describeStacks.data.Stacks[0].RoleARN.split("/")[1]
              ? describeStacks.data.Stacks[0].RoleARN.split("/")[1]
              : describeStacks.data.Stacks[0].RoleARN;
            var adminPrivileged;
            ...
            if (
              ...
              helpers.addResult(
                results,
                3,
                `Unable to query for IAM attached policy for role "${roleName}": ${helpers.addError(
                  listAttachedRolePolicies
                )}`,
                region,
                resource
            ...
            for (var policy of listAttachedRolePolicies.data.AttachedPolicies) {
              ...
              if (policy.PolicyArn === managedAdminPolicy) {
                helpers.addResult(
                  results,
                  2,
                  "CloudFormation stack has admin privileges",
                  region,
                  resource
                );
                return cb();
            ...
            if (!adminPrivileged) {
              helpers.addResult(
                results,
                0,
                "CloudFormation stack does not have admin privileges",
                region,
                resource
              );
            } else {
