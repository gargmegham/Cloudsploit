  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.mwaa_environmentdata_desired_encryption_level ||
        this.settings.mwaa_environmentdata_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listEnvironments.err || !listEnvironments.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query MWAA Environments: " +
              helpers.addError(listEnvironments),
            region
          );
          return rcb();
        }
        ...
        for (let environment of listEnvironments.data) {
          ...
          if (
            ...
            helpers.addResult(
              results,
              3,
              `Unable to get MWAA environment: ${helpers.addError(
                getEnvironment
              )}`,
              region,
              resource
          ...
          if (
            getEnvironment.data.Environment &&
            getEnvironment.data.Environment.KmsKey
          ) {
