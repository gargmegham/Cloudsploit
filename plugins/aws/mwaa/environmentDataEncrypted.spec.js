...
const listEnvironments = ["MyAirflowEnvironment"];
...
const getEnvironment = [
  {
    Environment: {
      ...
      ExecutionRoleArn:
        "arn:aws:iam::000111222333:role/service-role/AmazonMWAA-MyAirflowEnvironment-CKkXBd",
      KmsKey:
...
const createCache = (
  ...
  var environment =
    environments && environments.length ? environments[0] : null;
  return {
    mwaa: {
      listEnvironments: {
        "us-east-1": {
...
describe("environmentDataEncrypted", function () {
  describe("run", function () {
    it("should PASS if MWAA Environment Data is encrypted with desired encryption level", function (done) {
      ...
      environmentDataEncrypted.run(
        cache,
        { mwaa_environmentdata_desired_encryption_level: "awscmk" },
        (err, results) => {
    ...
    it("should UNKNOWN if unable to list MWAA Environment", function (done) {
      const cache = createCache(null, null, null, {
        message: "Unable to list MWAA Environment",
      });
      environmentDataEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if unable to list KMS keys", function (done) {
      const cache = createCache(listEnvironments, null, null, null, {
