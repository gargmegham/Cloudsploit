      ...
      function (region, rcb) {
        ...
        if (listEnvironments.err || !listEnvironments.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for Airflow environments: ${helpers.addError(
              listEnvironments
            )}`,
            region
          );
          ...
          function (airflowEnv, cb) {
            ...
            if (
              ...
              helpers.addResult(
                results,
                3,
                `Unable to get Airflow environment: ${helpers.addError(
                  getEnvironment
                )}`,
                region,
                resource
            ...
            if (!getEnvironment.data.Environment.ExecutionRoleArn) {
              helpers.addResult(
                results,
                0,
                "Airflow environment does not have a role attached",
                region,
                resource
              );
              return cb();
            ...
            var roleNameArr =
              getEnvironment.data.Environment.ExecutionRoleArn.split("/");
            ...
            for (var policy of listAttachedRolePolicies.data.AttachedPolicies) {
              ...
              if (policy.PolicyArn === managedAdminPolicy) {
                helpers.addResult(
                  results,
                  2,
                  "Airflow environment has admin privileges",
                  region,
                  resource
                );
                return cb();
            ...
            if (!adminPrivileged) {
              helpers.addResult(
                results,
                0,
                "Airflow environment does not have admin privileges",
                region,
                resource
              );
            } else {
