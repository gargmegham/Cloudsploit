...
const getEnvironment = [
  {
    Environment: {
      ...
      ExecutionRoleArn:
        "arn:aws:iam::000111222333:role/service-role/AmazonMWAA-role-1",
      Name: "env-1",
  ...
  },
  {
    Environment: {
      Arn: "arn:aws:airflow:us-east-1:000111222333:environment/env-1",
      ExecutionRoleArn:
        "arn:aws:iam::000111222333:role/service-role/AmazonMWAA-role-2",
      Name: "env-1",
      NetworkConfiguration: {
        SecurityGroupIds: ["sg-06bb33bc2a9d6cfa0", "sg-0356a73d9749f97ad"],
        SubnetIds: ["subnet-027b3e2dbd13be412", "subnet-0ba3663b2ac3734d2"],
...
describe("webServerPublicAccess", function () {
  describe("run", function () {
    ...
    it("should UNKNOWN if unable to get Ariflow environment", function (done) {
      const cache = createCache(listEnvironments, getEnvironment[0], null, {
        message: "error getting Airflow environment",
      });
      webServerPublicAccess.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(3);
