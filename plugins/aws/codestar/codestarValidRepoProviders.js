...
module.exports = {
  ...
  settings: {
    codestar_disallowed_repo_providers: {
      ...
      description:
        "A comma-separated list of repository providers which should not be used",
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      codestar_disallowed_repo_providers:
        ...
        this.settings.codestar_disallowed_repo_providers.default,
    };

    if (!config.codestar_disallowed_repo_providers.length)
      ...
      function (region, rcb) {
        ...
        if (listProjects.err || !listProjects.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query CodeStar projects: ${helpers.addError(
              listProjects
            )}`,
            region
          );
        ...
        async.each(
          ...
          function (project, cb) {
            ...
            var describeProject = helpers.addSource(cache, source, [
              "codestar",
              "describeProject",
              region,
              project.projectId,
            ...
            if (
              !describeProject ||
              describeProject.err ||
              !describeProject.data ||
              !describeProject.data.projectTemplateId
            ) {
              helpers.addResult(
                results,
                3,
                `Unable to query CodeStar project: ${helpers.addError(
                  describeProject
                )}`,
                region,
                project.projectId
            ...
            let repoProvider =
              describeProject.data.projectTemplateId.split("/").length > 1
                ? describeProject.data.projectTemplateId.split("/")[1]
                : "";

            if (
              config.codestar_disallowed_repo_providers.includes(repoProvider)
            ) {
              helpers.addResult(
                results,
                2,
