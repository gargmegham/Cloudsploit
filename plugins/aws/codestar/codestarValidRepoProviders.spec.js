...
const describeProject = {
  ...
  projectTemplateId:
    "arn:aws:codestar:us-east-1::project-template/github/webapp-nodeweb-lambda",
};
...
const createCache = (
  ...
  let project =
    listProjects && listProjects.length ? listProjects[0].projectId : null;
  return {
    codestar: {
      listProjects: {
        "us-east-1": {
...
describe("codestarValidRepoProviders", function () {
  describe("run", function () {
    it("should PASS if CodeStar project is using allowed repo providers", function (done) {
      ...
      codestarValidRepoProviders.run(
        cache,
        { codestar_disallowed_repo_providers: "codecommit" },
        (err, results) => {
    ...
    it("should FAIL if CodeStar project is not using allowed source providers", function (done) {
      ...
      codestarValidRepoProviders.run(
        cache,
        { codestar_disallowed_repo_providers: "github" },
        (err, results) => {
          expect(results.length).to.equal(1);
    ...
    it("should PASS if no CodeStar projects found", function (done) {
      ...
      codestarValidRepoProviders.run(
        cache,
        { codestar_disallowed_repo_providers: "s3" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          expect(results[0].region).to.equal("us-east-1");
    ...
    it("should UNKNOWN if unable to query CodeStar projects", function (done) {
      const cache = createCache(listProjects, {
        message: "Unable to query CodeStar projects",
      });
      codestarValidRepoProviders.run(
        cache,
        { codestar_disallowed_repo_providers: "s3" },
    ...
    it("should UNKNOWN if unable to query CodeStar project", function (done) {
      const cache = createCache(listProjects, null, null, {
        message: "Unable to query CodeStar project",
      });
      codestarValidRepoProviders.run(
        cache,
        { codestar_disallowed_repo_providers: "s3" },
