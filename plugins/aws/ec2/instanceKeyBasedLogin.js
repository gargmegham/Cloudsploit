...
module.exports = {
  ...
  settings: {
    instance_keypair_threshold: {
      ...
      description:
        "Plugin results will become aggregated when this value is breached. Max is 299.",
      regex: "^[1-2]{1}[0-9]{0,2}$",
      default: "10",
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      instance_keypair_threshold: parseInt(
        settings.instance_keypair_threshold ||
          this.settings.instance_keypair_threshold.default
      ),
    };
      ...
      function (region, rcb) {
          ...
          for (var j in describeInstances.data[i].Instances) {
            ...
            if (!instance.KeyName) {
              ...
              helpers.addResult(
                results,
                2,
                "Instance does not have associated keys for password-less SSH login",
                region,
                "arn:aws:ec2:" +
                  region +
                  ":" +
        ...
        if (found > config.instance_keypair_threshold) {
          ...
          helpers.addResult(
            ...
            2,
            "Over " +
              config.instance_keypair_threshold +
              " EC2 instances do not have associated keys for password-less SSH login",
            region,
            null,
            custom
          );
        ...
        if (!found) {
          helpers.addResult(
            ...
            0,
            "All " +
              describeInstances.data.length +
              " instances have associated keys for password-less SSH login",
            region
          );
        }
