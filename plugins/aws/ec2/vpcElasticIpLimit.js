...
module.exports = {
  ...
  settings: {
    vpc_elastic_ip_percentage_fail: {
      ...
      description:
        "Return a failing result when consumed VPC elastic IPs equals or exceeds this percentage",
      regex: "^(100|[1-9][0-9]?)$",
      default: 90,
    },
    vpc_elastic_ip_percentage_warn: {
      ...
      description:
        "Return a warning result when consumed VPC elastic IPs equals or exceeds this percentage",
      regex: "^(100|[1-9][0-9]?)$",
      default: 75,
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      vpc_elastic_ip_percentage_fail:
        settings.vpc_elastic_ip_percentage_fail ||
        this.settings.vpc_elastic_ip_percentage_fail.default,
      vpc_elastic_ip_percentage_warn:
        settings.vpc_elastic_ip_percentage_warn ||
        this.settings.vpc_elastic_ip_percentage_warn.default,
    };
      ...
      function (region, rcb) {
        ...
        if (describeAddresses.err || !describeAddresses.data) {
          helpers.addResult(
            results,
            3,
            "Unable to describe addresses for VPC Elastic IP limit: " +
              helpers.addError(describeAddresses),
            region
          );
          return rcb();
        }
        ...
        var percentage = Math.ceil(
          (eips / limits["vpc-max-elastic-ips"]) * 100
        );
        var returnMsg =
          "Account contains " +
          eips +
          " of " +
          limits["vpc-max-elastic-ips"] +
          " (" +
          percentage +
          "%) available VPC Elastic IPs";
