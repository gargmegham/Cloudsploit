      ...
      function (region, rcb) {
        ...
        if (!describeImages.data.length) {
          helpers.addResult(
            results,
            0,
            "No Amazon Machine Images found",
            region
          );
          return rcb();
        }
        ...
        if (
          ...
          helpers.addResult(
            results,
            3,
            `Unable to query EC2 instances: ${helpers.addError(
              describeInstances
            )}`,
            region
          );
        ...
        if (
          ...
          helpers.addResult(
            results,
            3,
            `Unable to query Auto Scaling launch configurations: ${helpers.addError(
              describeLaunchConfigurations
            )}`,
            region
          );
        ...
        if (
          !describeLaunchTemplates ||
          describeLaunchTemplates.err ||
          !describeLaunchTemplates.data
        ) {
          helpers.addResult(
            results,
            3,
            `Unable to query EC2 launch templates: ${helpers.addError(
              describeLaunchTemplates
            )}`,
            region
          );
        ...
        describeImages.data.forEach((image) => {
          ...
          if (
            ...
            new Date(image.DeprecationTime) < new Date()
          ) {
            found = true;
            let amiInUse = false;
            ...
            if (!amiInUse && describeLaunchTemplates.data.length) {
              for (const template of describeLaunchConfigurations.data) {
                if (template.LaunchTemplateDId) {
                  var describeLaunchTemplateVersions = helpers.addSource(
                    cache,
                    source,
                    [
                      "ec2",
                  ...
                  if (
                    ...
                    let templateVersion =
                      describeLaunchTemplateVersions.data.LaunchTemplateVersions.find(
                        (version) =>
                          version.VersionNumber == template.DefaultVersionNumber
                      );
                    if (
                      templateVersion &&
                      templateVersion.LaunchTemplateData &&
                      templateVersion.LaunchTemplateData.ImageId &&
                      image.ImageId ==
                        templateVersion.LaunchTemplateData.ImageId
                    ) {
                      amiInUse = true;
            ...
            helpers.addResult(
              results,
              status,
              `Deprecated Amazon Machine Image "${image.ImageId}" is ${
                amiInUse ? "" : "not "
              }in use`,
              region,
              resource
        ...
        if (!found) {
          helpers.addResult(
            results,
            0,
            "No deprecated Amazon Machine Image found",
            region
          );
        }
