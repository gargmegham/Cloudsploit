        ...
        async.parallel(
          [
            function (lcb) {
              ...
              if (
                describeInternetGateways.err ||
                !describeInternetGateways.data
              ) {
                helpers.addResult(
                  results,
                  3,
                  `Unable to query for Internet Gateways: ${helpers.addError(
                    describeInternetGateways
                  )}`,
                  region
                );
            ...
            function (lcb) {
              var describeEgressOnlyInternetGateways = helpers.addSource(
                cache,
                source,
              ...
              if (
                describeEgressOnlyInternetGateways.err ||
                !describeEgressOnlyInternetGateways.data
              ) {
                helpers.addResult(
                  results,
                  3,
                  `Unable to query for Egress-Only Internet Gateways: ${helpers.addError(
              ...
              if (!describeEgressOnlyInternetGateways.data.length) {
                helpers.addResult(
                  results,
                  0,
                  "No Egress-Only Internet Gateways found",
                  region
                );
              }
...
function loopGWForResults(gateways, results, region, resource, type = "") {
  gateways.data.forEach(function (gateway) {
    let gatewayId =
      gateway.EgressOnlyInternetGatewayId || gateway.InternetGatewayId;
