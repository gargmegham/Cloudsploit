...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      ec2_web_tier_tag_key:
        settings.ec2_web_tier_tag_key ||
        this.settings.ec2_web_tier_tag_key.default,
    };

    if (!config.ec2_web_tier_tag_key.length)
          ...
          for (var i in instance.Instances) {
            ...
            for (let t in describeTags.data) {
              ...
              if (
                ...
                tag.Key &&
                tag.Key === config.ec2_web_tier_tag_key
              ) {
                tagFound = true;
                break;
              }
            ...
            if (!tagFound) {
              helpers.addResult(
                results,
                0,
                "Instance does not have Web-Tier tag key",
                region,
                resource
              );
              return cb();
