...
module.exports = {
  ...
  compliance: {
    hipaa:
      "AWS VPC is the recommended location for processing of HIPAA-related " +
      ...
      function (region, rcb) {
        ...
        for (var i in describeInstances.data) {
          for (var j in describeInstances.data[i].Instances) {
            ...
            if (
              describeInstances.data[i].Instances[j].State &&
              describeInstances.data[i].Instances[j].State.Name &&
              describeInstances.data[i].Instances[j].State.Name !== "running"
            )
              continue;

            if (
              !describeInstances.data[i].Instances[j].NetworkInterfaces ||
              !describeInstances.data[i].Instances[j].NetworkInterfaces.length
            ) {
        ...
        if (notInVpc) {
          helpers.addResult(
            results,
            1,
            "There are " + notInVpc + " instances in EC2-Classic",
            region
          );
        } else if (inVpc) {
          helpers.addResult(
            results,
            0,
            "There are " + inVpc + " instances in a VPC",
            region
          );
        } else {
          helpers.addResult(results, 0, "No instances found", region);
