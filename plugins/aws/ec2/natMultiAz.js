  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        if (
          !describeNatGateways ||
          describeNatGateways.err ||
          !describeNatGateways.data
        ) {
          helpers.addResult(
            results,
            3,
            "Unable to query for NAT gateways: " +
              helpers.addError(describeNatGateways),
            region
          );
          return rcb();
        }
        ...
        for (var v in vpcMap) {
          ...
          if (numSubnets) {
            ...
            var vpcArn =
              "arn:aws:ec2:" + region + ":" + accountId + ":vpc/" + v;

            if (numSubnets === 1) {
              helpers.addResult(
                results,
                1,
                "VPC is using NAT gateways in only 1 subnet",
                region,
                vpcArn
              );
            } else {
              helpers.addResult(
                results,
                0,
                "VPC is using NAT gateways in " + numSubnets + " subnets",
                region,
                vpcArn
              );
            }
        ...
        if (!found) {
          helpers.addResult(
            results,
            0,
            "No VPCs with NAT gateways found",
            region
          );
        }
