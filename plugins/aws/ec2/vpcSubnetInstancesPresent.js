      ...
      function (region, rcb) {
        ...
        if (describeSubnets.err || !describeSubnets.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for VPC subnets: ${helpers.addError(
              describeSubnets
            )}`,
            region
          );
        ...
        if (
          ...
          helpers.addResult(
            results,
            3,
            `Unable to query for EC2 instances: ${helpers.addError(
              describeInstances
            )}`,
            region
          );
          ...
          describeInstances.data.forEach((instance) => {
            if (instance.Instances && instance.Instances.length) {
              instance.Instances.forEach((entry) => {
                if (entry.SubnetId && instanceSubnets[entry.SubnetId])
                  instanceSubnets[entry.SubnetId] += 1;
        ...
        describeSubnets.data.forEach((subnet) => {
          if (subnet.SubnetId && instanceSubnets[subnet.SubnetId]) {
            helpers.addResult(
              ...
              0,
              `Subnet has ${
                instanceSubnets[subnet.SubnetId]
              } instances attached`,
              region,
              subnet.SubnetArn
            );
