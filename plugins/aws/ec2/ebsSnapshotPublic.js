      ...
      function (region, rcb) {
        ...
        describeSnapshots.data.forEach((snapshot) => {
          ...
          if (
            ...
            helpers.addResult(
              results,
              3,
              `Unable to query EBS snapshot attribute: ${helpers.addError(
                describeSnapshotAttribute
              )}`,
              region,
              resource
          ...
          if (describeSnapshotAttribute.data.CreateVolumePermissions.length) {
            for (var p in describeSnapshotAttribute.data
              .CreateVolumePermissions) {
              var perm =
                describeSnapshotAttribute.data.CreateVolumePermissions[p];
        ...
        if (publicSnapshots.length > 20) {
          helpers.addResult(
            results,
            2,
            "More than 20 EBS snapshots are publicly shared",
            region
          );
