...
module.exports = {
  ...
  more_info:
    "VPC peering communication should be only between AWS accounts to keep organization resources private and isolated.",
  recommended_action:
      ...
      function (region, rcb) {
        ...
        if (
          ...
          helpers.addResult(
            results,
            3,
            `Unable to query for VPC peering connections: ${helpers.addError(
              describeVpcPeeringConnections
            )}`,
            region
          );
        ...
        if (!listAccounts || listAccounts.err || !listAccounts.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for Organization Accounts: ${helpers.addError(
              listAccounts
            )}`,
            region
          );
        ...
        describeVpcPeeringConnections.data.forEach((connection) => {
          ...
          if (
            connection.RequesterVpcInfo &&
            connection.RequesterVpcInfo.OwnerId &&
            organizationAccounts.includes(
              connection.RequesterVpcInfo.OwnerId
            ) &&
