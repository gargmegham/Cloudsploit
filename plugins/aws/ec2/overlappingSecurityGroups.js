...
module.exports = {
  ...
  recommended_action:
    "Structure security groups to provide a single category of access and do not " +
      ...
      function (region, rcb) {
              ...
              for (var w in sgMap[groupId]) {
                ...
                if (ruleMap[rule]) {
                  ...
                  if (!overlaps[arn]) {
                    ...
                    if (
                      overlaps[arn].indexOf(compOp1) === -1 &&
        ...
        if (!Object.keys(overlaps).length) {
          helpers.addResult(
            results,
            0,
            "No overlapping instance security groups found",
            region
          );
          ...
          helpers.addResult(
            results,
            1,
            "More than 20 EC2 instances have overlapping security groups",
            region
          );
        } else {
          for (var x in overlaps) {
            helpers.addResult(
              results,
              1,
              "Instance has overlapping security group rules via groups: " +
                overlaps[x].join(", "),
              region,
              x
            );
