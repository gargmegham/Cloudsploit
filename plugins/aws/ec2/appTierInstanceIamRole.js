...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      ec2_app_tier_tag_key:
        settings.ec2_app_tier_tag_key ||
        this.settings.ec2_app_tier_tag_key.default,
    };

    if (!config.ec2_app_tier_tag_key.length)
      ...
      function (region, rcb) {
        ...
        if (describeInstances.err || !describeInstances.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for instances: ${helpers.addError(
              describeInstances
            )}`,
            region
          );
        ...
        if (!describeTags || describeTags.err || !describeTags.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for tags: ${helpers.addError(describeTags)}`,
            region
          );
          return rcb();
        }
        ...
        async.each(describeInstances.data, function (instance, cb) {
          ...
          for (var i in instance.Instances) {
            ...
            for (let t in describeTags.data) {
              ...
              if (
                tag.ResourceId &&
                tag.ResourceId === entry.InstanceId &&
                tag.Key &&
                tag.Key === config.ec2_app_tier_tag_key
              ) {
                tagFound = true;
                break;
              }
            ...
            if (!tagFound) {
              helpers.addResult(
                results,
                0,
                "Instance does not have App-Tier tag key",
                region,
                resource
              );
              return cb();
            ...
            if (!entry.IamInstanceProfile || !entry.IamInstanceProfile.Arn) {
              helpers.addResult(
                results,
                2,
                "Instance does not use an IAM role",
                region,
                resource
              );
            } else {
              ...
              if (
                ...
                helpers.addResult(
                  results,
                  3,
                  `Unable to query for IAM role policy for role "${roleName}" ${helpers.addError(
                    listAttachedRolePolicies
                  )}`,
                  region,
                  resource
              ...
              if (listAttachedRolePolicies.data.AttachedPolicies.length) {
                helpers.addResult(
                  results,
                  0,
                  "IAM role attached with EC2 instance contains policies",
                  region,
                  resource
                );
                return cb();
              ...
              if (listRolePolicies.data.PolicyNames.length) {
                ...
                helpers.addResult(
                  results,
                  2,
                  "IAM role attached with EC2 instance does not contain any policies",
                  region,
                  resource
                );
              }
