      ...
      function (region, rcb) {
        ...
        if (describeVpcs.err || !describeVpcs.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for VPCs: " + helpers.addError(describeVpcs),
            region
          );
          return rcb();
        }
        ...
        if (
          ...
          helpers.addResult(
            results,
            3,
            "Unable to query for EC2 instances: " +
          ...
          for (var i in describeInstances.data) {
            for (var j in describeInstances.data[i].Instances) {
              if (
                describeInstances.data[i].Instances[j].VpcId &&
        ...
        if (
          ...
          describeLoadBalancers.err
        ) {
          helpers.addResult(
            results,
            3,
            "Unable to query for load balancers: " +
              helpers.addError(describeLoadBalancers),
            region
          );
          ...
          for (var k in describeLoadBalancers.data) {
            if (
              describeLoadBalancers.data[k].VPCId &&
        ...
        if (regions.lambda.indexOf(region) > -1) {
          ...
          if (!listFunctions || !listFunctions.data || listFunctions.err) {
            ...
            for (var l in listFunctions.data) {
              if (
                listFunctions.data[l].VpcConfig &&
        ...
        if (
          ...
          for (var m in describeDBInstances.data) {
            if (
              describeDBInstances.data[m].DBSubnetGroup &&
        ...
        if (
          !describeClusters ||
          !describeClusters.data ||
          describeClusters.err
        ) {
          helpers.addResult(
            results,
            3,
            "Unable to query for Redshift instances: " +
