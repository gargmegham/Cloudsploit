...
module.exports = {
  ...
  more_info:
    "Virtual Private Gateways allow communication between cloud infrastructure and the remote customer network. They help in establishing VPN connection between VPC and the customer gateway. " +
      ...
      function (region, rcb) {
        ...
        if (describeVpnGateways.err || !describeVpnGateways.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for Virtual Private Gateways: ${helpers.addError(
              describeVpnGateways
            )}`,
            region
          );
        ...
        if (!describeVpnGateways.data.length) {
          helpers.addResult(
            results,
            0,
            "No Virtual Private Gateways found",
            region
          );
          return rcb();
        }
        ...
        describeVpnGateways.data.forEach(function (vpn) {
          ...
          if (vpn.VpcAttachments && vpn.VpcAttachments.length) {
            let attached = vpn.VpcAttachments.find(
              (attachment) =>
                attachment.VpcId &&
                attachment.State &&
                attachment.State.toUpperCase() == "ATTACHED"
            );
