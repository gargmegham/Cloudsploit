...
module.exports = {
  ...
  settings: {
    allow_blank_whitelisted_principals: {
      ...
      description:
        "When set to true, VPC PrivateLink endpoints having zero/blank whitelisted pricipals will PASS",
      regex: "^(true|false)$",
      default: "false",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      allow_blank_whitelisted_principals:
        settings.allow_blank_whitelisted_principals ||
        this.settings.allow_blank_whitelisted_principals.default,
    };

    config.allow_blank_whitelisted_principals =
      config.allow_blank_whitelisted_principals == "true";
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        if (
          describeVpcEndpointServices.err ||
          !describeVpcEndpointServices.data
        ) {
          helpers.addResult(
            results,
            3,
            `Unable to query for VPC endpoint services: ${helpers.addError(
              describeVpcEndpointServices
            )}`,
            region
          );
        ...
        describeVpcEndpointServices.data =
          describeVpcEndpointServices.data.filter(
            (service) => service.Owner != "amazon"
          );
        ...
        if (!describeVpcEndpointServices.data.length) {
          helpers.addResult(
            results,
            0,
            "No user owned VPC endpoint services present",
            region
          );
          return rcb();
        }
        ...
        for (var service of describeVpcEndpointServices.data) {
          ...
          if (service.AcceptanceRequired) {
            ...
            if (config.allow_blank_whitelisted_principals) {
              var describeVpcEndpointServicePermissions = helpers.addSource(
                cache,
                source,
                [
                  "ec2",
