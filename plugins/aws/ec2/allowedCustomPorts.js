...
module.exports = {
  ...
  settings: {
    whitelisted_open_ports: {
      ...
      description:
        "A comma-separated list of ports/port-ranges that indicates open ports allowed for any connection. Example: tcp:80,tcp:443,tcp:80-443",
      regex: "[a-zA-Z0-9,:]",
      default: "",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var whitelisted_open_ports =
      settings.whitelisted_open_ports ||
      this.settings.whitelisted_open_ports.default;
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        for (var g in describeSecurityGroups.data) {
          ...
          IpPermissionsLoop: for (var p in group.IpPermissions) {
            var permission = group.IpPermissions[p];

            // Loop through each ip range for an ip permissions list
            IpRangesLoop: for (var r in permission.IpRanges) {
