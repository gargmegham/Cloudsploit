...
module.exports = {
  ...
  settings: {
    instance_limit_percentage_fail: {
      ...
      description:
        "Return a failing result when utilized instances equals or exceeds this percentage",
      regex: "^(100|[1-9][0-9]?)$",
      default: 90,
    },
    instance_limit_percentage_warn: {
      ...
      description:
        "Return a warning result when utilized instances equals or exceeds this percentage",
      regex: "^(100|[1-9][0-9]?)$",
      default: 75,
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      instance_limit_percentage_fail:
        settings.instance_limit_percentage_fail ||
        this.settings.instance_limit_percentage_fail.default,
      instance_limit_percentage_warn:
        settings.instance_limit_percentage_warn ||
    ...
    function checkLegacyRegion(describeInstances, region, rcb) {
      ...
      if (
        !describeAccountAttributes ||
        describeAccountAttributes.err ||
        !describeAccountAttributes.data ||
        !describeAccountAttributes.data.length
      ) {
        helpers.addResult(
          results,
          3,
          "Unable to query for account limits: " +
            helpers.addError(describeAccountAttributes) +
            ". Default limit of " +
      ...
      var percentage = Math.ceil(
        (ec2Instances / limits["max-instances"]) * 100
      );
      var returnMsg =
        "Account contains " +
        ec2Instances +
    ...
    function checkVcpusLimits(describeInstances, region, rcb) {
      ...
      if (
        !describeServiceQuotas ||
        describeServiceQuotas.err ||
        !describeServiceQuotas.data ||
        !describeServiceQuotas.data.length
      ) {
        helpers.addResult(
          results,
          3,
          "Unable to query for service quotas: " +
            helpers.addError(describeServiceQuotas),
          region
        );
        return rcb();
      }
      ...
      if (onDemandServiceQuotas && onDemandServiceQuotas.length > 0) {
        for (var sq in onDemandServiceQuotas) {
          var serviceQuota = onDemandServiceQuotas[sq];
          var instanceTypeName = serviceQuota.QuotaName.replace(
            "Running On-Demand ",
            ""
          )
            .replace(" instances", "")
          ...
          if (instanceTypeName.indexOf("standard") > -1) {
            var instanceTypes = instanceTypeName
              .replace("standard (", "")
              .replace(")", "")
              .replace(/ /g, "")
              .split(",");
        ...
        helpers.addResult(
          results,
          3,
          "Unable to query on-demand service quotas: " +
            helpers.addError(describeServiceQuotas),
          region
        );
        return rcb();
      ...
      describeInstances.data.forEach((instances) => {
        instances.Instances.forEach((i) => {
          ...
          var currentCoreCount = instanceMap.cores[instanceType]
            ? parseInt(instanceMap.cores[instanceType])
            : 0;
          instanceMap.cores[instanceType] = currentCoreCount + coreCount;

          var currentThreadsPerCore = instanceMap.threads[instanceType]
            ? parseInt(instanceMap.threads[instanceType])
            : 0;
          instanceMap.threads[instanceType] =
            currentThreadsPerCore + threadsPerCore;

          instanceMap.vcpus[instanceType] =
            instanceMap.cores[instanceType] * instanceMap.threads[instanceType];

          if (instanceMap.groupInstanceTypes[instanceType]) {
            if (
              instanceMap.groupInstanceTypes[instanceType].indexOf(
                instanceType
              ) < 0
            ) {
              var percentage = Math.ceil(
                (instanceMap.vcpus[instanceType] /
                  instanceMap.limit[instanceType]) *
                  100
              );
              ...
              instanceMap.groupVcpus[instanceType] =
                instanceMap.vcpus[instanceType];
            }
          }
        });
      ...
      var totalGroupVcpus = Object.values(instanceMap.groupVcpus).reduce(
        function (acc, val) {
          return acc + val;
        },
        0
      ...
      for (const it in instanceMap.instanceType) {
        ...
        if (instanceMap.groupInstanceTypes[it]) {
          percentage = Math.ceil(
            (instanceMap.groupVcpus[it] / instanceMap.limit[it]) * 100
          );
        } else {
          percentage = Math.ceil(
            (instanceMap.vcpus[it] / instanceMap.limit[it]) * 100
          );
        }
      ...
      for (const it in instanceMap.instanceType) {
        if (instanceMap.groupInstanceTypes[it] && runGroupOnce === false) {
          ...
          instanceMap.groupInstanceTypes[it] &&
          runGroupOnce === true
        ) {
          continue;
        } else {
        ...
        var returnMsg =
          ...
          (instanceMap.groupInstanceTypes[it]
            ? instanceMap.groupInstanceTypes[it]
                ...
                .toUpperCase()
                .replace(/,/g, ", ")
            : instanceMap.instanceType[it].toString().toUpperCase()) +
          " instances";

        if (ec2VcpusLimitUsage >= config.instance_limit_percentage_fail) {
          helpers.addResult(results, 2, returnMsg, region, null, custom);
        } else if (
          ec2VcpusLimitUsage >= config.instance_limit_percentage_warn
        ) {
          helpers.addResult(results, 1, returnMsg, region, null, custom);
        } else {
          helpers.addResult(results, 0, returnMsg, region, null, custom);
