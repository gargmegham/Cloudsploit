...
module.exports = {
  ...
  settings: {
    excessive_security_groups_fail: {
      ...
      description:
        "Return a failing result when the number of security groups exceeds this value",
      regex: "^[1-9]{1}[0-9]{0,5}$",
      default: 40,
    },
    excessive_security_groups_warn: {
      ...
      description:
        "Return a warning result when the number of security groups exceeds this value",
      regex: "^[1-9]{1}[0-9]{0,5}$",
      default: 30,
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      excessive_security_groups_fail:
        settings.excessive_security_groups_fail ||
        this.settings.excessive_security_groups_fail.default,
      excessive_security_groups_warn:
        settings.excessive_security_groups_warn ||
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        var returnMsg =
          ...
          describeSecurityGroups.data.length +
          " groups present";

        if (
          describeSecurityGroups.data.length >
          config.excessive_security_groups_fail
        ) {
          helpers.addResult(
            results,
            2,
          ...
          describeSecurityGroups.data.length >
          config.excessive_security_groups_warn
        ) {
          helpers.addResult(
            results,
            1,
