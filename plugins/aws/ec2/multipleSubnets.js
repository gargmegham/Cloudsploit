      ...
      function (region, rcb) {
        ...
        if (describeVpcs.data.length > 1) {
          helpers.addResult(
            results,
            0,
            "Multiple (" + describeVpcs.data.length + ") VPCs are used.",
            region
          );
          return rcb();
        }
        ...
        if (!vpcId) {
          helpers.addResult(
            results,
            3,
            "Unable to query for subnets for VPC.",
            region
          );
          return rcb();
        }
        ...
        if (
          ...
          !describeSubnets.data.Subnets
        ) {
          helpers.addResult(
            results,
            3,
            "Unable to query for subnets in VPC: " +
        ...
        if (describeSubnets.data.Subnets.length > 1) {
          helpers.addResult(
            ...
            0,
            "There are " +
              describeSubnets.data.Subnets.length +
              " subnets used in one VPC.",
            region,
            resource
          );
          ...
          helpers.addResult(
            ...
            2,
            "Only one subnet (" +
              describeSubnets.data.Subnets[0].SubnetId +
              ") in one VPC is used.",
            region,
            resource
          );
