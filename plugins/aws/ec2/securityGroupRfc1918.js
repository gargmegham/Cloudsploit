...
module.exports = {
  ...
  settings: {
    private_cidrs: {
      ...
      description:
        "A comma-separated list of CIDRs that indicates reserved private addresses",
  ...
  run: function (cache, settings, callback) {
    ...
    var private_cidrs =
      settings.private_cidrs || this.settings.private_cidrs.default;
        ...
        for (var g in describeSecurityGroups.data) {
          ...
          if (!group.IpPermissions || !group.IpPermissions.length) {
            helpers.addResult(
              ...
              "Security group :" +
                group.GroupName +
                ": does not have any IP permissions",
              region,
              resource
            );
            continue;
          ...
          for (var p in group.IpPermissions) {
            ...
            if (!privateCidrsFound.length) {
              helpers.addResult(
                ...
                'Security group "' +
                  group.GroupName +
                  '" is not configured to allow traffic from any reserved private addresses',
                region,
                resource
              );
            } else {
              helpers.addResult(
                ...
                'Security group "' +
                  group.GroupName +
                  '" is configured to allow inbound access for these reserved private addresses: ' +
                  privateCidrsFound.join(", "),
                region,
                resource
              );
            }
