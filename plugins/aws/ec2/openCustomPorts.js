...
module.exports = {
  ...
  settings: {
    restricted_open_ports: {
      ...
      description:
        "Comma separated list of ports/port-ranges that should be restricted and not publicly open. Example: tcp:80,tcp:443,tcp:80-443",
      regex: "[a-zA-Z0-9,:]",
      default: "",
    },
  ...
  run: function (cache, settings, callback) {
    ...
    var restricted_open_ports =
      settings.restricted_open_ports ||
      this.settings.restricted_open_ports.default;
      ...
      function (region, rcb) {
        ...
        if (describeSecurityGroups.err || !describeSecurityGroups.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for security groups: ${helpers.addError(
              describeSecurityGroups
            )}`,
            region
          );
