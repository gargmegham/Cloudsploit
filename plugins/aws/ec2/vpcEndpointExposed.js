      ...
      function (region, rcb) {
        ...
        if (describeVpcEndpoints.err || !describeVpcEndpoints.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for VPC endpoints: " +
              helpers.addError(describeVpcEndpoints),
            region
          );
          return rcb();
        }
        ...
        if (!describeVpcEndpoints.data.length) {
          helpers.addResult(results, 0, "No VPC endpoins present", region);
          return rcb();
        }
        ...
        if (!describeSubnets || describeSubnets.err || !describeSubnets.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for VPC subnets: " +
              helpers.addError(describeSubnets),
            region
          );
          return rcb();
        }
        ...
        if (
          ...
          helpers.addResult(
            results,
            3,
            "Unable to query for route tables: " +
        ...
        var subnetRouteTableMap = helpers.getSubnetRTMap(
          describeSubnets.data,
          describeRouteTables.data
        );
        var privateSubnets = helpers.getPrivateSubnets(
          subnetRouteTableMap,
          describeSubnets.data,
          describeRouteTables.data
        );
        ...
        for (var endpoint of describeVpcEndpoints.data) {
          ...
          if (
            ...
            endpoint.VpcEndpointType.toLowerCase() == "gateway"
          ) {
            helpers.addResult(
              results,
              0,
              `VPC endpoint is of ${endpoint.VpcEndpointId} is of Gateway type`,
              region,
              resource
            );
            continue;
          ...
          if (endpoint.SubnetIds && endpoint.SubnetIds.length) {
            if (
              endpoint.SubnetIds.find((subnetId) =>
                privateSubnets.includes(subnetId)
              )
            ) {
              helpers.addResult(
                results,
                0,
                "VPC endpoint is behind private subnet",
                region,
                resource
              );
              continue;
