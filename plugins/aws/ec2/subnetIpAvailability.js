...
module.exports = {
  ...
  settings: {
    subnet_ip_availability_percentage_fail: {
      ...
      description:
        "Return a failing result when consumed subnet IPs equals or exceeds this percentage",
      regex: "^(100|[1-9][0-9]?)$",
      default: 90,
    },
    subnet_ip_availability_percentage_warn: {
      ...
      description:
        "Return a warning result when consumed subnet IPs equals or exceeds this percentage",
      regex: "^(100|[1-9][0-9]?)$",
      default: 75,
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      subnet_ip_availability_percentage_fail:
        settings.subnet_ip_availability_percentage_fail ||
        this.settings.subnet_ip_availability_percentage_fail.default,
      subnet_ip_availability_percentage_warn:
        settings.subnet_ip_availability_percentage_warn ||
        this.settings.subnet_ip_availability_percentage_warn.default,
    };
      ...
      function (region, rcb) {
        ...
        for (var i in describeSubnets.data) {
          if (describeSubnets.data[i] && describeSubnets.data[i].CidrBlock) {
            var subnetSize = helpers.cidrSize(
              describeSubnets.data[i].CidrBlock
            );
            var consumedIPs =
              subnetSize - describeSubnets.data[i].AvailableIpAddressCount;
            var percentageConsumed = Math.ceil(
              (consumedIPs / subnetSize) * 100
            );
            var subnetArn =
              "arn:aws:ec2:" +
              region +
              ...
              accountId +
              ":subnet/" +
              describeSubnets.data[i].SubnetId;
            ...
            if (
              percentageConsumed >=
              config.subnet_ip_availability_percentage_fail
            ) {
              helpers.addResult(
                results,
                2,
              ...
              percentageConsumed >=
              config.subnet_ip_availability_percentage_warn
            ) {
              helpers.addResult(
                results,
                1,
