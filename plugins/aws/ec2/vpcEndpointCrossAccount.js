...
module.exports = {
  ...
  settings: {
    vpc_trusted_cross_account_arns: {
      ...
      description:
        "A comma-separated list of trusted cross account ARNs i.e. 'arn:aws:iam::000111222333:user/user1,arn:aws:iam::000111222333:user/user2'",
      regex: "^.*$",
      default: "",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var vpc_trusted_cross_account_arns =
      settings.vpc_trusted_cross_account_arns ||
      this.settings.vpc_trusted_cross_account_arns.default;
      ...
      function (region, rcb) {
        ...
        if (describeVpcEndpoints.err || !describeVpcEndpoints.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for VPC endpoints: ${helpers.addError(
              describeVpcEndpoints
            )}`,
            region
          );
        ...
        if (!describeVpcEndpoints.data.length) {
          helpers.addResult(results, 0, "No VPC endpoins found", region);
          return rcb();
        }
        ...
        describeVpcEndpoints.data.forEach((endpoint) => {
            ...
            for (var statement of statements) {
              if (
                ...
                for (var principal of principals) {
                  if (
                    !vpc_trusted_cross_account_arns.includes(principal) &&
          ...
          if (!unallowedCrossAccounts.length) {
            ...
            helpers.addResult(
              ...
              } allows cross account access to these principals: ${unallowedCrossAccounts.join(
                ", "
              )}`,
              region,
              resource
