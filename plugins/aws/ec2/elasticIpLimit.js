...
module.exports = {
  ...
  settings: {
    elastic_ip_percentage_fail: {
      ...
      description:
        "Return a failing result when consumed elastic IPs equals or exceeds this percentage",
      regex: "^(100|[1-9][0-9]?)$",
      default: 90,
    },
    elastic_ip_percentage_warn: {
      ...
      description:
        "Return a warning result when consumed elastic IPs equals or exceeds this percentage",
      regex: "^(100|[1-9][0-9]?)$",
      default: 75,
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      elastic_ip_percentage_fail:
        settings.elastic_ip_percentage_fail ||
        this.settings.elastic_ip_percentage_fail.default,
      elastic_ip_percentage_warn:
        settings.elastic_ip_percentage_warn ||
        this.settings.elastic_ip_percentage_warn.default,
    };
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        for (var i in describeAccountAttributes.data) {
          if (
            describeAccountAttributes.data[i].AttributeName &&
        ...
        if (describeAddresses.err || !describeAddresses.data) {
          helpers.addResult(
            results,
            3,
            "Unable to describe addresses for Elastic IP limit: " +
