...
module.exports = {
  ...
  compliance: {
    pci:
      "VPCs provide a firewall for compute resources that meets the network " +
      ...
      function (region, rcb) {
        ...
        if (!describeSubnets || !describeSubnets.data || describeSubnets.err) {
          helpers.addResult(
            results,
            3,
            "Unable to query for Subnets: " + helpers.addError(describeSubnets),
            region
          );
          return rcb();
        }
        ...
        if (
          !describeRouteTables ||
          !describeRouteTables.data ||
          describeRouteTables.err
        ) {
          helpers.addResult(
            results,
            3,
            "Unable to query for RouteTables: " +
              helpers.addError(describeRouteTables),
            region
          );
          return rcb();
        }
        ...
        if (
          !describeVpcPeeringConnections ||
          !describeVpcPeeringConnections.data ||
          describeVpcPeeringConnections.err
        ) {
          helpers.addResult(
            results,
            3,
            "Unable to query for VpcPeering: " +
              helpers.addError(describeVpcPeeringConnections),
            region
          );
          return rcb();
        }
        ...
        if (!describeVpcPeeringConnections.data.length) {
          helpers.addResult(
            results,
            0,
            "No public private subnets connection found",
            region
          );
          return rcb();
        }
        ...
        for (i in describeVpcPeeringConnections.data) {
          vpcPeeringInfo.push({
            peeringId:
              describeVpcPeeringConnections.data[i].VpcPeeringConnectionId,
            vpcId: describeVpcPeeringConnections.data[i].AccepterVpcInfo.VpcId,
            peercidr:
              describeVpcPeeringConnections.data[i].AccepterVpcInfo.CidrBlock,
            ownerId:
              describeVpcPeeringConnections.data[i].AccepterVpcInfo.OwnerId,
          });
          vpcPeeringInfo.push({
            peeringId:
              describeVpcPeeringConnections.data[i].VpcPeeringConnectionId,
            ...
            peercidr:
              describeVpcPeeringConnections.data[i].RequesterVpcInfo.CidrBlock,
            ownerId:
              describeVpcPeeringConnections.data[i].RequesterVpcInfo.OwnerId,
          });
        }
        ...
        for (var k in describeRouteTables.data) {
          ...
          for (var l in describeRouteTables.data[k].Routes) {
            routesInfo.push({
              ...
              rtVpcId:
                describeRouteTables.data[k].Routes[l].VpcPeeringConnectionId,
            });
          }
        ...
        for (var n in routes) {
          for (var o in routesInfo) {
            if (
              routes[n].rtId == routesInfo[o].rtId &&
              routesInfo[o].rtCB == "0.0.0.0/0" &&
              routesInfo[o].rtIg != "local"
            ) {
        ...
        for (var a in peeringRoutes) {
          for (var b in subnets) {
            if (
              peeringRoutes[a].vpcId == subnets[b].subVpcId &&
              peeringRoutes[a].peeringCidr == subnets[b].subCidr
            ) {
          ...
          for (var d in prvRecord) {
            if (pubRecord[c].peeringId == prvRecord[d].peeringId) {
              ...
              helpers.addResult(
                results,
                2,
                "A route between public and private subnets of different VPCs found, for Subnets: " +
                  pubRecord[c].subnetId +
                  " and " +
                  prvRecord[d].subnetId,
                region,
                "arn:aws:ec2:" +
                  region +
                  ":" +
        ...
        if (!register) {
          helpers.addResult(
            results,
            0,
            "No routes between public and private subnets of different VPCs found",
            region
          );
        }
