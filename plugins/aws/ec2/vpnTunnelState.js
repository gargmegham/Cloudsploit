...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    var enable_vpn_tunnel_state =
      settings.enable_vpn_tunnel_state ||
      this.settings.enable_vpn_tunnel_state.default;

    if (!enable_vpn_tunnel_state || enable_vpn_tunnel_state == "false")
      ...
      function (region, rcb) {
        ...
        if (describeVpnConnections.err || !describeVpnConnections.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for VPN connections: ${helpers.addError(
              describeVpnConnections
            )}`,
            region
          );
        ...
        if (!describeVpnConnections.data.length) {
          helpers.addResult(results, 0, "No VPN connections found", region);
          return rcb();
        }
