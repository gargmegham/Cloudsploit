...
module.exports = {
  ...
  settings: {
    ebs_snapshot_life: {
      ...
      description:
        "EBS volume snapshot will FAIL if its creation date is before this number of days in the past",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: "30",
    },
    ebs_result_limit: {
      ...
      description:
        "Plugin results will become aggregated when this value is breached",
      regex: "^[0-9]*$",
      default: "20",
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      ebs_snapshot_life: parseInt(
        settings.ebs_snapshot_life || this.settings.ebs_snapshot_life.default
      ),
      ebs_result_limit: parseInt(
        settings.ebs_result_limit || this.settings.ebs_result_limit.default
      ),
    };
        ...
        describeSnapshots.data.forEach((snapshot) => {
          ...
          if (Math.abs(difference) > config.ebs_snapshot_life) {
            helpers.addResult(
              results,
              2,
              `EBS snapshots is ${config.ebs_snapshot_life} days old`,
              region,
              resource
            );
          } else {
