...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    let config = {
      ignore_spot_instance_volumes:
        settings.ignore_spot_instance_volumes ||
        this.settings.ignore_spot_instance_volumes.default,
    };
    ...
    let accountId = helpers.addSource(cache, source, [
      "sts",
      "getCallerIdentity",
      acctRegion,
      "data",
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        if (
          !describeSnapshots ||
          describeSnapshots.err ||
          !describeSnapshots.data
        ) {
          helpers.addResult(
            results,
            3,
            `Unable to query for EBS Snapshots: ${helpers.addError(
              describeSnapshots
            )}`,
            region
          );
          ...
          for (var instance of describeInstances.data) {
            for (var entry of instance.Instances) {
              if (
                ...
                entry.InstanceLifecycle.toLowerCase() == "spot"
              )
                spotInstances.push(entry.InstanceId);
            }
          }
        }
        ...
        describeVolumes.data.forEach(function (volume) {
          ...
          if (volume.VolumeId) {
            if (ignoreSpot && volume.Attachments && volume.Attachments.length) {
              let found = volume.Attachments.find(
                (attachment) =>
                  attachment.InstanceId &&
                  !spotInstances.includes(attachment.InstanceId)
              );
              if (!found) return;
            }
