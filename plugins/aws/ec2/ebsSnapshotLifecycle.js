      ...
      function (region, rcb) {
        ...
        if (describeVolumes.err || !describeVolumes.data) {
          helpers.addResult(
            results,
            3,
            `Unable to describe EBS volumes: ${helpers.addError(
              describeVolumes
            )}`,
            region
          );
        ...
        if (
          !describeInstances ||
          describeInstances.err ||
          !describeInstances.data
        ) {
          helpers.addResult(
            results,
            3,
            `Unable to EC2 instances: ${helpers.addError(describeInstances)}`,
            region
          );
          return rcb();
        }
        ...
        if (
          !getLifecyclePolicies ||
          getLifecyclePolicies.err | !getLifecyclePolicies.data
        ) {
          helpers.addResult(
            results,
            3,
            `Unable to query DLM lifecycle policies: ${helpers.addError(
              getLifecyclePolicies
            )}`,
            region
          );
        ...
        getLifecyclePolicies.data.forEach((policy) => {
          ...
          if (
            !getLifecyclePolicy ||
            getLifecyclePolicy.err ||
            !getLifecyclePolicy.data ||
            !getLifecyclePolicy.data.Policy
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to query lifecycle policy: ${helpers.addError(
                getLifecyclePolicy
              )}`,
              region,
              policy.PolicyId
          ...
          if (
            getLifecyclePolicy.data.Policy.State &&
            ...
            if (
              getLifecyclePolicy.data.Policy.PolicyDetails.ResourceTypes &&
              getLifecyclePolicy.data.Policy.PolicyDetails.ResourceTypes.length
            ) {
              if (
                getLifecyclePolicy.data.Policy.PolicyDetails.ResourceTypes.includes(
                  "VOLUME"
                )
              ) {
                getLifecyclePolicy.data.Policy.PolicyDetails.TargetTags.forEach(
                  (tag) => {
                    if (tag.Key && tag.Value) {
                      if (passingVolumeTags[tag.Key])
                        passingVolumeTags[tag.Key].push(tag.Value);
              ...
              if (
                ...
                getLifecyclePolicy.data.Policy.PolicyDetails.TargetTags.forEach(
                  (tag) => {
                    if (tag.Key && tag.Value) {
                      if (passingInstanceTags[tag.Key])
                        passingInstanceTags[tag.Key].push(tag.Value);
              ...
              instance.Instances.forEach((entry) => {
                if (entry.Tags && entry.Tags.length) {
                  for (var it in entry.Tags) {
                    ...
                    if (
                      passingInstanceTags[itag.Key] &&
                      passingInstanceTags[itag.Key].includes(itag.Value)
                    ) {
                      if (
                        entry.BlockDeviceMappings &&
                        entry.BlockDeviceMappings.length
                      ) {
                        entry.BlockDeviceMappings.forEach((mapping) => {
                          if (mapping.Ebs && mapping.Ebs.VolumeId)
                            passingVolumes.push(mapping.Ebs.VolumeId);
                        });
                      }
                      break;
                    }
        ...
        async.each(describeVolumes.data, function (volume, vcb) {
          ...
          if (tagFound) {
            helpers.addResult(
              results,
              0,
              `EBS volume "${volume.VolumeId}" has lifecycle policy configured`,
              region,
              resource
            );
          } else {
            helpers.addResult(
              results,
              2,
              `EBS volume "${volume.VolumeId}" does not have lifecycle policy configured`,
              region,
              resource
            );
          }
