  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      ...
      function (region, rcb) {
          ...
          for (var instance of reservation.Instances) {
            ...
            if (!instance.MetadataOptions) {
              helpers.addResult(
                results,
                3,
                "Unable to get instance metadata options",
                region,
                arn
              );
              continue;
        ...
        var totalCount =
          instancesInsecure.length +
          instancesTokensRequired.length +
          instancesEndpointDisabled.length;
        ...
        if (!totalCount) {
          ...
          for (var iArn of instancesEndpointDisabled) {
            helpers.addResult(
              results,
              0,
              "Instance has instance metadata endpoint disabled",
              region,
              iArn
            );
          }
          ...
          for (var jArn of instancesTokensRequired) {
            helpers.addResult(
              results,
              0,
              "Instance requires tokens for instance metadata endpoint access",
              region,
              jArn
            );
          }
          ...
          for (var kArn of instancesInsecure) {
            helpers.addResult(
              results,
              2,
              "Instance has instance metadata endpoint enabled and does not require HttpTokens",
              region,
              kArn
            );
          }
