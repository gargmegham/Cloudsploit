...
module.exports = {
  ...
  compliance: {
    hipaa:
      "VPC Flow Logs provide a detailed traffic log of a VPC network " +
      "containing HIPAA data. Flow Logs should be enabled to satisfy " +
      "the audit controls of the HIPAA framework.",
    pci:
      "PCI requires logging of all network access to environments containing " +
  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        if (
          !describeFlowLogs ||
          describeFlowLogs.err ||
          !describeFlowLogs.data
        ) {
          helpers.addResult(
            results,
            3,
            "Unable to query for flow logs: " +
              helpers.addError(describeFlowLogs),
            region
          );
          return rcb();
        }
        ...
        for (var f in describeFlowLogs.data) {
          if (
            describeFlowLogs.data[f].ResourceId &&
            vpcMap[describeFlowLogs.data[f].ResourceId]
          ) {
            vpcMap[describeFlowLogs.data[f].ResourceId].push(
              describeFlowLogs.data[f]
            );
          }
        }
          ...
          if (!vpcMap[v].length) {
            ...
            if (activeLogs) {
              ...
              helpers.addResult(
                results,
                2,
                "VPC flow logs are enabled, but not active",
                region,
                resource
              );
            }
