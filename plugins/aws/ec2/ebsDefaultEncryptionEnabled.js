...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    var ebs_encryption_level =
      settings.ebs_encryption_level ||
      this.settings.ebs_encryption_level.default;
    var targetEncryptionLevel =
      helpers.ENCRYPTION_LEVELS.indexOf(ebs_encryption_level);
      ...
      function (region, rcb) {
        ...
        if (getEbsEncryptionByDefault.err) {
          helpers.addResult(
            results,
            3,
            "Unable to query for ebs encryption by default: " +
              helpers.addError(getEbsEncryptionByDefault),
            region
          );
          return rcb();
        }
        ...
        if (
          !getEbsDefaultKmsKeyId ||
          getEbsDefaultKmsKeyId.err ||
          !getEbsDefaultKmsKeyId.data
        ) {
          helpers.addResult(
            results,
            3,
            "Unable to query for ebs default kms key id: " +
              helpers.addError(getEbsDefaultKmsKeyId),
            region
          );
          return rcb();
        }
        ...
        if (getEbsDefaultKmsKeyId.data.split("/")[0] === "alias") {
          ...
          if (!listAliases || listAliases.err || !listAliases.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for list aliases: " +
                helpers.addError(listAliases),
              region
            );
            return rcb();
          }
        ...
        if (!isPredefinedAlias) {
          ...
          encryptionLevel = helpers.getEncryptionLevel(
            describeKey.data.KeyMetadata,
        ...
        if (encryptionLevel < targetEncryptionLevel) {
          helpers.addResult(
            results,
            2,
            `EBS default encryption is enabled but current encryption level ${helpers.ENCRYPTION_LEVELS[encryptionLevel]} is less than the target level ${helpers.ENCRYPTION_LEVELS[targetEncryptionLevel]}`,
            region
          );
        } else {
          helpers.addResult(
            results,
            0,
            `EBS default encryption is enabled and current encryption level ${helpers.ENCRYPTION_LEVELS[encryptionLevel]} is greater than or equal to the target level ${helpers.ENCRYPTION_LEVELS[targetEncryptionLevel]}`,
            region
          );
        }
