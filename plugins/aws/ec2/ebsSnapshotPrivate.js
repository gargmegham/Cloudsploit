...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      regions.support,
      function (region, rcb) {
        var describeTrustedAdvisorChecks = helpers.addSource(cache, source, [
          "support",
        ...
        if (
          !describeTrustedAdvisorChecks ||
          describeTrustedAdvisorChecks.err ||
          !describeTrustedAdvisorChecks.data
        ) {
          ...
          helpers.addResult(
            results,
            3,
            "Unable to query for Trusted Advisor checks: " + errMsg
          );
          return rcb();
        }
        ...
        async.each(
          ...
          function (check, cb) {
            ...
            var describeTrustedAdvisorCheckResult = helpers.addSource(
              cache,
              source,
              ["support", "describeTrustedAdvisorCheckResult", region, check.id]
            );

            if (
              !describeTrustedAdvisorCheckResult ||
              describeTrustedAdvisorCheckResult.err ||
              !describeTrustedAdvisorCheckResult.data
            ) {
              helpers.addResult(
                results,
                3,
                `Unable to query for Trusted Advisor check: "${checkName}" ${helpers.addError(
                  describeTrustedAdvisorCheckResult
                )}`
              );
            ...
            describeTrustedAdvisorCheckResult.data.result.flaggedResources.forEach(
              function (resource) {
                helpers.addResult(
                  results,
                  2,
