...
module.exports = {
  ...
  more_info:
    "AWS Transfer servers can log activity to CloudWatch if a proper IAM service role is provided. This role should be configured for all servers to ensure proper access logging.",
  ...
  compliance: {
    hipaa:
      "HIPAA requires that all data access is audited via proper logging configurations.",
      ...
      function (region, rcb) {
        ...
        if (listServers.err || !listServers.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for Transfer servers: " +
              helpers.addError(listServers),
            region
          );
          return rcb();
        }
        ...
        for (var i in listServers.data) {
          ...
          if (server.LoggingRole && server.LoggingRole.length) {
            helpers.addResult(
              results,
              0,
              "Logging role is properly configured for Transfer server",
              region,
              arn
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Logging role is not configured for Transfer server",
              region,
              arn
            );
          }
