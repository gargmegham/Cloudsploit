...
module.exports = {
  ...
  recommended_action:
    "1. Enter the WAF service. 2. Enter Web ACLs and filter by global. 3. If no Web ACL is found, Create a new global Web ACL and in Resource type to associate with web ACL, select the Cloudfront Distribution. ",
    ...
    listDistributions.data.forEach((distribution) => {
      if (!distribution.WebACLId || distribution.WebACLId === "") {
        helpers.addResult(
          results,
          2,
          "The Cloudfront Distribution does not have WAF enabled",
          "global",
          distribution.ARN
        );
      } else {
        helpers.addResult(
          results,
          0,
          "The Cloudfront Distribution has WAF enabled",
          "global",
          distribution.ARN
        );
      }
