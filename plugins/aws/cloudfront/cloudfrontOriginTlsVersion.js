...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    var results = [];
    var source = {};
    const deprecatedVersion = ["SSLv3", "TLSv1", "TLSv1.1"];
    ...
    for (let distribution of listDistributions.data) {
      ...
      if (
        ...
        helpers.addResult(
          results,
          0,
          "CloudFront distribution has no origins",
          "global",
          distribution.ARN
        );
      } else {
        ...
        for (let origin of origins) {
          if (
            ...
            helpers.addResult(
              results,
              0,
              "CloudFront distribution does not have custom origins or origins do not have SSL protocol items",
              "global",
              distribution.ARN
            );
            continue;
          ...
          if (isDeprecated) {
            helpers.addResult(
              results,
              2,
              "CloudFront distribution custom origin TLS version is deprecated",
              "global",
              distribution.ARN
            );
          } else {
            helpers.addResult(
              results,
              0,
              "CloudFront distribution custom origin TLS version is not deprecated",
              "global",
              distribution.ARN
            );
          }
