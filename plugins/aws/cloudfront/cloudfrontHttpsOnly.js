...
module.exports = {
      ...
      if (
        ...
        helpers.addResult(
          ...
          "CloudFront distribution " +
            "is configured to redirect non-HTTPS traffic to HTTPS",
          "global",
          Distribution.ARN
        );
      } else if (
        ...
        helpers.addResult(
          results,
          0,
          "The CloudFront " + "distribution is set to use HTTPS only.",
          "global",
          Distribution.ARN
        );
      } else {
        helpers.addResult(
          results,
          2,
          "CloudFront distribution " + "is not configured to use HTTPS",
          "global",
          Distribution.ARN
        );
      }
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    var distributionName =
      distributionNameArr[distributionNameArr.length - 1].split("/");
    ...
    if (
      ...
      if (settings.input && settings.input.cdnPolicyOption)
        params["DistributionConfig"]["ViewerProtocolPolicy"] =
          settings.input.cdnPolicyOption;
      else
