...
module.exports = {
  ...
  more_info:
    " With Origin Failover capability, you can setup two origins for your CloudFront web distributions primary and secondary. In the event of primary origin failure, " +
    ...
    listDistributions.data.forEach((distribution) => {
      if (distribution.OriginGroups && distribution.OriginGroups.Quantity) {
        helpers.addResult(
          results,
          0,
          "CloudFront distribution have origin failover enabled.",
          "global",
          distribution.ARN
        );
      } else {
        helpers.addResult(
          results,
          2,
          "CloudFront distribution does not have origin failover enabled.",
          "global",
          distribution.ARN
        );
      }
