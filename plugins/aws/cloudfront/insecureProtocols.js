...
module.exports = {
  ...
  compliance: {
    hipaa:
      "The transmission security aspect of HIPAA requires communication containing " +
      "sensitive data to be transmitted over secure connections. CloudFront " +
      "protocols must be up-to-date to avoid data exposure.",
    pci:
      "PCI requires secure transfer of cardholder data. It does not permit SSL or TLS " +
  ...
  settings: {
    insecure_cloudfront_ignore_default: {
      ...
      description:
        "When set to true, the use of the default CloudFront certificate is ignored, despite using an insecure TLS protocol.",
      regex: "^(true|false)$",
      default: "true",
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      insecure_cloudfront_ignore_default:
        settings.insecure_cloudfront_ignore_default ||
        this.settings.insecure_cloudfront_ignore_default.default,
    };

    config.insecure_cloudfront_ignore_default =
      config.insecure_cloudfront_ignore_default == "true";
      ...
      function (distribution, cb) {
        if (
          ...
          helpers.addResult(
            results,
            0,
            "Distribution is not configured for SSL delivery",
            "global",
            distribution.ARN
          );
          return cb();
        ...
        if (distribution.ViewerCertificate.CloudFrontDefaultCertificate) {
          if (config.insecure_cloudfront_ignore_default) {
            helpers.addResult(
              results,
              0,
              "Distribution is using secure default certificate",
              "global",
              distribution.ARN,
              custom
            );
            ...
            helpers.addResult(
              results,
              2,
              "Distribution is using the insecure default CloudFront TLS certificate",
              "global",
              distribution.ARN,
              custom
            );
          ...
          distribution.ViewerCertificate.MinimumProtocolVersion === "SSLv3"
        ) {
          helpers.addResult(
            results,
            2,
            "Distribution is using insecure SSLv3",
            "global",
            distribution.ARN
          );
        } else if (
          distribution.ViewerCertificate.MinimumProtocolVersion === "TLSv1"
        ) {
          helpers.addResult(
            results,
            2,
            "Distribution is using insecure TLSv1.0",
            "global",
            distribution.ARN
          );
        } else if (
          distribution.ViewerCertificate.MinimumProtocolVersion === "TLSv1_2016"
        ) {
          helpers.addResult(
            results,
            2,
            "Distribution is using insecure TLSv1_2016",
            "global",
            distribution.ARN
          );
        } else if (
          ...
          helpers.addResult(
            results,
            0,
            "Distribution is using secure TLSv1.1_2016",
            "global",
            distribution.ARN
          );
        } else {
          helpers.addResult(
            results,
            0,
            "Distribution is using secure TLSv1.2_2018",
            "global",
            distribution.ARN
          );
        }
