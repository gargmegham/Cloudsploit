      ...
      if (
        ...
        for (let origin of items) {
          ...
          let originProtocolPolicy =
            origin.CustomOriginConfig.OriginProtocolPolicy.toLowerCase();
        ...
        if (!isCorrectPolicy) {
          helpers.addResult(
            results,
            2,
            "CloudFront distribution custom origin is not configured to use HTTPS only",
            "global",
            distribution.ARN
          );
        } else {
          helpers.addResult(
            results,
            0,
            "CloudFront distribution custom origin is configured to use HTTPS only",
            "global",
            distribution.ARN
          );
        }
