...
module.exports = {
  ...
  more_info:
    "Logging requests to CloudFront " +
  ...
  compliance: {
    hipaa:
      "As part of the audit control requirement for HIPAA, request logging for " +
      "CloudFront should be enabled and logs should be stored in a secure location " +
      "with proper retention policies.",
    pci:
      "Request logging for networks hosting cardholder data is required " +
    ...
    listDistributions.data.forEach(function (Distribution) {
      ...
      if (
        ...
        helpers.addResult(
          results,
          3,
          `Unable to get CloudFront distribution: ${helpers.addError(
            getDistribution
          )}`
        );
        return;
