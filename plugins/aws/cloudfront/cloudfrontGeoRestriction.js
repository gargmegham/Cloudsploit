...
module.exports = {
  ...
  settings: {
    cloudfront_whitelisted_geo_locations: {
      ...
      description:
        "A comma-delimited list of shorthands for geographic locations which should be allowed within CloudFront distribution geographic restriction settings, i.e. AR=Argentine, EG=Egypt",
      regex: "[a-zA-Z0-9-,]",
      default: "",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      cloudfront_whitelisted_geo_locations:
        settings.cloudfront_whitelisted_geo_locations ||
        this.settings.cloudfront_whitelisted_geo_locations.default,
    };

    config.cloudfront_whitelisted_geo_locations =
      config.cloudfront_whitelisted_geo_locations
        .toUpperCase()
        .replace(/\s/g, "");
    ...
    listDistributions.data.forEach((distribution) => {
      if (
        ...
        distribution.Restrictions.GeoRestriction.RestrictionType.toLowerCase() ===
          "none"
      ) {
        helpers.addResult(
          results,
          2,
          "Geo restriction feature is not enabled within CloudFront distribution",
          "global",
          distribution.ARN
        );
      } else if (config.cloudfront_whitelisted_geo_locations.length) {
        config.cloudfront_whitelisted_geo_locations =
          config.cloudfront_whitelisted_geo_locations.split(",");
        if (
          distribution.Restrictions.GeoRestriction.RestrictionType.toLowerCase() ===
          "whitelist"
        ) {
          ...
          let missedLocations =
            config.cloudfront_whitelisted_geo_locations.filter(
              (location) => !items.includes(location)
            );
          ...
          let blockedLocations =
            config.cloudfront_whitelisted_geo_locations.filter((location) =>
        ...
        helpers.addResult(
          results,
          0,
          "Geo restriction feature is enabled within CloudFront distribution",
          "global",
          distribution.ARN
        );
      }
