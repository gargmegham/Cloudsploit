...
module.exports = {
  ...
  compliance: {
    hipaa:
      "HIPAA requires that all traffic transmitted over the network is encrypted " +
          ...
          if (
            ...
            if (
              ...
              helpers.addResult(
                results,
                0,
                "CloudFront origin: " + origin.Id + " is using https-only",
                "global",
                distribution.ARN
              );
            } else if (
              ...
              helpers.addResult(
                results,
                1,
                "CloudFront origin: " + origin.Id + " is using match-viewer",
                "global",
                distribution.ARN
              );
            } else if (
              ...
              helpers.addResult(
                results,
                2,
                "CloudFront origin: " + origin.Id + " is using http-only",
                "global",
                distribution.ARN
              );
            }
            ...
            if (
              ...
              if (
                protocols.indexOf("SSLv3") > -1 &&
                protocols.indexOf("TLSv1") > -1
              ) {
                helpers.addResult(
                  results,
                  2,
                  "CloudFront origin: " +
                    origin.Id +
                    " is using SSLv3 and TLSv1 protocols",
                  "global",
                  distribution.ARN
                );
      ...
      function () {
        if (!found) {
          helpers.addResult(
            results,
            0,
            "No CloudFront origins without HTTPS or with insecure protocols found"
          );
        }
