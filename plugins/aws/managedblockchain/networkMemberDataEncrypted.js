  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.blockchain_member_encryption_level ||
        this.settings.blockchain_member_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listNetworks.err || !listNetworks.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for Managed Blockchain networks: ${helpers.addError(
              listNetworks
            )}`,
            region
          );
        ...
        if (!listNetworks.data.length) {
          helpers.addResult(
            results,
            0,
            "No Managed Blockchain networks found",
            region
          );
          return rcb();
        }
        ...
        for (let network of listNetworks.data) {
          ...
          if (
            ...
            helpers.addResult(
              results,
              3,
              `Unable to query network members: ${helpers.addError(
                listMembers
              )}`,
              region,
              network.Arn
          ...
          for (let member of listMembers.data.Members) {
            ...
            if (
              ...
              helpers.addResult(
                results,
                3,
                `Unable to query network member: ${helpers.addError(
                  getMember
                )}`,
                region,
                member.Arn
            ...
            if (getMember.data.Member.KmsKeyArn) {
              if (getMember.data.Member.KmsKeyArn === "AWS_OWNED_KMS_KEY") {
                ...
                var kmsKeyId = getMember.data.Member.KmsKeyArn.split("/")[1]
                  ? getMember.data.Member.KmsKeyArn.split("/")[1]
                  : getMember.data.Member.KmsKeyArn;
