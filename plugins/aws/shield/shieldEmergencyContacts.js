...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    if (!describeEmergencyContactSettings)
      return callback(null, results, source);

    if (
      describeEmergencyContactSettings.err &&
    ...
    if (
      ...
      helpers.addResult(
        results,
        3,
        "Unable to query for Shield emergency contacts: " +
          helpers.addError(describeEmergencyContactSettings)
      );
      return callback(null, results, source);
    }
    ...
    if (!describeEmergencyContactSettings.data.length) {
      helpers.addResult(
        results,
        2,
        "Shield emergency contacts are not configured"
      );
    } else {
      helpers.addResult(
        results,
        0,
        "Shield emergency contacts are configured with: " +
          describeEmergencyContactSettings.data.length +
          " contacts"
      );
    }
