  ...
  run: function (cache, settings, callback) {
    ...
    if (describeSubscription.err || !describeSubscription.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query for Shield subscription: " +
          helpers.addError(describeSubscription)
      );
      return callback(null, results, source);
    }
    ...
    var now = new Date();
    var renewing =
      describeSubscription.data.AutoRenew &&
      describeSubscription.data.AutoRenew == "ENABLED";
    ...
    if (daysBetween <= 90 && !renewing) {
      helpers.addResult(
        results,
        2,
        "Shield subscription is expiring in " +
          daysBetween +
          " days and is not configured to auto-renew"
      );
    } else if (!renewing) {
      helpers.addResult(
        results,
      ...
      helpers.addResult(
        results,
        0,
        "Shield subscription is enabled, expiring in " +
          daysBetween +
          " days and is configured to auto-renew"
      );
    }
