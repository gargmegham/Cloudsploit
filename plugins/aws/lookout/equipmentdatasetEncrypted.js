  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.equipment_dataset_desired_encryption_level ||
        this.settings.equipment_dataset_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listDatasets.err || !listDatasets.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query Lookout for Equipment Dataset: " +
              helpers.addError(listDatasets),
            region
          );
          return rcb();
        }
        ...
        if (!listDatasets.data.length) {
          helpers.addResult(
            results,
            0,
            "No Lookout for Equipment Datasets found",
            region
          );
          return rcb();
        }
        ...
        for (let dataset of listDatasets.data) {
          ...
          if (
            ...
            helpers.addResult(
              results,
              3,
              `Unable to get Lookout for Equipment dataset: ${helpers.addError(
                describeDataset
              )}`,
              region,
              resource
