...
const listAnomalyDetectors = [
  {
    AnomalyDetectorArn:
      "arn:aws:lookoutmetrics:us-east-1:000011112222:AnomalyDetector:sadeed1",
  ...
  {
    AnomalyDetectorArn:
      "arn:aws:lookoutmetrics:us-east-1:000011112222:AnomalyDetector:sadeed2",
...
const createCache = (
  ...
  keysErr,
  describeKeyErr,
  describeAnomalyDetectorErr
) => {
  var keyId = keys && keys.length ? keys[0].KeyId : null;
  var detectorArn =
    detectors && detectors.length ? detectors[0].AnomalyDetectorArn : null;
  return {
    lookoutmetrics: {
  ...
  describe("run", function () {
    it("should PASS if LookoutMetrics Anomaly Detector is encrypted with desired encryption level", function (done) {
      const cache = createCache(
        [listAnomalyDetectors[0]],
        listKeys,
        describeAnomalyDetector[0],
        describeKey[0]
      );
    ...
    it("should FAIL if LookoutMetrics Anomaly Detector is not encrypted with desired encryption level", function (done) {
      const cache = createCache(
        [listAnomalyDetectors[1]],
        listKeys,
        describeAnomalyDetector[1],
        describeKey[1]
      );
      anomalyDetectorEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if unable to list LookoutMetrics Anomaly Detectors", function (done) {
      const cache = createCache(null, null, null, {
        message: "Unable to list LookoutMetrics Anomaly Detectors",
      });
      anomalyDetectorEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if unable to list KMS keys", function (done) {
      const cache = createCache(listAnomalyDetectors, null, null, null, {
