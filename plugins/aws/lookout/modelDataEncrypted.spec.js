...
const createCache = (
  ...
  var projectName =
    listProjects && listProjects.length ? listProjects[0].ProjectName : null;
  var modelArn =
    listModels && listModels.length ? listModels[0].ModelArn : null;
  return {
    lookoutvision: {
      listProjects: {
        "us-east-1": {
...
describe("modelDataEncrypted", function () {
  describe("run", function () {
    it("should PASS if LookoutVision model data is encrypted with desired level", function (done) {
      ...
      modelDataEncrypted.run(
        cache,
        { model_data_desired_encryption_level: "awscmk" },
        (err, results) => {
    ...
    it("should UNKNOWN if unable to list LookoutVision projects", function (done) {
      const cache = createCache(null, null, null, null, {
        message: "Unable to list LookoutVision model",
      });
      modelDataEncrypted.run(
        cache,
        { model_data_desired_encryption_level: "awscmk" },
    ...
    it("should UNKNOWN if unable to list LookoutVision models", function (done) {
      const cache = createCache(
        ...
        null,
        null,
        { message: "Unable to query LookoutVision models" }
      );
      modelDataEncrypted.run(
        cache,
        { model_data_desired_encryption_level: "awscmk" },
