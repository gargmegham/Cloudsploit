  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.model_data_desired_encryption_level ||
        this.settings.model_data_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listProjects.err || !listProjects.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for Lookout for Vision projects: " +
              helpers.addError(listProjects),
            region
          );
          return rcb();
        }
        ...
        if (!listProjects.data.length) {
          helpers.addResult(
            results,
            0,
            "No Lookout for Vision projects found",
            region
          );
          return rcb();
        }
        ...
        for (let project of listProjects.data) {
          ...
          if (!listModels || listModels.err || !listModels.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for Lookout for Vision models: " +
                project.ProjectName +
                ": " +
                helpers.addError(listModels),
              region,
              projectArn
            );
            continue;
          ...
          for (let model of listModels.data.Models) {
            ...
            if (
              ...
              describeModel.err ||
              !describeModel.data ||
              !describeModel.data.ModelDescription
            ) {
              helpers.addResult(
                results,
                3,
                "Unable to get Lookout for Vision models: " +
                  helpers.addError(describeModel),
                region,
                resource
              );
              continue;
