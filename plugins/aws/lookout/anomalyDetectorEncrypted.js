...
module.exports = {
  ...
  more_info:
    "Amazon Lookout for Metrics encrypts your data at rest with your choice of an encryption key. If you do not specify an encryption key, your data is encrypted with AWS owned key by default. " +
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.lookoutmetrics_anomalydetectors_desired_encryption_level ||
      ...
      function (region, rcb) {
        ...
        if (listAnomalyDetectors.err || !listAnomalyDetectors.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query LookoutMetrics Anomaly Detector: " +
              helpers.addError(listAnomalyDetectors),
            region
          );
          return rcb();
        }
        ...
        if (!listAnomalyDetectors.data.length) {
          helpers.addResult(
            results,
            0,
            "No LookoutMetrics Anomaly Detectors found",
            region
          );
          return rcb();
        }
        ...
        for (let detector of listAnomalyDetectors.data) {
          ...
          var describeAnomalyDetector = helpers.addSource(cache, source, [
            ...
            region,
            detector.AnomalyDetectorArn,
          ]);

          if (
            !describeAnomalyDetector ||
            describeAnomalyDetector.err ||
            !describeAnomalyDetector.data
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to get LookoutMetrics Anomaly Detector: ${helpers.addError(
                describeAnomalyDetector
              )}`,
              region,
              resource
