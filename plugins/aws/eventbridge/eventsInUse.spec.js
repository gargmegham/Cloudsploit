...
const listRules = [
  {
    ...
    EventPattern:
      '{"source":["aws.ec2"],"detail-type":["EC2 Instance Rebalance Recommendation","EC2 Spot Instance Interruption Warning"]}',
    State: "ENABLED",
    Description:
      "This rule is used to route Instance notifications to EC2 Auto Scaling",
  ...
  describe("run", function () {
    ...
    it("should UNKNOWN if Unable to list EventBridge events rules", function (done) {
      const cache = createCache(null, {
        message: "Unable to list EventBridge event rules",
      });
      eventsInUse.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        ...
        expect(results[0].region).to.equal("us-east-1");
        expect(results[0].message).to.include(
          "Unable to list EventBridge event rules"
        );
        done();
      });
    });
