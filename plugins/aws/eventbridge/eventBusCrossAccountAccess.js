...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    var config = {
      eventbridge_whitelisted_aws_account_principals:
        settings.eventbridge_whitelisted_aws_account_principals ||
        this.settings.eventbridge_whitelisted_aws_account_principals.default,
      eventbridge_whitelisted_aws_account_principals_regex:
        settings.eventbridge_whitelisted_aws_account_principals_regex ||
        this.settings.eventbridge_whitelisted_aws_account_principals_regex
          .default,
      eventbridge_whitelist_aws_organization_accounts:
        settings.eventbridge_whitelist_aws_organization_accounts ||
        this.settings.eventbridge_whitelist_aws_organization_accounts.default,
      eventbridge_policy_condition_keys:
        settings.eventbridge_policy_condition_keys ||
        this.settings.eventbridge_policy_condition_keys.default,
    };
    var allowedConditionKeys =
      config.eventbridge_policy_condition_keys.split(",");
    var makeRegexBased = config
      .eventbridge_whitelisted_aws_account_principals_regex.length
      ? true
      : false;
    var whitelistOrganization =
      config.eventbridge_whitelist_aws_organization_accounts == "true";
    config.eventbridge_whitelisted_aws_account_principals_regex = new RegExp(
      config.eventbridge_whitelisted_aws_account_principals_regex
    );
    var results = [];
    var source = {};
      ...
      function (region, rcb) {
        ...
        if (listEventBuses.err || !listEventBuses.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for Event Bus: ${helpers.addError(
              listEventBuses
            )}`,
            region
          );
        ...
        listEventBuses.data.forEach((eventBus) => {
          ...
          if (!eventBus.Policy) {
            helpers.addResult(
              results,
              0,
              "Event bus does not use custom policy",
              region,
              eventBus.Arn
            );
            return;
          ...
          statements.forEach((statement) => {
            ...
            if (
              ...
              crossAccountPrincipals.forEach((principal) => {
                ...
                if (makeRegexBased) {
                  if (
                    !config.eventbridge_whitelisted_aws_account_principals_regex.test(
                      principal
                    ) &&
                  ...
                  !config.eventbridge_whitelisted_aws_account_principals.includes(
                    principal
                  ) &&
                  !restrictedAccountPrincipals.includes(principal)
                )
          ...
          if (crossAccountEventBus && !restrictedAccountPrincipals.length) {
            helpers.addResult(
              results,
              0,
              "Event bus contains trusted account principals only",
              region,
              eventBus.Arn
            );
            ...
            helpers.addResult(
              results,
              0,
              "Event bus does not contain cross-account policy statement",
              region,
              eventBus.Arn
            );
          }
