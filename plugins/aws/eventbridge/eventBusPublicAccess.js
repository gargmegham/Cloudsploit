...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      event_bus_policy_condition_keys:
        settings.event_bus_policy_condition_keys ||
        this.settings.event_bus_policy_condition_keys.default,
    };
    config.event_bus_policy_condition_keys =
      config.event_bus_policy_condition_keys.replace(/\s/g, "");
    var allowedConditionKeys =
      config.event_bus_policy_condition_keys.split(",");
      ...
      function (region, rcb) {
        ...
        if (listEventBuses.err || !listEventBuses.data) {
          helpers.addResult(
            results,
            3,
            "Unable to list event bus: " + helpers.addError(listEventBuses),
            region
          );
          return rcb();
        }
