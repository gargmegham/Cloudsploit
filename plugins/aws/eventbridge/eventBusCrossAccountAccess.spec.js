...
const listEventBuses = [
  {
    ...
    Policy:
      '{"Version":"2012-10-17","Statement":[{"Sid":"allow_account_to_put_events","Effect":"Allow","Principal":{"AWS":"arn:aws:iam::211111111111:user/y"},"Action":"events:PutEvents","Resource":"arn:aws:events:us-east-1:111111111111:event-bus/test-bus"}]}',
  },
  {
    Name: "test-bus",
    Arn: "arn:aws:events:us-east-1:111111111111:event-bus/test-bus",
    Policy:
      '{"Version":"2012-10-17","Statement":[{"Sid":"allow_account_to_put_events","Effect":"Allow","Principal":{"AWS":"arn:aws:iam::111111111111:user/x"},"Action":"events:PutEvents","Resource":"arn:aws:events:us-east-1:111111111111:event-bus/test-bus"}]}',
  },
  {
    Name: "test-bus",
    Arn: "arn:aws:events:us-east-1:111111111111:event-bus/test-bus",
...
describe("eventBusCrossAccountAccess", function () {
  describe("run", function () {
    it("should PASS if Event Bus has cross-account access policy attached", function (done) {
      ...
      eventBusCrossAccountAccess.run(
        cache,
        {
    ...
    it("should PASS if cross-account role contains organization account ID and setting to allow organization account is true", function (done) {
      ...
      eventBusCrossAccountAccess.run(
        cache,
        { eventbridge_whitelist_aws_organization_accounts: "true" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          done();
    ...
    it("should UNKNOWN if unable to query event bus", function (done) {
      const cache = createCache(
        [],
        {},
        { message: "Unable to list event bus" }
      );
      eventBusCrossAccountAccess.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
