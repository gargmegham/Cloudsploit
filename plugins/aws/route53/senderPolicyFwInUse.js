  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      ...
      function (zone, cb) {
        ...
        let enabled = listResourceRecordSets.data.ResourceRecordSets.find(
          (recordSet) =>
            recordSet.Type &&
            recordSet.Type.toUpperCase() == "TXT" &&
            recordSet.ResourceRecords.find(
              (record) =>
                record.Value && record.Value.toLowerCase().includes("v=spf1")
            )
        );
