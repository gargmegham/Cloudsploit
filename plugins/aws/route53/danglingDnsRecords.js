...
module.exports = {
  ...
  settings: {
    dns_allow_private_ips: {
      ...
      description:
        "When true, allows Route53 DNS records to point to IP addresses outside AWS",
      regex: "^(true|false)$",
      default: "false",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      dns_allow_private_ips:
        settings.dns_allow_private_ips ||
        this.settings.dns_allow_private_ips.default,
    };
    ...
    if (listHostedZones.err || !listHostedZones.data) {
      helpers.addResult(
        results,
        3,
        `Unable to query for hosted zones: ${helpers.addError(
          listHostedZones
        )}`,
        region
      );
      ...
      async.each(helpers.regions(settings).ec2, function (region, rcb) {
        ...
        if (describeAddresses.err || !describeAddresses.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for elastic IP addresses: ${helpers.addError(
              describeAddresses
            )}`,
            region
          );
    ...
    async.each(
      ...
      function (zone, cb) {
        ...
        if (
          !listResourceRecordSets ||
          listResourceRecordSets.err ||
          !listResourceRecordSets.data
        ) {
          helpers.addResult(
            results,
            3,
            `Unable to query for resource record sets: ${helpers.addError(
              listResourceRecordSets
            )}`,
            region,
            resource
        ...
        if (
          !listResourceRecordSets.data.ResourceRecordSets ||
          !listResourceRecordSets.data.ResourceRecordSets.length
        ) {
          helpers.addResult(
            results,
            0,
        ...
        listResourceRecordSets.data.ResourceRecordSets.forEach((recordSet) => {
          if (
            ...
            recordSet.ResourceRecords.length
          ) {
            recordSet.ResourceRecords.forEach((record) => {
              if (
                !allowPrivateIps &&
                record.Value &&
                !addresses.includes(record.Value)
              ) {
          ...
          if (
            ...
            recordSet.Type === "A" &&
            recordSet.Name &&
            !bucketNames.includes(recordSet.Name) &&
            recordSet.AliasTarget &&
            recordSet.AliasTarget.DNSName &&
            recordSet.AliasTarget.DNSName.startsWith("s3-website")
          ) {
        ...
        if (!danglingDnsRecords.length) {
          ...
          helpers.addResult(
            ...
            `Hosted Zone "${
              zone.Name
            }" has these dangling DNS records: ${danglingDnsRecords.join(
              ", "
            )}`,
            region,
            resource
