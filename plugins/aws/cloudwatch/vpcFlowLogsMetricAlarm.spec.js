...
const describeAlarms = [
  {
    ...
    AlarmArn:
      "arn:aws:cloudwatch:us-east-1:000011112222:alarm:vpc_flow_log_alarm",
  ...
  },
  {
    ...
    AlarmArn:
      "arn:aws:cloudwatch:us-east-1:000011112222:alarm:vpd_flow_log_alarm",
    AlarmDescription: "Triggered by 'REJECT' packets.",
    AlarmConfigurationUpdatedTimestamp: "2022-02-28T16:16:32.485000+00:00",
    ActionsEnabled: true,
    OKActions: [],
  ...
  describe("run", function () {
    it("should PASS if CloudWatch alarm is configured for the VPC Flow Logs", function (done) {
      const cache = createCache(
        [describeMetricFilters[0]],
        [describeAlarms[0]]
      );
      vpcFlowLogsMetricAlarm.run(
        cache,
        { vpc_flow_log_group: "vpc_flow_log_group_name" },
        (err, results) => {
    ...
    it("should UNKNOWN if Unable to describe CloudWatch logs metric filters", function (done) {
      ...
      vpcFlowLogsMetricAlarm.run(
        ...
        (err, results) => {
          ...
          expect(results[0].status).to.equal(3);
          expect(results[0].message).to.include(
            "Unable to describe CloudWatch logs metric filters"
          );
          done();
        }
      );
