...
module.exports = {
  ...
  settings: {
    vpc_flow_log_group: {
      ...
      description:
        "Existing CloudWatch log group name created to log VPC flow logs",
      regex: "^.*$",
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      vpc_flow_log_group:
        settings.vpc_flow_log_group || this.settings.vpc_flow_log_group.default,
    };

    if (!config.vpc_flow_log_group.length)
      ...
      function (region, rcb) {
        ...
        if (describeMetricFilters.err || !describeMetricFilters.data) {
          helpers.addResult(
            results,
            3,
            `Unable to describe CloudWatch logs metric filters: ${helpers.addError(
              describeMetricFilters
            )}`,
            region
          );
        ...
        if (!describeMetricFilters.data.length) {
          helpers.addResult(
            results,
            2,
            "No CloudWatch logs metric filters found",
            region
          );
          return rcb();
        }

        let cwVpcLogGroup = describeMetricFilters.data.find(
          (metrics) => metrics.logGroupName === config.vpc_flow_log_group
        );
        ...
        if (!cwVpcLogGroup) {
          helpers.addResult(
            results,
            2,
            "Unable to locate the specified log group",
            region
          );
          return rcb();
        }
        ...
        let metricTransformations =
          ...
          cwVpcLogGroup.metricTransformations.length
            ? cwVpcLogGroup.metricTransformations.map(
                (transformation) => transformation.metricName
              )
            : [];
        ...
        if (!describeAlarms || describeAlarms.err || !describeAlarms.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for CloudWatch metric alarms: " +
              helpers.addError(describeAlarms),
            region
          );
          return rcb();
        }
        ...
        if (!describeAlarms.data.length) {
          helpers.addResult(
            results,
            2,
            "No CloudWatch metric alarms found",
            region
          );
          return rcb();
        }

        let metricAlarm = describeAlarms.data.find((alarm) =>
          metricTransformations.includes(alarm.MetricName)
        );

        if (
          metricAlarm &&
          metricAlarm.AlarmActions &&
          metricAlarm.AlarmActions.length
        ) {
          helpers.addResult(
            results,
            0,
