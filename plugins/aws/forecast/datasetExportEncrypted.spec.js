...
const listForecastExportJobs = [
  {
    ForecastExportJobArn:
      "arn:aws:forecast:us-east-1:111122223333:forecast-export-job/test_fc/ewd",
    ForecastExportJobName: "ewd",
    Destination: {
      S3Config: {
        Path: "s3://amazon-connect-5bc142a71067/data",
        RoleArn:
          "arn:aws:iam::111122223333:role/service-role/AmazonForecast-ExecutionRole-1637334836508",
      },
    },
    Status: "ACTIVE",
  ...
  {
    ForecastExportJobArn:
      "arn:aws:forecast:us-east-1:111122223333:forecast-export-job/test_fc/samxbqwd",
  ...
  describe("run", function () {
    it("should PASS if Forecast Dataset Export is encrypted with desired encryption level", function (done) {
      const cache = createCache(
        [listForecastExportJobs[1]],
        listKeys,
        describeKey[0]
      );
      datasetExportEncrypted.run(
        cache,
        { forecast_dataset_desired_encryption_level: "awscmk" },
        (err, results) => {
