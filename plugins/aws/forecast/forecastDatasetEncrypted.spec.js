...
const describeDataset = [
  {
    ...
    EncryptionConfig: {
      RoleArn:
        "arn:aws:iam::000011112222:role/service-role/AmazonForecast-ExecutionRole-1637334836508",
      KMSKeyArn:
...
describe("forecastDatasetEncrypted", function () {
  describe("run", function () {
      ...
      forecastDatasetEncrypted.run(
        ...
        (err, results) => {
          ...
          expect(results[0].message).to.include(
            "Forecast dataset is encrypted with awscmk"
          );
          done();
        }
      );
    ...
    it("should FAIL if Forecast dataset is not encrypted with desired KMS key", function (done) {
      ...
      forecastDatasetEncrypted.run(
        cache,
        { forecast_dataset_desired_encryption_level: "externalcmk" },
        (err, results) => {
          expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if unable to list Forecast datasets", function (done) {
      ...
      forecastDatasetEncrypted.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Unable to query Forecast datasets"
        );
