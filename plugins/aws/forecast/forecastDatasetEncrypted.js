  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.forecast_dataset_desired_encryption_level ||
        this.settings.forecast_dataset_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listDatasets.err || !listDatasets.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query Forecast datasets: ${helpers.addError(
              listDatasets
            )}`,
            region
          );
        ...
        for (let dataset of listDatasets.data) {
          ...
          if (
            ...
            helpers.addResult(
              results,
              3,
              `Unable to query Forecast dataset: ${helpers.addError(
                describeDataset
              )}`,
              region,
              resource
          ...
          if (
            describeDataset.data.EncryptionConfig &&
            ...
            helpers.addResult(
              results,
              2,
              "Forecast dataset does not have encryption enabled",
              region,
              resource
            );
          }
