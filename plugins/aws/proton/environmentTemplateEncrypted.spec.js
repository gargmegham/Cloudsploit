  ...
  describe("run", function () {
    it("should PASS if Proton environment template is encrypted with desired encryption level", function (done) {
      const cache = createCache(
        [listEnvironmentTemplates[0]],
        listKeys,
        getEnvironmentTemplate[0],
        describeKey[0]
      );
      environmentTemplateEncrypted.run(
        cache,
        { proton_environmenttemplate_desired_encryption_level: "awscmk" },
        (err, results) => {
    ...
    it("should UNKNOWN if unable to list Proton environment template", function (done) {
      const cache = createCache(null, null, null, {
        message: "Unable to list Proton environment template",
      });
      environmentTemplateEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if unable to list KMS keys", function (done) {
      const cache = createCache(listEnvironmentTemplates, null, null, null, {
