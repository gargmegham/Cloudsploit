  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.proton_environmenttemplate_desired_encryption_level ||
      ...
      function (region, rcb) {
        ...
        if (listEnvironmentTemplates.err || !listEnvironmentTemplates.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query Environment Template: " +
              helpers.addError(listEnvironmentTemplates),
            region
          );
          return rcb();
        }
        ...
        for (let template of listEnvironmentTemplates.data) {
          ...
          if (
            !getEnvironmentTemplate ||
            getEnvironmentTemplate.err ||
            !getEnvironmentTemplate.data ||
            !getEnvironmentTemplate.data.environmentTemplate
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to get Environment Template description: ${helpers.addError(
                getEnvironmentTemplate
              )}`,
              region,
              resource
          ...
          if (getEnvironmentTemplate.data.environmentTemplate.encryptionKey) {
            var encryptionKey =
              getEnvironmentTemplate.data.environmentTemplate.encryptionKey;
