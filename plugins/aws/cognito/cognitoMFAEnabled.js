...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      regions.cognitoidentityserviceprovider,
      function (region, rcb) {
        ...
        if (userPools.err || !userPools.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query Cognito user pools: " +
              helpers.addError(userPools),
            region
          );
          return rcb();
        }
        ...
        for (let userPool of userPools.data) {
          ...
          if (
            !describeUserPool ||
            describeUserPool.err ||
            !describeUserPool.data ||
            !describeUserPool.data.UserPool
          ) {
            helpers.addResult(
              results,
              3,
              "Unable to describe Cognito user pool: " +
                helpers.addError(describeUserPool),
              region,
              userPoolArn
            );
            continue;
          ...
          if (
            describeUserPool.data.UserPool.MfaConfiguration &&
            describeUserPool.data.UserPool.MfaConfiguration.toUpperCase() ==
              "ON"
          ) {
            helpers.addResult(
              results,
              0,
              "User pool has MFA enabled",
              region,
              userPoolArn
            );
          } else {
            helpers.addResult(
              results,
              2,
              "User pool does not have MFA enabled",
              region,
              userPoolArn
            );
          }
