...
module.exports = {
  ...
  apis: [
    ...
    "WAFV2:getWebACLForCognitoUserPool",
        ...
        for (let userPool of userPools.data) {
          ...
          if (!webACLResource || webACLResource.err || !webACLResource.data) {
            helpers.addResult(
              results,
              3,
              "Unable to get WebACL resource for cognito user pool: " +
                helpers.addError(webACLResource),
              region,
              arn
            );
            continue;
          ...
          if (webACLResource.data.WebACL) {
            ...
            helpers.addResult(
              results,
              2,
              "User pool does not have WAFV2 enabled",
              region,
              arn
            );
          }
