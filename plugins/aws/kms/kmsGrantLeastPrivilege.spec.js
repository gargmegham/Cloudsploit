...
const listGrants = [
  [
    {
      ...
      GrantId:
        "02c191300546eb259de3e7c4c29be970bdc8b0a209af7d27581d87df37310068",
      Name: "Allperm",
    ...
    },
    {
      KeyId:
        "arn:aws:kms:us-east-1:000011112222:key/ad013a33-b01d-4d88-ac97-127399c18b3e",
      GrantId:
        "04df4a33699d9a76c3baed37ea9b95ee25328236952c6833810cfae0a751686e",
    ...
    },
    {
      KeyId:
        "arn:aws:kms:us-east-1:000011112222:key/ad013a33-b01d-4d88-ac97-127399c18b3e",
      GrantId:
        "0978f48c86e449fc2c64f2440313813393909e43963ec3a2e457bbfe0e38b19d",
  ...
  [
    {
      KeyId:
        "arn:aws:kms:us-east-1:000011112222:key/fb8ab834-47f3-4434-810a-e9cb1634de69",
      GrantId:
        "1521390bb3dcbdf8a5754cffcd2f3ce45e5f15d724d70f11ca53ccbfa9bfda6e",
      Name: "",
    ...
    },
    {
      KeyId:
        "arn:aws:kms:us-east-1:000011112222:key/fb8ab834-47f3-4434-810a-e9cb1634de69",
      GrantId:
        "a752caf60c95a013f6318e0f9d1f3a8a202f14f9ff9569af4bd338eddc1f19c5",
      Name: "",
  ...
  describe("run", function () {
    it("should PASS if KMS key does not provide * permission for any grants", function (done) {
      ...
      kmsGrantLeastPrivilege.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "KMS key does not provide * permission for any grants"
        );
        done();
      });
    });
    ...
    it("should FAIL if KMS key provides * permission for grants", function (done) {
      ...
      kmsGrantLeastPrivilege.run(cache, {}, (err, results) => {
        ...
        expect(results[0].region).to.equal("us-east-1");
        expect(results[0].message).to.include(
          "KMS key provides * permission for these grants"
        );
        done();
      });
    });
    ...
    it("should UNKNOWN if unable to query for KMS Key grants", function (done) {
      ...
      kmsGrantLeastPrivilege.run(cache, {}, (err, results) => {
        ...
        expect(results[0].region).to.equal("us-east-1");
        expect(results[0].message).to.include(
          "Unable to query for KMS Key grants"
        );
        done();
      });
    });
