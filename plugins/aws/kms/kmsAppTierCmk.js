...
module.exports = {
  ...
  settings: {
    kms_cmk_tag_key: {
      ...
      description:
        "When this tag key exists in the AWS account, there should also exist a KMS CMK with the same tag key",
      regex: "^.*$",
      default: "",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      kms_cmk_tag_key:
        settings.kms_cmk_tag_key || this.settings.kms_cmk_tag_key.default,
    };
      ...
      function (region, rcb) {
        ...
        if (getTagKeys.err || !getTagKeys.data) {
          helpers.addResult(
            results,
            3,
            `Unable to get tag keys: ${helpers.addError(getTagKeys)}`,
            region
          );
          return rcb();
        }
        ...
        if (!getTagKeys.data.includes(config.kms_cmk_tag_key)) {
          helpers.addResult(
            results,
            2,
            `No key with "${config.kms_cmk_tag_key}" tag found`,
            region
          );
          return rcb();
        }
        ...
        async.each(
          ...
          function (kmsKey, kcb) {
            ...
            if (
              !listResourceTags ||
              listResourceTags.err ||
              !listResourceTags.data ||
              !listResourceTags.data.Tags
            ) {
              helpers.addResult(
                results,
                3,
                `Unable to describe resource tags: ${helpers.addError(
                  listResourceTags
                )}`,
                region,
                kmsKey.KeyArn
