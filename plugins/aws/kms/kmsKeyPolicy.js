...
module.exports = {
  ...
  settings: {
    kms_key_policy_max_user_count: {
      ...
      description:
        "Return a failing result when KMS key policies contain more than this many trusted users",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: 10,
    },
    kms_key_policy_max_third_parties_count: {
      ...
      description:
        "Return a warning result when KMS key policies contain more than this many trusted third parties",
      regex: "^[0-9]{0,3}$",
      default: 0,
    },
    kms_key_policy_whitelisted_account_ids: {
      ...
      description:
        "A comma-delimited list of known third-party AWS account IDs that should be trusted",
      regex: "^\\d{12}(?:,\\d{12})*$",
      default: "",
    },
    kms_key_policy_whitelisted_policy_ids: {
      ...
      description:
        "A comma-delimited list of known Key Policy IDs that should be trusted",
      regex: "^.{1,255}$",
      default: "aqua-cspm",
    },
    kms_key_policy_condition_keys: {
      ...
      description:
        "Comma separated list of AWS IAM condition keys that should be allowed i.e. aws:SourceAccount,kms:CallerAccount." +
      ...
      regex: "^.*$",
      default:
        "aws:PrincipalArn,aws:PrincipalAccount,aws:PrincipalOrgID,aws:SourceAccount,aws:SourceArn,aws:SourceOwner,kms:CallerAccount",
    },
  ...
  run: function (cache, settings, callback) {
    var config = {
      kms_key_policy_max_user_count:
        settings.kms_key_policy_max_user_count ||
        this.settings.kms_key_policy_max_user_count.default,
      kms_key_policy_max_third_parties_count:
        settings.kms_key_policy_max_third_parties_count ||
        this.settings.kms_key_policy_max_third_parties_count.default,
      kms_key_policy_whitelisted_account_ids:
        settings.kms_key_policy_whitelisted_account_ids ||
        this.settings.kms_key_policy_whitelisted_account_ids.default,
      kms_key_policy_whitelisted_policy_ids:
        settings.kms_key_policy_whitelisted_policy_ids ||
        this.settings.kms_key_policy_whitelisted_policy_ids.default,
      kms_key_policy_condition_keys:
        settings.kms_key_policy_condition_keys ||
        this.settings.kms_key_policy_condition_keys.default,
      kms_ignore_aws_managed_keys:
        settings.kms_ignore_aws_managed_keys ||
        this.settings.kms_ignore_aws_managed_keys.default,
      kms_whitelist_aws_organization_accounts:
        settings.kms_whitelist_aws_organization_accounts ||
        this.settings.kms_whitelist_aws_organization_accounts.default,
    };

    if (
      config.kms_key_policy_whitelisted_account_ids &&
      config.kms_key_policy_whitelisted_account_ids.length
    ) {
      config.kms_key_policy_whitelisted_account_ids =
        config.kms_key_policy_whitelisted_account_ids.split(",");
    } else {
      config.kms_key_policy_whitelisted_account_ids = [];
    }
    ...
    if (
      ...
      config.kms_key_policy_whitelisted_policy_ids =
        config.kms_key_policy_whitelisted_policy_ids.split(",");
    } else {
      config.kms_key_policy_whitelisted_policy_ids = [];
    }

    config.kms_ignore_aws_managed_keys =
      config.kms_ignore_aws_managed_keys == "true";
    var whitelistOrganization =
      config.kms_whitelist_aws_organization_accounts == "true";
    ...
    if (whitelistOrganization) {
      ...
      if (!listAccounts || listAccounts.err || !listAccounts.data) {
        helpers.addResult(
          results,
          3,
          `Unable to query organization accounts: ${helpers.addError(
            listAccounts
          )}`,
          acctRegion
        );
      ...
      organizationAccounts = helpers.getOrganizationAccounts(
        listAccounts,
        accountId
      );
    }
    ...
    config.kms_key_policy_whitelisted_account_ids =
      config.kms_key_policy_whitelisted_account_ids.concat(
      ...
      function (region, rcb) {
        ...
        async.each(
          ...
          function (kmsKey, kcb) {
            if (config.kms_ignore_aws_managed_keys) {
              ...
              if (
                ...
                helpers.addResult(
                  results,
                  3,
                  `Unable to query for KMS Key: ${helpers.addError(
                    describeKey
                  )}`,
                  region
                );
              ...
              let keyLevel = helpers.getEncryptionLevel(
                describeKey.data.KeyMetadata,
            ...
            if (
              getKeyPolicy.data.Id &&
              config.kms_key_policy_whitelisted_policy_ids.length &&
              config.kms_key_policy_whitelisted_policy_ids.indexOf(
                getKeyPolicy.data.Id
              ) > -1
            ) {
              helpers.addResult(
                results,
                0,
                "The key " + kmsKey.KeyArn + " is whitelisted.",
                region,
                kmsKey.KeyArn
              );
              return kcb();
            ...
            for (var s in statements) {
              ...
              var newUsers = principal.AWS.filter(function (newUser) {
                return (
                  totalUsers.filter(function (existingUser) {
              ...
              if (statement.Condition) {
                conditionalCaller = helpers.isValidCondition(
                  statement,
                  allowedConditionKeys,
                  helpers.IAM_CONDITION_OPERATORS,
                  true,
                  accountId
                );
              }
              ...
              if (principal.AWS.indexOf("*") > -1 && !conditionalCaller) {
                ...
                for (var caller of conditionalCaller) {
                  if (
                    caller !== accountId &&
                    config.kms_key_policy_whitelisted_account_ids.indexOf(
                      caller
                    ) === -1
                  ) {
                    thirdPartyTrusted += 1;
                ...
                for (var u in principal.AWS) {
                  if (
                    ...
                    for (var i in config.kms_key_policy_whitelisted_account_ids) {
                      if (
                        principal.AWS[u].indexOf(
                          config.kms_key_policy_whitelisted_account_ids[i]
                        ) > -1
                      ) {
                        wlFound = true;
                      }
            ...
            if (totalUsers.length > config.kms_key_policy_max_user_count) {
              ...
              helpers.addResult(
                results,
                2,
                "Key trusts " + totalUsers.length + " users",
                region,
                kmsKey.KeyArn,
                custom
              );
            ...
            if (
              thirdPartyTrusted > config.kms_key_policy_max_third_parties_count
            ) {
              found = true;
              helpers.addResult(
                results,
                1,
                "Key trusts " + thirdPartyTrusted + " third parties",
                region,
                kmsKey.KeyArn,
                custom
              );
            ...
            if (wildcardTrusted) {
              ...
              helpers.addResult(
                results,
                2,
                "Key trusts " + wildcardTrusted + " principals with wildcards",
                region,
                kmsKey.KeyArn,
                custom
              );
