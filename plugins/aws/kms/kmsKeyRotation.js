...
module.exports = {
  ...
  more_info:
    "All KMS keys should have key rotation enabled. AWS will handle the rotation of the encryption key itself, as well as storage of previous keys, so previous data does not need to be re-encrypted before the rotation occurs.",
  ...
  compliance: {
    pci:
      "PCI has strict requirements regarding the use of encryption keys " +
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        listKeys.data.forEach((kmsKey) => {
          ...
          const currentEncryptionLevel = helpers.getEncryptionLevel(
            describeKeyData.KeyMetadata,
            helpers.ENCRYPTION_LEVELS
          );
          if (
            currentEncryptionLevel <= 2 ||
            (describeKeyData.KeyMetadata.KeyState &&
              describeKeyData.KeyMetadata.KeyState.toUpperCase() ===
                "PENDINGDELETION")
          )
            return;
          ...
          if (
            !getKeyRotationStatus ||
            getKeyRotationStatus.err ||
            !getKeyRotationStatus.data
          ) {
            helpers.addResult(
              results,
              3,
              "Unable to get key rotation status: " +
                helpers.addError(getKeyRotationStatus),
              region,
              kmsKey.KeyArn
            );
            return;
          ...
          helpers.addResult(
            results,
            status,
            `Key rotation is ${enabled ? "" : "not"} enabled`,
            region,
            kmsKey.KeyArn
          );
        });
