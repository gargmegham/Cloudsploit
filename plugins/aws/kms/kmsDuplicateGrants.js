      ...
      function (region, rcb) {
        ...
        listKeys.data.forEach((kmsKey) => {
          ...
          for (let entry of listGrants.data.Grants) {
            let dupGrant = listGrants.data.Grants.filter(
              (grant) =>
                grant.KeyId === entry.KeyId &&
                grant.Name === entry.Name &&
                grant.GranteePrincipal == entry.GranteePrincipal &&
                (grant.Operations && entry.Operations
                  ? JSON.stringify(grant.Operations) ==
                    JSON.stringify(entry.Operations)
                  : true) &&
                (grant.Constraints && entry.Constraints
          ...
          if (found) {
            ...
            helpers.addResult(
              results,
              0,
              "KMS key does not have duplicate grants",
              region,
              resource
            );
          }
