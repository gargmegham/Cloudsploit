...
module.exports = {
  ...
  compliance: {
    pci:
      "PCI requires vendor defaults to be changed. While KMS keys " +
      ...
      function (region, rcb) {
        ...
        if (listAliases.err || !listAliases.data) {
          helpers.addResult(
            results,
            3,
            "Unable to list KMS key aliases: " + helpers.addError(listAliases),
            region
          );
          return rcb();
        }
        ...
        if (describeTrails) {
          if (describeTrails.err || !describeTrails.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for CloudTrail: " +
                helpers.addError(describeTrails),
              region
            );
          } else {
          ...
          if (describeVolumes.err || !describeVolumes.data) {
            helpers.addResult(
              ...
              "Unable to query for EBS volumes: " +
                helpers.addError(describeVolumes),
              region
            );
          } else {
              ...
              if (describeVolumes.data[j].KmsKeyId) {
                services.push({
                  ...
                  resource:
                    ...
                    accountId +
                    ":volume/" +
                    describeVolumes.data[j].VolumeId,
          ...
          if (listPipelines) {
            if (listPipelines.err || !listPipelines.data) {
              helpers.addResult(
                results,
                3,
                "Unable to query for ElasticTranscoder pipelines: " +
                  helpers.addError(listPipelines),
                region
              );
            } else {
        ...
        if (describeDBInstances) {
          if (describeDBInstances.err || !describeDBInstances.data) {
            ...
            for (var l in describeDBInstances.data) {
              if (
                describeDBInstances.data[l].StorageEncrypted &&
        ...
        if (describeClusters) {
          if (describeClusters.err || !describeClusters.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for Redshift clusters: " +
                helpers.addError(describeClusters),
              region
            );
          } else {
              ...
              if (describeClusters.data[m].KmsKeyId) {
                services.push({
                  ...
                  resource:
                    ...
                    accountId +
                    ":cluster:" +
                    describeClusters.data[m].ClusterIdentifier,
          ...
          if (describeActiveReceiptRuleSet) {
            if (describeActiveReceiptRuleSet.err) {
              helpers.addResult(
                results,
                3,
                "Unable to query for SES: " +
                  helpers.addError(describeActiveReceiptRuleSet),
                region
              );
            } else if (
              describeActiveReceiptRuleSet.data &&
              describeActiveReceiptRuleSet.data.Rules
            ) {
              for (var rule of describeActiveReceiptRuleSet.data.Rules) {
                if (rule.Actions) {
                  for (var o in rule.Actions) {
                    if (
                      rule.Actions[o].S3Action &&
        ...
        if (region in regions.workspaces) {
          var describeWorkspaces = helpers.addSource(cache, source, [
            "workspaces",
            "describeWorkspaces",
            region,
          ]);
          ...
          if (describeWorkspaces) {
            if (describeWorkspaces.err || !describeWorkspaces.data) {
              helpers.addResult(
                results,
                3,
                "Unable to query for workspaces: " +
                  helpers.addError(describeWorkspaces),
                region
              );
            } else {
                ...
                if (describeWorkspaces.data[p].VolumeEncryptionKey) {
                  services.push({
                    ...
                    resource:
                      ...
                      accountId +
                      ":workspace/" +
                      describeWorkspaces.data[p].WorkspaceId,
        ...
        if (listFunctions) {
          if (listFunctions.err || !listFunctions.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for Lambda functions: " +
                helpers.addError(listFunctions),
              region
            );
          } else {
        ...
        if (describeLogGroups) {
          if (describeLogGroups.err || !describeLogGroups.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for CloudWatch Logs groups: " +
                helpers.addError(describeLogGroups),
              region
            );
          } else {
          ...
          if (describeFileSystems) {
            if (describeFileSystems.err || !describeFileSystems.data) {
              helpers.addResult(
                results,
                3,
                "Unable to query for EFS file systems: " +
                  helpers.addError(describeFileSystems),
                region
              );
            } else {
                ...
                if (describeFileSystems.data[s].KmsKeyId) {
                  services.push({
                    ...
                    resource:
                      ...
                      accountId +
                      ":file-system/" +
                      describeFileSystems.data[s].FileSystemId,
          ...
          if (listBuckets) {
            if (listBuckets.err || !listBuckets.data) {
              helpers.addResult(
                results,
                3,
                "Unable to query for S3 buckets: " +
                  helpers.addError(listBuckets),
                region
              );
            } else {
                      ...
                      for (var u in getBucketEncryption.data) {
                        if (getBucketEncryption.data[u].Rules) {
                          for (var v in getBucketEncryption.data[u].Rules) {
                            if (
                              getBucketEncryption.data[u].Rules[v]
          ...
          function (key, kcb) {
            ...
            if (!describeKey || describeKey.err || !describeKey.data) {
              helpers.addResult(
                results,
                3,
                "Unable to query for KMS key: " +
                  key.KeyId +
                  ": " +
                  helpers.addError(describeKey),
                region
              );
              return kcb();
            }
          ...
          function () {
            ...
            if (!reg)
              helpers.addResult(
                results,
                0,
                "No default KMS keys found in use",
                region
              );
            rcb();
          }
