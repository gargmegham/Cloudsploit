      ...
      function (region, rcb) {
        ...
        listKeys.data.forEach((kmsKey) => {
          ...
          if (
            ...
            listGrants.err ||
            !listGrants.data ||
            !listGrants.data.Grants
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to query for KMS Key grants: ${helpers.addError(
                describeKey
              )}`,
              region,
              resource
          ...
          for (let grant of listGrants.data.Grants) {
            if (keySpec && keySpec.startsWith("SYMMETRIC")) {
              if (
                ...
                grant.Operations.length >= symmetricOperations.length
              )
                privilegedGrants.push(grant.GrantId);
            } else {
              if (
                grant.Operations &&
                grant.Operations.length &&
                grant.Operations.length >= asymmetricEDOperations.length
              )
                privilegedGrants.push(grant.GrantId);
            }
          }
          ...
          if (privilegedGrants.length) {
            helpers.addResult(
              results,
              2,
              `KMS key provides * permission for these grants: ${privilegedGrants.join(
                ", "
              )}`,
              region,
              resource
