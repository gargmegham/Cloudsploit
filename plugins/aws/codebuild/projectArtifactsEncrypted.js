  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.project_artifacts_desired_encryption_level ||
        this.settings.project_artifacts_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listProjects.err || !listProjects.data) {
          helpers.addResult(
            results,
            3,
            `Unable to list CodeBuild projects: ${helpers.addError(
              listProjects
            )}`,
            region
          );
        ...
        for (let project of listProjects.data) {
          ...
          if (
            batchGetProjects.data.projects[0].encryptionKey &&
            batchGetProjects.data.projects[0].encryptionKey.includes(
              "alias/aws/s3"
            )
          ) {
            ...
            var kmsKeyId = kmsKeyArn.split("/")[1]
              ? kmsKeyArn.split("/")[1]
              : kmsKeyArn;
