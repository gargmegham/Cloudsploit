...
const listProjects = ["test-project"];
...
describe("codebuildValidSourceProviders", function () {
  describe("run", function () {
    it("should PASS if CodeBuild project is using allowed source providers", function (done) {
      ...
      codebuildValidSourceProviders.run(
        cache,
        { codebuild_disallowed_source_providers: "bitbucket" },
        (err, results) => {
    ...
    it("should FAIL if CodeBuild project is using disallowed source providers", function (done) {
      ...
      codebuildValidSourceProviders.run(
        cache,
        { codebuild_disallowed_source_providers: "s3" },
        (err, results) => {
          expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if unable to query CodeBuild projects", function (done) {
      const cache = createCache(listProjects, {
        message: "Unable to query CodeBuild projects",
      });
      codebuildValidSourceProviders.run(
        cache,
        { codebuild_disallowed_source_providers: "s3" },
    ...
    it("should UNKNOWN if unable to query CodeBuild project", function (done) {
      const cache = createCache(listProjects, null, null, {
        message: "Unable to query CodeBuild project",
      });
      codebuildValidSourceProviders.run(
        cache,
        { codebuild_disallowed_source_providers: "s3" },
