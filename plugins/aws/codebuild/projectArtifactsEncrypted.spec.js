    ...
    projects: [
      {
        ...
        environment: [Object],
        serviceRole:
          "arn:aws:iam::000011112222:role/service-role/codebuild-testproj-service-role",
        timeoutInMinutes: "60",
...
const createCache = (
  ...
  var projectName =
    listProjects && listProjects.length ? listProjects[0] : null;
  return {
    codebuild: {
      listProjects: {
        "us-east-1": {
...
describe("projectArtifactsEncrypted", function () {
  describe("run", function () {
    it("should PASS if CodeBuild project artifact is encrypted with desired encryption level", function (done) {
      ...
      projectArtifactsEncrypted.run(
        cache,
        { projects_artifact_desired_encryption_level: "awscmk" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          expect(results[0].region).to.equal("us-east-1");
          expect(results[0].message).to.include(
            "CodeBuild project artifacts are encrypted with awscmk"
          );
          done();
        }
      );
    ...
    it("should FAIL if CodeBuild project artifact is not encrypted with desired encryption level", function (done) {
      ...
      projectArtifactsEncrypted.run(
        ...
        (err, results) => {
          ...
          expect(results[0].region).to.equal("us-east-1");
          expect(results[0].message).to.include(
            "CodeBuild project artifacts are encrypted with awskms"
          );
          done();
        }
      );
    ...
    it("should UNKNOWN if unable to list project artifacts", function (done) {
      const cache = createCache(null, null, null, {
        message: "Unable to list project artifacts",
      });
      projectArtifactsEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if unable to list KMS keys", function (done) {
      const cache = createCache(listProjects, null, null, null, {
