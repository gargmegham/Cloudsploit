        ...
        if (autoScalingGroups.err || !autoScalingGroups.data) {
          helpers.addResult(
            ...
            "Unable to query for AutoScaling groups: " +
              helpers.addError(autoScalingGroups),
            region
          );
          return rcb();
        }
        ...
        if (elasticLoadBalancers.err || !elasticLoadBalancers.data) {
          helpers.addResult(
            ...
            "Unable to query for Classic load balancers: " +
              helpers.addError(elasticLoadBalancers),
            region
          );
          return rcb();
        }
        ...
        if (elasticLoadBalancersV2.err || !elasticLoadBalancersV2.data) {
          helpers.addResult(
            ...
            "Unable to query for Application/Network load balancers: " +
              helpers.addError(elasticLoadBalancersV2),
            region
          );
          return rcb();
        }
          ...
          if (asg.HealthCheckType == "ELB") {
            if (asg.LoadBalancerNames && asg.LoadBalancerNames.length) {
              asg.LoadBalancerNames.forEach(function (elbName) {
                if (loadBalancers[elbName]) {
                  ...
                  if (elbAvailabilityZones && elbAvailabilityZones.length) {
                    elbAvailabilityZones.forEach(function (elbAz) {
                      if (
                        ...
                        asgAvailabilityZones.length &&
                        !asgAvailabilityZones.includes(elbAz)
                      ) {
                  ...
                  if (distinctAzs.length) {
                    helpers.addResult(
                      ...
                      'Auto scaling group "' +
                        ...
                        '" has load balancers in these different availability zones: ' +
                        distinctAzs.join(", "),
                      region,
                      resource
                    );
                    ...
                    helpers.addResult(
                      ...
                      'Auto scaling group "' +
                        asg.AutoScalingGroupName +
                        '" has all load balancers in same availability zones',
                      region,
                      resource
                    );
                  }
                  ...
                  helpers.addResult(
                    ...
                    'AutoScaling group "' +
                      asg.AutoScalingGroupName +
                      '" utilizes inactive load balancers',
                    region,
                    resource
                  );
                }
              ...
              helpers.addResult(
                results,
                0,
                "AutoScaling group does not have any Load Balancer associated",
                region,
                resource
              );
            }
            ...
            helpers.addResult(
              results,
              0,
              "AutoScaling group does not utilize a load balancer",
              region,
              resource
            );
          }
