...
module.exports = {
  ...
  settings: {
    app_tier_tag_key: {
      ...
      description:
        "App-Tier tag key used by Auto Scaling groups to indicate App-Tier groups",
      regex: "^.*$",
      default: "",
    },
    cw_log_agent_install_command: {
      ...
      regex: "^.*$",
      default:
        "#!/bin/bash curl https://s3.amazonaws.com//aws-cloudwatch/downloads/latest/awslogs-agent-setup.py -O " +
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      app_tier_tag_key:
        settings.app_tier_tag_key || this.settings.app_tier_tag_key.default,
      cw_log_agent_install_command:
        settings.cw_log_agent_install_command ||
        this.settings.cw_log_agent_install_command.default,
      s3_cw_agent_config_file:
        settings.s3_cw_agent_config_file ||
        this.settings.s3_cw_agent_config_file.default,
    };
      ...
      function (region, rcb) {
        ...
        if (describeAutoScalingGroups.err || !describeAutoScalingGroups.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for Auto Scaling groups: ${helpers.addError(
              describeAutoScalingGroups
            )}`,
            region
          );
        ...
        async.each(
          ...
          function (asg, cb) {
            ...
            if (appTierTag) {
              ...
              var describeLaunchConfigurations = helpers.addSource(
                cache,
                source,
                [
                  "autoscaling",
              ...
              if (
                ...
                helpers.addResult(
                  results,
                  3,
                  `Unable to query launch configurations for auto scaling group "${
                    asg.AutoScalingGroupName
                  }": ${helpers.addError(describeLaunchConfigurations)}`,
                  region,
                  resource
                );
                return cb();
              ...
              describeLaunchConfigurations.data.LaunchConfigurations.forEach(
                function (launchConfig) {
                  config.cw_log_agent_install_command =
                    config.cw_log_agent_install_command.replace(
                      "<AWS_REGION>",
                      region
                    );
                  config.cw_log_agent_install_command =
                    config.cw_log_agent_install_command.replace(
                      "<S3_CLOUDWATCH_AGENT_CONFIG_FILE_LOCATION>",
                      config.s3_cw_agent_config_file
                    );
                  if (
                    launchConfig.UserData &&
                    launchConfig.UserData.indexOf(
                      config.cw_log_agent_install_command
                    ) > -1
                  ) {
                    logsEnabled = true;
            ...
            if (!appTierAsgFound) {
              helpers.addResult(
                results,
                0,
                "No App-Tier Auto Scaling groups found",
                region
              );
            }
            cb();
