    ...
    LaunchConfigurations: [
      {
        ...
        LaunchConfigurationARN:
          "arn:aws:autoscaling:us-east-1:111122223333:launchConfiguration:b7772b0b-5b9e-4fc2-8509-e298fed9fae2:launchConfigurationName/test-36",
        ImageId: "ami-0001903fe8544444c",
        KeyName: "auto-scaling-test-instance",
        SecurityGroups: ["sg-08f7cb8776a0176ef"],
        ClassicLinkVPCSecurityGroups: [],
        UserData:
          "#!/bin/bash curl https://s3.amazonaws.com//aws-cloudwatch/downloads/latest/awslogs-agent-setup.py -O chmod +x ./awslogs-agent-setup.py ./awslogs-agent-setup.py -n -r us-east-1 -c s3://bucket-test-13/configFile.config",
        InstanceType: "t2.micro",
        KernelId: "",
        RamdiskId: "",
        BlockDeviceMappings: [
    ...
    autoscaling: {
      describeAutoScalingGroups: {
        ...
        describeLaunchConfigurations: {
          "us-east-1": {
            message:
              "error describing auto scaling group notification configurations",
          },
        },
      },
    },
...
describe("appTierAsgCloudWatchLogs", function () {
  describe("run", function () {
    it("should PASS if App-Tier Auto Scaling launch configuration is using CloudWatch Logs agent", function (done) {
      ...
      const settings = {
        s3_cw_agent_config_file: "s3://bucket-test-13/configFile.config",
        app_tier_tag_key: "app_tier",
      };
    ...
    it("should PASS if no App-Tier Auto Scaling groups found", function (done) {
      const cache = createCache(
        ...
        describeLaunchConfigurations[2]
      );
      appTierAsgCloudWatchLogs.run(
        cache,
        { app_tier_tag_key: "app_tier" },
