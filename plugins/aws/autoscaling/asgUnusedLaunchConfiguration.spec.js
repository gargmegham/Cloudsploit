...
const describeAutoScalingGroups = [
  {
    ...
    AutoScalingGroupARN:
      "arn:aws:autoscaling:us-east-1:000111222333:autoScalingGroup:1efc7b66-fbd4-4a5b-8086-8b95b0b74603:autoScalingGroupName/sadeed-grp1",
    ...
    ServiceLinkedRoleARN:
      "arn:aws:iam::000011112222:role/aws-service-role/autoscaling.amazonaws.com/AWSServiceRoleForAutoScaling",
  },
];
...
const describeLaunchConfigurations = [
  {
    ...
    LaunchConfigurationARN:
      "arn:aws:autoscaling:us-east-1:000011112222:launchConfiguration:8fceeaaa-c984-4cad-a20d-3e09eaea2440:launchConfigurationName/mine2",
    ImageId: "ami-085b20a79fc1af8f5",
    KeyName: "test",
    SecurityGroups: ["sg-008a9126e4f284b6c"],
  ...
  },
  {
    ...
    LaunchConfigurationARN:
      "arn:aws:autoscaling:us-east-1:000011112222:launchConfiguration:917c072a-383b-48b6-9675-ed0e1f4e6f51:launchConfigurationName/mine3",
    ImageId: "ami-085b20a79fc1af8f5",
    KeyName: "test",
    SecurityGroups: ["sg-0d55142f499300efb"],
    ClassicLinkVPCSecurityGroups: [],
    UserData: "",
    InstanceType: "a1.medium",
    KernelId: "",
...
describe("asgUnusedLaunchConfiguration", function () {
  describe("run", function () {
    ...
    it("should FAIL if Auto Scaling launch configuration is not being used", function (done) {
      const cache = createCache(
        [describeAutoScalingGroups[0]],
        [describeLaunchConfigurations[1]]
      );
      asgUnusedLaunchConfiguration.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should PASS if no Auto Scaling launch configurations found", function (done) {
      ...
      asgUnusedLaunchConfiguration.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "No Auto Scaling launch configurations found"
        );
        done();
      });
    });
    ...
    it("should UNKNOWN if Unable to query for Auto Scaling launch configurations", function (done) {
      ...
      asgUnusedLaunchConfiguration.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Unable to query for Auto Scaling launch configurations"
        );
        done();
      });
    });
