...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      ...
      approved_amis:
        settings.approved_amis || this.settings.approved_amis.default,
    };
              ...
              describeLaunchConfigurations.data.LaunchConfigurations.forEach(
                function (launchConfig) {
                  if (launchConfig.ImageId) {
                    imageFound = true;
                    if (
                      config.approved_amis.indexOf(launchConfig.ImageId) === -1
                    ) {
              ...
              if (imageFound) {
                if (!unapprovedAmis.length) {
                  ...
                  helpers.addResult(
                    results,
                    2,
                    `Launch Configuration for App-Tier Auto Scaling group "${
                      asg.AutoScalingGroupName
                    }" is using this unapproved AMI: ${unapprovedAmis.join(
                      ", "
                    )}`,
                    region,
                    resource
