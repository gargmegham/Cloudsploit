      ...
      function (region, rcb) {
        ...
        if (
          ...
          helpers.addResult(
            results,
            3,
            `Unable to query for Auto Scaling launch configurations: ${helpers.addError(
              describeLaunchConfigurations
            )}`,
            region
          );
        ...
        if (
          ...
          helpers.addResult(
            results,
            3,
            `Unable to query for EC2 security groups: ${helpers.addError(
              describeSecurityGroups
            )}`,
            region
          );
          ...
          if (!config.SecurityGroups || !config.SecurityGroups.length) {
            ...
            if (missingSecurityGroups.length) {
              helpers.addResult(
                ...
                }" references these missing EC2 security groups: ${missingSecurityGroups.join(
                  ", "
                )}`,
                region,
                resource
