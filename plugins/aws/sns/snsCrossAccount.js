...
module.exports = {
  ...
  settings: {
    ...
    sns_topic_policy_condition_keys: {
      ...
      regex: "^.*$",
      default:
        "aws:PrincipalArn,aws:PrincipalAccount,aws:PrincipalOrgID,aws:SourceAccount,aws:SourceArn,aws:SourceOwner,sns:Endpoint",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      sns_whitelisted_aws_account_principals:
        settings.sns_whitelisted_aws_account_principals ||
        this.settings.sns_whitelisted_aws_account_principals.default,
      sns_whitelist_aws_organization_accounts:
        settings.sns_whitelist_aws_organization_accounts ||
        this.settings.sns_whitelist_aws_organization_accounts.default,
      sns_topic_policy_condition_keys:
        settings.sns_topic_policy_condition_keys ||
        this.settings.sns_topic_policy_condition_keys.default,
    };
    var allowedConditionKeys =
      config.sns_topic_policy_condition_keys.split(",");
    var whitelistOrganization =
      config.sns_whitelist_aws_organization_accounts == "true";
      ...
      function (region, rcb) {
        ...
        if (listTopics.err || !listTopics.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for SNS topics: " + helpers.addError(listTopics),
            region
          );
          return rcb();
        }
        ...
        async.each(
          ...
          function (topic, cb) {
            ...
            if (
              !getTopicAttributes ||
              getTopicAttributes.err ||
              !getTopicAttributes.data
            ) {
              helpers.addResult(
                results,
                3,
                "Unable to query SNS topic for attributes: " +
                  helpers.addError(getTopicAttributes),
                region,
                topic.TopicArn
              );
              return cb();
            ...
            if (
              !getTopicAttributes.data.Attributes ||
            ...
            for (var statement of statements) {
              ...
              if (
                ...
                for (let principal of crossAccountPrincipals) {
                  if (
                    config.sns_whitelisted_aws_account_principals.includes(
                      principal
                    )
                  )
                    continue;
                  ...
                  if (whitelistOrganization) {
                    orgAccount = organizationAccounts.find((account) =>
                ...
                if (crossAccount) {
                  for (let a in statement.Action) {
                    if (
                      crossAccountActions.indexOf(statement.Action[a]) === -1
                    ) {
            ...
            if (crossAccountActions.length) {
              helpers.addResult(
                results,
                2,
                "SNS topic policy allows cross-account access to the action(s): " +
                  crossAccountActions,
                region,
                topic.TopicArn
              );
