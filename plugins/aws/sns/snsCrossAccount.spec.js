...
const getTopicAttributes = [
  {
    ResponseMetadata:
      "{\"RequestId\": '2a205b73-5c0d-55e2-8129-0ca612f4a41c' }",
    Attributes: {
      Policy:
  ...
  },
  {
    ...
    Attributes: {
      ...
      SubscriptionsPending: "0",
      KmsMasterKeyId:
        "arn:aws:kms:us-east-1:111122223333:key/b8789907-b7f7-438d-847e-7d468bac86b2",
...
const createCache = (listTopics, getTopicAttributes) => {
  var topicArn =
    listTopics && listTopics.length ? listTopics[0].TopicArn : null;
  return {
    sns: {
      listTopics: {
        "us-east-1": {
...
describe("snsCrossAccount", function () {
  describe("run", function () {
    ...
    it("should PASS if cross-account is whitelisted", function (done) {
      ...
      snsCrossAccount.run(
        cache,
        { sns_whitelisted_aws_account_principals: "111122223333" },
        (err, results) => {
          expect(results.length).to.equal(1);
    ...
    it("should PASS if SNS topic policy allows cross-account access to organization account and setting is set to true", function (done) {
      ...
      snsCrossAccount.run(
        cache,
        { sns_whitelist_aws_organization_accounts: "true" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          done();
