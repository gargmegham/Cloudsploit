...
module.exports = {
  ...
  remediation_description:
    "Server-Side Encryption to protect the content of SNS topic messages will be enabled.",
          ...
          function (topic, cb) {
            ...
            if (
              ...
              helpers.addResult(
                results,
                3,
                "Unable to query SNS topic attributes: " +
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    var defaultKeyDesc =
      "Default master key that protects my SNS data when no other key is defined";
    ...
    if (settings.input && settings.input.kmsKeyIdforSns) {
      params = {
      ...
      if (!defaultKmsKeyId)
        return callback(`No default SNS key for the region ${config.region}`);
      params = {
        AttributeName: "KmsMasterKeyId",
        TopicArn: resource,
