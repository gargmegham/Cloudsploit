...
module.exports = {
  ...
  settings: {
    sns_unwanted_subscribers: {
      ...
      description:
        "Comma-separated list of subscription endpoint i.e. xyz@aquasec.com",
      regex: "^.*$",
      default: "",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      sns_unwanted_subscribers:
        settings.sns_unwanted_subscribers ||
        this.settings.sns_unwanted_subscribers.default,
    };

    config.sns_unwanted_subscribers = config.sns_unwanted_subscribers.replace(
      /\s+/g,
      ""
    );

    if (!config.sns_unwanted_subscribers.length)
      return callback(null, results, source);

    config.sns_unwanted_subscribers =
      config.sns_unwanted_subscribers.toLowerCase();
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        if (listSubscriptions.err) {
          helpers.addResult(
            ...
            "Unable to query for SNS subscriptions: " +
              helpers.addError(listSubscriptions),
            region
          );
          return rcb();
        }
        ...
        for (let subscriber of listSubscriptions.data) {
          ...
          if (
            subscriber.Endpoint &&
            config.sns_unwanted_subscribers.includes(
              subscriber.Endpoint.toLowerCase()
            )
          ) {
            helpers.addResult(
              results,
              2,
              "SNS subscription is an unwanted subscription",
              region,
              resource
            );
          } else {
            helpers.addResult(
              results,
              0,
              "SNS subscription is a wanted subscription",
              region,
              resource
            );
          }
