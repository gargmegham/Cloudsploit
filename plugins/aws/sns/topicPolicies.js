...
module.exports = {
  ...
  settings: {
    sns_topic_policy_condition_keys: {
      ...
      regex: "^.*$",
      default:
        "aws:PrincipalArn,aws:PrincipalAccount,aws:PrincipalOrgID,aws:SourceOwner,aws:SourceArn,aws:SourceAccount,sns:Endpoint",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    config.sns_topic_policy_condition_keys =
      config.sns_topic_policy_condition_keys.replace(/\s/g, "");
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        listTopics.data.forEach((topic) => {
          ...
          if (
            !getTopicAttributes ||
            (!getTopicAttributes.err && !getTopicAttributes.data)
          )
            return;
