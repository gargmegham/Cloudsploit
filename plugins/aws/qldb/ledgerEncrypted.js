  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.qldb_ledger_desired_encryption_level ||
        this.settings.qldb_ledger_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listLedgers.err || !listLedgers.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query Ledgers: " + helpers.addError(listLedgers),
            region
          );
          return rcb();
        }
        ...
        for (let ledger of listLedgers.data) {
          ...
          if (!describeLedger || describeLedger.err || !describeLedger.data) {
            helpers.addResult(
              results,
              3,
              `Unable to get Ledgers description: ${helpers.addError(
                describeLedger
              )}`,
              region,
              resource
          ...
          if (
            describeLedger.data.EncryptionDescription &&
