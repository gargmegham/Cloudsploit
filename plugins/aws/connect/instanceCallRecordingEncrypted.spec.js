    ...
    StorageConfigs: [
      {
        ...
        S3Config: {
          ...
          EncryptionConfig: {
            EncryptionType: "KMS",
            KeyId:
              "arn:aws:kms:us-east-1:000111222333:key/2fe84bbe-30c3-4535-92b0-6d593085c84f",
          },
        },
      },
    ],
  ...
  describe("run", function () {
    it("should FAIL if Connect instance is not using desired encryption level", function (done) {
      const cache = createCache(
        listInstances,
        listInstanceCallRecordingStorageConfigs[0],
        listKeys,
        describeKey[1]
      );
    ...
    it("should FAIL if Connect instance does not have encryption enabled for call recordings", function (done) {
      const cache = createCache(
        listInstances,
        listInstanceCallRecordingStorageConfigs[1],
        listKeys,
        describeKey[1]
      );
      instanceCallRecordingEncrypted.run(cache, {}, (err, results) => {
    ...
    it("should PASS if Connect instance does not have any storage config for call recordings", function (done) {
      const cache = createCache(
        listInstances,
        listInstanceCallRecordingStorageConfigs[2],
        listKeys,
        describeKey[0]
      );
      instanceCallRecordingEncrypted.run(cache, {}, (err, results) => {
