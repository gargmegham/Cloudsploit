...
module.exports = {
  ...
  more_info:
    "You can configure Amazon Connect instance to save attachments on S3. When you save " +
  ...
  apis: [
    ...
    "Connect:instanceAttachmentStorageConfigs",
    "KMS:listKeys",
    "KMS:describeKey",
  ],
  settings: {
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.connect_attachments_encryption_level ||
        this.settings.connect_attachments_encryption_level.default,
    };
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        for (let instance of listInstances.data) {
          ...
          var instanceAttachmentStorageConfigs = helpers.addSource(
            cache,
            source,
          ...
          if (
            ...
            instanceAttachmentStorageConfigs.err ||
            !instanceAttachmentStorageConfigs.data ||
            !instanceAttachmentStorageConfigs.data.StorageConfigs
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to describe Connect instance Attachments storage config: ${helpers.addError(
                instanceAttachmentStorageConfigs
              )}`,
              region,
              resource
          ...
          let storageConfig =
            instanceAttachmentStorageConfigs.data.StorageConfigs[0];
