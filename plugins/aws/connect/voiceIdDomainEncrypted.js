  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.voice_id_desired_encryption_level ||
        this.settings.voice_id_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listDomains.err || !listDomains.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query Connect Voice ID domains: " +
        ...
        if (!listDomains.data.length) {
          helpers.addResult(
            results,
            0,
            "No Connect Voice ID domains found",
            region
          );
          return rcb();
        }
        ...
        for (let domain of listDomains.data) {
          ...
          if (
            ...
            domain.ServerSideEncryptionConfiguration.KmsKeyId
          ) {
            let encryptionKey =
              domain.ServerSideEncryptionConfiguration.KmsKeyId;
