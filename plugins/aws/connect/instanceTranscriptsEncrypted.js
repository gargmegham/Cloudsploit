...
module.exports = {
  ...
  more_info:
    "You can configure Amazon Connect instance to save transcripts for chats to be saved on S3. When you save " +
  ...
  apis: [
    ...
    "Connect:listInstanceChatTranscriptStorageConfigs",
    "KMS:listKeys",
    "KMS:describeKey",
  ],
  settings: {
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.connect_chat_transcripts_encryption_level ||
        this.settings.connect_chat_transcripts_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listInstances.err || !listInstances.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query Connect instances: ${helpers.addError(
              listInstances
            )}`,
            region
          );
        ...
        for (let instance of listInstances.data) {
          ...
          var listInstanceChatTranscriptStorageConfigs = helpers.addSource(
            cache,
            source,
            [
              "connect",
          ...
          if (
            !listInstanceChatTranscriptStorageConfigs ||
            listInstanceChatTranscriptStorageConfigs.err ||
            !listInstanceChatTranscriptStorageConfigs.data ||
            !listInstanceChatTranscriptStorageConfigs.data.StorageConfigs
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to describe Connect instance chat transcripts storage config: ${helpers.addError(
                listInstanceChatTranscriptStorageConfigs
              )}`,
              region,
              resource
          ...
          if (
            !listInstanceChatTranscriptStorageConfigs.data.StorageConfigs.length
          ) {
            helpers.addResult(
              results,
              0,
