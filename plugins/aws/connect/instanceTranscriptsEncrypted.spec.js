...
const listInstances = [
  {
    ...
    ServiceRole:
      "arn:aws:iam::000111222333:role/aws-service-role/connect.amazonaws.com/AWSServiceRoleForAmazonConnect_XYUbWO6kTpYfc9uUNk5i",
...
const listInstanceChatTranscriptStorageConfigs = [
  {
    StorageConfigs: [
      {
        AssociationId:
          "190014ee5aa69dd33835164f250fb5938636b3ad8225f323bebc0ecd13da16a8",
        StorageType: "S3",
        S3Config: {
  ...
  },
  {
    StorageConfigs: [
      {
        AssociationId:
          "519cada57abf190bef871bb031d6c2813a0f76ffd638e531d1d7dd77984bd9a2",
        StorageType: "S3",
        S3Config: {
          BucketName: "amazon-connect-a72b5bc76263",
          BucketPrefix: "connect/akhtar/ChatTranscripts",
  ...
  describe("run", function () {
    it("should FAIL if Connect instance is not using desired encryption level", function (done) {
      const cache = createCache(
        listInstances,
        listInstanceChatTranscriptStorageConfigs[0],
        listKeys,
        describeKey[1]
      );
    ...
    it("should FAIL if Connect instance does not have encryption enabled for chat transcripts", function (done) {
      const cache = createCache(
        listInstances,
        listInstanceChatTranscriptStorageConfigs[1],
        listKeys,
        describeKey[1]
      );
      instanceTranscriptsEncrypted.run(cache, {}, (err, results) => {
    ...
    it("should PASS if Connect instance does not have any storage config for chat transcripts", function (done) {
      const cache = createCache(
        listInstances,
        listInstanceChatTranscriptStorageConfigs[2],
        listKeys,
        describeKey[0]
      );
      instanceTranscriptsEncrypted.run(cache, {}, (err, results) => {
    ...
    it("should UNKNOWN Unable to query Connect instances", function (done) {
      const cache = createCache([], null, listKeys, describeKey[0], {
