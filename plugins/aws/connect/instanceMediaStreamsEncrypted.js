...
module.exports = {
  ...
  apis: [
    ...
    "Connect:listInstanceMediaStreamStorageConfigs",
    "KMS:listKeys",
    "KMS:describeKey",
  ],
  settings: {
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.connect_media_streams_encryption_level ||
        this.settings.connect_media_streams_encryption_level.default,
    };
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        for (let instance of listInstances.data) {
          ...
          var listInstanceMediaStreamStorageConfigs = helpers.addSource(
            cache,
            source,
            [
              "connect",
          ...
          if (
            !listInstanceMediaStreamStorageConfigs ||
            listInstanceMediaStreamStorageConfigs.err ||
            !listInstanceMediaStreamStorageConfigs.data ||
            !listInstanceMediaStreamStorageConfigs.data.StorageConfigs
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to describe Connect instance media streams storage config: ${helpers.addError(
                listInstanceMediaStreamStorageConfigs
              )}`,
              region,
              resource
          ...
          if (
            !listInstanceMediaStreamStorageConfigs.data.StorageConfigs.length
          ) {
            helpers.addResult(
              results,
              0,
          ...
          let storageConfig =
            listInstanceMediaStreamStorageConfigs.data.StorageConfigs[0];
