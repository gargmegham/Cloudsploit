...
module.exports = {
  ...
  settings: {
    customer_profiles_desired_encryption_level: {
      ...
      description:
        "In order (lowest to highest) awscmk=Customer managed KMS; externalcmk=Customer managed externally sourced KMS; cloudhsm=Customer managed CloudHSM sourced KMS",
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.customer_profiles_desired_encryption_level ||
        this.settings.customer_profiles_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listDomains.err || !listDomains.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query customerprofiles domain: " +
              helpers.addError(listDomains),
            region
          );
          return rcb();
        }
        ...
        if (!listDomains.data.length) {
          helpers.addResult(
            results,
            0,
            "No customerprofiles domain found",
            region
          );
          return rcb();
        }
        ...
        for (let domain of listDomains.data) {
          ...
          if (!getDomain || getDomain.err || !getDomain.data) {
            helpers.addResult(
              results,
              3,
              `Unable to get customerprofiles domain description: ${helpers.addError(
                getDomain
              )}`,
              region,
              resource
          ...
          if (getDomain.data.DefaultEncryptionKey) {
            ...
            var keyId = DefaultEncryptionKey.split("/")[1]
              ? DefaultEncryptionKey.split("/")[1]
            ...
            helpers.addResult(
              results,
              3,
              "Unable to find Customer Profile domain encryption key",
              region,
              resource
            );
            continue;
