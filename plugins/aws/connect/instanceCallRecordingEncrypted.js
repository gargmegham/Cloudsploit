...
module.exports = {
  ...
  more_info:
    "You can configure Amazon Connect instance to save recordings for incoming call to be saved on S3. When you save " +
  ...
  apis: [
    ...
    "Connect:listInstanceCallRecordingStorageConfigs",
    "KMS:listKeys",
    "KMS:describeKey",
  ],
  settings: {
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.connect_call_recording_encryption_level ||
        this.settings.connect_call_recording_encryption_level.default,
    };
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        for (let instance of listInstances.data) {
          ...
          var listInstanceCallRecordingStorageConfigs = helpers.addSource(
            cache,
            source,
            [
              "connect",
          ...
          if (
            !listInstanceCallRecordingStorageConfigs ||
            listInstanceCallRecordingStorageConfigs.err ||
            !listInstanceCallRecordingStorageConfigs.data ||
            !listInstanceCallRecordingStorageConfigs.data.StorageConfigs
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to describe Connect instance call recording storage config: ${helpers.addError(
                listInstanceCallRecordingStorageConfigs
              )}`,
              region,
              resource
          ...
          if (
            !listInstanceCallRecordingStorageConfigs.data.StorageConfigs.length
          ) {
            helpers.addResult(
              results,
              0,
          ...
          let storageConfig =
            listInstanceCallRecordingStorageConfigs.data.StorageConfigs[0];
