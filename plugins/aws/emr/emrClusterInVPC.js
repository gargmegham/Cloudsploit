      ...
      function (region, rcb) {
        ...
        if (
          ...
          helpers.addResult(
            results,
            3,
            "Unable to query for supported platforms: " +
        ...
        if (describeAccountAttributes.data.length) {
          var supportedPlatforms = describeAccountAttributes.data.find(
            (attribute) => attribute.AttributeName == "supported-platforms"
          );

          if (supportedPlatforms && supportedPlatforms.AttributeValues) {
            let ec2ClassicFound = supportedPlatforms.AttributeValues.find(
              (value) =>
                value.AttributeValue &&
                value.AttributeValue.toUpperCase() === "EC2"
            );
            if (!ec2ClassicFound) {
              helpers.addResult(
                results,
                0,
                "EC2 account attribute allows only VPC supported platform",
                region
              );
              return rcb();
            }
        ...
        for (let cluster of listClusters.data) {
          ...
          if (
            describeCluster.data.Cluster.Ec2InstanceAttributes &&
            ...
            helpers.addResult(
              results,
              0,
              `EMR cluster  "${cluster.Name}" is in VPC`,
              region,
              resource
            );
          } else {
            helpers.addResult(
              results,
              2,
              `EMR cluster  "${cluster.Name}" is not in VPC`,
              region,
              resource
            );
          }
