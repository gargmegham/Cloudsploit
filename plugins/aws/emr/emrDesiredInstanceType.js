...
module.exports = {
  ...
  settings: {
    emr_desired_master_instance_type: {
      ...
      description:
        "Comma-separated list of desired master instance types for EMR",
      regex: "^.*$",
      default: "",
    },
    emr_desired_core_instance_type: {
      ...
      description:
        "Comma-separated list of desired core instance types for EMR",
      regex: "^.*$",
      default: "",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    const emr_desired_master_instance_type =
      settings.emr_desired_master_instance_type ||
      this.settings.emr_desired_master_instance_type.default;
    const emr_desired_core_instance_type =
      settings.emr_desired_core_instance_type ||
      this.settings.emr_desired_core_instance_type.default;

    if (
      !emr_desired_master_instance_type.length &&
      !emr_desired_core_instance_type.length
    )
      ...
      function (region, rcb) {
        ...
        if (listClusters.err || !listClusters.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for EMR cluster: " +
        ...
        for (const cluster of listClusters.data) {
          ...
          if (
            ...
            helpers.addResult(
              results,
              3,
              "Unable to query for EMR cluster config: " +
          ...
          const masterGroup = config.find(
            (InstanceGroup) => InstanceGroup.InstanceGroupType === "MASTER"
          );
          const coreGroup = config.find(
            (InstanceGroup) => InstanceGroup.InstanceGroupType === "CORE"
          );
          const masterInstanceType = masterGroup
            ? masterGroup.InstanceType
            : null;
          ...
          if (
            masterInstanceType &&
            !emr_desired_master_instance_type.includes(masterInstanceType) &&
            coreInstanceType &&
            !emr_desired_core_instance_type.includes(coreInstanceType)
          ) {
            helpers.addResult(
              results,
              2,
