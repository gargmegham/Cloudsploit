...
module.exports = {
  ...
  more_info:
    "Setting threshold for the number of EMR cluster instances provisioned within your AWS account will help to manage EMR compute resources and" +
  ...
  settings: {
    emr_instance_count_global_threshold: {
      ...
      description:
        "Checks for the number of running EMR instances across all regions and triggers a failing result if it exceeds the specified count",
      regex: "^[0-9]{1,4}$",
      default: 200,
    },
    emr_instance_count_regional_threshold: {
      ...
      description:
        "Checks for the number of running EMR instances per region and triggers a failing result if it exceeds the specified count",
      regex: "^[0-9]{1,4}$",
      default: 100,
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      emr_instance_count_global_threshold:
        settings.emr_instance_count_global_threshold ||
        this.settings.emr_instance_count_global_threshold.default,
      emr_instance_count_regional_threshold:
        settings.emr_instance_count_regional_threshold ||
        this.settings.emr_instance_count_regional_threshold.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listClusters.err || !listClusters.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for EMR clusters: " +
        ...
        for (const cluster of listClusters.data) {
          ...
          if (
            !listInstanceGroups ||
            listInstanceGroups.err ||
            !listInstanceGroups.data ||
            !listInstanceGroups.data.InstanceGroups
          ) {
            helpers.addResult(
              results,
              3,
              "Unable to query for EMR cluster instance groups: " +
                helpers.addError(listInstanceGroups),
              region,
              resource
            );
            continue;
          ...
          const masterGroup = instanceGroups.find(
            (InstanceGroup) => InstanceGroup.InstanceGroupType === "MASTER"
          );
          const coreGroup = instanceGroups.find(
            (InstanceGroup) => InstanceGroup.InstanceGroupType === "CORE"
          );
          const masterInstanceCount = masterGroup
            ? masterGroup.RunningInstanceCount
            : 0;
          const coreInstanceCount = coreGroup
            ? coreGroup.RunningInstanceCount
            : 0;
        ...
        if (instanceCount > config.emr_instance_count_regional_threshold) {
          helpers.addResult(
            ...
            2,
            instanceCount +
              " EMR instances running in " +
              region +
              " region, exceed limit of: " +
              regionThreshold,
            region
          );
        } else {
          helpers.addResult(
            ...
            0,
            instanceCount +
              " EMR instances in the region are within the regional expected count of: " +
              regionThreshold,
            region
          );
        }
      ...
      function () {
        ...
        if (instanceCountGlobal > globalThreshold) {
          helpers.addResult(
            ...
            2,
            instanceCountGlobal +
              " EMR instances running in all regions, exceed limit of: " +
              globalThreshold,
            "global"
          );
        } else {
          helpers.addResult(
            ...
            0,
            instanceCountGlobal +
              " EMR instances in the account are within the global expected count of: " +
              globalThreshold,
            "global"
          );
        }
