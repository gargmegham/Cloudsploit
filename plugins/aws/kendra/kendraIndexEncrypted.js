  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.kendra_index_desired_encryption_level ||
        this.settings.kendra_index_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listIndices.err || !listIndices.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query Kendra Indices: " + helpers.addError(listIndices),
            region
          );
          return rcb();
        }
        ...
        for (let index of listIndices.data) {
          ...
          if (!describeIndex || describeIndex.err || !describeIndex.data) {
            helpers.addResult(
              results,
              3,
              `Unable to get Kendra index description: ${helpers.addError(
                describeIndex
              )}`,
              region,
              resource
          ...
          if (
            describeIndex.data.ServerSideEncryptionConfiguration &&
