...
const describeIndex = [
  {
    ...
    Edition: "DEVELOPER_EDITION",
    RoleArn:
      "arn:aws:iam::000011112222:role/service-role/AmazonKendra-us-east-1-role1",
...
describe("kendraIndexEncrypted", function () {
  describe("run", function () {
    it("should PASS if Kendra Indices is encrypted with desired encryption level", function (done) {
      ...
      kendraIndexEncrypted.run(
        cache,
        { kendra_index_desired_encryption_level: "awscmk" },
        (err, results) => {
    ...
    it("should UNKNOWN if unable to list Kendra Indices", function (done) {
      const cache = createCache(null, null, null, {
        message: "Unable to list Kendra Indices",
      });
      kendraIndexEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if unable to list KMS keys", function (done) {
      const cache = createCache(listIndices, null, null, null, null, {
