      ...
      Configuration: {
        ResultConfiguration: {
          ...
          EncryptionConfiguration: {
            ...
            KmsKey:
              "arn:aws:kms:us-east-1:000011112222:key/7cb78370-cdc6-4ccb-a973-ecc8cbdc0dec",
          },
        },
...
describe("workgroupEnforceConfiguration", function () {
  describe("run", function () {
    ...
    it("should UNKNOWN if unable to list Athena workgroups", function (done) {
      const cache = createCache(listWorkGroups, null, {
        message: "Unable to list workgroups",
      });
      workgroupEnforceConfiguration.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if unable to describe Athena workgroup", function (done) {
      const cache = createCache([listWorkGroups[0]], getWorkGroup, null, {
        message: "Unable to describe Athena workgroup",
      });
      workgroupEnforceConfiguration.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(3);
