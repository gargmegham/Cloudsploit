...
module.exports = {
  ...
  remediation_description:
    "Encryption for the affected workgroups will be enabled.",
  ...
  remediation_inputs: {
    ...
    kmsKeyIdforWg: {
      ...
      description:
        "The KMS Key ID used for encryption if encryption option is SSE_KMS or CSE_KMS",
      regex:
      ...
      function (region, rcb) {
        ...
        if (listWorkGroups.err || !listWorkGroups.data) {
          helpers.addResult(
            results,
            3,
            "Unable to list Athena workgroups: " +
              helpers.addError(listWorkGroups),
            region
          );
          return rcb();
        }
        ...
        listWorkGroups.data.forEach(function (wg) {
          ...
          if (!getWorkGroup || getWorkGroup.err || !getWorkGroup.data) {
            helpers.addResult(
              results,
              3,
              "Unable to describe Athena workgroup: " +
                helpers.addError(getWorkGroup),
              region,
              arn
            );
          } else if (
            ...
            if (
              ...
              helpers.addResult(
                results,
                0,
                "Athena primary workgroup does not have encryption enabled but is not in use.",
                region,
                arn
              );
            } else {
              helpers.addResult(
                results,
                2,
                "Athena workgroup is not using encryption",
                region,
                arn
              );
            }
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    if (settings.input && settings.input.encryptionOption) {
      if (
        settings.input.encryptionOption === "SSE_KMS" ||
        ...
        if (settings.input.kmsKeyIdforWg) {
          ...
          return callback(
            `Key is mandatory for workgroup update with ${settings.input.encryptionOption}`
          );
        }
