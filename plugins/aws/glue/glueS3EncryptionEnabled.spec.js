      ...
      S3Encryption: [
        {
          ...
          KmsKeyArn:
            "arn:aws:kms:us-east-1:111122223333:key/7cb78370-cdc6-4ccb-a973-ecc8cbdc0dec",
        },
      ],
  ...
  describe("run", function () {
    it("should PASS if AWS Glue security configuration has s3 encryption enabled", function (done) {
      const cache = createCache(
        [getSecurityConfigurations[0]],
        listKeys,
        describeKey[0]
      );
      s3EncryptionMode.run(
        cache,
        { glue_s3_encryption_level: "awskms" },
        (err, results) => {
    ...
    it("should FAIL if AWS Glue security configuration does not hsve s3 encryption enabled at desired level", function (done) {
      ...
      s3EncryptionMode.run(
        cache,
        { glue_s3_encryption_level: "awscmk" },
        (err, results) => {
          expect(results.length).to.equal(1);
