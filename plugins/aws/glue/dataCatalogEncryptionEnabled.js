...
module.exports = {
  ...
  remediation_description:
    "Glue data catalog encryption for the affected regions will be enabled.",
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.glue_datacatalog_encryption_level ||
        this.settings.glue_datacatalog_encryption_level.default,
    };
    ...
    async.each(
      regions.glue,
      function (region, rcb) {
        var getDataCatalogEncryptionSettings = helpers.addSource(
          cache,
          source,
        ...
        if (
          getDataCatalogEncryptionSettings.err ||
          !getDataCatalogEncryptionSettings.data
        ) {
          helpers.addResult(
            results,
            3,
            `Unable to query Glue data catalog encryption settings: ${helpers.addError(
        ...
        if (
          ...
          encryptionSettings.EncryptionAtRest.CatalogEncryptionMode.toUpperCase() !==
            "DISABLED"
        ) {
          var kmsKeyId =
            encryptionSettings.EncryptionAtRest.SseAwsKmsKeyId.split("/")[1]
              ? encryptionSettings.EncryptionAtRest.SseAwsKmsKeyId.split("/")[1]
              : encryptionSettings.EncryptionAtRest.SseAwsKmsKeyId;
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    var defaultKeyDesc =
      "Default master key that protects my Glue data when no other key is defined";
    ...
    if (settings.input && settings.input.kmsKeyIdforDataCatalog) {
      ...
      if (!defaultKmsKeyId)
        return callback(`No default Glue key for the region ${config.region}`);
      params = {
        DataCatalogEncryptionSettings: {
          EncryptionAtRest: {
            CatalogEncryptionMode: "SSE-KMS",
