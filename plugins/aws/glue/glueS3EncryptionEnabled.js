...
module.exports = {
  ...
  settings: {
    glue_s3_encryption_level: {
      ...
      description:
        "In order (lowest to highest) sse=S3 Server-Side; awskms=AWS-managed KMS; awscmk=Customer managed KMS;",
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      glue_s3_encryption_level:
        settings.glue_s3_encryption_level ||
        this.settings.glue_s3_encryption_level.default,
    };

    var desiredEncryptionLevel = helpers.ENCRYPTION_LEVELS.indexOf(
      config.glue_s3_encryption_level
    );
      ...
      function (region, rcb) {
        ...
        if (getSecurityConfigurations.err || !getSecurityConfigurations.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for AWS Glue security configurations: ${helpers.addError(
              getSecurityConfigurations
            )}`,
            region
          );
        ...
        if (!getSecurityConfigurations.data.length) {
          helpers.addResult(
            results,
            0,
            "No AWS Glue security configurations found",
            region
          );
          return rcb();
        }
        ...
        async.each(
          getSecurityConfigurations.data,
          function (configuration, cb) {
            ...
            if (
              ...
              for (var encryptionConfig of configuration.EncryptionConfiguration
                .S3Encryption) {
                if (
                  encryptionConfig.S3EncryptionMode &&
                  encryptionConfig.S3EncryptionMode.toUpperCase() !== "DISABLED"
                ) {
                  encryptionEnabled = true;
                  if (
                    encryptionConfig.S3EncryptionMode.toUpperCase() === "SSE-S3"
                  ) {
                    currentEncryptionLevel =
                      helpers.ENCRYPTION_LEVELS.indexOf("sse");
                  } else {
                    if (encryptionConfig.KmsKeyArn) {
                      ...
                      if (
                        ...
                        helpers.addResult(
                          results,
                          3,
                          `Unable to query associated KMS Key for configuration "${
                            configuration.Name
                      ...
                      helpers.addResult(
                        results,
                        3,
                        "Unable to find associated KMS key for security configuration",
                        region,
                        resource
                      );
                      return cb();
