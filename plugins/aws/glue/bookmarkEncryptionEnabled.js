      ...
      function (region, rcb) {
        ...
        if (getSecurityConfigurations.err || !getSecurityConfigurations.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query Glue security configurations: ${helpers.addError(
              getSecurityConfigurations
            )}`,
            region
          );
        ...
        for (var configuration of getSecurityConfigurations.data) {
          ...
          if (
            ...
            configuration.EncryptionConfiguration.JobBookmarksEncryption.JobBookmarksEncryptionMode.toUpperCase() !==
              "DISABLED"
          ) {
            helpers.addResult(
              results,
