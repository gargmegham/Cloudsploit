...
module.exports = {
  ...
  remediation_description:
    "The policy that deny all S3 actions when SecureTransport is false will be added in the impacted buckets.",
    ...
    for (let bucket of listBuckets.data) {
      ...
      if (
        ...
        helpers.addResult(
          results,
          2,
          "No bucket policy found; encryption in transit not enforced",
          bucketLocation,
          bucketResource
        );
        continue;
      ...
      if (!policyJson.Statement.length) {
        helpers.addResult(
          results,
          2,
          "Bucket policy does not contain any statements; encryption in transit not enforced",
          bucketLocation,
          bucketResource
        );
        continue;
      ...
      if (
        policyJson.Statement.find((statement) =>
          statementDeniesInsecureTransport(statement, bucketResource)
        )
      ) {
        helpers.addResult(
          results,
          0,
          "Bucket policy enforces encryption in transit",
          bucketLocation,
          bucketResource
        );
      } else {
        helpers.addResult(
          results,
          2,
          "Bucket does not enforce encryption in transit",
          bucketLocation,
          bucketResource
        );
      }
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    if (
      !bucketPolicies ||
      bucketPolicies.err ||
      Object.keys(bucketPolicies).length
    ) {
  ...
  rollback: function (config, cache, settings, resource, callback) {
    console.log(
      "Rollback support for this plugin has not yet been implemented"
    );
