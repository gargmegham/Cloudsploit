...
module.exports = {
  ...
  remediation_description:
    "The impacted bucket will be configured to use either AES-256 encryption, or CMK-based encryption if a KMS key ID is provided.",
  ...
  settings: {
    s3_encryption_require_cmk: {
      ...
      description:
        "When set to true, S3 encryption using default KMS keys or AES will be marked as failing",
      regex: "^(true|false)$",
      default: "false",
    },
    s3_encryption_allow_pattern: {
      ...
      description:
        "When set, whitelists buckets matching the given pattern. Useful for overriding buckets outside the account control.",
      regex: "^.{1,255}$",
      default: "",
    },
    s3_encryption_kms_alias: {
      ...
      description:
        "If set, S3 encryption must be configured using the KMS key alias specified. Be sure to include the alias/ prefix. Comma-delimited.",
    ...
    s3_encryption_allow_cloudfront: {
      ...
      description:
        "When set to true, buckets that serve as CloudFront origins will not be required to have CMK encryption (which is unsupported by CloudFront).",
      regex: "^(true|false)$",
      default: "false",
    },
    s3_allow_unencrypted_static_websites: {
      ...
      description:
        "Allow buckets having static website enabled to skip encryption",
      regex: "^(true|false)$",
      default: "false",
    },
    whitelist_appconfig_s3_buckets: {
      ...
      description:
        "When set to true, whitelists buckets which are source to AppConfig configuration profiles",
      regex: "^(true|false)$",
      default: "false",
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      s3_encryption_require_cmk:
        settings.s3_encryption_require_cmk ||
        this.settings.s3_encryption_require_cmk.default,
      s3_encryption_allow_pattern:
        settings.s3_encryption_allow_pattern ||
        this.settings.s3_encryption_allow_pattern.default,
      s3_encryption_kms_alias:
        settings.s3_encryption_kms_alias ||
        this.settings.s3_encryption_kms_alias.default,
      s3_encryption_allow_cloudfront:
        settings.s3_encryption_allow_cloudfront ||
        this.settings.s3_encryption_allow_cloudfront.default,
      s3_allow_unencrypted_static_websites:
        settings.s3_allow_unencrypted_static_websites ||
        this.settings.s3_allow_unencrypted_static_websites.default,
      whitelist_appconfig_s3_buckets:
        settings.whitelist_appconfig_s3_buckets ||
        this.settings.whitelist_appconfig_s3_buckets.default,
    };

    config.s3_encryption_require_cmk =
      config.s3_encryption_require_cmk == "true";
    config.s3_encryption_allow_cloudfront =
      config.s3_encryption_allow_cloudfront == "true";
    config.s3_allow_unencrypted_static_websites =
      config.s3_allow_unencrypted_static_websites == "true";
    config.whitelist_appconfig_s3_buckets =
      config.whitelist_appconfig_s3_buckets == "true";
    ...
    async.series(
      [
            ...
            function (region, rcb) {
              ...
              if (listKeys.err || !listKeys.data) {
                helpers.addResult(
                  results,
                  3,
                  "Unable to query for KMS: " + helpers.addError(listKeys),
                  region
                );
                return rcb();
              }
          ...
          if (listDistributions.err || !listDistributions.data) {
            helpers.addResult(
              ...
              "Unable to query for CloudFront distributions: " +
                helpers.addError(listDistributions)
            );
            return cb();
          }
            ...
            function (region, rcb) {
              ...
              if (listApplications.err || !listApplications.data) {
                helpers.addResult(
                  results,
                  3,
                  "Unable to query for AppConfig applications: " +
                    helpers.addError(listApplications),
                  region
                );
                return rcb();
              }
              ...
              if (listApplications.data.length) {
                listApplications.data.forEach(function (application) {
                  ...
                  var listConfigurationProfiles = helpers.addSource(
                    cache,
                    source,
                    [
                      "appconfig",
                  ...
                  if (
                    !listConfigurationProfiles ||
                    listConfigurationProfiles.err ||
                    !listConfigurationProfiles.data ||
                    !listConfigurationProfiles.data.Items
                  ) {
                    helpers.addResult(
                      results,
                      3,
                      `Unable to get configuration profiles description: ${helpers.addError(
                        listConfigurationProfiles
                      )}`,
                      region,
                      resource
                  ...
                  if (listConfigurationProfiles.data.Items.length) {
                    for (let config of listConfigurationProfiles.data.Items) {
                      if (
                        config.LocationUri &&
                        config.LocationUri.startsWith("s3://")
                      ) {
                        let bucketName = config.LocationUri.split("/")[2];
                        if (!appConfigBuckets.includes(bucketName))
                          appConfigBuckets.push(bucketName);
                      }
                    }
                  }
                });
        ...
        function (cb) {
          ...
          var allowRegex =
            config.s3_encryption_allow_pattern &&
            config.s3_encryption_allow_pattern.length
              ? new RegExp(config.s3_encryption_allow_pattern)
              : false;

          listBuckets.data.forEach(function (bucket) {
            let bucketResource = "arn:aws:s3:::" + bucket.Name;
            var bucketLocation = helpers.getS3BucketLocation(
              cache,
              region,
              bucket.Name
            );
            if (
              config.whitelist_appconfig_s3_buckets &&
              appConfigBuckets.includes(bucket.Name)
            ) {
              helpers.addResult(
                results,
                0,
            ...
            if (allowRegex && allowRegex.test(bucket.Name)) {
              helpers.addResult(
                ...
                "Bucket: " +
                  bucket.Name +
                  " is whitelisted via custom setting.",
                bucketLocation,
              ...
              if (config.s3_allow_unencrypted_static_websites) {
                ...
                if (
                  getBucketWebsite &&
                  getBucketWebsite.err &&
                  getBucketWebsite.err.code &&
                  getBucketWebsite.err.code === "NoSuchWebsiteConfiguration"
                ) {
                  // do nothing
                } else if (
                  !getBucketWebsite ||
                  getBucketWebsite.err ||
                  !getBucketWebsite.data
                ) {
                  helpers.addResult(
                    results,
                    3,
                    `Error querying for bucket website: ${
                      bucket.Name
              ...
              if (
                ...
                !getBucketEncryption ||
                getBucketEncryption.err ||
                !getBucketEncryption.data
              ) {
                helpers.addResult(
                  results,
                  3,
                  "Error querying bucket encryption for: " +
                    bucket.Name +
                    ": " +
                ...
                getBucketEncryption.data.ServerSideEncryptionConfiguration &&
                getBucketEncryption.data.ServerSideEncryptionConfiguration
                  .Rules &&
                getBucketEncryption.data.ServerSideEncryptionConfiguration
                  .Rules[0] &&
                ...
                if (
                  config.s3_encryption_require_cmk &&
                  (algo == "AES256" ||
                    (algo == "aws:kms" && defaultKeyIds.indexOf(keyArn) > -1))
                ) {
                  if (
                    config.s3_encryption_allow_cloudfront &&
                    ...
                    helpers.addResult(
                      ...
                      "Bucket: " +
                        ...
                        " has " +
                        algo +
                        " encryption enabled without a CMK but is a CloudFront origin",
                      bucketLocation,
                      bucketResource,
                      custom
                    );
                    ...
                    helpers.addResult(
                      ...
                      "Bucket: " +
                        ...
                        " has " +
                        algo +
                        " encryption enabled but is not using a CMK",
                      bucketLocation,
                      bucketResource,
                      custom
                    );
                    ...
                    if (
                      ...
                      helpers.addResult(
                        ...
                        "Bucket: " +
                          ...
                          " has " +
                          algo +
                          " encryption enabled but is a CloudFront origin",
                        bucketLocation,
                        bucketResource,
                        custom
                      );
                      ...
                      helpers.addResult(
                        ...
                        "Bucket: " +
                          bucket.Name +
                          " has encryption enabled but matching KMS key alias " +
                          config.s3_encryption_kms_alias +
                          " could not be found in the account",
                        bucketLocation,
                        bucketResource,
                        custom
                      );
                      ...
                      helpers.addResult(
                        ...
                        "Bucket: " +
                          ...
                          " has " +
                          algo +
                          " encryption enabled using required KMS key: " +
                          keyArn,
                        bucketLocation,
                        bucketResource,
                        custom
                      ...
                      if (algo !== "aws:kms") {
                        msg =
                          ...
                          " encryption (" +
                          algo +
                          ") is not configured to use required KMS key";
                      } else {
                        msg =
                          "Bucket: " +
                          bucket.Name +
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    if (settings.input && settings.input.kmsKeyId) {
      params = {
        Bucket: bucketName,
