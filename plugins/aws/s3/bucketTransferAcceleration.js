...
module.exports = {
  ...
  run: function (cache, settings, callback) {
    ...
    listBuckets.data.forEach(function (bucket) {
      ...
      if (
        !getBucketAccelerateConfiguration ||
        getBucketAccelerateConfiguration.err ||
        !getBucketAccelerateConfiguration.data
      ) {
        helpers.addResult(
          results,
          3,
          `Unable to get bucket acceleration configuration: ${helpers.addError(
            getBucketAccelerateConfiguration
          )}`,
          bucketLocation,
          resource
      ...
      if (
        getBucketAccelerateConfiguration.data.Status &&
        getBucketAccelerateConfiguration.data.Status.toUpperCase() === "ENABLED"
      ) {
        helpers.addResult(
          results,
          0,
