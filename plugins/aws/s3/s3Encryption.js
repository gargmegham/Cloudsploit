      ...
      if (statementTargetsAction(statement, "s3:PutObject")) {
        if (statement.Condition && statement.Condition.StringNotEquals) {
          ...
          if (
            ...
            return {
              key: statement.Condition.StringNotEquals[
                "s3:x-amz-server-side-encryption-aws-kms-key-id"
              ],
            };
          }
...
function getKeyEncryptionLevel(kmsKey) {
  return kmsKey.Origin === "AWS_CLOUDHSM"
    ? "cloudhsm"
    : kmsKey.Origin === "EXTERNAL"
    ? "externalcmk"
    : kmsKey.KeyManager === "CUSTOMER"
    ? "awscmk"
    : "awskms";
}
...
module.exports = {
  ...
  settings: {
    s3_required_encryption_level: {
      ...
      description:
        "In order (low to high) sse=Server-Side Encryption; awskms=AWS KMS; awscmk=Customer KMS; externalcmk=Customer external KMS; cloudhsm=Customer CloudHSM",
  ...
  run: function (cache, settings, callback) {
    ...
    var desiredEncryptionLevelString =
      settings.s3_required_encryption_level ||
      this.settings.s3_required_encryption_level.default;
    var s3_allow_unencrypted_static_websites =
      settings.s3_allow_unencrypted_static_websites ||
      this.settings.s3_allow_unencrypted_static_websites.default;
    if (
      !desiredEncryptionLevelString.match(
        this.settings.s3_required_encryption_level.regex
      )
    ) {
      helpers.addResult(
        results,
        3,
        "Settings misconfigured for S3 Encryption Enforcement."
      );
    ...
    for (let bucket of listBuckets.data) {
      ...
      if (
        getBucketPolicy &&
        getBucketPolicy.err &&
        getBucketPolicy.err.code &&
        getBucketPolicy.err.code === "NoSuchBucketPolicy"
      ) {
        helpers.addResult(
          results,
          2,
          "No bucket policy found; encryption not enforced",
          bucketLocation,
          bucketResource
        );
        continue;
      ...
      if (
        ...
        !getBucketPolicy.data.Policy
      ) {
        helpers.addResult(
          results,
          3,
          `Error querying for bucket policy on bucket: ${
            bucket.Name
      ...
      try {
        ...
        helpers.addResult(
          results,
          3,
          `Bucket policy on bucket [${bucket.Name}] could not be parsed.`,
          bucketLocation,
          bucketResource
        );
        continue;
      ...
      if (!policyJson.Statement.length) {
        helpers.addResult(
          results,
          2,
          "Bucket policy does not contain any statements; encryption not enforced",
          bucketLocation,
          bucketResource
        );
        continue;
      ...
      var statementEncryptionLevels = policyJson.Statement.map((statement) => {
        ...
        if (encryptionLevel.key) {
          ...
          const describeKey = helpers.addSource(cache, source, [
            "kms",
            "describeKey",
            region,
            keyId,
      ...
      const currentEncryptionLevel = statementEncryptionLevels.reduce(
        (max, level) =>
          encryptionLevelMap[level] > encryptionLevelMap[max] ? level : max,
        "none"
      );

      if (
        encryptionLevelMap[currentEncryptionLevel] <
        encryptionLevelMap[desiredEncryptionLevelString]
      ) {
        helpers.addResult(
          results,
          2,
          `Bucket policy does not enforce encryption to ${desiredEncryptionLevelString}, policy currently enforces: ${currentEncryptionLevel}`,
          bucketLocation,
          bucketResource
        );
      } else {
        helpers.addResult(
          results,
          0,
          `Bucket policy enforces encryption to ${desiredEncryptionLevelString}, policy currently enforces: ${currentEncryptionLevel}`,
          bucketLocation,
          bucketResource
        );
      }
