          ...
          Rules: [
            {
              ApplyServerSideEncryptionByDefault: {
                ...
                KMSMasterKeyID:
                  "arn:aws:kms:us-east-1:0123456789101:key/abc0123",
              },
            },
          ],
        },
          ...
          abc0123: {
            data: {
              KeyMetadata: {
                ...
                Description: cmk
                  ? "My key"
                  : "Default master key that protects my S3 objects when no other key is defined",
              ...
              Items: [
                {
                  ...
                  Name: "testConfig",
                  LocationUri:
                    "s3://bucket1/mine/AWSLogs/000011112222/Config/test",
                  ValidatorTypes: ["LAMBDA"],
                  Type: "AWS.Freeform",
                },
              ],
            },
  ...
  describe("run", function () {
    ...
    it("should give passing result if S3 bucket has CMK KMS encryption with provided alias", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          "has aws:kms encryption enabled using required KMS key"
        );
        expect(results[0].region).to.equal("us-east-1");
        done();
      };
    ...
    it("should give passing result if S3 bucket has CMK KMS encryption with one of provided aliases", function (done) {
      ...
      s3.run(
        cache,
        { s3_encryption_kms_alias: "alias/some-other-alias,alias/my-alias" },
        callback
      );
    });
    ...
    it("should give failing result if S3 bucket has CMK KMS encryption without provided alias", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].status).to.equal(2);
        expect(results[0].message).to.include(
          "but matching KMS key alias alias/my-unknown-alias could not be found in the account"
        );
        expect(results[0].region).to.equal("us-east-1");
        done();
      };
      ...
      s3.run(
        cache,
        { s3_encryption_kms_alias: "alias/my-unknown-alias" },
        callback
      );
    });
    ...
    it("should give passing result if S3 bucket requires CMK but has AES256 KMS encryption as a CloudFront origin", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          "has AES256 encryption enabled without a CMK but is a CloudFront origin"
        );
        expect(results[0].region).to.equal("us-east-1");
        done();
      };
    ...
    it("should give passing result if S3 bucket requires alias but has AES256 KMS encryption as a CloudFront origin", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          "has AES256 encryption enabled but is a CloudFront origin"
        );
        expect(results[0].region).to.equal("us-east-1");
        done();
      };
    ...
    it("should give failing result if S3 bucket requires alias but has AES256 KMS encryption not as a CloudFront origin", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].status).to.equal(2);
        expect(results[0].message).to.include(
          "encryption (AES256) is not configured to use required KMS key"
        );
        expect(results[0].region).to.equal("us-east-1");
        done();
      };
