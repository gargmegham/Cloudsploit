...
module.exports = {
  ...
  more_info:
    "S3 buckets should be configured to strictly require SSL connections " +
    ...
    listBuckets.data.forEach((bucket) => {
      ...
      if (
        ...
        statements.forEach((statement) => {
          if (!statement.Sid || !statement.Sid.length)
            statement.Sid = Math.random().toString(36).substring(2, 7);
        });
        ...
        for (var statement of statements) {
          if (
            ...
            if (!helpers.isEffectiveStatement(statement, denyPermissionsMap))
              continue;
            ...
            var denyActionResourceMap = helpers.getDenyPermissionsMap(
              statements,
              statement.Sid
            );
            if (!helpers.isEffectiveStatement(statement, denyActionResourceMap))
              continue;
