...
module.exports = {
  ...
  compliance: {
    hipaa:
      "HIPAA requires that access to protected information is controlled and audited. " +
  ...
  run: function (cache, settings, callback) {
        ...
        for (let origin of distribution.Origins.Items) {
          if (origin.S3OriginConfig && origin.DomainName) {
            ...
            if (
              ...
              let oaiId = origin.S3OriginConfig.OriginAccessIdentity.substring(
                origin.S3OriginConfig.OriginAccessIdentity.lastIndexOf("/") + 1
              );
              let cfUser = `arn:aws:iam::cloudfront:user/CloudFront Origin Access Identity ${oaiId}`;
              if (s3BucketAssociations[bucketName]) {
                if (s3BucketAssociations[bucketName][distribution.Id])
                  s3BucketAssociations[bucketName][distribution.Id].push(
                    cfUser
                  );
                else {
    ...
    if (!s3OriginFound) {
      helpers.addResult(
        results,
        0,
        "No S3 origins found for CloudFront distributions"
      );
      return callback(null, results, source);
    }

    async.each(
      Object.entries(s3BucketAssociations),
      function ([bucketName, cfDistributions], cb) {
        ...
        if (Object.keys(cfDistributions).length > 1) {
          helpers.addResult(
            results,
            2,
            `S3 bucket is origin to more than one distributions which are these: ${Object.keys(
              cfDistributions
            ).join(", ")}`,
            bucketLocation,
        ...
        if (
          ...
          helpers.addResult(
            results,
            3,
            `Error querying for bucket policy for bucket "${bucketName}": ${helpers.addError(
              getBucketPolicy
            )}`,
            bucketLocation,
            `arn:aws:s3:::${bucketName}`
        ...
        for (var statement of statements) {
          ...
          for (var principal of principals) {
            if (
              ...
              !s3BucketAssociations[bucketName][distributionId].includes(
                principal
              ) &&
        ...
        var missingOrigins = s3BucketAssociations[bucketName][
          distributionId
        ].filter(function (item) {
          return (
            !allowedOrigins.includes(item) && !restrictedOrigins.includes(item)
          );
        });
        ...
        if (unknownPrincipals.length || restrictedOrigins.length) {
          ...
          helpers.addResult(
            results,
            0,
            `S3 bucket is origin to only one CloudFront distribution which is: ${distributionId}`,
            bucketLocation,
            `arn:aws:s3:::${bucketName}`
          );
        }
