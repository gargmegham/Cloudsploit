    ...
    listBuckets.data.forEach(function (bucket) {
      ...
      if (
        ...
        helpers.addResult(
          results,
          3,
          "Error querying bucket versioning for : " +
            bucket.Name +
            ": " +
        ...
        if (
          getBucketLifecycleConfiguration &&
          getBucketLifecycleConfiguration.err &&
          getBucketLifecycleConfiguration.err.code &&
          getBucketLifecycleConfiguration.err.code ==
          ...
          !getBucketLifecycleConfiguration ||
          getBucketLifecycleConfiguration.err ||
          !getBucketLifecycleConfiguration.data
        ) {
          helpers.addResult(
            results,
            3,
            `Unable to query for S3 bucket lifecycle configuration: ${helpers.addError(
              getBucketLifecycleConfiguration
            )}`,
            bucketLocation,
            "arn:aws:s3:::" + bucket.Name
          ...
          var ruleExists = getBucketLifecycleConfiguration.data.Rules.find(
            (rule) => rule.Status && rule.Status.toUpperCase() === "ENABLED"
          );

          if (ruleExists) {
            var ruleForNonCurrent =
              getBucketLifecycleConfiguration.data.Rules.find(
                (rule) =>
                  rule.Status &&
