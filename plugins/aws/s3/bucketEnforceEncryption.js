...
module.exports = {
  ...
  settings: {
    s3_enforce_encryption_require_cmk: {
      ...
      description:
        "When set to true S3 policies that enforce encryption but use AWS SSE will fail",
      regex: "^(true|false)$",
      default: "false",
    },
  ...
  run: function (cache, settings, callback) {
    var config = {
      s3_enforce_encryption_require_cmk:
        settings.s3_enforce_encryption_require_cmk ||
        this.settings.s3_enforce_encryption_require_cmk.default,
      s3_enforce_encryption_allow_pattern:
        settings.s3_enforce_encryption_allow_pattern ||
        this.settings.s3_enforce_encryption_allow_pattern.default,
    };

    config.s3_enforce_encryption_require_cmk =
      config.s3_enforce_encryption_require_cmk == "true";
    ...
    var allowRegex =
      config.s3_enforce_encryption_allow_pattern &&
      config.s3_enforce_encryption_allow_pattern.length
        ? new RegExp(config.s3_enforce_encryption_allow_pattern)
        : false;
    ...
    for (var i in listBuckets.data) {
      ...
      if (
        ...
        helpers.addResult(
          results,
          3,
          "Error querying for bucket policy for bucket: " +
            bucket.Name +
            ": " +
          ...
          if (!policyJson || !policyJson.Statement) {
            ...
            if (nullCondition && encryptionType) {
              ...
              helpers.addResult(
                results,
                2,
                "Bucket is missing required encryption enforcement policies.",
                bucketLocation,
                bucketResource
              );
            }
