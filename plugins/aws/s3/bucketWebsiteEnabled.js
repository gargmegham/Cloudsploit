...
module.exports = {
  ...
  settings: {
    s3_website_whitelist_empty_buckets: {
      ...
      description:
        "When set to true, whitelist empty buckets without checking bucket website configutation",
      regex: "^(true|false)$",
      default: "false",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      s3_website_whitelist_empty_buckets:
        settings.s3_website_whitelist_empty_buckets ||
        this.settings.s3_website_whitelist_empty_buckets.default,
    };
    var ignoreEmptyBuckets =
      config.s3_website_whitelist_empty_buckets == "true";
    ...
    listBuckets.data.forEach(function (bucket) {
      ...
      if (ignoreEmptyBuckets) {
        ...
        if (!listObjects || listObjects.err || !listObjects.data) {
          helpers.addResult(
            results,
            3,
            "Unable to list S3 bucket objects: " +
              helpers.addError(listObjects),
            bucketLocation,
      ...
      if (
        ...
        helpers.addResult(
          ...
          "Bucket : " +
            bucket.Name +
            " does not have static website hosting enabled",
          bucketLocation,
          "arn:aws:s3:::" + bucket.Name
        );
      } else if (
        ...
        helpers.addResult(
          results,
          3,
          "Error querying bucket website for : " +
            bucket.Name +
            ": " +
