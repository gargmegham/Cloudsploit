...
module.exports = {
  ...
  more_info:
    "S3 buckets can be configured to allow anyone, regardless of whether they are an AWS user or not, to write objects to a bucket or delete objects. This option should not be configured unless there is a strong business requirement.",
  recommended_action:
    ...
    for (var i in listBuckets.data) {
      ...
      if (!getBucketAcl || getBucketAcl.err || !getBucketAcl.data) {
        helpers.addResult(
          results,
          3,
          "Error querying for bucket ACL for bucket: " +
            bucket.Name +
            ": " +
        ...
        for (var g in getBucketAcl.data.Grants) {
          ...
          if (
            ...
            if (uri === ACL_ALL_USERS) {
              bucketIssues.push(
                "ACL Grantee AllUsers allowed permission: " + permission
              );
              if (permission === "READ") {
              ...
              bucketIssues.push(
                "Grantee AuthenticatedUsers allowed permission: " + permission
              );
              if (permission === "READ") {
                if (bucketResult < 1) bucketResult = 1;
              } else {
