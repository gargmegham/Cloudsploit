  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.app_runner_service_desired_encryption_level ||
        this.settings.app_runner_service_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listServices.err || !listServices.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query Service: " + helpers.addError(listServices),
            region
          );
          return rcb();
        }
        ...
        for (let service of listServices.data) {
          ...
          if (
            !describeService ||
            describeService.err ||
            !describeService.data
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to get Service description: ${helpers.addError(
                describeService
              )}`,
              region,
              resource
          ...
          if (
            describeService.data.Service &&
            describeService.data.Service.EncryptionConfiguration &&
