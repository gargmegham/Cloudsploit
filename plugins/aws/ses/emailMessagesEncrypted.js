...
module.exports = {
  ...
  settings: {
    ses_email_desired_encryption_level: {
      ...
      description:
        "Desired encryption level for email messages to encrypt them before they get saves on S3",
      regex:
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.ses_email_desired_encryption_level ||
        this.settings.ses_email_desired_encryption_level.default,
    };
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        if (
          describeActiveReceiptRuleSet.err &&
          describeActiveReceiptRuleSet.err.message &&
          describeActiveReceiptRuleSet.err.message.includes(
        ...
        if (
          ...
          helpers.addResult(
            results,
            3,
            "Unable to query for SES active rule set: " +
        ...
        if (
          ...
          !describeActiveReceiptRuleSet.data.Rules.length
        ) {
          helpers.addResult(
            results,
            0,
            "SES active rule set does not have any rules",
            region,
            resource
          );
          return rcb();
        ...
        for (let rule of describeActiveReceiptRuleSet.data.Rules) {
          ...
          if (!rule.Enabled) {
            helpers.addResult(
              results,
              0,
              "SES active rule set rule is not enabled",
              region,
              resource
            );
            continue;
          ...
          if (!s3Action)
            helpers.addResult(
              results,
              0,
              "SES active rule set rule does not have action to deliver to S3",
              region,
              resource
            );
          else {
            if (s3Action.S3Action.KmsKeyArn) {
              if (s3Action.S3Action.KmsKeyArn.includes("alias/aws/ses"))
                currentEncryptionLevel = 2;
              else {
                let kmsKeyId = s3Action.S3Action.KmsKeyArn.split("/")[1]
                  ? s3Action.S3Action.KmsKeyArn.split("/")[1]
                  : s3Action.S3Action.KmsKeyArn;
              ...
              let currentEncryptionLevelString =
