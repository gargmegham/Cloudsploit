      ...
      function (region, rcb) {
        ...
        if (listIdentities.err || !listIdentities.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for SES identities: " +
              helpers.addError(listIdentities),
            region
          );
          return rcb();
        }
        ...
        if (
          ...
          helpers.addResult(
            results,
            3,
            "Unable to get SES DKIM attributes: " +
              helpers.addError(getIdentityDkimAttributes),
            region
          );
          return rcb();
        }
        ...
        for (var i in getIdentityDkimAttributes.data.DkimAttributes) {
          ...
          if (!identity.DkimEnabled) {
            ...
            helpers.addResult(
              results,
              1,
              "DKIM is enabled, but not configured properly",
              region,
              resource
            );
          } else {
            helpers.addResult(
              results,
              0,
              "DKIM is enabled and configured properly",
              region,
              resource
            );
          }
