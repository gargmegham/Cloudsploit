  ...
  describe("run", function () {
    ...
    it("should UNKNOWN if Unable to query for App Mesh meshes", function (done) {
      const cache = createCache(null, null, {
        message: "Unable to query for App Mesh meshes",
      });
      restrictExternalTraffic.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if Unable to describe App Mesh mesh", function (done) {
      const cache = createCache([listMeshes[0]], null, null, {
        message: "Unable to describe App Mesh mesh",
      });
      restrictExternalTraffic.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(3);
        expect(results[0].region).to.equal("us-east-1");
        expect(results[0].message).to.include(
          "Unable to describe App Mesh mesh"
        );
        done();
      });
    });
