...
const createCache = (
  ...
  listMeshesErr,
  listVirtualGatewaysErr,
  describeVirtualGatewayErr
) => {
  var name = listMeshes && listMeshes.length ? listMeshes[0].meshName : null;
  var gatewayName =
    listVirtualGateways && listVirtualGateways.length
      ? listVirtualGateways[0].virtualGatewayName
      : null;
  return {
    appmesh: {
      listMeshes: {
  ...
  describe("run", function () {
    it("should PASS if App Mesh vitual gateway listeners restrict TLS enabled connections", function (done) {
      const cache = createCache(
        [listMeshes[0]],
        [listVirtualGateways[0]],
        describeVirtualGateway[0]
      );
      appmeshTLSRequired.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "App Mesh virtual gateway listeners restrict TLS enabled connections"
        );
        done();
      });
    });
    ...
    it("should FAIL if App Mesh vitual gateway listeners does not restrict TLS enabled connections", function (done) {
      const cache = createCache(
        [listMeshes[1]],
        [listVirtualGateways[1]],
        describeVirtualGateway[1]
      );
      appmeshTLSRequired.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        ...
        expect(results[0].region).to.equal("us-east-1");
        expect(results[0].message).to.include(
          "App Mesh virtual gateway listeners does not restrict TLS enabled connections"
        );
        done();
      });
    });
    ...
    it("should UNKNOWN if Unable to list App Mesh meshes", function (done) {
      const cache = createCache(null, null, null, {
        message: "Unable to list App Mesh meshes",
      });
      appmeshTLSRequired.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if unable to list App Mesh virtual gateways", function (done) {
      const cache = createCache(
        ...
        describeVirtualGateway[0],
        null,
        { message: "Unable to list App Mesh virtual gateways" }
      );
      appmeshTLSRequired.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
        expect(results[0].status).to.equal(3);
