      ...
      function (region, rcb) {
        ...
        if (listMeshes.err || !listMeshes.data) {
          helpers.addResult(
            results,
            3,
            `Unable to query for App Mesh meshes: ${helpers.addError(
              listMeshes
            )}`,
            region
          );
        ...
        for (let mesh of listMeshes.data) {
          ...
          if (
            ...
            helpers.addResult(
              results,
              3,
              `Unable to describe App Mesh mesh: ${helpers.addError(
                describeMesh
              )}`,
              region,
              resource
          ...
          if (
            describeMesh.data.mesh.spec &&
            describeMesh.data.mesh.spec.egressFilter &&
            describeMesh.data.mesh.spec.egressFilter.type.toUpperCase() ===
              "ALLOW_ALL"
          ) {
            helpers.addResult(
              results,
