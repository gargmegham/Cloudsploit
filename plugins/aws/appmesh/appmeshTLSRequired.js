      ...
      function (region, rcb) {
        ...
        if (listMeshes.err || !listMeshes.data) {
          helpers.addResult(
            results,
            3,
            "Unable to list App Mesh meshes: " + helpers.addError(listMeshes),
            region
          );
          return rcb();
        }
        ...
        for (let mesh of listMeshes.data) {
          ...
          if (
            !listVirtualGateways ||
            listVirtualGateways.err ||
            !listVirtualGateways.data
          ) {
            helpers.addResult(
              results,
              3,
              "Unable to list App Mesh virtual gateways: " +
                mesh.meshName +
                ": " +
                helpers.addError(listVirtualGateways),
              region,
              resource
            );
            continue;
          ...
          if (
            !listVirtualGateways.data.virtualGateways ||
            !listVirtualGateways.data.virtualGateways.length
          ) {
            helpers.addResult(
              results,
              0,
              "No App Mesh virtual gateways found",
              region,
              resource
            );
            continue;
          ...
          for (let gateway of listVirtualGateways.data.virtualGateways) {
            ...
            if (
              ...
              helpers.addResult(
                results,
                3,
                "Unable to describe App Mesh virtual gateway: " +
                  helpers.addError(describeVirtualGateway),
                region,
                gatewayArn
              );
              continue;
            ...
            if (
              ...
              helpers.addResult(
                results,
                0,
                "App Mesh virtual gateway does not have listeners",
                region,
                gatewayArn
              );
            } else {
              const tlsEnabled =
                describeVirtualGateway.data.virtualGateway.spec.listeners.every(
                  (listener) =>
                    listener.tls &&
                    listener.tls.mode &&
                    listener.tls.mode.toUpperCase() === "STRICT"
                );
              const status = tlsEnabled ? 0 : 2;
              helpers.addResult(
                results,
                status,
                `App Mesh virtual gateway listeners ${
                  tlsEnabled ? "restrict" : "does not restrict"
