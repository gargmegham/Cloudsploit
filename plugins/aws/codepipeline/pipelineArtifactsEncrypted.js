  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.pipeline_artifacts_desired_encryption_level ||
        this.settings.pipeline_artifacts_desired_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listPipelines.err || !listPipelines.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query Pipeline Artifacts: " +
        ...
        for (let pipeline of listPipelines.data) {
          ...
          if (!getPipeline || getPipeline.err || !getPipeline.data) {
            helpers.addResult(
              results,
              3,
              `Unable to get Pipeline Artifacts description: ${helpers.addError(
                getPipeline
              )}`,
              region,
              resource
          ...
          if (
            ...
            var kmsKey =
              getPipeline.data.pipeline.artifactStore.encryptionKey.id;
            var kmsKeyArn = kmsAliasArnMap[kmsKey]
              ? kmsAliasArnMap[kmsKey]
              : null;
