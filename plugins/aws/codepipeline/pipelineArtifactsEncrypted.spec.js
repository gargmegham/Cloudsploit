...
const getPipeline = [
  {
    pipeline: {
      name: "sad",
      roleArn:
        "arn:aws:iam::000111222333:role/service-role/AWSCodePipelineServiceRole-us-east-1-sad",
      artifactStore: {
        type: "S3",
...
describe("pipelineArtifactsEncrypted", function () {
  describe("run", function () {
    it("should PASS if Pipeline Artifacts is encrypted with desired encryption level", function (done) {
      ...
      pipelineArtifactsEncrypted.run(
        cache,
        { pipeline_artifacts_encryption: "awscmk" },
        (err, results) => {
    ...
    it("should UNKNOWN if unable to list Pipeline Artifacts", function (done) {
      const cache = createCache(null, null, null, {
        message: "Unable to list Pipeline Artifacts",
      });
      pipelineArtifactsEncrypted.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
    ...
    it("should UNKNOWN if unable to list KMS keys", function (done) {
      const cache = createCache(listPipelines, null, null, null, {
