...
module.exports = {
  ...
  settings: {
    acm_certificate_expiry_pass: {
      ...
      description:
        "Return a passing result when certificate expiration date exceeds this number of days in the future",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: 45,
    },
    acm_certificate_expiry_warn: {
      ...
      description:
        "Return a warning result when certificate expiration date exceeds this number of days in the future",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: 30,
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      acm_certificate_expiry_pass:
        settings.acm_certificate_expiry_pass ||
        this.settings.acm_certificate_expiry_pass.default,
      acm_certificate_expiry_warn:
        settings.acm_certificate_expiry_warn ||
        this.settings.acm_certificate_expiry_warn.default,
    };
    ...
    async.each(
      ...
      function (region, rcb) {
        ...
        if (listCertificates.err || !listCertificates.data) {
          helpers.addResult(
            ...
            "Unable to list ACM certificates: " +
              helpers.addError(listCertificates),
            region
          );
          return rcb();
        }
        ...
        listCertificates.data.forEach(function (cert) {
          ...
          if (
            !describeCertificate ||
            describeCertificate.err ||
            !describeCertificate.data
          ) {
            helpers.addResult(
              results,
              3,
              "Unable to describe ACM certificate: " +
                helpers.addError(describeCertificate),
              region,
              cert.CertificateArn
            );
          } else if (
            !describeCertificate.data.Certificate ||
            !describeCertificate.data.Certificate.NotAfter
          ) {
            if (
              describeCertificate.data.Certificate &&
              ...
              helpers.addResult(
                results,
                1,
                "ACM certificate is not eligible for renewal",
                region,
                cert.CertificateArn
              );
            } else {
            ...
            var difference = Math.round(
              (new Date(then).getTime() - new Date(now).getTime()) /
                (24 * 60 * 60 * 1000)
            );
            var expiresInMsg =
