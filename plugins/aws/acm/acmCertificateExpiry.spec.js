...
const listCertificates = [
  {
    CertificateArn:
      "arn:aws:acm:us-east-1:000011112222:certificate/a59bf6c5-faba-4e45-8b20-054d90e41500",
  ...
  describe("run", function () {
    ...
    it("should WARN if certificate expiration date exceed set WARN number of days in the future", function (done) {
      const cache = createCache(
        [listCertificates[0]],
        null,
        describeCertificate[3]
      );
      acmCertificateExpiry.run(
        cache,
        { acm_certificate_expiry_warn: "25" },
        (err, results) => {
          expect(results.length).to.equal(1);
