...
const listCertificates = [
  {
    CertificateArn:
      "arn:aws:acm:us-east-1:000011122222:certificate/4eb67297-7e64-4f9b-bfd1-38f962df491c",
    DomainName: "CoolDude69.com",
  },
  {
    CertificateArn:
      "arn:aws:acm:us-east-1:000011122222:certificate/8951d876-8417-4d5c-8caa-5f73a3b1211e",
...
const describeCertificate = [
  {
    Certificate: {
      CertificateArn:
        "arn:aws:acm:us-east-1:000011122222:certificate/4eb67297-7e64-4f9b-bfd1-38f962df491c",
      DomainName: "cooldude69.com",
      SubjectAlternativeNames: ["cooldude69.com"],
      DomainValidationOptions: [
        {
          ...
          ResourceRecord: {
            ...
            Type: "CNAME",
            Value:
              "_788985d4b083a63504b1d46c7db65abb.gbwdrhjxvn.acm-validations.aws.",
          },
  ...
  },
  {
    Certificate: {
      CertificateArn:
        "arn:aws:acm:us-east-1:000011122222:certificate/8951d876-8417-4d5c-8caa-5f73a3b1211e",
      DomainName: "*.viteace.com",
      SubjectAlternativeNames: ["*.viteace.com"],
      DomainValidationOptions: [
        {
          DomainName: "*.viteace.com",
          ...
          ResourceRecord: {
            ...
            Type: "CNAME",
            Value:
              "_2711d4a887c089c7518bfdb01655daf3.gbwdrhjxvn.acm-validations.aws.",
          },
          ValidationMethod: "DNS",
        },
      ],
...
describe("acmSingleDomainNameCertificate", function () {
  describe("run", function () {
    it("should PASS if ACM certificate is a single domain name certificate", function (done) {
      ...
      acmSingleDomainNameCertificate.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "ACM certificate is a single domain name certificate"
        );
        done();
      });
    });
    ...
    it("should FAIL if ACM certificate is a wildcard certificate", function (done) {
      ...
      acmSingleDomainNameCertificate.run(
        cache,
        { acm_certificate_expiry_warn: "35" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(2);
          expect(results[0].region).to.equal("us-east-1");
          expect(results[0].message).to.include(
            "ACM certificate is a wildcard certificate"
          );
          done();
        }
      );
    ...
    it("should UNKNOWN if unable to describe ACM certificate", function (done) {
      const cache = createCache([listCertificates[0]], null, null, {
        message: "err",
      });
      acmSingleDomainNameCertificate.run(cache, {}, (err, results) => {
        expect(results.length).to.equal(1);
