        ...
        listCertificates.data.forEach(function (cert) {
          ...
          if (
            ...
            helpers.addResult(
              results,
              0,
              "Non AWS-issued certificates do not support AWS DNS validation",
              region,
              cert.CertificateArn
            );
          } else if (
            ...
            helpers.addResult(
              results,
              2,
              "ACM certificate does not have DomainValidationOptions",
              region,
              cert.CertificateArn
            );
          } else {
            describeCertificate.data.Certificate.DomainValidationOptions.forEach(
              function (domain) {
                if (
                  !domain.ValidationStatus ||
                  domain.ValidationStatus != "SUCCESS"
                ) {
                  helpers.addResult(
                    results,
                    2,
