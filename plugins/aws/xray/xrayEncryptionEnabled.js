...
module.exports = {
  ...
  settings: {
    xray_encryption_level: {
      ...
      description:
        "In order (lowest to highest) sse=Default Encryption; awskms=AWS-managed KMS; awscmk=Customer managed KMS; externalcmk=Customer managed externally sourced KMS; cloudhsm=Customer managed CloudHSM sourced KMS",
  ...
  run: function (cache, settings, callback) {
    var config = {
      xray_encryption_level:
        settings.xray_encryption_level ||
        this.settings.xray_encryption_level.default,
    };

    var desiredEncryptionLevel = helpers.ENCRYPTION_LEVELS.indexOf(
      config.xray_encryption_level
    );
      ...
      function (region, rcb) {
        ...
        if (getEncryptionConfig.err || !getEncryptionConfig.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for XRay encryption configuration: " +
              helpers.addError(getEncryptionConfig),
            region
          );
          return rcb();
        }
        ...
        if (
          ...
          if (currentEncryptionLevel >= desiredEncryptionLevel) {
            helpers.addResult(
              results,
              0,
              `XRay is configured to use encryption at level ${currentEncryptionLevelString} which is greater than or equal to desired level ${config.xray_encryption_level}`,
              region
            );
          } else {
            helpers.addResult(
              results,
              2,
              `XRay is configured to use encryption at level ${currentEncryptionLevelString} which is less than desired level ${config.xray_encryption_level}`,
              region
            );
          }
        } else {
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    let defaultKeyDesc =
      "Default master key that protects my X-Ray data when no other key is defined";
    ...
    if (settings.input && settings.input.kmsKeyIdforXray) {
      params = {
        Type: "KMS",
        KeyId: settings.input.kmsKeyIdforXray,
      };
    } else {
      let defaultKmsKeyId = helpers.getDefaultKeyId(
        cache,
        config.region,
        defaultKeyDesc
      );
      if (!defaultKmsKeyId)
        return callback(`No default X-Ray key for the region ${config.region}`);
      params = {
        Type: "KMS",
    ...
    helpers.remediatePlugin(config, putCall[0], params, function (err) {
      ...
      remediation_file["post_remediate"]["actions"][pluginName][region] =
        action;
