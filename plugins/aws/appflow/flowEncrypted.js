  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      desiredEncryptionLevelString:
        settings.appflow_flow_encryption_level ||
        this.settings.appflow_flow_encryption_level.default,
    };
      ...
      function (region, rcb) {
        ...
        if (listFlows.err || !listFlows.data) {
          helpers.addResult(
            results,
            3,
            `Unable to list AppFlow flows: ${helpers.addError(listFlows)}`,
            region
          );
          return rcb();
        }
        ...
        if (!listFlows.data.length) {
          helpers.addResult(results, 0, "No AppFlow flows found", region);
          return rcb();
        }
        ...
        for (let flow of listFlows.data) {
          ...
          if (!describeFlow || describeFlow.err || !describeFlow.data) {
            helpers.addResult(
              results,
              3,
              `Unable to describe flow: ${helpers.addError(describeFlow)}`,
              region,
              resource
            );
            continue;
          ...
          if (describeFlow.data.kmsArn) {
            var kmsKeyId = describeFlow.data.kmsArn.split("/")[1]
              ? describeFlow.data.kmsArn.split("/")[1]
              : describeFlow.data.kmsArn;
