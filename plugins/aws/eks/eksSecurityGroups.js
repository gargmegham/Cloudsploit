      ...
      function (region, rcb) {
        ...
        if (listClusters.err || !listClusters.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for EKS clusters: " +
        ...
        if (
          !describeSecurityGroups ||
          describeSecurityGroups.err ||
          !describeSecurityGroups.data
        ) {
          helpers.addResult(
            results,
            3,
            "Unable to describe security group rules: " +
        ...
        for (var c in listClusters.data) {
          ...
          if (
            ...
            helpers.addResult(
              results,
              3,
              "Unable to describe EKS cluster: " +
          ...
          if (
            ...
            for (s in groups) {
              var group = groups[s];
              if (sgMap[group])
                effectiveInbound = effectiveInbound.concat(sgMap[group]);
            }
            ...
            if (effectiveInbound.length) {
              helpers.addResult(
                results,
                2,
                "EKS control plane security groups allow additional access on unnecessary port ranges: " +
                  effectiveInbound.join(", "),
                region,
                arn
              );
            } else {
              helpers.addResult(
                results,
                0,
                "EKS control plane security groups do not contain unnecessary ports",
                region,
                arn
              );
            }
            ...
            helpers.addResult(
              results,
              1,
              "EKS control plane does not have security groups configured",
              region,
              arn
            );
          }
