      ...
      function (region, rcb) {
        ...
        for (var c in listClusters.data) {
          ...
          if (
            ...
            let versionDeprecationDate = deprecatedVersions[version]
              ? deprecatedVersions[version]
              : null;
            let versionOutdatedDate = outdatedVersions[version]
              ? outdatedVersions[version]
              : null;
            let today = new Date();
            let dateToday =
            ...
            if (versionDeprecationDate && today > versionDeprecationDate) {
              helpers.addResult(
                results,
                2,
                "EKS cluster is running Kubernetes: " +
                  version +
              ...
              helpers.addResult(
                results,
                0,
                "EKS cluster is running a current version of Kubernetes: " +
                  version,
                region,
                arn
              );
            ...
            helpers.addResult(
              results,
              2,
              "Unknown Kubernetes version found",
              region,
              arn
            );
          }
