...
module.exports = {
  ...
  settings: {
    glacier_vault_policy_condition_keys: {
      ...
      description:
        "Comma separated list of AWS IAM condition keys that should be allowed i.e. aws:SourceAccount, aws:SourceArn",
      regex: "^.*$",
      default:
        "aws:PrincipalArn,aws:PrincipalAccount,aws:PrincipalOrgID,aws:SourceOwner,aws:SourceArn,aws:SourceAccount",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      glacier_vault_policy_condition_keys:
        settings.glacier_vault_policy_condition_keys ||
        this.settings.glacier_vault_policy_condition_keys.default,
    };
    config.glacier_vault_policy_condition_keys =
      config.glacier_vault_policy_condition_keys.replace(/\s/g, "");
    var allowedConditionKeys =
      config.glacier_vault_policy_condition_keys.split(",");
    ...
    regions.glacier.forEach((region) => {
      ...
      if (listVaults.err || !listVaults.data) {
        helpers.addResult(
          results,
          3,
          `Unable to list S3 Glacier vaults: ${helpers.addError(listVaults)}`,
          region
        );
        return;
      }
      ...
      for (let vault of listVaults.data) {
        ...
        if (
          getVaultAccessPolicy &&
          getVaultAccessPolicy.err &&
          getVaultAccessPolicy.err.code &&
          getVaultAccessPolicy.err.code == "ResourceNotFoundException"
        ) {
          helpers.addResult(
            results,
            0,
            "S3 Glacier vault does not have any policy attached",
            region,
            resource
          );
          continue;
        ...
        if (
          !getVaultAccessPolicy ||
          getVaultAccessPolicy.err ||
          !getVaultAccessPolicy.data
        ) {
          helpers.addResult(
            results,
            3,
            `Unable to get vault policy: ${helpers.addError(
              getVaultAccessPolicy
            )}`,
            region,
            resource
        ...
        var statements =
          getVaultAccessPolicy.data.policy &&
          getVaultAccessPolicy.data.policy.Policy
            ? helpers.normalizePolicyDocument(
                getVaultAccessPolicy.data.policy.Policy
              )
            : [];
        ...
        for (let statement of statements) {
          var effectEval =
            statement.Effect && statement.Effect == "Allow" ? true : false;
          ...
          if (statement.Condition)
            scopedCondition = helpers.isValidCondition(
              statement,
        ...
        if (actions.length) {
          helpers.addResult(
            results,
            2,
            "S3 Glacoer vault policy allows global access to the action(s): " +
              actions,
            region,
            resource
          );
