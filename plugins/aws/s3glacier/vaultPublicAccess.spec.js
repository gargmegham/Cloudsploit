...
const vaultPolicy = [
  {
    policy: {
      Policy:
        '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":"111111111111","Action":"glacier:*","Resource":"arn:aws:glacier:us-east-1:111111111111:vaults/vault-access"}]}',
    },
  },
  {
    policy: {
      Policy:
        '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"AWS":"*"},"Action":"glacier:*","Resource":"arn:aws:glacier:us-east-1:111111111111:vaults/vault-access"}]}',
    },
  },
  {
    policy: {
...
const createCache = (vault, policy) => {
  var vaultName =
    vault && vault.length && vault[0].VaultName ? vault[0].VaultName : null;
  return {
    glacier: {
      listVaults: {
        "us-east-1": {
  ...
  describe("run", function () {
    ...
    it("should FAIL if S3 Glacoer vault policy allows global access to the actions", function (done) {
      ...
      vaultPublicAccess.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          `S3 Glacoer vault policy allows global access`
        );
        expect(results[0].region).to.equal("us-east-1");
        done();
      });
    ...
    it("should UNKNOWN if unable to list vaults", function (done) {
      ...
      vaultPublicAccess.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.equal(
          `Unable to list S3 Glacier vaults: ${message}`
        );
        done();
      });
    });
