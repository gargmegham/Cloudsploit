...
module.exports = {
  ...
  description:
    "Ensures HTTPS Only is enabled for App Services, redirecting all HTTP traffic to HTTPS",
  more_info:
    "Enabling HTTPS Only traffic will redirect all non-secure HTTP requests to HTTPS. HTTPS uses the SSL/TLS protocol to provide a secure connection.",
  recommended_action:
    "Enable HTTPS Only support SSL settings for all App Services",
  ...
  remediation_description:
    "The HTTPS-only option will be enabled for the web app",
  ...
  compliance: {
    hipaa:
      "HIPAA requires all data to be transmitted over secure channels. " +
      "App Service HTTPS redirection should be used to ensure site visitors " +
      "are always connecting over a secure channel.",
    pci:
      "All card holder data must be transmitted over secure channels. " +
      "App Service HTTPS redirection should be used to ensure site visitors " +
      "are always connecting over a secure channel.",
  },
      ...
      function (location, rcb) {
        ...
        if (webApps.err || !webApps.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query App Service: " + helpers.addError(webApps),
            location
          );
          return rcb();
        }
        ...
        webApps.data.forEach(function (webApp) {
          if (webApp.httpsOnly) {
            helpers.addResult(
              results,
              0,
              "The App Service has HTTPS Only enabled",
              location,
              webApp.id
            );
          } else {
            helpers.addResult(
              results,
              2,
              "The App Service does not have HTTPS Only enabled",
              location,
              webApp.id
            );
          }
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    var baseUrl =
      "https://management.azure.com/{resource}?api-version=2019-08-01";
    var method = "PUT";
