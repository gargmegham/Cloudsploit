...
module.exports = {
  ...
  more_info:
    "Enabling HTTP2.0 ensures that the App Service has the latest technology which improves server performance",
  recommended_action:
    "Enable HTTP 2.0 support in the general settings for all App Services",
  ...
  remediation_description:
    "The HTTP 2.0 option will be enabled for the web app",
          ...
          if (!webConfigs || webConfigs.err || !webConfigs.data) {
            ...
            if (webConfigs.data[0] && webConfigs.data[0].http20Enabled) {
              helpers.addResult(
                results,
                0,
                "App Service has HTTP 2.0 enabled",
                location,
                webApp.id
              );
            } else {
              helpers.addResult(
                results,
                2,
                "App Service does not have HTTP 2.0 enabled",
                location,
                webApp.id
              );
            }
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    var baseUrl =
      "https://management.azure.com/{resource}/config/web?api-version=2019-08-01";
    var method = "PATCH";
