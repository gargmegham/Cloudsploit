...
module.exports = {
  ...
  description:
    "Ensures Authentication is enabled for App Services, redirecting unauthenticated users to the login page.",
  more_info:
    "Enabling authentication will redirect all unauthenticated requests to the login page. It also handles authentication of users with specific providers (Azure Active Directory, Facebook, Google, Microsoft Account, and Twitter).",
  ...
  remediation_description:
    "The App Service Authentication option will be enabled for the web app",
  apis_remediate: ["webApps:list"],
  actions: {
    remediate: ["webApps:updateAuthSettings"],
  ...
  compliance: {
    hipaa:
      "HIPAA requires all application access to be restricted to known users " +
      ...
      function (location, rcb) {
        ...
        if (webApps.err || !webApps.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for App Services: " + helpers.addError(webApps),
            location
          );
          return rcb();
        }
        ...
        if (!webApps.data.length) {
          helpers.addResult(
            results,
            0,
            "No existing App Services found",
            location
          );
          return rcb();
        }
        ...
        webApps.data.forEach(function (webApp) {
          const authSettings = helpers.addSource(cache, source, [
            "webApps",
            "getAuthSettings",
            location,
            webApp.id,
          ...
          if (!authSettings || authSettings.err || !authSettings.data) {
            ...
            if (authSettings.data.enabled) {
              helpers.addResult(
                results,
                0,
                "App Service has App Service Authentication enabled",
                location,
                webApp.id
              );
            } else {
              helpers.addResult(
                results,
                2,
                "App Service does not have App Service Authentication enabled",
                location,
                webApp.id
              );
            }
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    var baseUrl =
      "https://management.azure.com/{resource}/config/authsettings?api-version=2019-08-01";
    var method = "PUT";
