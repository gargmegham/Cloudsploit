...
const backupConfigs = {
  ...
  enabled: true,
  storageAccountUrl:
    "https://akhtarrgdiag.blob.core.windows.net/appbackup?sp=rwdl&st=2022-03-16T07:51:37Z&se=2295-12-29T08:51:37Z&sv=2020-08-04&sr=c&sig=FeC0hGUrqJb6b%2Bh5qbIif84725sMjeqyNUzWa4tL3L4%3D",
  backupSchedule: {
    frequencyInterval: 7,
    frequencyUnit: "Day",
  ...
  describe("run", function () {
    ...
    it("should give passing result if web apps backups can not be configured", function (done) {
      ...
      backupRetentionPeriod.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "WebApps backup can not be configured for the function App"
        );
        expect(results[0].region).to.equal("eastus");
        done();
      });
    ...
    it("should give failing result if no web app backup config found", function (done) {
      ...
      backupRetentionPeriod.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "No backup configurations found for this WebApp"
        );
        expect(results[0].region).to.equal("eastus");
        done();
      });
    ...
    it("should give unknown result if unable to query for web app backup configs", function (done) {
      ...
      backupRetentionPeriod.run(cache, {}, (err, results) => {
        ...
        expect(results[0].status).to.equal(3);
        expect(results[0].message).to.include(
          "Unable to query for Web App Backup Configs"
        );
        expect(results[0].region).to.equal("eastus");
        done();
      });
    ...
    it("should give passing result if web app backup retention period is within the recommended threshold", function (done) {
      ...
      backupRetentionPeriod.run(
        cache,
        { webapps_backup_retention_period: 7 },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          expect(results[0].message).to.include(
            "WebApp has a backup retention period of 7 of 7 days"
          );
          expect(results[0].region).to.equal("eastus");
          done();
        }
    ...
    it("should give failing result if web app backup retention period is below the recommended threshold", function (done) {
      ...
      backupRetentionPeriod.run(
        cache,
        { webapps_backup_retention_period: 10 },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(2);
          expect(results[0].message).to.include(
            "WebApp has a backup retention period of 7 of 10 days limit"
          );
          expect(results[0].region).to.equal("eastus");
          done();
        }
