...
module.exports = {
  ...
  more_info:
    "Remote debugging feature requires specific inbound ports to be opened which can increase chances of unauthorized access.",
      ...
      function (location, rcb) {
        ...
        async.each(
          ...
          function (webApp, scb) {
            ...
            const remoteDebugging = configs.data.some(
              (config) => config.remoteDebuggingEnabled
            );
            if (!remoteDebugging) {
              helpers.addResult(
                results,
                0,
                "Remote debugging is disabled for web app",
                location,
                webApp.id
              );
            } else {
              helpers.addResult(
                results,
                2,
                "Remote debugging is enabled for web app",
                location,
                webApp.id
              );
            }
