      ...
      function (location, rcb) {
        ...
        async.each(
          ...
          function (webApp, scb) {
            ...
            const alwaysOnEnabled = configs.data.some(
              (config) => config.alwaysOn
            );
            if (alwaysOnEnabled) {
              helpers.addResult(
                results,
                0,
                "Always On feature is enabled for the Web App",
                location,
                webApp.id
              );
            } else {
              helpers.addResult(
                results,
                2,
                "Always On feature is disabled for the Web App",
                location,
                webApp.id
              );
            }
