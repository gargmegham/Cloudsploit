...
module.exports = {
  ...
  more_info:
    "FTPS-only access for your Azure App Services applications, can guarantee that the encrypted traffic between the web application servers and the FTP clients cannot be decrypted by malicious actors.",
      ...
      function (location, rcb) {
        ...
        async.each(
          ...
          function (webApp, scb) {
            ...
            const ftpsOnlyAcces = configs.data.every(
              (config) =>
                config.ftpsState && config.ftpsState.toLowerCase() == "ftpsonly"
            );
