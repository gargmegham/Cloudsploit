...
module.exports = {
  ...
  description:
    "Ensures that Azure Network Security Groups (NSGs) have a sufficient flow log retention period",
  more_info:
    "A flow log data retention period of 90 days or more, allows you to collect the necessary amount of logging data required to check for anomalies and provide details about any potential security breach.",
  recommended_action:
    "Modify NSG flow logs and set desired value in days for retention period",
  ...
  settings: {
    nsg_flowlog_retention_period: {
      ...
      default: "90",
      description:
        "Desired number of days for which NSG flow logs data will be retained.",
  ...
  run: function (cache, settings, callback) {
    ...
    const config = {
      retentionDays: parseInt(
        settings.nsg_flowlog_retention_period ||
          this.settings.nsg_flowlog_retention_period.default
      ),
    };

    async.each(
      locations.networkWatchers,
      function (location, rcb) {
        ...
        if (networkWatchers.err || !networkWatchers.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for Network Watchers: " +
              helpers.addError(networkWatchers),
            location
          );
          return rcb();
        }
        ...
        async.each(
          networkWatchers.data,
          function (networkWatcher, scb) {
            ...
            if (!flowLogs || flowLogs.err || !flowLogs.data) {
              helpers.addResult(
                results,
                3,
                "Unable to query for flow logs data: " +
                  helpers.addError(networkWatchers),
                location
              );
              return scb();
