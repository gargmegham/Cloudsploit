...
module.exports = {
  ...
  more_info:
    "After expiry date has reached for Key Vault key, it cannot be used for cryptographic operations anymore.",
  recommended_action:
    "Ensure that Key Vault keys are rotated before they get expired.",
  ...
  settings: {
    key_vault_key_expiry_fail: {
      ...
      description:
        "Return a failing result when key expiration date is within this number of days in the future",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: "30",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      key_vault_key_expiry_fail: parseInt(
        settings.key_vault_key_expiry_fail ||
          this.settings.key_vault_key_expiry_fail.default
      ),
    };
        ...
        vaults.data.forEach(function (vault) {
          ...
          if (!keys || keys.err || !keys.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for Key Vault keys: " + helpers.addError(keys),
              location,
              vault.id
            );
            ...
            keys.data.forEach(function (key) {
              ...
              if (!key.attributes || !key.attributes.enabled) {
                ...
                key.attributes &&
                (key.attributes.expires || key.attributes.exp)
              ) {
                let keyExpiry = key.attributes.exp
                  ? key.attributes.exp * 1000
                  : key.attributes.expires;
                let difference = Math.round(
                  (new Date(keyExpiry).getTime() - new Date().getTime()) /
                    (24 * 60 * 60 * 1000)
                );
                if (difference > config.key_vault_key_expiry_fail) {
                  helpers.addResult(
                    results,
                    0,
                    `Key expires in ${difference} days`,
                    location,
                    keyId
                  );
                  ...
                  helpers.addResult(
                    results,
                    2,
                    `Key expired ${Math.abs(difference)} days ago`,
                    location,
                    keyId
                  );
                }
