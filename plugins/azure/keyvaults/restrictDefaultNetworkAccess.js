          ...
          if (vault.networkAcls) {
            if (
              ...
              (!vault.networkAcls.defaultAction ||
                (vault.networkAcls.defaultAction &&
                  vault.networkAcls.defaultAction === "Allow"))
            ) {
              helpers.addResult(
                results,
                2,
                "Key Vault allows access to all networks",
                location,
                vault.id
              );
            } else {
              helpers.addResult(
                results,
                0,
                "Key Vault does not allow access to all networks",
                location,
                vault.id
              );
            }
            ...
            helpers.addResult(
              results,
              0,
              "Network Acl is not configured for Key Vault",
              location,
              vault.id
            );
          }
