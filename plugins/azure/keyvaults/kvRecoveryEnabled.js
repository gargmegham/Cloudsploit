...
module.exports = {
  ...
  recommended_action:
    "Once Key Vaults are created, the Azure CLI must be used to update the vault Soft Delete and Purge Protection settings.",
  link: "https://docs.microsoft.com/en-us/azure/key-vault/key-vault-ovw-soft-delete",
  apis: ["vaults:list"],
  compliance: {
    hipaa:
      "HIPAA requires that all encryption mechanisms be protected against " +
        ...
        vaults.data.forEach(function (vault) {
          if (vault.enablePurgeProtection && vault.enableSoftDelete) {
            helpers.addResult(
              results,
              0,
              "Purge protection and soft delete are enabled for the Key Vault",
              location,
              vault.id
            );
          } else {
            let msg = [
              `Purge protection is ${
                vault.enablePurgeProtection ? "" : "not"
              } enabled.`,
