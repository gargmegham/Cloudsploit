...
module.exports = {
  ...
  more_info:
    "Enabling network firewall rules for your Key Vaults will block access to incoming requests for data, including from other Azure services. " +
  ...
  recommended_action:
    "Ensure that Microsoft Azure Key Vault network firewall configuration allows trusted Microsoft services to bypass the firewall.",
      ...
      function (location, rcb) {
        ...
        vaults.data.forEach((vault) => {
          if (vault.networkAcls) {
            if (
              vault.networkAcls &&
              vault.networkAcls.bypass &&
              vault.networkAcls.bypass === "None"
            ) {
              helpers.addResult(
                results,
                2,
                "Trusted Microsoft services are not allowed to access the key vault resources",
                location,
                vault.id
              );
            } else {
              helpers.addResult(
                results,
                0,
                "Trusted Microsoft services are allowed to access the key vault resources",
                location,
                vault.id
              );
            }
            ...
            helpers.addResult(
              results,
              0,
              "Network Acls are not configured for key vault",
              location,
              vault.id
            );
          }
