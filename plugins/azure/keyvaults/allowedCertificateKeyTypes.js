...
module.exports = {
  ...
  recommended_action:
    "Ensure that Key Vault SSL certificates are using the allowed key types.",
  ...
  settings: {
    allowed_certificate_key_types: {
      ...
      description:
        "Comma separated key types supported for certificates in Azure Key Vault",
      regex: "^(RSA|EC)$",
      default: "",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      allowed_certificate_key_types:
        settings.allowed_certificate_key_types ||
        this.settings.allowed_certificate_key_types.default,
    };

    if (!config.allowed_certificate_key_types.length)
            ...
            certificates.data.forEach((certificate) => {
              ...
              if (
                ...
                if (certificateKeys && certificateKeys.kty) {
                  if (
                    config.allowed_certificate_key_types
                      .toLowerCase()
                      .includes(certificateKeys.kty.toLowerCase())
                  ) {
                    helpers.addResult(
                      results,
                      0,
                      "Certificate key type is " + certificateKeys.kty,
                      location,
                      certificate.id
                    );
                  } else {
