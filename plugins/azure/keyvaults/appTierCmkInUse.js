...
module.exports = {
  ...
  more_info:
    "Setting a CMK for app tier, you gain full control over who can use this key to access the application data, implementing the principle of least privilege on the encryption key ownership and usage.",
  recommended_action:
    "Ensure a CMK created and configured for application tier in each region.",
      ...
      function (location, rcb) {
        ...
        if (appTierKey) {
          helpers.addResult(
            results,
            0,
            `CMK exists for application tier: ${appTierKey}`,
            location
          );
        } else {
          helpers.addResult(
            results,
            2,
            "CMK does not exist for application tier",
            location
          );
        }
