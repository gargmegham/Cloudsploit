  ...
  describe("run", function () {
    ...
    it("should give unkown result if Unable to query for Key Vaults keys", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Unable to query for Key Vault keys"
        );
        expect(results[0].region).to.equal("eastus");
        done();
      };
    ...
    it("should give passing result if CMK exists for application tier", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "CMK exists for application tier"
        );
        expect(results[0].region).to.equal("eastus");
        done();
      };
      ...
      auth.run(
        createCache(null, [listVaults[1]], [getKeys[0]]),
        { app_tier_tag_key: "apptier" },
        callback
      );
    });
