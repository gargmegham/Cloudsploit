  ...
  describe("run", function () {
    ...
    it("should give failing result if postgresql server has geo-redundant backup storage enabled", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "PostgreSQL Server does not have geo-redundant backup storage enabled"
        );
        expect(results[0].region).to.equal("eastus");
        done();
      };

      const cache = createCache(null, [listPostgres[0]]);
    ...
    it("should give passing result if postgresql server does not have geo-redundant backup storage enabled", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          "PostgreSQL Server has geo-redundant backup storage enabled"
        );
        expect(results[0].region).to.equal("eastus");
        done();
      };

      const cache = createCache(null, [listPostgres[1]]);
