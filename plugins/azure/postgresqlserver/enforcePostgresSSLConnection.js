...
module.exports = {
  ...
  recommended_action:
    "Ensure the connection security settings of each PostgreSQL server are configured to enforce SSL connections.",
  ...
  remediation_description:
    "The SSL enforcement option will be enabled for the affected PostreSQL servers",
    ...
    async.each(
      ...
      (location, rcb) => {
        ...
        for (let res in servers.data) {
          ...
          if (
            postgresServer.sslEnforcement &&
            postgresServer.sslEnforcement.toLowerCase() == "enabled"
          ) {
            helpers.addResult(
              results,
              0,
              "The PostgreSQL Server is configured to enforce SSL connections",
              location,
              postgresServer.id
            );
          } else {
            helpers.addResult(
              results,
              2,
              "The PostgreSQL Server is not configured to enforce SSL connections",
              location,
              postgresServer.id
            );
          }
