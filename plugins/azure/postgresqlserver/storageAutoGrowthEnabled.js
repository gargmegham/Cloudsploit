...
module.exports = {
  ...
  recommended_action:
    "Modify PostgreSQL servers to enable storage auto-growth feature",
      ...
      (location, rcb) => {
        ...
        for (let postgresServer of listPostgres.data) {
          if (
            ...
            postgresServer.storageProfile.storageAutogrow.toLowerCase() ==
              "enabled"
          ) {
            helpers.addResult(
              results,
              0,
              "Storage Auto Growth is enabled for PostgreSQL Server",
              location,
              postgresServer.id
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Storage Auto Growth is not enabled for PostgreSQL Server",
              location,
              postgresServer.id
            );
          }
