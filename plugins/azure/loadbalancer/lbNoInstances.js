...
module.exports = {
  ...
  recommended_action:
    "Delete old load balancers that no longer have backend resources.",
        ...
        loadBalancers.data.forEach((loadBalancer) => {
          ...
          if (
            ...
            helpers.addResult(
              results,
              2,
              "Load Balancer does not have any backend instances",
              location,
              loadBalancer.id
            );
          } else {
            loadBalancer.backendAddressPools.forEach((backendAddressPool) => {
              if (
                ...
                backendAmt +=
                  backendAddressPool.properties.backendIPConfigurations.length;
              } else if (
                backendAddressPool.properties &&
            ...
            if (backendAmt) {
              ...
              helpers.addResult(
                results,
                2,
                "Load Balancer does not have any backend instances or addresses",
                location,
                loadBalancer.id
              );
            }
