...
module.exports = {
  ...
  recommended_action:
    "Ensure that each load balancer only accepts connections on port 443.",
      ...
      function (location, rcb) {
        ...
        if (loadBalancers.err || !loadBalancers.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query Load Balancers: " +
        ...
        loadBalancers.data.forEach((loadBalancer) => {
          ...
          if (notHTTPSRules && isHTTPS) {
            helpers.addResult(
              results,
              2,
              "HTTPS is configured but other ports are open",
              location,
              loadBalancer.id
            );
            ...
            helpers.addResult(
              results,
              2,
              "HTTPS is not configured and other ports are open",
              location,
              loadBalancer.id
            );
          } else if (isHTTPS) {
