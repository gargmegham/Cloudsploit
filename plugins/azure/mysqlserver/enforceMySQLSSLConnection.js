...
module.exports = {
  ...
  more_info:
    "MySQL servers should be set to use SSL for data transmission to ensure all data is encrypted in transit.",
  recommended_action:
    "Ensure the connection security of each Azure Database for MySQL is configured to enforce SSL connections.",
  ...
  remediation_description:
    "The SSL enforcement option will be enabled for the affected MySQL servers",
      ...
      (location, rcb) => {
        ...
        if (servers.err || !servers.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for MySQL servers: " + helpers.addError(servers),
            location
          );
          return rcb();
        }
        ...
        if (!servers.data.length) {
          helpers.addResult(
            results,
            0,
            "No existing MySQL servers found",
            location
          );
          return rcb();
        }
        ...
        servers.data.forEach(function (server) {
          if (
            server.sslEnforcement &&
            server.sslEnforcement.toLowerCase() == "enabled"
          ) {
            helpers.addResult(
              results,
              0,
              "The MySQL server enforces SSL connections",
              location,
              server.id
            );
          } else {
            helpers.addResult(
              results,
              2,
              "The MySQL server does not enforce SSL connections",
              location,
              server.id
            );
          }
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    var baseUrl =
      "https://management.azure.com{resource}?api-version=2017-12-01";
    var method = "PATCH";
