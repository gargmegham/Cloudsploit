...
module.exports = {
  ...
  recommended_action:
    "1. Go to Resources. 2. Select the resource. 3. Select the Locks blade under settings on the left side. 4. Add a lock 5. Enter the Tags Blade and  add cloudsploitLock as a tag with true as its value.",
  ...
  settings: {
    tag: {
      ...
      description:
        "This tag will be required to indicate that the management lock is enabled.",
      ...
      (location, rcb) => {
        ...
        if (managementLocks.err || !managementLocks.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query Management Locks: " +
              helpers.addError(managementLocks),
            location
          );
          return rcb();
        }
        ...
        managementLocks.data.forEach((managementLock) => {
          ...
          myLockedResourceObj[myLockedResource[resourceLength - 6]].push(
            myLockedResource[resourceLength - 5]
          );
        });
        ...
        async.each(locations.resources, (loc, lcb) => {
          ...
          if (resources.err || !resources.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query Resources: " + helpers.addError(resources),
              loc
            );
            return lcb();
          }
          ...
          async.each(
            ...
            (resource, resCb) => {
              ...
              if (
                myLockedResourceObj[myResource[myResource.length - 2]] &&
                myLockedResourceObj[myResource[myResource.length - 2]] ==
                  myResource[myResource.length - 1]
              ) {
                helpers.addResult(
                  results,
                  0,
                  "Resource has Management Lock Enabled",
                  loc,
                  resource.id
                );
                return resCb();
                ...
                helpers.addResult(
                  results,
                  2,
                  "Resource does not have Management Lock Enabled",
                  loc,
                  resource.id
                );
                return resCb();
