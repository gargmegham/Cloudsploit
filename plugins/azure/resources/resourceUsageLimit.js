...
module.exports = {
  ...
  description:
    "Determines if resources are close to the Azure per-account limit",
  more_info:
    "Azure limits accounts to certain numbers of resources. Exceeding those limits could prevent resources from launching.",
  recommended_action:
    "Check if resources are close to the account limit to avoid resource launch failures",
      ...
      function (location, rcb) {
        ...
        if (subLocations.err || !subLocations.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for resource subscription locations: " +
              helpers.addError(subLocations),
            location
          );
          return rcb();
        }
        ...
        if (!subLocations.data.length) {
          helpers.addResult(
            results,
            0,
            "No resource subscription locations found",
            location
          );
          return rcb();
        }
          ...
          if (!usages || usages.err || !usages.data) {
            if (
              ...
              helpers.addResult(
                results,
                0,
                "Usage tracking is not supported by this location",
                location,
                sloc.id
              );
            } else {
              helpers.addResult(
                results,
                3,
                "Unable to query for Resource Usages: " +
                  helpers.addError(usages),
                location,
                sloc.id
              );
            }
            ...
            usages.data.forEach((resource) => {
              if (
                ...
                ["Network Watchers"].indexOf(resource.name.localizedValue) ===
                  -1
              ) {
                var resourceName = resource.name.localizedValue;
                var percentUsed = parseInt(
                  (resource.currentValue / resource.limit) * 100
                );
                if (percentUsed >= 100) {
            ...
            if (limit100.length || limit90.length || limit70.length) {
              ...
              helpers.addResult(
                results,
                2,
                "The following usage quotas are exceeded:" + msgStr,
                location,
                sloc.id
              );
            } else {
              helpers.addResult(
                results,
                0,
                "None of the resource usage limits exceed 70%",
                location,
                sloc.id
              );
            }
