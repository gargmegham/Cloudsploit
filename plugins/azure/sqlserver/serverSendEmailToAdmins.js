...
module.exports = {
  ...
  recommended_action:
    "Configure Send scan reports to email addresses of concerned data owners/stakeholders for critical SQL servers.",
        ...
        servers.data.forEach(function (server) {
          ...
          if (
            ...
            if (!vulnerabilityAssessments.data.length) {
              ...
              let emailNotificationEnabled = vulnerabilityAssessments.data.find(
                (vulnerabilityAssessment) =>
                  vulnerabilityAssessment.recurringScans &&
                  vulnerabilityAssessment.recurringScans.emailSubscriptionAdmins
              );
              if (emailNotificationEnabled) {
                helpers.addResult(
                  results,
                  0,
                  "Send Email notifications to admins for the SQL server is enabled",
                  location,
                  server.id
                );
              } else {
                helpers.addResult(
                  results,
                  2,
                  "Send Email notifications to admins for the SQL server is disabled",
                  location,
                  server.id
                );
              }
