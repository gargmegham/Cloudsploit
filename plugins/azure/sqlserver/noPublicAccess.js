...
module.exports = {
  ...
  recommended_action:
    "Ensure that the firewall of each SQL Server is configured to prohibit traffic from the public 0.0.0.0 global IP address.",
  ...
  settings: {
    server_firewall_end_ip: {
      ...
      description:
        "Comma separated list of IP addresses which cannot be end IPs for firewall rule",
      regex:
        "((25[0-5]|2[0-4]|[01]??)(25[0-5]|2[0-4]|[01]??)(25[0-5]|2[0-4]|[01]??)(25[0-5]|2[0-4]|[01]??)(,\n|,?$))",
      default: "",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    var config = {
      server_firewall_end_ip:
        settings.server_firewall_end_ip ||
        this.settings.server_firewall_end_ip.default,
    };
        ...
        servers.data.forEach(function (server) {
          ...
          if (!firewallRules || firewallRules.err || !firewallRules.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query SQL Server Firewall Rules: " +
                helpers.addError(firewallRules),
              location,
              server.id
            );
          } else {
            if (!firewallRules.data.length) {
              helpers.addResult(
                results,
                0,
                "No existing SQL Server Firewall Rules found",
                location,
                server.id
              );
            } else {
              ...
              firewallRules.data.forEach((firewallRule) => {
                ...
                if (checkEndIp) {
                  ...
                  if (
                    startIpAddr &&
                    startIpAddr.toString().indexOf("0.0.0.0") > -1 &&
                    endIpAddr &&
              ...
              if (publicAccess) {
                helpers.addResult(
                  results,
                  2,
                  "The SQL Server is open to outside traffic",
                  location,
                  server.id
                );
              } else {
                helpers.addResult(
                  results,
                  0,
                  "The SQL server is protected from outside traffic",
                  location,
                  server.id
                );
              }
