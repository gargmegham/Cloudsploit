...
module.exports = {
  ...
  recommended_action:
    "Ensure that an email address is activated under send alerts in advanced data security for all SQL servers.",
        ...
        servers.data.forEach(function (server) {
          ...
          if (
            ...
            if (!serverSecurityAlertPolicies.data.length) {
              ...
              let alertsEnabled = serverSecurityAlertPolicies.data.find(
              ...
              if (alertsEnabled) {
                helpers.addResult(
                  results,
                  0,
                  "Send alerts is enabled on the SQL server",
                  location,
                  server.id
                );
              } else {
                helpers.addResult(
                  results,
                  2,
                  "Send alerts is not enabled on the SQL server",
                  location,
                  server.id
                );
              }
