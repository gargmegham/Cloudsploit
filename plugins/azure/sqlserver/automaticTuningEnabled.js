...
module.exports = {
  ...
  domain: "Databases",
  description:
    "Ensures that Microsoft Azure SQL servers have automatic tuning configured.",
  more_info:
          ...
          function (server, scb) {
            ...
            if (!configs || configs.err || !configs.data) {
              helpers.addResult(
                results,
                3,
                "Unable to query for SQL server automatic tuning configurations: " +
            ...
            if (!config) {
              helpers.addResult(
                results,
                2,
                "No automatic tuning configurations found for SQL server",
                location,
                server.id
              );
              return scb();
            ...
            if (
              config.forceLastGoodPlan &&
              config.forceLastGoodPlan.reasonDesc &&
              config.forceLastGoodPlan.reasonDesc.toLowerCase() ===
                "autoconfigured"
            ) {
              configuredFlagsCount++;
            }
            ...
            if (configuredFlagsCount === 3) {
              helpers.addResult(
                results,
                0,
                "SQL server has Azure automatic tuning enabled",
                location,
                server.id
              );
            } else {
              helpers.addResult(
                results,
                2,
                "SQL server does not have Azure automatic tuning enabled",
                location,
                server.id
              );
            }
