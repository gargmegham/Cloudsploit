...
module.exports = {
  ...
  recommended_action:
    "Ensure that the storage account retention policy for each SQL server is set to greater than 90 days.",
            ...
            if (!serverBlobAuditingPolicies.data.length) {
              ...
              serverBlobAuditingPolicies.data.forEach(
                (serverBlobAuditingPolicy) => {
                  if (
                    serverBlobAuditingPolicy.retentionDays &&
                    serverBlobAuditingPolicy.retentionDays >= 90
                  ) {
                    helpers.addResult(
                      results,
                      0,
                      "Server Auditing retention is greater than 90 days",
                      location,
                      server.id
                    );
                  } else if (
                    ...
                    helpers.addResult(
                      results,
                      2,
                      `Server Auditing retention is ${serverBlobAuditingPolicy.retentionDays} days`,
                      location,
                      server.id
                    );
                  } else {
                    helpers.addResult(
                      results,
                      2,
                      "Server Auditing is not being stored in a storage account",
                      location,
                      server.id
                    );
                  }
