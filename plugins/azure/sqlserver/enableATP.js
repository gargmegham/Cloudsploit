    ...
    async.each(
      ...
      function (location, rcb) {
        ...
        servers.data.forEach(function (server) {
          const advancedThreatProtectionSettings = helpers.addSource(
            cache,
            source,
            [
          ...
          if (
            !advancedThreatProtectionSettings ||
            advancedThreatProtectionSettings.err ||
            !advancedThreatProtectionSettings.data
          ) {
            helpers.addResult(
              results,
              3,
              "Unable to query for Advanced Threat Protection settings: " +
                helpers.addError(advancedThreatProtectionSettings),
              location,
              server.id
            );
          } else {
            if (!advancedThreatProtectionSettings.data.length) {
              helpers.addResult(
                results,
                2,
                "No Advanced Threat Protection setting found",
                location,
                server.id
              );
            } else {
              let atpEnabled = advancedThreatProtectionSettings.data.find(
                (threadProtectionSetting) =>
              ...
              if (atpEnabled) {
                helpers.addResult(
                  results,
                  0,
                  "Advanced Threat Protection for the SQL server is enabled",
                  location,
                  server.id
                );
              } else {
                helpers.addResult(
                  results,
                  2,
                  "Advanced Threat Protection for the SQL server is disabled",
                  location,
                  server.id
                );
              }
