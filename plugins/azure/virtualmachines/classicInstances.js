...
module.exports = {
  ...
  domain: "Compute",
  description:
    "Ensures Azure Resource Manager is being used for instances instead of Cloud Services (VM Classic)",
  more_info:
    "ARM is the latest and most secure method of launching Azure resources. VM Classic should not be used.",
      ...
      function (location, rcb) {
        ...
        if (classicVms) {
          helpers.addResult(
            results,
            2,
            `There are ${classicVms} classic VM instances`,
            location
          );
        } else {
          helpers.addResult(
            results,
            0,
            "There are no classic VM instances",
            location
          );
        }
