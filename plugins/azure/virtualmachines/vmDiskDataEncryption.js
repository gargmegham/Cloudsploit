...
module.exports = {
  ...
  domain: "Compute",
  description:
    "Ensure that Data Disk Encryption is enabled for virtual machines",
  more_info:
    "Encrypting VM Data Disks (non-boot volume) ensures that its entire contents are fully unrecoverable without a key, protecting the volume from unwarranted reads",
      ...
      function (location, rcb) {
        ...
        if (disks.err || !disks.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query Disks: " + helpers.addError(disks),
            location
          );
          return rcb();
        }
