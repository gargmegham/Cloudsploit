...
module.exports = {
  ...
  more_info:
    "Extensions are small applications that provide post-deployment configuration and automation on Azure VMs. Extensions installed should be approved by the organization to meet the organizational security requirements.",
  ...
  run: function (cache, settings, callback) {
    ...
    const config = {
      approvedExtensions:
        settings.vm_approved_extensions ||
        this.settings.vm_approved_extensions.default,
    };
        ...
        async.each(
          ...
          function (virtualMachine, scb) {
            ...
            virtualMachineExtensions.data.forEach(function (
              ...
              let found = extensionsList.some(
                (extension) => extension.trim() === virtualMachineExtension.name
              );
              ...
              if (found) {
                helpers.addResult(
                  results,
                  0,
                  "Installed extensions are approved by the organization",
                  location,
                  virtualMachineExtension.id
                );
              } else {
                helpers.addResult(
                  results,
                  2,
                  "Installed extensions are not approved by the organization",
                  location,
                  virtualMachineExtension.id
                );
              }
