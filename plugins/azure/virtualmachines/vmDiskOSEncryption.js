...
module.exports = {
  ...
  domain: "Compute",
  description:
    "Ensures that VM OS Disk Encryption is enabled for virtual machines",
  more_info:
    "Encrypting VM OS disks (boot volume) ensures that the entire contents are fully unrecoverable without a key, protecting the volume from unwarranted reads.",
      ...
      function (location, rcb) {
        ...
        if (disks.err || !disks.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for Disks: " + helpers.addError(disks),
            location
          );
          return rcb();
        }
            ...
            if (
              ...
              if (disk.encryption) {
                ...
                helpers.addResult(
                  results,
                  2,
                  "OS disk encryption is disabled",
                  location,
                  disk.id
                );
              }
