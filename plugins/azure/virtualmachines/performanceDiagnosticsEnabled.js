...
module.exports = {
  ...
  more_info:
    "The performance diagnostics tool helps in troubleshooting performance issues that can affect a Windows or Linux virtual machine (VM).",
  recommended_action:
    "Enable performance diagnostics on Azure virtual machines",
          ...
          function (virtualMachine, scb) {
            ...
            if (!virtualMachineExtensions.data.length) {
              helpers.addResult(
                results,
                2,
                "Performance Diagnostics is disabled on the virtual machine",
                location,
                virtualMachine.id
              );
              return scb();
            ...
            const adEnabled = virtualMachineExtensions.data.some(
              (virtualMachineExtension) =>
                ...
                virtualMachineExtension.provisioningState &&
                virtualMachineExtension.provisioningState === "Succeeded"
            );
            ...
            if (adEnabled) {
              helpers.addResult(
                results,
                0,
                "Performance Diagnostics is enabled on the virtual machine",
                location,
                virtualMachine.id
              );
            } else {
