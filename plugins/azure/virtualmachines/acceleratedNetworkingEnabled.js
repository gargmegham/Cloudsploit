...
module.exports = {
  ...
  domain: "Compute",
  description:
    "Ensures that accelerated networking is enabled on Azure virtual machines(VM).",
  more_info:
  ...
  recommended_action:
    "Enable accelerated networking in virtual machine network interfaces",
  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      ...
      function (location, rcb) {
        ...
        if (
          !networkInterfaces ||
          networkInterfaces.err ||
          !networkInterfaces.data ||
          !networkInterfaces.data.length
        ) {
          helpers.addResult(
            results,
            3,
            "Unable to query for network interfaces: " +
              helpers.addError(networkInterfaces),
            location
          );
          return rcb();
        }
        ...
        networkInterfaces.data.forEach((networkInterface) => {
          nicMap.set(
            networkInterface.id,
            networkInterface.enableAcceleratedNetworking
          );
        });
        ...
        virtualMachines.data.forEach((virtualMachine) => {
          ...
          if (
            ...
            acclNetwrork = virtualMachine.networkProfile.networkInterfaces.find(
              (interface) => {
          ...
          if (acclNetwrork) {
            helpers.addResult(
              results,
              0,
              "Accelerated Networking is enabled on Azure Virtual Machine(VM)",
              location,
              virtualMachine.id
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Accelerated Networking is not enabled on Azure Virtual Machine(VM)",
              location,
              virtualMachine.id
            );
          }
