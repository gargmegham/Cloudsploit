      ...
      function (location, rcb) {
        ...
        if (!recoveryVaults || recoveryVaults.err || !recoveryVaults.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for backup vaults: " +
        ...
        for (const virtualMachine of virtualMachines.data) {
          ...
          if (vmPolicies && vmPolicies.length) {
            vmBackupsEnabled = vmPolicies.some(
              (policy) => policy && policy.length
            );
          }
          ...
          if (vmBackupsEnabled) {
            helpers.addResult(
              results,
              0,
              "Azure virtual machine has backups enabled",
              location,
              virtualMachine.id
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Azure virtual machine does not have backups enabled",
              location,
              virtualMachine.id
            );
          }
