    ...
    async.each(
      ...
      (location, rcb) => {
        ...
        async.each(
          ...
          (virtualMachineScaleSet, scb) => {
            const scaleSetExtensions =
              virtualMachineScaleSet.virtualMachineProfile &&
            ...
            const healthMonitoring = scaleSetExtensions.length
              ? scaleSetExtensions.some(
                  (extension) =>
                    extension.name === "healthRepairExtension" ||
                    extension.type === "ApplicationHealthLinux"
                )
              : false;
            ...
            if (healthMonitoring) {
              helpers.addResult(
                results,
                0,
                "Virtual Machine Scale Set has health monitoring enabled",
                location,
              ...
              helpers.addResult(
                results,
                2,
                "Virtual Machine Scale Set has health monitoring disabled",
                location,
                virtualMachineScaleSet.id
              );
            }
