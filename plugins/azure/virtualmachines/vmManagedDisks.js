...
module.exports = {
  ...
  domain: "Compute",
  description:
    "Ensures that Azure virtual machines are configured to use Azure managed disks.",
  more_info:
    "Azure managed disks are block-level storage volumes that are managed by Azure are like physical disks in an on-premises server but, virtualized. Azure managed disks provide high durability and security.",
        ...
        for (let virtualMachine of virtualMachines.data) {
          if (
            ...
            helpers.addResult(
              results,
              0,
              "Virtual machine is configured to use Azure managed disks",
              location,
              virtualMachine.id
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Virtual machine is not configured to use Azure managed disks",
              location,
              virtualMachine.id
            );
          }
