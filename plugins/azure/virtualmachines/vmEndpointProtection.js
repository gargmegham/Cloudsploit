...
module.exports = {
  ...
  domain: "Compute",
  description:
    "Ensures that VM Endpoint Protection is enabled for all virtual machines",
  more_info:
    "Installing endpoint protection systems provides for real-time protection capabilities that help identify and remove viruses, spyware, and other malicious software, with configurable alerts for malicious or unwanted software.",
  ...
  compliance: {
    pci:
      "PCI requires the use of anti-virus and anti-malware solutions. Enabling " +
      "VM endpoint protection provides real-time VM monitoring for malicious activity.",
    hipaa:
      "HIPAA requires protection of all network systems, including monitoring " +
            ...
            if (
              ...
              if (!windowsImg) {
                ...
                helpers.addResult(
                  results,
                  0,
                  "The Microsoft VM does not offer endpoint protection",
                  location,
                  virtualMachine.id
                );
              }
              ...
              virtualMachineExtensions.data.forEach(function (
                virtualMachineExtension
              ) {
                if (
                  virtualMachineExtension.type &&
              ...
              if (antiMalware) {
                helpers.addResult(
                  results,
                  0,
                  "Endpoint protection is installed on the virtual machine",
                  location,
                  virtualMachine.id
                );
              } else {
                if (!windowsImg) {
                  helpers.addResult(
                    results,
                    2,
                    "Endpoint protection is not installed on the virtual machine",
                    location,
                    virtualMachine.id
                  );
                } else {
