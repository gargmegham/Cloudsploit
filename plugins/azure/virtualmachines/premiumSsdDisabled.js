...
module.exports = {
  ...
  more_info:
    "Azure standard SSD disks store data on solid state drives (SSDs), like Azure's existing premium storage disks. Standard SSD disks are a cost-effective storage option optimized for workloads that need consistent performance at lower IOPS levels.",
  recommended_action:
    "Modify virtual machines disks to use standard SSD disk volumes instead of premium SSD disk volumes",
      ...
      function (location, rcb) {
        ...
        virtualMachines.data.forEach((virtualMachine) => {
          ...
          if (
            ...
            virtualMachine.storageProfile.osDisk.managedDisk
              .storageAccountType &&
            virtualMachine.storageProfile.osDisk.managedDisk.storageAccountType.toLowerCase() ===
              "premium_lrs"
          ) {
            helpers.addResult(
              results,
              2,
              "Attached OS disk volume is of Premium SSD type",
              location,
              virtualMachine.storageProfile.osDisk.managedDisk.id
            );
            foundDisk = true;
            ...
            helpers.addResult(
              results,
              0,
              "Attached OS disk volume is not of Premium SSD type",
              location,
              virtualMachine.storageProfile.osDisk.managedDisk.id
            );
            foundDisk = true;
          ...
          for (const dataDisk of dataDisks) {
            if (
              ...
              dataDisk.managedDisk.id &&
              dataDisk.managedDisk.storageAccountType.toLowerCase() ===
                "premium_lrs"
            ) {
              helpers.addResult(
                results,
                2,
                "Attached data disk volume is of Premium SSD type",
                location,
                dataDisk.managedDisk.id
              );
              foundDisk = true;
              ...
              helpers.addResult(
                results,
                0,
                "Attached data disk volume is not of Premium SSD type",
                location,
                dataDisk.managedDisk.id
              );
              foundDisk = true;
          ...
          if (!foundDisk) {
            helpers.addResult(
              results,
              0,
              "No disks found for the Virtual machine",
              location,
              virtualMachine.id
            );
          }
