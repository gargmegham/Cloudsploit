...
module.exports = {
  ...
  domain: "Compute",
  description:
    "Determine if the number of VM instances is close to the Azure per-availability set limit",
  more_info:
    "Azure limits availability sets to certain numbers of resources. Exceeding those limits could prevent resources from launching.",
  link: "https://docs.microsoft.com/en-us/azure/virtual-machines/windows/overview",
  recommended_action:
    "Contact Azure support to increase the number of instances available",
      ...
      function (location, rcb) {
        ...
        if (resourceGroups.err || !resourceGroups.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query Resource Groups: " +
        ...
        async.each(
          ...
          function (resourceGroup, scb) {
            ...
            if (
              !availabilitySets ||
              availabilitySets.err ||
              !availabilitySets.data
            ) {
              helpers.addResult(
                results,
                3,
                "Unable to query Availability Sets: " +
                  helpers.addError(availabilitySets),
                location
              );
              return scb();
            }
            ...
            availabilitySets.data.forEach((availabilitySet) => {
              ...
              var percentage = Math.ceil(
                (vmInstances / limits["max-instances"]) * 100
              );
              var returnMsg =
                "Availability Set contains " +
                vmInstances +
                " of " +
