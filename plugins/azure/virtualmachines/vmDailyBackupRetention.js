...
module.exports = {
  ...
  more_info:
    "Azure Backup provides independent and isolated backups to guard against unintended destruction of the data on your VMs. These backups should be retained for a specific amount of time to recover destroyed VM.",
  recommended_action:
    "Configure virtual machine daily backup retention policy to retain backups for desired number of days",
  ...
  apis: [
    ...
    "backupProtectedItems:listByVault",
    "backupPolicies:listByVault",
  ],
  settings: {
    vm_daily_backup_retention_period: {
      name: "VM Daily Backup Retention Period",
      description:
        "Number of days that a VM backup will be retained until it is permanently deleted (7 or above)",
  ...
  run: function (cache, settings, callback) {
    ...
    const config = {
      retentionPeriod: parseInt(
        settings.vm_daily_backup_retention_period ||
          this.settings.vm_daily_backup_retention_period.default
      ),
    };
      ...
      function (location, rcb) {
        ...
        if (virtualMachines.err || !virtualMachines.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for virtual Machines: " +
              helpers.addError(virtualMachines),
            location
          );
          return rcb();
        }
        ...
        if (!recoveryVaults || recoveryVaults.err || !recoveryVaults.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for backup recovery vaults: " +
              helpers.addError(recoveryVaults),
            location
          );
          return rcb();
        }
        ...
        if (!recoveryVaults.data.length) {
          helpers.addResult(
            results,
            2,
            "No backup recovery vaults found",
            location
          );
          return rcb();
        }
        ...
        for (const vault of recoveryVaults.data) {
          ...
          if (
            !backupProtectedItems ||
            backupProtectedItems.err ||
            !backupProtectedItems.data
          ) {
            helpers.addResult(
              results,
              3,
              "Unable to query for backup retention policies : " +
                helpers.addError(backupProtectedItems),
              location
            );
            return rcb();
          }
          ...
          if (!backupPolicies || backupPolicies.err || !backupPolicies.data) {
            helpers.addResult(
              ...
              3,
              "Unable to query for backup retention policies : " +
                helpers.addError(backupPolicies),
              location
            );
            return rcb();
          }
          ...
          for (const bpItem of backupProtectedItems.data) {
            if (
              bpItem.virtualMachineId &&
              vmPoliciesMap.get(bpItem.virtualMachineId.toLowerCase())
            ) {
              vmPoliciesMap
                .get(bpItem.virtualMachineId.toLowerCase())
                .push(bpItem.policyId);
        ...
        async.each(
          ...
          function (virtualMachine, scb) {
            const vmPolicies = vmPoliciesMap.get(
              virtualMachine.id.toLowerCase()
            );
            if (vmPolicies && vmPolicies.length) {
              ...
              for (const vmPolicy of vmPolicies) {
                ...
                if (
                  backupPolicy &&
                  backupPolicy.retentionPolicy &&
                  backupPolicy.retentionPolicy.dailySchedule &&
                  backupPolicy.retentionPolicy.dailySchedule
                    .retentionDuration &&
                  backupPolicy.retentionPolicy.dailySchedule.retentionDuration
                    .count &&
                  backupPolicy.retentionPolicy.dailySchedule.retentionDuration
                    .count > retentionDays
                ) {
              ...
              if (retentionDays >= config.retentionPeriod) {
                helpers.addResult(
                  results,
                  0,
                  `VM daily backups are configured to be retained for ${retentionDays} of ${config.retentionPeriod} days desired limit`,
                  location,
                  virtualMachine.id
                );
              } else {
              ...
              helpers.addResult(
                results,
                2,
                "No backup policies are configured for the virtual machine",
                location,
                virtualMachine.id
              );
            }
