...
module.exports = {
  ...
  more_info:
    "Guest Level Diagnostics should be enabled to collect information about VMs processing and state of VM applications.",
    ...
    async.each(
      ...
      function (location, rcb) {
        ...
        for (let virtualMachine of virtualMachines.data) {
          const virtualMachineData = helpers.addSource(cache, source, [
            "virtualMachines",
            "get",
            location,
            virtualMachine.id,
          ...
          if (
            !(
              ...
              virtualMachineData.data.resources &&
              virtualMachineData.data.resources.length
            ) ||
            virtualMachineData.err
          ) {
            helpers.addResult(
              results,
              3,
              "unable to query for virtual machine data",
              location,
              virtualMachine.id
            );
          } else {
            const diagnosticSetting = virtualMachineData.data.resources.find(
              (resource) =>
                resource.properties &&
            ...
            if (diagnosticSetting) {
              helpers.addResult(
                results,
                0,
                "Guest Level Diagnostics are enabled for the virtual machine",
                location,
                virtualMachine.id
              );
            } else {
              helpers.addResult(
                results,
                2,
                "Guest Level Diagnostics are disabled for the virtual machine",
                location,
                virtualMachine.id
              );
            }
