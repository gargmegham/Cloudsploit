  ...
  describe("run", function () {
    ...
    it("should give passing result if snapshot is not older than desired limit", function (done) {
      ...
      const daysCreated = helpers.daysBetween(
        new Date(),
        new Date(snapshots[0].timeCreated)
      );
      ...
      oldVmDiskSnapshots.run(cache, {}, (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          `VM disk snapshot is ${daysCreated} days older which is equal to or less than 30 days limit`
        );
        expect(results[0].region).to.equal("eastus");
        done();
      });
    ...
    it("should give failing result if snapshot is older than desired limit", function (done) {
      ...
      const daysCreated = helpers.daysBetween(
        new Date(),
        new Date(snapshots[1].timeCreated)
      );
      ...
      oldVmDiskSnapshots.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          `VM disk snapshot is ${daysCreated} days older which is more than 30 days limit`
        );
        expect(results[0].region).to.equal("eastus");
        done();
      });
