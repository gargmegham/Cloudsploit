...
module.exports = {
  ...
  more_info:
    "Enabling Availability Sets ensures that during either a planned or unplanned maintenance event, the virtual machine will still be available.",
  recommended_action:
    "Virtual Machine Availability Sets can only be configured when creating a new virtual machine. Recreate the Virtual Machine with Availability Sets enabled.",
        ...
        virtualMachines.data.forEach((virtualMachine) => {
          if (virtualMachine.availabilitySet) {
            helpers.addResult(
              results,
              0,
              "The Virtual Machine has Availability Set enabled",
              location,
              virtualMachine.id
            );
          } else {
            helpers.addResult(
              results,
              2,
              "The Virtual Machine does not have Availability Set enabled",
              location,
              virtualMachine.id
            );
          }
