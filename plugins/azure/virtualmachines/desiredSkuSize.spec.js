...
describe("desiredSkuSize", function () {
  describe("run", function () {
    ...
    it("should give passing result if no virtual machines", function (done) {
      ...
      desiredSkuSize.run(
        cache,
        { vm_desired_sku_size: "standard_ds3_v2" },
        (err, results) => {
    ...
    it("should give passing result if virtual machine is of desired SKU size", function (done) {
      ...
      desiredSkuSize.run(
        ...
        (err, results) => {
          ...
          expect(results[0].status).to.equal(0);
          expect(results[0].message).to.include(
            "Virtual machine is using the desired SKU size of 'standard_ds3_v2'"
          );
          expect(results[0].region).to.equal("eastus");
          done();
        }
    ...
    it("should give failing result if virtual machine is not of desired sku size", function (done) {
      ...
      desiredSkuSize.run(
        ...
        (err, results) => {
          ...
          expect(results[0].message).to.include(
            "Virtual machine is not using the desired SKU size of 'standard_ds3_v2'"
          );
          expect(results[0].region).to.equal("eastus");
          done();
        }
