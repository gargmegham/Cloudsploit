...
module.exports = {
  ...
  recommended_action:
    "Configure virtual machine instant restore backup retention policy to retain backups for desired number of days",
  ...
  settings: {
    vm_instant_restore_retention_period: {
      ...
      description:
        "Number of days that a VM instant restore backup will be retained",
      regex: "^[1-5]$",
      default: "5",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    const config = {
      retentionPeriod: parseInt(
        settings.vm_instant_restore_retention_period ||
          this.settings.vm_instant_restore_retention_period.default
      ),
    };
          ...
          function (virtualMachine, scb) {
            ...
            if (vmPolicies && vmPolicies.length) {
              ...
              for (const vmPolicy of vmPolicies) {
                ...
                if (
                  backupPolicy &&
                  backupPolicy.instantRpRetentionRangeInDays
                ) {
              ...
              if (retentionDays === 0) {
                helpers.addResult(
                  results,
                  2,
                  "No instant restore backup is configured",
                  location,
                  virtualMachine.id
                );
                ...
                helpers.addResult(
                  results,
                  0,
                  `VM instant restore backups are configured to be retained for ${retentionDays} of ${config.retentionPeriod} days desired limit`,
                  location,
                  virtualMachine.id
                );
              } else {
