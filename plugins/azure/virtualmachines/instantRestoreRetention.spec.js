...
const backupProtectedItems = [
  {
    ...
    virtualMachineId:
      "/subscriptions/123/resourceGroups/aqua-resource-group/providers/Microsoft.Compute/virtualMachines/test-vm",
    policyId:
      "/Subscriptions/123/resourceGroups/aqua-resource-group/providers/Microsoft.RecoveryServices/vaults/test-vault/backupPolicies/DailyPolicy",
  },
  {
    ...
    type: "Microsoft.RecoveryServices/vaults/backupFabrics/protectionContainers/protectedItems",
    virtualMachineId:
      "/subscriptions/123/resourceGroups/aqua-resource-group/providers/Microsoft.Compute/virtualMachines/test-vm-2",
    policyId:
      "/Subscriptions/123/resourceGroups/aqua-resource-group/providers/Microsoft.RecoveryServices/vaults/test-vault/backupPolicies/DailyPolicy",
  },
];
  ...
  describe("run", function () {
    ...
    it("should give passing result if instant restore backup retention period is configured", function (done) {
      ...
      instantRestoreRetention.run(cache, {}, (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          "VM instant restore backups are configured to be retained for 5 of 5 days desired limit"
        );
        expect(results[0].region).to.equal("eastus");
        done();
      });
    ...
    it("should give failing result if backup retention period is not configured", function (done) {
      ...
      instantRestoreRetention.run(cache, {}, (err, results) => {
        ...
        expect(results[0].status).to.equal(2);
        expect(results[0].message).to.include(
          "VM instant restore backups are configured to be retained for 2 of 5 days desired limit"
        );
        expect(results[0].region).to.equal("eastus");
        done();
      });
