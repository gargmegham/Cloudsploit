...
module.exports = {
  ...
  more_info:
    "Autoscale automatically creates new instances when certain metrics are surpassed, or can destroy instances that are being underutilized. Autoscale notifications should be enabled to know about the status of autoscale operation.",
  recommended_action:
    "Ensure that autoscale notifications are enabled for all Virtual Machine Scale Sets",
  link: "https://docs.microsoft.com/en-us/azure/azure-monitor/autoscale/autoscale-overview",
  apis: [
    "virtualMachineScaleSets:listAll",
    "autoscaleSettings:listBySubscription",
  ],
      ...
      (location, rcb) => {
        ...
        if (virtualMachineScaleSets.err || !virtualMachineScaleSets.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for Virtual Machine Scale Sets: " +
              helpers.addError(virtualMachineScaleSets),
            location
          );
          return rcb();
        }
        ...
        if (!virtualMachineScaleSets.data.length) {
          helpers.addResult(
            results,
            0,
            "No existing Virtual Machine Scale Sets found",
            location
          );
          return rcb();
        }
        ...
        if (
          !autoscaleSettings ||
          autoscaleSettings.err ||
          !autoscaleSettings.data
        ) {
          helpers.addResult(
            results,
            3,
            "Unable to query for AutoScale settings: " +
              helpers.addError(autoscaleSettings),
            location
          );
          return rcb();
        }
        ...
        if (!autoscaleSettings.data.length) {
          helpers.addResult(
            results,
            2,
            "No Virtual Machine Scale Sets have autoscale enabled",
            location
          );
          return rcb();
        }
        ...
        autoscaleSettings.data.forEach(function (autoscaleSetting) {
          if (autoscaleSetting.targetResourceUri) {
            asMap[autoscaleSetting.targetResourceUri.toLowerCase()] =
              autoscaleSetting;
          }
        });
        ...
        virtualMachineScaleSets.data.forEach((virtualMachineScaleSet) => {
          ...
          let found = autoScaleNotifications.find(
            (notification) =>
              (notification.email &&
                (notification.email.sendToSubscriptionAdministrator ||
                  ...
                  (notification.email.customEmails &&
                    notification.email.customEmails.length))) ||
          ...
          if (found) {
            helpers.addResult(
              results,
              0,
              "Virtual Machine Scale Set has autoscale notifications enabled",
              location,
              virtualMachineScaleSet.id
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Virtual Machine Scale Set has autoscale notifications disabled",
              location,
              virtualMachineScaleSet.id
            );
          }
