...
module.exports = {
  ...
  more_info:
    "SSL Access only should be enabled for Azure Cache for Redis to meet the organization's security compliance requirements.",
        ...
        for (let cache of caches.data) {
          if (cache.enableNonSslPort) {
            helpers.addResult(
              results,
              2,
              "SSL Access Only is not enabled for Azure Cache for Redis",
              location,
              cache.id
            );
          } else {
            helpers.addResult(
              results,
              0,
              "SSL Access Only is enabled for Azure Cache for Redis",
              location,
              cache.id
            );
          }
