...
module.exports = {
  ...
  more_info:
    "Monitoring for rename SQL Server Database events gives insight into network access changes and may reduce the time it takes to detect suspicious activity.",
  recommended_action:
    "Add a new log alert to the Alerts service that monitors for SQL Server Database rename events.",
      ...
      function (location, rcb) {
          ...
          if (conditionOperation && conditionOperation.length) {
            allConditions.allOf.forEach((condition) => {
              if (
                ...
                condition.equals
                  .toLowerCase()
                  .indexOf(conditionResource + "/move") > -1
              ) {
                alertRenameEnabled =
                  !alertRenameEnabled && activityLogAlertResource.enabled
                    ? true
                    : alertRenameEnabled;
              }
            });
        ...
        if (!alertRenameEnabled) {
          helpers.addResult(
            results,
            2,
            `Log alert for ${text} rename is not enabled`,
            location,
            subscriptionId
          );
        } else {
          helpers.addResult(
            results,
            0,
            `Log alert for ${text} rename is enabled`,
            location,
            subscriptionId
          );
        }
