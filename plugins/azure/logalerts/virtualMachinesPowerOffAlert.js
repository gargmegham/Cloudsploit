...
module.exports = {
  ...
  more_info:
    "Monitoring for power off Virtual Machine events gives insight into network access changes and may reduce the time it takes to detect suspicious activity.",
  recommended_action:
    "Add a new log alert to the Alerts service that monitors for Virtual Machine power off events.",
      ...
      function (location, rcb) {
          ...
          if (conditionOperation && conditionOperation.length) {
            allConditions.allOf.forEach((condition) => {
              if (
                ...
                condition.equals
                  .toLowerCase()
                  .indexOf(conditionResource + "/poweroff") > -1
              ) {
                alertPowerOffEnabled =
                  !alertPowerOffEnabled && activityLogAlertResource.enabled
                    ? true
                    : alertPowerOffEnabled;
              }
            });
        ...
        if (!alertPowerOffEnabled) {
          helpers.addResult(
            results,
            2,
            `Log alert for ${text} powerOff is not enabled`,
            location,
            subscriptionId
          );
        } else {
          helpers.addResult(
            results,
            0,
            `Log alert for ${text} powerOff is enabled`,
            location,
            subscriptionId
          );
        }
