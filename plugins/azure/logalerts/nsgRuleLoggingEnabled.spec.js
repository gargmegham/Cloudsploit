  ...
  describe("run", function () {
    ...
    it("should give failing result if nsg rule write not enabled", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[1].message).to.include(
          "Log alert for Network Security Groups rule write is not enabled"
        );
        expect(results[0].region).to.equal("global");
        done();
      };
    ...
    it("should give failing result if nsg rule delete not enabled", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[1].status).to.equal(2);
        expect(results[1].message).to.include(
          "Log Alert for Network Security Groups rule delete is not enabled"
        );
        expect(results[0].region).to.equal("global");
        done();
      };
    ...
    it("should give passing result if nsg rule write enabled", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].message).to.include(
          "Log alert for Network Security Groups rule write is enabled"
        );
        expect(results[0].region).to.equal("global");
        done();
      };
    ...
    it("should give passing result if nsg rule delete enabled", function (done) {
      const callback = (err, results) => {
        ...
        expect(results[0].status).to.equal(0);
        expect(results[0].message).to.include(
          "Log alert for Network Security Groups rule delete is enabled"
        );
        expect(results[0].region).to.equal("global");
        done();
      };
