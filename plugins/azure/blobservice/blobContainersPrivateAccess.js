...
module.exports = {
  ...
  recommended_action:
    "Ensure each blob container is configured to restrict anonymous access",
  ...
  compliance: {
    hipaa:
      "Strict access controls to all data is a core requirement for HIPAA. " +
      "Restricting anonymous blob access ensures all access is limited to those " +
      "with explicit approval.",
    pci:
      "PCI requires all access to be restricted and identified. Limiting public blob " +
  ...
  actions: {
    remediate: ["blobContainers:update"],
    rollback: ["blobContainers:update"],
  },
  permissions: {
    remediate: ["blobContainers:update"],
    rollback: ["blobContainers:update"],
  ...
  realtime_triggers: [
    "microsoftstorage:storageaccounts:blobservices:containers:write",
  ],
        ...
        storageAccounts.data.forEach(function (storageAccount) {
          if (storageAccount.allowBlobPublicAccess === false) {
            helpers.addResult(
              results,
              0,
              "Storage Account does not allow blob containers public access",
              location,
              storageAccount.id
            );
          } else {
              ...
              blobContainers.data.forEach(function (blob) {
                if (
                  ...
                  helpers.addResult(
                    results,
                    0,
                    "Blob container does not allow public access",
                    location,
                    blob.id
                  );
                } else {
                  helpers.addResult(
                    results,
                    2,
                    "Blob container allows public access",
                    location,
                    blob.id
                  );
                }
