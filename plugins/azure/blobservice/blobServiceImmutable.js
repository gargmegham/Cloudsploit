...
module.exports = {
  ...
  recommended_action:
    "Enable a data immutability policy for all storage containers in the Azure storage account.",
  ...
  compliance: {
    hipaa:
      "Blob immutability preserves the integrity of stored data and protects against " +
      ...
      (location, rcb) => {
        ...
        if (!storageAccounts.data.length) {
          helpers.addResult(
            results,
            0,
            "No existing Storage Accounts found",
            location
          );
          return rcb();
        }
          ...
          function (storageAccount, cb) {
            ...
            if (!blobServices || blobServices.err || !blobServices.data) {
              helpers.addResult(
                results,
                3,
                "Unable to query Blob Services: " +
                  helpers.addError(blobServices),
                location,
                storageAccount.id
              );
              return cb();
              ...
              helpers.addResult(
                results,
                0,
                "Storage Account does not contain blob services",
                location,
                storageAccount.id
              );
              return cb();
              ...
              async.each(
                ...
                function (blob, bcb) {
                  if (
                    ...
                    blob.deleteRetentionPolicy.enabled
                  ) {
                    helpers.addResult(
                      results,
                      0,
                      "Immutability has been configured for the blob service",
                      location,
                      blob.id
                    );
                  } else {
                    if (blob.id) {
                      ...
                      failingStorageName.splice(
                        failingStorageName.length - 2,
                        2
                      );
                    ...
                    function (storageAccountId, scb) {
                      ...
                      if (
                        ...
                        helpers.addResult(
                          results,
                          3,
                          "Unable to query Blob Containers: " +
                            helpers.addError(blobContainers),
                          location,
                          storageAccount.id
                        );
                        return scb();
                        ...
                        helpers.addResult(
                          results,
                          0,
                          "Storage Account does not contain blob containers",
                          location,
                          storageAccount.id
                        );
                        return scb();
                        ...
                        async.each(
                          blobContainers.data,
                          function (blobContainer, ccb) {
                            if (blobContainer.hasImmutabilityPolicy) {
                              helpers.addResult(
                                results,
                                0,
                                "Immutability has been configured for the blob container",
                                location,
                                blobContainer.id
                              );
                            } else {
                              helpers.addResult(
                                results,
                                2,
                                "Immutability has not been configured for the blob container",
                                location,
                                blobContainer.id
                              );
                            }
