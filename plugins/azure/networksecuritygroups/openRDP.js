...
module.exports = {
  ...
  recommended_action:
    "For each VM, open the Networking blade and verify that the Inbound Port Rules do not have a rule for RDP with a source equal to Any or Internet",
  apis: ["networkSecurityGroups:listAll"],
  apis_remediate: ["networkSecurityGroups:listAll"],
  remediation_min_version: "202011201836",
  remediation_description:
    "The impacted network security group rule will be deleted if no input is provided. If the failing port is in a port range and no input is provided, the range will be deleted. Otherwise, any input will replace the open CIDR rule.",
  remediation_inputs: {
  ...
  actions: {
    remediate: ["networkSecurityGroups:update"],
    rollback: ["networkSecurityGroups:update"],
  },
  permissions: {
    remediate: ["networkSecurityGroups:update"],
    rollback: ["networkSecurityGroups:update"],
  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      locations.networkSecurityGroups,
      function (location, rcb) {
        var networkSecurityGroups = helpers.addSource(cache, source, [
        ...
        if (networkSecurityGroups.err || !networkSecurityGroups.data) {
          helpers.addResult(
            ...
            "Unable to query for Network Security Groups: " +
              helpers.addError(networkSecurityGroups),
            location
          );
          return rcb();
        }
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    var baseUrl =
      "https://management.azure.com/{resource}?api-version=2020-05-01";
    var method = "PUT";
    var protocols = ["TCP", "*"];
    var ports = [3389];
    var actions = [];
    var errors = [];
    helpers.remediateOpenPortsHelper(
      putCall,
      pluginName,
      protocols,
      ports,
