...
describe("excessiveSecurityGroups", function () {
  describe("run", function () {
    ...
    it("should give passing result if the count of Network Security Group is Acceptable", function (done) {
      ...
      excessiveSecurityGroups.run(
        cache,
        {
          excessive_security_groups_fail: 5,
          excessive_security_groups_warn: 4,
        },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          expect(results[0].message).to.include(
            "Acceptable number of security groups:"
          );
          expect(results[0].region).to.equal("eastus");
          done();
        }
    ...
    it("should give failing result if the count of Network Security Group is Excessive", function (done) {
      ...
      excessiveSecurityGroups.run(
        ...
        (err, results) => {
          ...
          expect(results[0].message).to.include(
            "Excessive number of security groups:"
          );
          expect(results[0].region).to.equal("eastus");
          done();
        }
    ...
    it("should give warning result if the count of Network Security Group is Large", function (done) {
      ...
      excessiveSecurityGroups.run(
        cache,
        {
          excessive_security_groups_fail: 3,
          excessive_security_groups_warn: 2,
        },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(1);
          expect(results[0].message).to.include(
            "Large number of security groups:"
          );
          expect(results[0].region).to.equal("eastus");
          done();
        }
