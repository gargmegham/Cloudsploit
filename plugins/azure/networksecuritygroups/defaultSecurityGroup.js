    ...
    async.each(
      ...
      function (location, rcb) {
        ...
        for (let s in networkSecurityGroups.data) {
          ...
          if (sg.defaultSecurityRules && sg.defaultSecurityRules.length) {
            ...
            if (denyRuleInbound && denyRuleOutbound) {
              helpers.addResult(
                ...
                "The security group: " +
                  sg.name +
                  " has all required default inbound and outbound rules",
                location,
                sg.id
              );
            } else {
              helpers.addResult(
                ...
                "The security group: " +
                  sg.name +
                  " does not have required default inbound and outbound rules: " +
                  (denyRuleInbound ? "Inbound: OK; " : "Inbound: FAIL; ") +
                  (denyRuleOutbound ? "Outbound: OK" : "Outbound: FAIL"),
                location,
                sg.id
              );
            }
            ...
            helpers.addResult(
              ...
              "Default security group for: " +
                sg.name +
                " is missing one or more default inbound or outbound rules",
              location,
              sg.id
            );
          }
