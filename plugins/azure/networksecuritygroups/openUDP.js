...
module.exports = {
  ...
  recommended_action:
    "Disable direct UDP access to your Azure Virtual Machines from the Internet",
    ...
    async.each(
      ...
      function (location, rcb) {
        ...
        for (let s in networkSecurityGroups.data) {
          ...
          if (sg.securityRules && sg.securityRules.length) {
            let InvalidSourceAddressPrefixes = [
              "*",
              "0.0.0.0",
              "<nw>/0",
              "/0",
            ...
            for (var rule in accessRules) {
              ...
              if (
                ...
                InvalidSourceAddressPrefixes.indexOf(
                  dRule.sourceAddressPrefix
                ) > -1
              ) {
                openUdpPorts = true;
            ...
            if (openUdpPorts) {
              helpers.addResult(
                ...
                "The security group: " +
                  sg.name +
                  " has open UDP ports for internet access",
                location,
                sg.id
              );
            } else {
              helpers.addResult(
                ...
                "The security group: " +
                  sg.name +
                  " does not have open UDP ports for internet access",
                location,
                sg.id
              );
            }
            ...
            helpers.addResult(
              ...
              "The security group: " +
                sg.name +
                " does not have any security rules",
              location,
              sg.id
            );
          }
