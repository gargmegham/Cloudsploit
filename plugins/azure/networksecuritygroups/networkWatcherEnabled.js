...
module.exports = {
  ...
  more_info:
    "Network Watcher helps locate, diagnose, and gain insights into Azure networks. Enabling Network Watcher in all locations ensures that no resources are being used in locations that are not authorized.",
    ...
    async.each(
      ...
      function (location, rcb) {
        ...
        if (!networkWatchers.data.length && virtualNetworks.data.length) {
          ...
          !networkWatchers.data.length &&
          !virtualNetworks.data.length
        ) {
          helpers.addResult(
            results,
            0,
            "No Virtual Networks or Network Watchers in the region",
            location
          );
        }
        ...
        networkWatchers.data.forEach((networkWatcher) => {
          if (
            ...
            networkWatcher.properties.provisioningState.toLowerCase() ==
              "succeeded"
          ) {
            helpers.addResult(
              results,
              0,
              "Network Watcher is enabled",
              location,
              networkWatcher.id
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Network Watcher is not successfully provisioned for the region",
              location,
              networkWatcher.id
            );
          }
