...
const logProfile = [
  {
    ...
    identity: null,
    storageAccountId:
      "/subscriptions/1234/resourceGroups/Default-ActivityLogAlerts/providers/Microsoft.Storage/storageAccounts/devstoragetwo",
  ...
  describe("run", function () {
    ...
    it("should give failing result if Log Profile has the following issues", function (done) {
      ...
      logProfileArchiveData.run(cache, {}, (err, results) => {
        ...
        expect(results[0].status).to.equal(2);
        expect(results[0].message).to.include(
          "Log Profile has the following issues"
        );
        expect(results[0].region).to.equal("global");
        done();
      });
