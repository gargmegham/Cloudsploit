...
module.exports = {
  ...
  more_info:
    "Monitor diagnostic setting in Azure controls how the diagnostic logs are exported. When a diagnostic setting is created, " +
  ...
  recommended_action:
    "Ensure the categories Administrative, Alert, Policy, and Security are set to Enabled for all diagnostic settings.",
        ...
        diagnosticSettings.data.forEach((settings) => {
          ...
          if (settings.logs && settings.logs.length) {
            settings.logs.forEach((log) => {
              if (
                log.enabled &&
                appropriateCategories.indexOf(log.category) > -1
              ) {
            ...
            if (appropriateCategories.length == enabledCategories.length) {
              helpers.addResult(
                results,
                0,
                "Diagnostic Setting is configured to log required categories",
                location,
                settings.id
              );
            } else {
              helpers.addResult(
                results,
                2,
                "Diagnostic Setting is not configured to log required categories",
                location,
                settings.id
              );
            }
            ...
            helpers.addResult(
              results,
              2,
              "Diagnostic Setting does not have any logs configured",
              location,
              settings.id
            );
          }
