...
module.exports = {
  ...
  recommended_action:
    "Enable sending of logs to Log Analytics for each Network Security Group resource in the Azure Monitor.",
  link: "https://docs.microsoft.com/en-us/azure/azure-monitor/platform/collect-activity-logs",
  apis: [
    "networkSecurityGroups:listAll",
    "diagnosticSettings:listByNetworkSecurityGroup",
  ],
  compliance: {
    pci:
      "PCI requires monitoring and logging of all network traffic. " +
      "These include malicious attempts to access services within the " +
      "infrastructure.",
    hipaa:
      "HIPAA requires monitoring and logging of all network traffic. " +
      "These include malicious attempts to access services within the " +
      "infrastructure.",
  },
      ...
      (location, rcb) => {
        ...
        if (!networkSecurityGroups.data.length) {
          helpers.addResult(
            results,
            0,
            "No existing Network Security Groups found",
            location
          );
          return rcb();
        }
        ...
        networkSecurityGroups.data.forEach(function (nsg) {
          ...
          if (
            !diagnosticSettings ||
            diagnosticSettings.err ||
            !diagnosticSettings.data
          ) {
            helpers.addResult(
              results,
              3,
              "Unable to query diagnostics settings: " +
            ...
            helpers.addResult(
              results,
              2,
              "No existing diagnostics settings",
              location,
              nsg.id
            );
          } else {
            ...
            if (found) {
              helpers.addResult(
                results,
                0,
                "NSG Log Analytics is enabled for NSG",
                location,
                nsg.id
              );
            } else {
              helpers.addResult(
                results,
                2,
                "NSG Log Analytics is not enabled for NSG",
                location,
                nsg.id
              );
            }
