...
module.exports = {
  ...
  recommended_action:
    "Ensure that all activity is logged to the Event Hub or storage account for archiving.",
  ...
  compliance: {
    ...
    pci:
      "Log profiles satisfy the PCI requirement to log all account activity " +
        ...
        logProfiles.data.forEach(function (logProfile) {
          ...
          if (logProfile.locations && logProfile.locations.length) {
            ...
            locations.all.forEach(function (region) {
              if (
                region !== "global" &&
                logProfile.locations.indexOf(region) === -1
              ) {
            ...
            if (unmatchedRegions.length) {
              issues.push(
                "the following regions are not being monitored: " +
                  unmatchedRegions.join(", ")
              );
            }
          } else {
          ...
          if (logProfile.categories && logProfile.categories.length) {
            ...
            if (unmatchedCats.length) {
              issues.push(
                "the following categories are not being monitored: " +
                  unmatchedCats.join(", ")
              );
            }
          } else {
          ...
          if (issues.length) {
            helpers.addResult(
              results,
              2,
              "Log Profile has the following issues: " + issues.join("; "),
              location,
              logProfile.id
            );
          } else {
            helpers.addResult(
              results,
              0,
              "Log Profile is archiving all activities in all regions.",
              location,
              logProfile.id
            );
          }
