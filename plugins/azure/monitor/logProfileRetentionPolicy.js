...
module.exports = {
  ...
  more_info:
    "Log retention policies should be configured with sufficient retention to aid in investigation of prior security incidents and for compliance purposes.",
  recommended_action:
    "Ensure that the Activity Log export to Event Hub is configured with a retention policy of at least 365 days.",
  ...
  compliance: {
    pci:
      "PCI requires log profile retention history to be" +
      " a minimum of 365 days.",
    hipaa:
      "HIPAA requires log profile data to be archived " +
      ...
      (location, rcb) => {
        ...
        if (logProfiles.err || !logProfiles.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for Log Profiles: " +
              helpers.addError(logProfiles),
            location
          );
          return rcb();
        }
        ...
        if (!logProfiles.data.length) {
          helpers.addResult(
            results,
            2,
            "No existing Log Profiles found",
            location
          );
          return rcb();
        }
        ...
        logProfiles.data.forEach(function (logProfile) {
          if (!logProfile.retentionPolicy) {
            ...
            !logProfile.retentionPolicy.days ||
            logProfile.retentionPolicy.days < 365
          ) {
            helpers.addResult(
              results,
              2,
              `The Log Profile retention policy of ${
                logProfile.retentionPolicy.days || "0"
              } days is not sufficient (at least 365 days).`,
              location,
              logProfile.id
            );
          } else {
