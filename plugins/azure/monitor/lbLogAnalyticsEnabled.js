...
module.exports = {
  ...
  more_info:
    "Enabling Send to Log Analytics ensures that all Load Balancer logs are being properly monitored and managed.",
  recommended_action:
    "Send all diagnostic logs for Load Balancers from the Azure Monitor service to Log Analytics.",
  ...
  compliance: {
    hipaa:
      "HIPAA requires that a secure audit log record for " +
      ...
      (location, rcb) => {
        ...
        if (loadBalancers.err || !loadBalancers.data) {
          helpers.addResult(
            ...
            "Unable to query for Load Balancers: " +
              helpers.addError(loadBalancers),
            location
          );
          return rcb();
        }
        ...
        if (!loadBalancers.data.length) {
          helpers.addResult(
            results,
            0,
            "No existing Load Balancers found",
            location
          );
          return rcb();
        }
          ...
          if (
            ...
            if (found) {
              helpers.addResult(
                results,
                0,
                "Log analytics is enabled for load balancer",
                location,
                loadBalancer.id
              );
            } else {
              helpers.addResult(
                results,
                2,
                "Log analytics is not enabled for load balancer",
                location,
                loadBalancer.id
              );
            }
