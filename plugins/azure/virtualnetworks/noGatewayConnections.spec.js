  ...
  describe("run", function () {
    ...
    it("should give failing result if there are network gateway connections", function (done) {
      ...
      noGatewayConnections.run(cache, {}, (err, results) => {
        ...
        expect(results[0].message).to.include(
          "test-gateway has an established connection with test-gateway-2"
        );

        expect(results[1].status).to.equal(2);
        expect(results[1].message).to.include(
          "test-gateway-2 has an established connection with test-gateway"
        );
