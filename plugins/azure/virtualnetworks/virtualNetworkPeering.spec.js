...
describe("virtualNetworkPeering", function () {
  describe("run", function () {
    ...
    it("should give passing result if No existing virtual networks found", function (done) {
      const cache = createCache([]);
      virtualNetworkPeering.run(
        cache,
        { enable_virtual_network_peering: "true" },
        (err, results) => {
    ...
    it("should give passing result if subscription is whitelisted", function (done) {
      const cache = createCache(
        [virtualNetworks[0]],
        [virtualNetworkPeerings[0]]
      );
      virtualNetworkPeering.run(
        cache,
        { whitelisted_peering_subscriptions: "123" },
        (err, results) => {
          expect(results.length).to.equal(1);
          expect(results[0].status).to.equal(0);
          expect(results[0].message).to.include(
    ...
    it("should give failing result if subscription is not whitelisted", function (done) {
      ...
      virtualNetworkPeering.run(
        ...
        (err, results) => {
          ...
          expect(results[0].message).to.include(
            "Vitual network has peering with these unknown subscriptions: 123"
          );
          expect(results[0].region).to.equal("eastus");
          done();
        }
