...
module.exports = {
  ...
  more_info:
    "Use subnets and network security groups to control network traffic instead of using virtual network gateways to meet your organization's security and compliance requirements.",
  link: "https://docs.microsoft.com/en-us/azure/vpn-gateway/vpn-gateway-about-vpngateways",
  recommended_action:
    "Configure subnets and network security groups instead of virtual network gateways",
  apis: [
    "resourceGroups:list",
    "virtualNetworks:listAll",
    "virtualNetworkGateways:listByResourceGroup",
  ],
      ...
      function (location, rcb) {
        ...
        if (!resourceGroups || resourceGroups.err || !resourceGroups.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for resource groups: " +
              helpers.addError(resourceGroups),
            location
          );
          return rcb();
        }
        ...
        if (!resourceGroups.data.length) {
          helpers.addResult(
            results,
            0,
            "No existing resource groups found",
            location
          );
          return rcb();
        }
        ...
        resourceGroups.data.forEach((resourceGroup) => {
          ...
          if (
            !virtualNetworkGateways ||
            virtualNetworkGateways.err ||
            !virtualNetworkGateways.data
          ) {
            helpers.addResult(
              results,
              3,
              "Unable to query for virtual Network Gateways: " +
                helpers.addError(virtualNetworkGateways),
              location
            );
            return;
          }
        ...
        virtualNetworks.data.forEach((virtualNetwork) => {
          ...
          if (virtualNetwork.subnets.length) {
            for (let subnet of virtualNetwork.subnets) {
              if (
                ...
                subnet.properties.ipConfigurations.length
              ) {
                let gatewayFound = false;
                for (let gatewayId of gatewaysList) {
                  gatewayFound = subnet.properties.ipConfigurations.some(
                    (configuration) => configuration.id.indexOf(gatewayId) > -1
                  );
                }
          ...
          if ((gatewayUsed && !subnetFound) || !gatewayUsed) {
            helpers.addResult(
              results,
              0,
              "Virtual network is not using network gateways",
              location,
              virtualNetwork.id
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Virtual network is using network gateways",
              location,
              virtualNetwork.id
            );
          }
