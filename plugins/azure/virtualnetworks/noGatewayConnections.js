...
module.exports = {
  ...
  apis: [
    "resourceGroups:list",
    "networkGatewayConnections:listByResourceGroup",
  ],
    ...
    async.each(
      ...
      function (location, rcb) {
        ...
        async.each(resourceGroups.data, function (resourceGroup, scb) {
          ...
          if (
            !networkGatewayConnections ||
            networkGatewayConnections.err ||
            !networkGatewayConnections.data
          ) {
            helpers.addResult(
              results,
              3,
              "Unable to query for network gateway connections : " +
                helpers.addError(networkGatewayConnections),
              location
            );
            return scb();
          }
          ...
          if (!networkGatewayConnections.data.length) {
            helpers.addResult(
              results,
              0,
              "No connections found for network gateways",
              location
            );
            return scb();
          }
          ...
          networkGatewayConnections.data.forEach((networkGatewayConnection) => {
            if (
              ...
              networkGatewayConnection.virtualNetworkGateway1.id &&
              networkGatewayConnection.virtualNetworkGateway2 &&
              networkGatewayConnection.virtualNetworkGateway2.id
            ) {
              let gateway1 =
                networkGatewayConnection.virtualNetworkGateway1.id.split("/");
              gateway1 = gateway1[gateway1.length - 1];
              let gateway2 =
                networkGatewayConnection.virtualNetworkGateway2.id.split("/");
              gateway2 = gateway2[gateway2.length - 1];
              helpers.addResult(
                results,
                2,
                `${gateway1} has an established connection with ${gateway2}`,
                location,
                networkGatewayConnection.virtualNetworkGateway1.id
              );
              helpers.addResult(
                results,
                2,
                `${gateway2} has an established connection with ${gateway1}`,
                location,
                networkGatewayConnection.virtualNetworkGateway2.id
              );
            }
