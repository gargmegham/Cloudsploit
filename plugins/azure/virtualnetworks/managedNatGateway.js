      ...
      function (location, rcb) {
        ...
        if (!natGateways || natGateways.err || !natGateways.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for Virtual Network NAT Gateways: " +
              helpers.addError(natGateways),
            location
          );
          return rcb();
        }
        ...
        virtualNetworks.data.forEach((virtualNetwork) => {
          ...
          let found =
            virtualNetwork.subnets && virtualNetwork.subnets.length
              ? virtualNetwork.subnets.some((subnet) =>
                  natSubnets.includes(subnet.id)
                )
              : false;
          ...
          if (found) {
            helpers.addResult(
              results,
              0,
              "Virtual Network Managed NAT (Network Address Translation) Gateway service is enabled for Virtual Network",
              location,
              virtualNetwork.id
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Virtual Network Managed NAT (Network Address Translation) Gateway service is disabled for Virtual Network",
              location,
              virtualNetwork.id
            );
          }
