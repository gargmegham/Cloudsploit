...
module.exports = {
  ...
  more_info:
    "When soft delete for blobs is enabled for a storage account, blobs, blob versions, and snapshots in that storage account may be recovered after they are deleted, within a retention period that you specify.",
  recommended_action:
    "Enable soft delete for blobs and set deletion retention policy to keep blobs for more than desired number of days",
  ...
  settings: {
    keep_deleted_blobs_for_days: {
      ...
      description:
        "Number of days that a blob is marked for deletion persists until it is permanently deleted",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: "30",
    },
  },
  ...
  run: function (cache, settings, callback) {
    ...
    const config = {
      keepForDays: parseInt(
        settings.keep_deleted_blobs_for_days ||
          this.settings.keep_deleted_blobs_for_days.default
      ),
    };
    ...
    async.each(
      ...
      function (location, rcb) {
        ...
        storageAccounts.data.forEach((storageAccount) => {
          ...
          if (
            !getServiceProperties ||
            getServiceProperties.err ||
            !getServiceProperties.data
          ) {
            helpers.addResult(
              results,
              3,
              `Unable to get blob service properties: ${helpers.addError(
                getServiceProperties
              )}`,
              location,
              storageAccount.id
