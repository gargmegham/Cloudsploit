...
module.exports = {
  ...
  recommended_action:
    "Ensure the Storage Account used by Activity Logs is configured with a BYOK key.",
  ...
  compliance: {
    hipaa:
      "HIPAA requires that all data is encrypted, including data at rest. " +
        ...
        storageAccounts.data.forEach((storageAccount) => {
          ...
          if (checkSA) {
            ...
            if (
              ...
              storageAccount.encryption.keySource.toLowerCase() ==
                "microsoft.keyvault"
            ) {
              helpers.addResult(
                results,
                0,
                "Activity Logs container for the storage account is encrypted with BYOK",
                location,
                storageAccount.id
              );
            } else {
              helpers.addResult(
                results,
                2,
                "Activity Logs container for the storage account is not encrypted with BYOK",
                location,
                storageAccount.id
              );
            }
