...
module.exports = {
  ...
  recommended_action:
    "Configure the firewall of each Storage Account to allow access only from known virtual networks.",
  link: "https://docs.microsoft.com/en-us/azure/storage/common/storage-network-security",
  apis: ["storageAccounts:list"],
  compliance: {
    pci:
      "PCI requires data access to be configured to use a firewall. Removing the " +
      "default network access action enables a more granular level of access controls.",
    hipaa:
      "HIPAA access controls require data access to be restricted to known sources. " +
  ...
  remediation_description:
    "Default network action will be set to deny all traffic for affected storage accounts",
  apis_remediate: ["storageAccounts:list"],
  actions: {
    remediate: ["storageAccounts:update"],
    rollback: ["storageAccounts:update"],
  },
  permissions: {
    remediate: ["storageAccounts:update"],
    rollback: ["storageAccounts:update"],
        ...
        if (storageAccount.err || !storageAccount.data) {
          helpers.addResult(
            ...
            "Unable to query for Storage Accounts: " +
              helpers.addError(storageAccount),
            location
          );
          return rcb();
        }
          ...
          if (allowRegex && allowRegex.test(account.name)) {
            ...
            if (
              ...
              helpers.addResult(
                results,
                0,
                "Storage Account default network access rule set to deny",
                location,
                account.id
              );
            } else if (
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    var baseUrl =
      "https://management.azure.com/{resource}?api-version=2021-04-01";
    var method = "PATCH";
