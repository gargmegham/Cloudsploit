...
module.exports = {
  ...
  remediation_description:
    "The HTTPS-only option will be enabled for the storage account",
        ...
        if (!storageAccount.data.length) {
          ...
          for (var acct in storageAccount.data) {
            ...
            if (
              ...
              account.supportsHttpsTrafficOnly
            ) {
              helpers.addResult(
                results,
                0,
                "Storage Account is configured with HTTPS-only traffic",
                location,
                account.id
              );
            } else {
              helpers.addResult(
                results,
                2,
                "Storage Account is not configured with HTTPS-only traffic",
                location,
                account.id
              );
            }
  ...
  remediate: function (config, cache, settings, resource, callback) {
    ...
    var baseUrl =
      "https://management.azure.com/{resource}?api-version=2019-06-01";
    var method = "PATCH";

    // for logging purposes
    var storageAccountNameArr = resource.split("/");
    var storageAccountName =
      storageAccountNameArr[storageAccountNameArr.length - 1];
