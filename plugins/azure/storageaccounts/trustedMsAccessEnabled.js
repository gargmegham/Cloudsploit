...
module.exports = {
  ...
  recommended_action:
    "For each Storage Account, configure an exception for trusted Microsoft services.",
      ...
      function (location, rcb) {
        ...
        storageAccount.data.forEach((account) => {
          if (
            ...
            account.networkAcls.bypass.toLowerCase().indexOf("azureservices") >
              -1
          ) {
            helpers.addResult(
              results,
              0,
              "Storage Account is set to allow trusted Microsoft services",
              location,
              account.id
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Storage Account is not set to allow trusted Microsoft services",
              location,
              account.id
            );
          }
