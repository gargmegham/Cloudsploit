...
module.exports = {
  ...
  more_info:
    "Subscription owners should not include permissions to create custom owner roles. This follows the principle of least privilege.",
  link: "https://docs.microsoft.com/en-us/azure/role-based-access-control/custom-roles",
  recommended_action:
    "Remove roles that allow permissions to create custom owner roles.",
      ...
      function (location, rcb) {
        ...
        if (roleDefinitions.err || !roleDefinitions.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for role definitions: " +
              helpers.addError(roleDefinitions),
            location
          );
          return rcb();
        }
        ...
        roleDefinitions.data.forEach((roleDefinition) => {
          if (
            ...
            roleDefinition.roleType !== "BuiltInRole"
          ) {
        ...
        customRoles.forEach(function (roleDefinition) {
          ...
          if (roleDefinition.permissions && roleDefinition.permissions.length) {
            roleDefinition.permissions.forEach((permission) => {
              if (permission.actions && permission.actions.indexOf("*") > -1) {
                action = true;
              }
            });
          }
          ...
          if (action && scope) {
            helpers.addResult(
              results,
              2,
              "Permission to create custom owner roles enabled",
              location,
              roleDefinition.id
            );
          } else {
            helpers.addResult(
              results,
              0,
              "Permission to create custom owner roles is not enabled",
              location,
              roleDefinition.id
            );
          }
