...
module.exports = {
  ...
  recommended_action:
    "Disable global read, write, and delete policies on all tables and ensure the ACL is configured with least privileges.",
  ...
  compliance: {
    ...
    pci:
      "PCI data must be secured via least-privileged ACLs. Table Service ACLs " +
        ...
        storageAccounts.data.forEach(function (storageAccount) {
          ...
          if (!listKeys || listKeys.err || !listKeys.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for for Table Service using Storage Account SAS: " +
            ...
            if (
              !listTablesSegmented ||
              listTablesSegmented.err ||
              !listTablesSegmented.data
            ) {
              helpers.addResult(
                results,
                3,
                "Unable to query for for Table Service: " +
                  helpers.addError(listTablesSegmented),
                location,
                storageAccount.id
              );
              ...
              helpers.addResult(
                results,
                0,
                "No existing Table Service tables found",
                location,
                storageAccount.id
              );
            } else {
              listTablesSegmented.data.forEach(function (table) {
                ...
                if (!getTableAcl || getTableAcl.err || !getTableAcl.data) {
                  helpers.addResult(
                    results,
                    3,
                    "Unable to query Table Service table ACL: " +
                      helpers.addError(getTableAcl),
                    location,
                    tableId
                  );
                } else {
                  ...
                  if (
                    ...
                    if (fullPermissions.length) {
                      helpers.addResult(
                        results,
                        2,
                        `Table ACL allows: ${fullPermissions.join(", ")}`,
                        location,
                        tableId
                      );
                    } else {
                      helpers.addResult(
                        results,
                        0,
                        "Table ACL does not contain full access permissions",
                        location,
                        tableId
                      );
                    }
                    ...
                    helpers.addResult(
                      results,
                      0,
                      "Table ACL has not been configured",
                      location,
                      tableId
                    );
                  }
