...
module.exports = {
  ...
  recommended_action:
    "Enable HTTPS and disable HTTP for each custom origin endpoint for each CDN profile.",
        ...
        profiles.data.forEach(function (profile) {
          ...
          if (!endpoints || endpoints.err || !endpoints.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for CDN Profile endpoints: " +
                helpers.addError(endpoints),
              location,
              profile.id
            );
          } else {
            if (!endpoints.data.length) {
              helpers.addResult(
                results,
                0,
                "CDN profile does not contain any endpoints",
                location,
                profile.id
              );
            } else {
              ...
              endpoints.data.forEach(function (endpoint) {
                if (endpoint.isHttpAllowed) {
                  helpers.addResult(
                    results,
                    2,
                    "CDN profile endpoint allows insecure HTTP origin",
                    location,
                    endpoint.id
                  );
                } else {
                  helpers.addResult(
                    results,
                    0,
                    "CDN profile endpoint does not allow insecure HTTP origin",
                    location,
                    endpoint.id
                  );
                }
