...
module.exports = {
  ...
  recommended_action:
    "Ensure that diagnostic logging is enabled for each CDN endpoint for each CDN profile",
  link: "https://docs.microsoft.com/en-us/azure/cdn/cdn-azure-diagnostic-logs",
  apis: [
    "profiles:list",
    "endpoints:listByProfile",
    "diagnosticSettings:listByEndpoint",
  ],
      ...
      (location, rcb) => {
        ...
        if (profiles.err || !profiles.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query CDN profiles: " + helpers.addError(profiles),
            location
          );
          return rcb();
        }
        ...
        if (!profiles.data.length) {
          helpers.addResult(
            results,
            0,
            "No existing CDN profiles found",
            location
          );
          return rcb();
        }
        ...
        profiles.data.forEach(function (profile) {
          ...
          if (!endpoints || endpoints.err || !endpoints.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query CDN endpoints: " + helpers.addError(endpoints),
              location,
              profile.id
            );
            ...
            helpers.addResult(
              results,
              0,
              "No existing CDN endpoints found",
              location,
              profile.id
            );
          } else {
              ...
              if (
                ...
                if (found) {
                  helpers.addResult(
                    results,
                    0,
                    "Request logging is enabled for endpoint",
                    location,
                    endpoint.id
                  );
                } else {
                  helpers.addResult(
                    results,
                    2,
                    "Request logging is not enabled for endpoint",
                    location,
                    endpoint.id
                  );
                }
