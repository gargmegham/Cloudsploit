...
module.exports = {
  ...
  recommended_action:
    "Disable global read, write, delete policies on all queues and ensure the ACL is configured with least privileges.",
  ...
  compliance: {
    ...
    pci:
      "PCI data must be secured via least-privileged ACLs. Queue Service ACLs " +
        ...
        storageAccounts.data.forEach(function (storageAccount) {
          ...
          if (!listKeys || listKeys.err || !listKeys.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for for Queue Service using Storage Account SAS: " +
            ...
            if (
              !listQueuesSegmented ||
              listQueuesSegmented.err ||
              !listQueuesSegmented.data
            ) {
              helpers.addResult(
                results,
                3,
                "Unable to query for for Queue Service: " +
                  helpers.addError(listQueuesSegmented),
                location,
                storageAccount.id
              );
              ...
              helpers.addResult(
                results,
                0,
                "No existing Queue Service queues found",
                location,
                storageAccount.id
              );
            } else {
              listQueuesSegmented.data.forEach(function (queue) {
                ...
                if (!getQueueAcl || getQueueAcl.err || !getQueueAcl.data) {
                  helpers.addResult(
                    results,
                    3,
                    "Unable to query Queue Service queue ACL: " +
                      helpers.addError(getQueueAcl),
                    location,
                    queue.id
                  );
                } else {
                  ...
                  if (
                        ...
                        switch (permissions.charAt(i)) {
                          ...
                          case "a":
                            fullPermissions.push(
                              `add (via identifier ${ident})`
                            );
                            break;
                          case "u":
                            fullPermissions.push(
                              `update (via identifier ${ident})`
                            );
                            break;
                          case "p":
                            fullPermissions.push(
                              `process (via identifier ${ident})`
                            );
                            break;
                          default:
                            break;
                    ...
                    if (fullPermissions.length) {
                      helpers.addResult(
                        results,
                        2,
                        `Queue ACL allows: ${fullPermissions.join(", ")}`,
                        location,
                        queue.id
                      );
                    } else {
                      helpers.addResult(
                        results,
                        0,
                        "Queue ACL does not contain full access permissions",
                        location,
                        queue.id
                      );
                    }
                    ...
                    helpers.addResult(
                      results,
                      0,
                      "Queue ACL has not been configured",
                      location,
                      queue.id
                    );
                  }
