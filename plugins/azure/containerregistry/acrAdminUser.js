...
module.exports = {
  ...
  recommended_action:
    "Ensure that the admin user is disabled for each container registry.",
      ...
      function (location, rcb) {
        ...
        if (registries.err || !registries.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for container registries: " +
              helpers.addError(registries),
            location
          );
          return rcb();
        }
        ...
        if (!registries.data.length) {
          helpers.addResult(
            results,
            0,
            "No existing container registries found",
            location
          );
          return rcb();
        }
        ...
        registries.data.forEach((registry) => {
          if (registry.adminUserEnabled) {
            helpers.addResult(
              results,
              2,
              "Admin user is enabled on the container registry",
              location,
              registry.id
            );
          } else {
            helpers.addResult(
              results,
              0,
              "Admin user is not enabled on the container registry",
              location,
              registry.id
            );
          }
