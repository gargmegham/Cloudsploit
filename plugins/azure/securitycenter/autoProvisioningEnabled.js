...
module.exports = {
  ...
  recommended_action:
    "Ensure that the data collection settings of the subscription have Auto Provisioning set to enabled.",
  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      locations.autoProvisioningSettings,
      (location, rcb) => {
        ...
        if (autoProvisioningSettings.err || !autoProvisioningSettings.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query auto provisioning settings: " +
              helpers.addError(autoProvisioningSettings),
            location
          );
          return rcb();
        }
        ...
        if (!autoProvisioningSettings.data.length) {
          helpers.addResult(
            results,
            2,
            "No existing auto provisioning settings found",
            location
          );
          return rcb();
        }
        ...
        autoProvisioningSettings.data.forEach(function (aps) {
          if (aps.autoProvision && aps.autoProvision.toLowerCase() == "on") {
            helpers.addResult(
              results,
              0,
              "Monitoring Agent Auto Provisioning is enabled",
              location,
              aps.id
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Monitoring Agent Auto Provisioning is disabled",
              location,
              aps.id
            );
          }
