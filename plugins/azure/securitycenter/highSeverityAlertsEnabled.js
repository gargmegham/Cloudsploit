...
module.exports = {
  ...
  recommended_action:
    "Ensure that high severity alerts are configured to be sent.",
    ...
    async.each(
      ...
      (location, rcb) => {
        ...
        securityContacts.data.forEach((securityContact) => {
          if (
            securityContact.alertNotifications &&
            securityContact.alertNotifications.toLowerCase() == "on"
          ) {
            helpers.addResult(
              results,
              0,
              "High severity alerts for the subscription are configured",
              location,
              securityContact.id
            );
          } else {
            helpers.addResult(
              results,
              2,
              "High severity alerts for the subscription are not configured",
              location,
              securityContacts.id
            );
          }
