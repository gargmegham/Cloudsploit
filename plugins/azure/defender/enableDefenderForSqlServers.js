...
module.exports = {
  ...
  more_info:
    "Turning on Microsoft Defender for Azure SQL Server Databases enables threat detection for Azure SQL database servers, providing threat intelligence, anomaly detection, and behavior analytics in the Microsoft Defender for Cloud.",
  recommended_action:
      ...
      function (location, rcb) {
        ...
        if (pricings.err || !pricings.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for Pricing: " + helpers.addError(pricings),
            location
          );
          return rcb();
        }
        ...
        let sqlServersPricing = pricings.data.find(
          (pricing) =>
            pricing.name && pricing.name.toLowerCase() === "sqlservers"
        );
        ...
        if (sqlServersPricing) {
          if (sqlServersPricing.pricingTier.toLowerCase() === "standard") {
            helpers.addResult(
              results,
              0,
              "Azure Defender is enabled for SQL Server Databases",
              location,
              sqlServersPricing.id
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Azure Defender is not enabled for SQL Server Databases",
              location,
              sqlServersPricing.id
            );
          }
