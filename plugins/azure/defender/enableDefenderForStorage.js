...
module.exports = {
  ...
  recommended_action:
    "Enable Microsoft Defender for Storage in Defender plans for the subscription.",
  ...
  run: function (cache, settings, callback) {
    ...
    async.each(
      ...
      function (location, rcb) {
        ...
        let storagePricing = pricings.data.find(
          (pricing) => pricing.name.toLowerCase() === "storageaccounts"
        );
        if (storagePricing) {
          if (storagePricing.pricingTier.toLowerCase() === "standard") {
            helpers.addResult(
              results,
              0,
              "Azure Defender is enabled for Storage Accounts",
              location,
              storagePricing.id
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Azure Defender is not enabled for Storage Accounts",
              location,
              storagePricing.id
            );
          }
