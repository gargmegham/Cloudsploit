...
module.exports = {
  ...
  more_info:
    "It is strongly recommended to configure the Azure Cosmos DB accounts used for production workloads to enable automatic failover. " +
        ...
        databaseAccounts.data.forEach((account) => {
          ...
          if (account.enableAutomaticFailover) {
            helpers.addResult(
              results,
              0,
              "Automatic failover is enabled for Cosmos DB account",
              location,
              account.id
            );
          } else {
            helpers.addResult(
              results,
              2,
              "Automatic failover is not enabled for Cosmos DB account",
              location,
              account.id
            );
          }
