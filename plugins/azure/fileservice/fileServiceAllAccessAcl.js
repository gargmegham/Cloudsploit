...
module.exports = {
  ...
  recommended_action:
    "Disable global read, write, and delete policies on all file shares and ensure the share ACL is configured with least privileges.",
  ...
  compliance: {
    hipaa:
      "HIPAA access controls require data to be secured with least-privileged " +
      "ACLs. File Service ACLs enable granular permissions for data access.",
    pci:
      "PCI data must be secured via least-privileged ACLs. File Service ACLs " +
      ...
      function (location, rcb) {
        ...
        if (storageAccounts.err || !storageAccounts.data) {
          helpers.addResult(
            results,
            3,
            "Unable to query for for storage accounts: " +
        ...
        storageAccounts.data.forEach(function (storageAccount) {
          ...
          if (!listKeys || listKeys.err || !listKeys.data) {
            helpers.addResult(
              results,
              3,
              "Unable to query for for File Service using Storage Account SAS: " +
                helpers.addError(listKeys),
              location,
              storageAccount.id
            );
          } else {
            ...
            if (
              !listSharesSegmented ||
              listSharesSegmented.err ||
              !listSharesSegmented.data
            ) {
              helpers.addResult(
                results,
                3,
                "Unable to query for for File Shares: " +
                  helpers.addError(listSharesSegmented),
                location,
                storageAccount.id
              );
              ...
              helpers.addResult(
                results,
                0,
                "No existing File Service shares found",
                location,
                storageAccount.id
              );
            } else {
              listSharesSegmented.data.forEach(function (fileShare) {
                ...
                if (!getShareAcl || getShareAcl.err || !getShareAcl.data) {
                  helpers.addResult(
                    results,
                    3,
                    "Unable to query File Service share ACL: " +
                      helpers.addError(getShareAcl),
                    location,
                    fileShare.id
                  );
                } else {
                  ...
                  if (
                    ...
                    Object.keys(acl.signedIdentifiers).length
                  ) {
                    for (var ident in acl.signedIdentifiers) {
                      var permissions =
                        acl.signedIdentifiers[ident].Permissions;
                        ...
                        switch (permissions.charAt(i)) {
                          ...
                          case "c":
                            fullPermissions.push(
                              `create (via identifier ${ident})`
                            );
                            break;
                          case "w":
                            fullPermissions.push(
                              `write (via identifier ${ident})`
                            );
                            break;
                          case "d":
                            fullPermissions.push(
                              `delete (via identifier ${ident})`
                            );
                            break;
                          case "l":
                            fullPermissions.push(
                              `list (via identifier ${ident})`
                            );
                            break;
                          default:
                            break;
                    ...
                    if (fullPermissions.length) {
                      helpers.addResult(
                        results,
                        2,
                        `File Share ACL allows: ${fullPermissions.join(", ")}`,
                        location,
                        fileShare.id
                      );
                    } else {
                      helpers.addResult(
                        results,
                        0,
                        "File Share ACL does not contain full access permissions",
                        location,
                        fileShare.id
                      );
                    }
                    ...
                    helpers.addResult(
                      results,
                      0,
                      "File Share ACL has not been configured",
                      location,
                      fileShare.id
                    );
                  }
