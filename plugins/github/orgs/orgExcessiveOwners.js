...
module.exports = {
  ...
  category: "Orgs",
  description:
    "Checks whether the organization has an excessive number of owners relative to its size.",
  more_info:
  ...
  recommended_action:
    "Reduce the number of owners for the organization and use repository-level permissions for more granular control.",
  ...
  run: function (cache, settings, callback) {
    ...
    if (listMembers.err || !listMembers.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query for organization members: " +
          helpers.addError(listMembers)
      );
      ...
      if (getMembership && getMembership.data && getMembership.data.role) {
        ...
        helpers.addResult(
          ...
          "Unable to get organization membership for user: " +
            member.login +
            ": " +
            helpers.addError(getMembership)
        );
      }
    }
    ...
    if (count.admin + count.users <= 5) {
      ...
      var percent = Math.ceil(
        (count.admin / (count.admin + count.users)) * 100
      );
      if (percent > 20) {
        helpers.addResult(
          results,
