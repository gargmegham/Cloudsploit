  ...
  run: function (cache, settings, callback) {
    ...
    if (getOrg.err || !getOrg.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query for organization permission information: " +
    ...
    if (
      !getOrg.data.default_repository_permission ||
      getOrg.data.default_repository_permission == "none"
    ) {
      helpers.addResult(
        results,
        0,
        "The default organization permission level is set to none."
      );
    } else if (getOrg.data.default_repository_permission == "read") {
      helpers.addResult(
        results,
        1,
        "The default organization permission level is set to read-only."
      );
    } else if (getOrg.data.default_repository_permission == "write") {
      helpers.addResult(
        results,
        2,
        "The default organization permission level is set to write."
      );
    } else if (getOrg.data.default_repository_permission == "admin") {
      helpers.addResult(
        results,
        2,
        "The default organization permission level is set to admin."
      );
    }
