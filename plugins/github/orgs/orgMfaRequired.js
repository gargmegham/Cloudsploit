...
module.exports = {
  ...
  category: "Orgs",
  description:
    "Checks whether multi-factor authentication is required at the org-level.",
  more_info:
  ...
  recommended_action:
    "Enable the setting that requires two-factor authentication for everyone in the organization.",
  apis: ["orgs:get"],
  ...
  run: function (cache, settings, callback) {
    ...
    if (getOrg.err || !getOrg.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query for organization MFA information: " +
    ...
    if (
      !getOrg.data.two_factor_requirement_enabled ||
      getOrg.data.two_factor_requirement_enabled == "none"
    ) {
      helpers.addResult(
        results,
        2,
        "MFA is not enforced for the organization."
      );
    } else {
