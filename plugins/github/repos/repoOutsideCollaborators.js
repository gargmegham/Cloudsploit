...
module.exports = {
  ...
  category: "Repos",
  description:
    "Ensures organization repositories do not have outside collaborators with admin or push permissions.",
  more_info:
  ...
  recommended_action:
    "For outside collaborators that need access to organization code, provide read access and require the collaborator to fork the repo and submit a pull request that can be reviewed by organization members.",
  ...
  run: function (cache, settings, callback) {
    ...
    var listMembers = helpers.addSource(cache, source, ["orgs", "listMembers"]);
    ...
    for (r in listRepos.data) {
      ...
      if (
        ...
        listCollaborators.err
      ) {
        helpers.addResult(
          results,
          3,
          "Unable to list collaborators for repo: " +
            repo.full_name +
            ": " +
            helpers.addError(listCollaborators),
          "global",
          resource
        );
        continue;
      ...
      if (!listCollaborators.data.length) {
        helpers.addResult(
          results,
          0,
          "No collaborators found for repository: " + repo.full_name,
          "global",
          resource
        );
        continue;
      ...
      for (c in listCollaborators.data) {
        ...
        if (orgMembers.indexOf(collaborator.login) == -1) {
          for (p in collaborator.permissions) {
            if (collaborator.permissions[p] && outside[p])
              outside[p].push(collaborator);
          }
        }
      }
      ...
      var message =
        "Repository has outside collaborators with the following permissions:" +
        " Admin: " +
        outside.admin.length +
        " Push: " +
        outside.push.length +
        " Pull: " +
        outside.push.length;
