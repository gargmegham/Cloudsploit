...
module.exports = {
  ...
  types: ["user"],
  description:
    "Ensures multi-factor authentication is enabled for the default user account",
  more_info:
    "GitHub MFA provides additional account security by requiring an additional login device or code. All accounts should have MFA enabled.",
  ...
  run: function (cache, settings, callback) {
    ...
    if (getAuthenticated.err || !getAuthenticated.data) {
      helpers.addResult(
        results,
        3,
        "Unable to query for user MFA status: " +
          helpers.addError(getAuthenticated)
      );
    ...
    if (getAuthenticated.data.two_factor_authentication) {
      helpers.addResult(
        ...
        "User MFA is enabled",
        "global",
        getAuthenticated.data.url || "N/A"
      );
    } else {
      helpers.addResult(
        results,
        2,
        "User MFA is not enabled",
        "global",
        getAuthenticated.data.url || "N/A"
      );
    }
