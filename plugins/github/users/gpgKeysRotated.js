...
module.exports = {
  ...
  settings: {
    github_gpg_keys_rotated_fail: {
      ...
      description:
        "Return a failing result when GPG keys exceed this number of days without being rotated",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: 365,
    },
    github_gpg_keys_rotated_warn: {
      ...
      description:
        "Return a warning result when GPG keys exceed this number of days without being rotated",
      regex: "^[1-9]{1}[0-9]{0,3}$",
      default: 180,
    },
  },
  ...
  run: function (cache, settings, callback) {
    var config = {
      github_gpg_keys_rotated_fail:
        settings.github_gpg_keys_rotated_fail ||
        this.settings.github_gpg_keys_rotated_fail.default,
      github_gpg_keys_rotated_warn:
        settings.github_gpg_keys_rotated_warn ||
        this.settings.github_gpg_keys_rotated_warn.default,
    };
    ...
    for (p in listGpgKeys.data) {
      ...
      if (key.created_at) {
        ...
        if (
          helpers.daysAgo(key.created_at) > config.github_gpg_keys_rotated_fail
        ) {
          returnCode = 2;
        } else if (
          helpers.daysAgo(key.created_at) > config.github_gpg_keys_rotated_warn
        ) {
          returnCode = 1;
        }
        ...
        helpers.addResult(
          results,
          3,
          "User GPG key: " + keyName + " does not have a created date",
          "global",
          keyResourceName,
          custom
        );
