...
parser.add_argument("--config", {
  help:
    "The path to a CloudSploit config file containing cloud credentials. See config_example.js. " +
...
parser.add_argument("--collection", {
  help: "Output: full collection JSON as file",
});
...
if (settings.compliance && settings.compliance.indexOf("cis") > -1) {
  ...
  settings.compliance = settings.compliance.filter(function (e) {
    return e !== "cis";
  });
}
...
function checkRequiredKeys(obj, keys) {
  keys.forEach(function (key) {
    if (!obj[key] || !obj[key].length) {
      console.error(
        `ERROR: The credential config did not contain a valid value for: ${key}`
      );
      process.exit(1);
    }
  });
...
if (
  config.credentials.aws.credential_file &&
  (!settings.cloud || settings.cloud == "aws")
) {
  settings.cloud = "aws";
  ...
  if (
    ...
    console.error(
      "ERROR: AWS credential file does not have accessKeyId or secretAccessKey properties"
    );
    process.exit(1);
  }
} else if (
  config.credentials.aws.access_key &&
  (!settings.cloud || settings.cloud == "aws")
) {
  settings.cloud = "aws";
  ...
  config.credentials.azure.credential_file &&
  (!settings.cloud || settings.cloud == "azure")
) {
  settings.cloud = "azure";
  cloudConfig = loadHelperFile(config.credentials.azure.credential_file);
  if (
    !cloudConfig ||
    !cloudConfig.ApplicationID ||
    !cloudConfig.KeyValue ||
    !cloudConfig.DirectoryID ||
    !cloudConfig.SubscriptionID
  ) {
    console.error(
      "ERROR: Azure credential file does not have ApplicationID, KeyValue, DirectoryID, or SubscriptionID"
    );
    process.exit(1);
  }
  cloudConfig.location = "East US";
} else if (
  config.credentials.azure.application_id &&
  (!settings.cloud || settings.cloud == "azure")
) {
  settings.cloud = "azure";
  checkRequiredKeys(config.credentials.azure, [
    "key_value",
    "directory_id",
    "subscription_id",
  ]);
  ...
  config.credentials.google.credential_file &&
  (!settings.cloud || settings.cloud == "google")
) {
  ...
  config.credentials.google.project &&
  (!settings.cloud || settings.cloud == "google")
) {
  settings.cloud = "google";
  ...
  config.credentials.oracle.credential_file &&
  (!settings.cloud || settings.cloud == "oracle")
) {
  settings.cloud = "oracle";
  cloudConfig = loadHelperFile(config.credentials.oracle.credential_file);
  if (
    !cloudConfig ||
    !cloudConfig.tenancyId ||
    !cloudConfig.compartmentId ||
    !cloudConfig.userId ||
    !cloudConfig.keyValue ||
    !cloudConfig.region
  ) {
    console.error(
      "ERROR: Oracle credential file does not have tenancyId, compartmentId, userId, region, or keyValue"
    );
    process.exit(1);
  }
  ...
  config.credentials.oracle.tenancy_id &&
  (!settings.cloud || settings.cloud == "oracle")
) {
  settings.cloud = "oracle";
  checkRequiredKeys(config.credentials.oracle, [
    "compartment_id",
    "user_id",
    "key_fingerprint",
    "key_value",
  ...
  config.credentials.github.credential_file &&
  (!settings.cloud || settings.cloud == "github")
) {
  ...
  config.credentials.alibaba.credential_file &&
  (!settings.cloud || settings.cloud == "alibaba")
) {
  settings.cloud = "alibaba";
  cloudConfig = loadHelperFile(config.credentials.alibaba.credential_file);
} else if (
  config.credentials.alibaba.access_key_id &&
  (!settings.cloud || settings.cloud == "alibaba")
) {
  settings.cloud = "alibaba";
...
if (settings.remediate && settings.remediate.length) {
  ...
  if (
    config.credentials.aws_remediate &&
    config.credentials.aws_remediate.credential_file
  ) {
    cloudConfig.remediate = loadHelperFile(
      config.credentials.aws_remediate.credential_file
    );
    if (
      !cloudConfig.remediate ||
      !cloudConfig.remediate.accessKeyId ||
      !cloudConfig.remediate.secretAccessKey
    ) {
      console.error(
        "ERROR: AWS credential file for remediation does not have accessKeyId or secretAccessKey properties"
      );
      process.exit(1);
    }
  } else if (
    config.credentials.aws_remediate &&
    config.credentials.aws_remediate.access_key
  ) {
    ...
    config.credentials.azure_remediate &&
    config.credentials.azure_remediate.credential_file
  ) {
    cloudConfig.remediate = loadHelperFile(
      config.credentials.azure_remediate.credential_file
    );
    if (
      !cloudConfig.remediate ||
      !cloudConfig.remediate.ApplicationID ||
      !cloudConfig.remediate.KeyValue ||
      !cloudConfig.remediate.DirectoryID ||
      !cloudConfig.remediate.SubscriptionID
    ) {
      console.error(
        "ERROR: Azure credential file for remediation does not have ApplicationID, KeyValue, DirectoryID, or SubscriptionID"
      );
      process.exit(1);
    }
  } else if (
    config.credentials.azure_remediate &&
    config.credentials.azure_remediate.application_id
  ) {
    checkRequiredKeys(config.credentials.azure_remediate, [
      "key_value",
      "directory_id",
      "subscription_id",
    ]);
    ...
    config.credentials.google_remediate &&
    config.credentials.google_remediate.project
  ) {
    checkRequiredKeys(config.credentials.google, [
      "client_email",
      "private_key",
    ]);
    cloudConfig = {
    ...
    console.error(
      "ERROR: Config file does not contain any valid credential configs for remediation."
    );
    process.exit(1);
  }
}
