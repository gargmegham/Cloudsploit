...
var collect = function (AzureConfig, settings, callback) {
  ...
  helpers.login(AzureConfig, function (loginErr, loginData) {
      ...
      helpers.call(
        {
          ...
          post: obj.post,
          token: obj.graph
            ? loginData.graphToken
            : obj.vault
            ? loginData.vaultToken
            : loginData.token,
        },
        function (err, data) {
          ...
          if (
            ...
            (!data ||
              !((obj.paginate && data[obj.paginate]) || data["nextLink"]))
          ) {
    ...
    var processCall = function (obj, cb, localData) {
      var localUrl =
        obj.nextUrl ||
        obj.url.replace(/\{subscriptionId\}/g, AzureConfig.SubscriptionID);
      if (obj.rateLimit) {
        setTimeout(function () {
    ...
    let integrationCall = function (
      ...
      settings.previousCollection = Object.keys(
        settings.previousCollection
      ...
      if (
        ...
        try {
          ...
          console.log(
            `Error in storing ${service} service data: ${JSON.stringify(e)}`
          );
          cback();
        }
      } else {
    ...
    async.series([
          ...
          function (callObj, service, callCb) {
            ...
            async.eachOf(
              ...
              function () {
                if (
                  settings.identifier &&
                  calls[service].sendIntegration &&
                  calls[service].sendIntegration.enabled
                ) {
      ...
      function (cb) {
        function processTopCall(collectionObj, service, subCallObj, subCallCb) {
          ...
          var regionsToLoop = parseCollection(
            subCallObj.reliesOnPath,
            collection
          );
                ...
                if (regionObj && regionObj.data && regionObj.data.length) {
                  ...
                  async.eachLimit(
                    ...
                    function (regionData, regionDataCb) {
                      ...
                      processCall(
                        localReq,
                        function (processCallErr, processCallData) {
                          if (processCallErr)
                            collectionObj[region][regionData.id].err =
                              processCallErr;
                          if (processCallData) {
                            if (processCallData.value) {
                              collectionObj[region][regionData.id].data =
                                processCallData.value;
                            } else {
                              collectionObj[region][regionData.id].data =
                                processCallData;
                            }
                            helpers.reduceProperties(
                              service,
                              collectionObj[region][regionData.id].data
                            );
                          }
          ...
          function (callObj, service, callCb) {
            ...
            async.eachOf(
              ...
              function () {
                if (
                  settings.identifier &&
                  postcalls[service].sendIntegration &&
                  postcalls[service].sendIntegration.enabled
                ) {
            ...
            async.eachOf(
              ...
              function (regionObj, region, regionCb) {
                ...
                async.eachOfLimit(
                  regionObj,
                  5,
                  function (resourceObj, resourceId, resourceCb) {
                    function processResource(resourceData, resourceDataCb) {
                      ...
                      processCall(
                        ...
                        function (processCallErr, processCallData) {
                          if (processCallErr)
                            collectionObj[region][resourceData.id].err =
                              processCallErr;
                          if (processCallData) {
                            if (processCallData.value) {
                              collectionObj[region][resourceData.id].data =
                                processCallData.value;
                            } else {
                              collectionObj[region][resourceData.id].data =
                                processCallData;
                    ...
                    if (Array.isArray(resourceObj)) {
                      async.eachLimit(
                        resourceObj,
                        10,
                        function (resourceData, resourceDataCb) {
            ...
            async.eachOf(
              ...
              function (subCallObj, one, subCallCb) {
                ...
                if (subCallObj.url) {
                  ...
                  async.eachOf(
                    subCallObj,
                    function (innerCallObj, two, innerCb) {
                      if (
                        settings &&
                        settings.api_calls &&
                        settings.api_calls.indexOf(
                          [service, one, two].join(":")
                        ) === -1
                      )
                        return subCallCb();

                      if (!collection[service][one][two])
                        collection[service][one][two] = {};
                      processTopCall(
                        collection[service][one][two],
                        service,
                        innerCallObj,
              ...
              function () {
                if (
                  settings.identifier &&
                  tertiarycalls[service].sendIntegration &&
                  tertiarycalls[service].sendIntegration.enabled
                ) {
                  if (
                    !tertiarycalls[service].sendIntegration.integrationReliesOn
                  ) {
                    integrationCall(
                      collection,
                      settings,
          ...
          function (callObj, service, callCb) {
            ...
            async.eachOfLimit(
              ...
              function () {
                if (
                  settings.identifier &&
                  specialcalls[service].sendIntegration &&
                  specialcalls[service].sendIntegration.enabled
                ) {
                  if (
                    !specialcalls[service].sendIntegration.integrationReliesOn
                  ) {
                    integrationCall(
                      collection,
                      settings,
