...
var collect = function (GitHubConfig, settings, callback) {
  var collection = {};
  var appConfig = {
    id: GitHubConfig.application_id,
    privateKey: GitHubConfig.private_key,
  };
  ...
  var path = GitHubConfig.organization
    ? "/orgs/:org/installation"
    : "/users/:username/installation";
  var param = GitHubConfig.organization ? "org" : "username";
  reqObj[param] = GitHubConfig.login;

  Octoreq("GET " + path, reqObj)
    .then(function (data) {
      if (!data || !data.data || !data.data.id)
        return callback(
          "No installation ID found. Please ensure the GitHub app is installed."
        );
      ...
      if (
        ...
        GitHubConfig.installation_id !== installationId
      ) {
        return callback(
          "Installation ID misconfigured. Please reinstall the GitHub app."
        );
      }

      app
        .getInstallationAccessToken({ installationId })
        .then(function (installationToken) {
          if (!installationToken)
            return callback(
              "Installation token could not be obtained. Please ensure the GitHub app is installed."
            );
          ...
          var octokit = {
            server: new Octokit({
              ...
              previews: ["hellcat-preview", "machine-man-preview"],
            }),
            user: new Octokit({
              baseUrl: GitHubConfig.url,
            ...
            if (callObj.paginate && results) {
              if (callObj.paginate !== "self") {
                ...
                for (var r in results) {
                  if (results[r][callObj.paginate])
                    masterList = masterList.concat(
                      results[r][callObj.paginate]
                    );
                }
                results = masterList;
              }
              ...
              async.eachOfLimit(
                ...
                function (callObj, callKey, callCb) {
                  if (
                    ...
                    return callCb();
                  if (!collection[service][callKey])
                    collection[service][callKey] = {};
                  ...
                  if (callObj.override) {
                    ...
                    var processResults = function (results) {
                      collection[service][callKey].data = processPagination(
                        callObj,
                        results
                      );
                      finish();
                    ...
                    if (callObj.paginate) {
                      var options =
                        octokit[type][service][callKey].endpoint.merge(params);
                      octokit[type]
                        .paginate(options)
                        .then(processResults, processErr);
                    } else {
                      octokit[type][service][callKey](params).then(
                        processResults,
                        processErr
                      );
                    }
                        ...
                        function (callObj, callKey, callCb) {
                          ...
                          if (callObj.override) {
                            ...
                            if (
                              ...
                              var params = callObj.params || {};
                              if (callObj.inject_org)
                                params.org = GitHubConfig.login;

                              var processResults = function (results) {
                                collection[service][callKey].data =
                                  processPagination(callObj, results);
                                callCb();
                              };
                              ...
                              if (callObj.paginate) {
                                var options =
                                  octokit[type][service][
                                    callKey
                                  ].endpoint.merge(params);
                                octokit[type]
                                  .paginate(options)
                                  .then(processResults, processErr);
                              } else {
                                ...
                                function (dep, depCb) {
                                  ...
                                  if (callObj.paginate) {
                                    var options =
                                      octokit[type][service][
                                        callKey
                                      ].endpoint.merge(filter);
                                    octokit[type]
                                      .paginate(options)
                                      .then(processResults, processErr);
                                  } else {
                                    octokit[type][service][callKey](
                                      filter
                                    ).then(processResults, processErr);
                                  }
                                },
