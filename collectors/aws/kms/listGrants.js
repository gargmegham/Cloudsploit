...
module.exports = function (AWSConfig, collection, retries, callback) {
  ...
  async.eachLimit(
    collection.kms.listKeys[AWSConfig.region].data,
    15,
    function (key, cb) {
      ...
      var paginateCb = function (err, data) {
        if (err)
          collection.kms.listGrants[AWSConfig.region][key.KeyId].err = err;
        ...
        if (
          ...
          collection.kms.listGrants[AWSConfig.region][key.KeyId].data.Grants =
            collection.kms.listGrants[AWSConfig.region][
              key.KeyId
            ].data.Grants.concat(data.Grants);
        } else {
