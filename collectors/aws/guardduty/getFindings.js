...
module.exports = function (AWSConfig, collection, retries, callback) {
  ...
  async.eachLimit(
    collection.guardduty.listDetectors[AWSConfig.region].data,
    15,
    function (detectorId, dcb) {
      if (
        !collection.guardduty ||
      ...
      helpers.makeCustomCollectorCall(
        ...
        function (err, data) {
          if (err) {
            collection.guardduty.getFindings[AWSConfig.region][detectorId].err =
              err;
          }
          collection.guardduty.getFindings[AWSConfig.region][detectorId].data =
            data;
          dcb();
        }
      );
