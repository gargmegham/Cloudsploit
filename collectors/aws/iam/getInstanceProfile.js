...
module.exports = function (AWSConfig, collection, retries, callback) {
  ...
  if (
    ...
    !Object.keys(collection.ec2.describeInstances).length
  )
    return callback();
  async.eachOfLimit(
    collection.ec2.describeInstances,
    5,
    function (regionInstances, region, rcb) {
      if (
        !collection.ec2 ||
        !regionInstances.data ||
        !regionInstances.data.length
      )
        return rcb();
      ...
      async.eachLimit(
        ...
        function (parent, cb) {
          ...
          let iamInstanceProfileName =
            instance.IamInstanceProfile.Arn.split("/")[1];

          if (
            collection.iam.getInstanceProfile[AWSConfig.region][
              instance.IamInstanceProfile.Arn
            ]
          )
            return cb();
