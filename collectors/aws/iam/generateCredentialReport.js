...
module.exports = function (AWSConfig, collection, retries, callback) {
  ...
  var generateCredentialReport = function (genCb) {
    iam.generateCredentialReport(function (err, data) {
      ...
      if (err || !data || !data.State)
        return genCb(err || "Unable to generate credential report");
      genCb();
    });
  };
  ...
  async.retry(
    ...
    function (genErr) {
      if (genErr) {
        collection.iam.generateCredentialReport[AWSConfig.region].err =
          genErr || "Unable to download credential report";
        return callback();
      }
      ...
      async.retry(
        ...
        function (reportErr, reportData) {
          if (reportErr || !reportData || !reportData.Content) {
            collection.iam.generateCredentialReport[AWSConfig.region].err =
              reportErr || "Unable to download credential report";
            return callback();
          }
          ...
          try {
            ...
            collection.iam.generateCredentialReport[AWSConfig.region].err =
              "Error converting credential CSV to string: " + e;
            return callback();
          }

          if (!csvRows[0]) {
            collection.iam.generateCredentialReport[AWSConfig.region].err =
              "Error reading credential CSV";
            return callback();
          }
          ...
          collection.iam.generateCredentialReport[AWSConfig.region].data =
            entries;
          callback();
        }
      );
