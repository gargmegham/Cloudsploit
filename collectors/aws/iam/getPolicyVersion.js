...
module.exports = function (AWSConfig, collection, retries, callback) {
  ...
  if (
    ...
    !collection.iam.listPolicies[AWSConfig.region].data
  )
    return callback();
  ...
  async.eachLimit(
    ...
    function (policy, cb) {
      ...
      helpers.makeCustomCollectorCall(
        iam,
        "getPolicyVersion",
        { PolicyArn: policy.Arn, VersionId: versionId },
        retries,
        null,
        null,
        null,
        function (err, data) {
          if (err) {
            collection.iam.getPolicyVersion[AWSConfig.region][policy.Arn].err =
              err;
          }
          collection.iam.getPolicyVersion[AWSConfig.region][policy.Arn].data =
            data;
          cb();
        }
      );
