...
module.exports = function (AWSConfig, collection, retries, callback) {
  ...
  async.eachLimit(
    collection.accessanalyzer.listAnalyzers[AWSConfig.region].data,
    15,
    function (analyzer, cb) {
      ...
      var paginateCb = function (err, data) {
        if (err)
          collection.accessanalyzer.listFindings[AWSConfig.region][
            analyzer.arn
          ].err = err;
        ...
        if (
          ...
          collection.accessanalyzer.listFindings[AWSConfig.region][
            analyzer.arn
          ].data.findings = collection.accessanalyzer.listFindings[
            AWSConfig.region
          ][analyzer.arn].data.findings.concat(data.findings);
        } else {
          collection.accessanalyzer.listFindings[AWSConfig.region][
            analyzer.arn
          ].data = data;
