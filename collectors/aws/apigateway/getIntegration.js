...
module.exports = function (AWSConfig, collection, retries, callback) {
  ...
  async.eachLimit(
    collection.apigateway.getRestApis[AWSConfig.region].data,
    5,
    function (api, cb) {
      if (
        !collection.apigateway.getResources ||
      ...
      async.eachLimit(
        ...
        function (resource, pCb) {
          ...
          async.eachOfLimit(
            ...
            3,
            function (methodVal, methodKey, mCb) {
              collection.apigateway.getIntegration[AWSConfig.region][api.id][
                resource.id
              ][methodKey] = {};
              ...
              helpers.makeCustomCollectorCall(
                ...
                function (err, data) {
                  if (err) {
                    collection.apigateway.getIntegration[AWSConfig.region][
                      api.id
                    ][resource.id][methodKey].err = err;
                    return mCb();
                  }

                  collection.apigateway.getIntegration[AWSConfig.region][
                    api.id
                  ][resource.id][methodKey].data = data;
                  mCb();
                }
              );
            },
