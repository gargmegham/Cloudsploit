...
module.exports = function (
  ...
  if (
    ...
    knownBuckets = collection.s3.listBuckets[AWSConfig.region].data.map(
      function (bucket) {
        return bucket.Name;
      }
    );
  ...
  if (
    collection &&
    collection.cloudtrail &&
    collection.cloudtrail.describeTrails
  ) {
    for (var region in collection.cloudtrail.describeTrails) {
      if (
        !collection.cloudtrail.describeTrails[region].data ||
        !collection.cloudtrail.describeTrails[region].data.length
      )
        continue;
          ...
          if (bErr) {
            ...
            if (bErr.statusCode && bErr.statusCode == 301) {
              helpers.makeCustomCollectorCall(
                ...
                null,
                null,
                function (locErr, locData) {
                  if (locErr || !locData || !locData.LocationConstraint)
                    return bcb();
                  // Special case where location constraint is EU - rewrite as eu-west-1
                  if (locData.LocationConstraint == "EU")
                    locData.LocationConstraint = "eu-west-1";
                  ...
                  s3Alt[callKey](
                    { Bucket: bucket },
                    function (altErr, altData) {
                      if (altErr) {
                        collection["s3"][callKey][AWSConfig.region][
                          bucket
                        ].err = altErr;
                      } else {
                        collection["s3"][callKey][AWSConfig.region][
