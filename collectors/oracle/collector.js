    ...
    function (callObj, callKey, callCb) {
      ...
      async.eachLimit(
        ...
        function (region, regionCb) {
          ...
          if (
            collection[regionSubscriptionService.name] &&
            collection[regionSubscriptionService.name][
              regionSubscriptionService.call
            ] &&
            collection[regionSubscriptionService.name][
              regionSubscriptionService.call
            ][regionSubscriptionService.region] &&
            collection[regionSubscriptionService.name][
              regionSubscriptionService.call
            ][regionSubscriptionService.region].data &&
            collection[regionSubscriptionService.name][
              regionSubscriptionService.call
            ][regionSubscriptionService.region].data.filter(
...
var getRegionSubscription = function (
  OracleConfig,
  collection,
  settings,
  calls,
...
var collect = function (OracleConfig, settings, callback) {
  var collection = {};
  OracleConfig.region = OracleConfig.region
    ? OracleConfig.region
    : "us-ashburn-1";
  OracleConfig.maxRetries = 5;
  OracleConfig.retryDelayOptions = { base: 300 };
  regionSubscriptionService = {
    name: "regionSubscription",
    call: "list",
    region: OracleConfig.region,
  };
  ...
  getRegionSubscription(
    ...
    settings,
    calls,
    regionSubscriptionService.name,
    regionSubscriptionService.call,
    regionSubscriptionService.region,
    function () {
      async.eachOfLimit(
        ...
        function () {
          ...
          async.eachOfLimit(
            postcalls,
            10,
            function (postCall, service, serviceCb) {
              if (!collection[service]) collection[service] = {};
            ...
            function () {
              ...
              async.eachOfLimit(
                finalcalls,
                10,
                function (finalCall, service, serviceCb) {
                  if (!collection[service]) collection[service] = {};
