...
var collect = function (GoogleConfig, settings, callback) {
  ...
  helpers.authenticate(GoogleConfig).then((client) => {
    async.series(
      [
        function (cb) {
          async.eachOfLimit(
        ...
        function (cb) {
          async.eachOfLimit(
            postcalls,
            10,
            function (postcallObj, service, postcallCb) {
              helpers.processCall(
                GoogleConfig,
                collection,
                settings,
                ...
                function () {
                  if (
                    ...
                    if (
                      !postcalls[service].sendIntegration.integrationReliesOn
                    ) {
                      integrationCall(
                        collection,
                        settings,
        ...
        function (cb) {
          async.eachOfLimit(
            tertiarycalls,
            10,
            function (tertiaryCallObj, service, tertiaryCallCb) {
              helpers.processCall(
                GoogleConfig,
                collection,
                settings,
