          ...
          async.eachLimit(
            ...
            function (region, regionCb) {
              if (
                ...
                globalServices.indexOf(service) === -1
              )
                return regionCb();
              if (!collection[serviceLower][callKey][region])
                collection[serviceLower][callKey][region] = {};

              let LocalAlibabaConfig = JSON.parse(
                JSON.stringify(AlibabaConfig)
              );
              ...
              if (callObj.override) {
                ...
                let endpoint =
                  regionEndpointMap[serviceLower] &&
                  regionEndpointMap[serviceLower].includes(region)
                    ? `https://${serviceLower}.${region}.aliyuncs.com`
                    : `https://${serviceLower}.aliyuncs.com`;
                ...
                var clientCb = function (err, data) {
                  ...
                  if (
                    callObj.subProperty &&
                    !data[callObj.property][callObj.subProperty]
                  )
                    return regionCb();

                  var dataToAdd = callObj.subProperty
                    ? data[callObj.property][callObj.subProperty]
                    : callObj.property
                  ...
                  if (
                    ...
                    if (
                      ...
                      let pageSize =
                        callObj.pageSize || parseInt(data["PageSize"]);
                ...
                function execute(nextToken, paginateParams) {
                  ...
                  if (nextToken) localParams[nextToken[0]] = nextToken[1];
                    ...
                    localParams = { ...localParams, ...paginateParams };

                  client.request(callKey, localParams, requestOption).then(
                    (result) => {
                        ...
                        async.eachLimit(
                          ...
                          function (val, valCb) {
                            ...
                            for (let key in callObj.filterKey) {
                              params[callObj.filterKey[key]] =
                                val[callObj.filterValue[key]];
                            }
                            ...
                            var requestCb = function (err, data) {
                              ...
                              collection[serviceLower][callKey][region][
                                ...
                                callObj.resultFilter &&
                                data[callObj.resultFilter]
                                  ? data[callObj.resultFilter]
                                  : data;
                            ...
                            var execute = function () {
                              client
                                .request(callKey, params, requestOption)
                                .then(
                                  (result) => {
