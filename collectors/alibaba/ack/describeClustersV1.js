...
module.exports = function (AlibabaConfig, collection, region, callback) {
  ...
  var execute = function () {
    ...
    client
      .request(httpMethod, uriPath, queries, body, headers, requestOption)
      .then(
        (res) => {
          callCB(null, res);
        },
  ...
  var callCB = function (err, data) {
    ...
    collection.ack.describeClustersV1[region].data =
      collection.ack.describeClustersV1[region].data.concat(data.clusters);
    if (
      data["page_info"] &&
      data["page_info"]["page_size"] &&
      data["page_info"]["page_number"] &&
      data["page_info"]["total_count"] &&
      data["page_info"]["page_size"] * data["page_info"]["page_number"] <
        data["page_info"]["total_count"]
    ) {
      pageNumber += 1;
      execute();
