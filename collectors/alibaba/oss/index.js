...
module.exports = function (
  ...
  async.eachLimit(
    ...
    function (bucket, bcb) {
      ...
      store[callKey](bucketName).then(
        (result) => {
          if (callKey == "getBucketPolicy")
            collection.oss[callKey][region][bucketName].data = result.policy
              ? result.policy
              : result;
          else
            collection.oss[callKey][region][bucketName].data =
