...
describe("output", function () {
  describe("junit", function () {
    ...
    it("should indicate one error there is one failing error", function () {
      ...
      handler.writeResult(
        { status: 3, message: "error message" },
        { title: "myTitle" },
        "key"
      );
      handler.close();
  ...
  describe("json", function () {
    ...
    it("should indicate one pass there is one passing result", function () {
      ...
      handler.close();
      expect(JSON.stringify(JSON.parse(buffer.cache))).to.equal(
        '[{"plugin":"key","title":"myTitle","description":"myDescription","resource":"N/A","region":"Global","status":"OK"}]'
      );
    });
  });
  ...
  describe("create", function () {
    it("should write to console without errors", function () {
      ...
      handler.writeResult(
        { status: 0, message: "Certificate has validation enabled" },
        {
          category: "ACM",
          title: "ACM Certificate Validation",
          description:
            "Testing the ACM certificate which must have DNS validation enabled",
        },
        "key"
      );
      handler.close();
