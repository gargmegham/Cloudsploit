...
function addResult(
  ...
  var processError = function (errorObj) {
    if (errorObj && errorObj.code && errorObj.code == 404) {
      pushResult(
        0,
        "Project is deleted or pending deletion.",
        region,
        resource,
        custom
      );
      ...
      pushResult(
        required ? 2 : 0,
        required
          ? "Service is not enabled, but it is recommended to run a secure workload in GCP."
      ...
      errorObj.errors.forEach(function (errError) {
        if (
          errError &&
          errError.message &&
          disabledKeywords.some((substring) =>
            errError.message.includes(substring)
          )
        ) {
          pushResult(
            required ? 2 : 0,
          ...
          pushResult(
            3,
            errError.message ? errError.message : message,
            region,
            resource,
            custom
          );
      ...
      pushResult(
        ...
        errorObj.message
          ? errorObj.message
          : "Unable to query the API: " + errorObj,
        region,
        resource,
        custom
      );
  ...
  if (err && err.code) {
    processError(err);
  } else if (err && err[region] && err[region].length) {
    err[region].forEach(function (errRegion) {
      if (errRegion && err[region][errRegion] && err[region][errRegion].code) {
        processError(err[region][errRegion]);
      } else {
        pushResult(
          3,
          err[region][errRegion].message
            ? err[region][errRegion].message
            : message,
          region,
          resource,
...
function findOpenPorts(
  ...
  let projects = shared.addSource(cache, source, ["projects", "get", "global"]);

  if (!projects || projects.err || !projects.data || !projects.data.length) {
    addResult(
      results,
      3,
      "Unable to query for projects: " + shared.addError(projects),
      "global",
      null,
      null,
      projects ? projects.err : null
    );
    return;
  }
  ...
  for (let sgroups of ngs) {
    let strings = [];
    let resource = createResourceName(
      "firewalls",
      sgroups.name,
      project,
      "global"
      ...
      for (let firewallRule of firewallRules) {
        for (let protocol in protocols) {
          ...
          for (let port of ports) {
            if (
              sgroups["direction"] &&
              sgroups["direction"] === "INGRESS" &&
              firewallRule["IPProtocol"] &&
              firewallRule["IPProtocol"] === protocol &&
              !sgroups["disabled"] &&
              (sourceAddressPrefix.includes("*") ||
                sourceAddressPrefix.includes("") ||
                sourceAddressPrefix.includes("0.0.0.0/0") ||
                sourceAddressPrefix.includes("<nw>/0") ||
                sourceAddressPrefix.includes("/0") ||
                sourceAddressPrefix.includes("internet"))
            ) {
              var sourcefilter =
  ...
  for (let sgroups of ngs) {
            ...
            if (firewallRule["ports"]) {
              firewallRule["ports"].forEach((portRange) => {
                if (portRange.includes("-")) {
                  ...
                  let endPort = portRange[1];
                  if (
                    parseInt(startPort) === 0 &&
                    parseInt(endPort) === 65535
                  ) {
    ...
    if (strings.length) {
      shared.addResult(
        results,
        2,
        "Firewall Rule:(" + sgroups.name + ") has " + strings.join(" and "),
        location,
        resource
      );
    } else {
      shared.addResult(
        ...
        "Firewall Rule:(" +
          sgroups.name +
          ") does not have all ports open to the public",
        location,
        resource
      );
    }
...
function hasBuckets(buckets) {
  if (buckets.length && Object.keys(buckets[0]).length > 1) {
    return true;
  } else {
    return false;
  }
...
function getProtectionLevel(cryptographickey, encryptionLevels) {
  if (
    cryptographickey &&
    cryptographickey.versionTemplate &&
    cryptographickey.versionTemplate.protectionLevel
  ) {
    if (cryptographickey.versionTemplate.protectionLevel == "SOFTWARE")
      return encryptionLevels.indexOf("cloudcmek");
    else if (cryptographickey.versionTemplate.protectionLevel == "HSM")
      return encryptionLevels.indexOf("cloudhsm");
    else if (cryptographickey.versionTemplate.protectionLevel == "EXTERNAL")
      return encryptionLevels.indexOf("external");
  }
...
function checkOrgPolicy(
  ...
  if (orgPolicies && orgPolicies.policies) {
    let policyToCheck = orgPolicies.policies.find(
      (policy) =>
        policy.constraint && policy.constraint.includes(constraintName)
    );
    if (policyToCheck) {
      if (constraintType == "listPolicy" && policyToCheck.listPolicy) {
        if (policyToCheck.listPolicy.allValues) {
          isEnabled =
            policyToCheck.listPolicy.allValues == "ALLOW" ? false : true;
        } else if (
          (policyToCheck.listPolicy.allowedValues &&
            policyToCheck.listPolicy.allowedValues.length) ||
          (policyToCheck.listPolicy.deniedValues &&
            policyToCheck.listPolicy.deniedValues.length)
        ) {
          isEnabled = true;
        }
      } else if (
        constraintType == "booleanPolicy" &&
        policyToCheck.booleanPolicy &&
        policyToCheck.booleanPolicy.enforced
      ) {
        isEnabled = true;
      }
    } else {
