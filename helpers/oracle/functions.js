  ...
  if (isSecurityRule) {
    for (var p in groups) {
      ...
      if (
        ...
        for (var portIndex in ports) {
          ...
          if (
            ...
            var message =
              portIndex.toUpperCase() + " port " + port + " open to 0.0.0.0/0";
      ...
      if (
        ...
        for (portIndex in ports) {
          ...
          if (
            ...
            var message =
              portIndex.toUpperCase() + " port " + port + " open to ::/0";
            if (messages.indexOf(message) === -1) messages.push(message);
          }
        }
      } else if (
      ...
      if (messages.length) {
        if (
          securityGroups &&
          securityGroups.data.find(
            (group) => group.id == groups[p].networkSecurityGroups
          )
        ) {
          var securityGroupName = securityGroups.data.find(
            (group) => group.id == groups[p].networkSecurityGroups
          ).displayName;
        }
        shared.addResult(
          ...
          "The Security Group: " +
            (securityGroupName
              ? securityGroupName
              : groups[p].networkSecurityGroups) +
            " has " +
            service +
            ": " +
            messages.join(" and "),
...
function checkEventRules(
  ...
  rulesFound = eventsToCheck.every((event) =>
    enabledRules.includes(event.value)
  );
  let activeRules = [];
  let inactiveRules = [];
  if (!rulesFound) {
    activeRules = eventsToCheck
      .filter((event) => enabledRules.includes(event.value))
      .map((rule) => rule.displayName);
    inactiveRules = eventsToCheck
      .filter((event) => !enabledRules.includes(event.value))
      .map((rule) => rule.displayName);
  }
  ...
  if (!rulesFound && !activeRules.length) {
    shared.addResult(
      results,
      2,
      `No event rules are configured for ${displayName} changes`,
      region,
      compartment
    );
    ...
    shared.addResult(
      results,
      2,
      `Event rules are missing for ${displayName} ${inactiveRules.join(
        ", "
      )} events`,
      region,
      compartment
    ...
    shared.addResult(
      results,
      0,
      `Event rules are configured for all ${displayName} changes`,
      region,
      compartment
    );
  }
...
function checkRegionSubscription(cache, source, results, region) {
  var regionSubscription = shared.addSource(cache, source, [
    "regionSubscription",
    "list",
    shared.objectFirstKey(cache["regionSubscription"]["list"]),
  ]);
...
function normalizePolicyStatement(policyStatement) {
  ...
  if (statementArr[statementArr.indexOf("in") + 1] === "tenancy") {
    ...
    statementObj["location"] =
      "compartment " + statementArr[statementArr.indexOf("in") + 2];
  }

  if (statementArr.indexOf("where") > -1)
    statementObj["condition"] = statementArr.slice(
      statementArr.indexOf("where") + 1,
      statementArr.length
    );
  return statementObj;
}
...
function getProtectionLevel(cryptographickey, encryptionLevels) {
  if (cryptographickey && cryptographickey.protectionMode) {
    if (cryptographickey.protectionMode.toUpperCase() == "SOFTWARE")
      return encryptionLevels.indexOf("cloudcmek");
    else if (cryptographickey.protectionMode.toUpperCase() == "HSM")
...
function testStatement(statementObj, resourceTypes, policyAdmins, verbs) {
  ...
  if (resourceTypes.indexOf("all-resources") === -1)
    resourceTypes.push("all-resources");

  let subjectArr = statementObj["subject"].split(" ");
  policyAdmins = policyAdmins.toLowerCase();
  subjectArr.forEach((subject) => {
    if (policyAdmins.indexOf(subject) > -1)
      subjectArr.splice(subjectArr.indexOf(subject), 1);
  });
