    ...
    if (callObj.reliesOnService) {
      ...
      function ociMany(callObj, OracleConfig) {
        ...
        async.eachLimit(
          ...
          10,
          function (service, serviceCb) {
            var records =
              callObj.collection[service][
                callObj.reliesOnCall[callObj.reliesOnService.indexOf(service)]
              ][OracleConfig.region].data;
              ...
              function (record, recordCb) {
                for (var filter in callObj.filterKey) {
                  if (callObj.filterConfig && callObj.filterConfig[filter]) {
                    parameters[callObj.filterKey[filter]] =
                      OracleConfig[callObj.filterValue[filter]];
                ...
                try {
                  OracleConfig.privateKey = sshpk.parsePrivateKey(
                    OracleConfig.keyValue,
                    "pem"
                  );
                  if (
                    !sshpk.PrivateKey.isPrivateKey(
                  ...
                  !OracleConfig.RESTversion
                    ? (OracleConfig.RESTversion = "/20160918")
                    : false;
                } catch (e) {
                  console.log("Could not read the Oracle Private Key.");
                }
                if (callObj.restVersion || callObj.restVersion == "") {
