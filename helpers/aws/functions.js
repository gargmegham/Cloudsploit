...
function waitForCredentialReport(iam, callback, CREDENTIAL_DOWNLOAD_STARTED) {
  if (!CREDENTIAL_DOWNLOAD_STARTED) {
    iam.generateCredentialReport(function (err, data) {
      if (
        (err && err.code && err.code == "ReportInProgress") ||
        (data && data.State)
      ) {
    ...
    var pingCredentialReport = function (pingCb) {
      iam.getCredentialReport(function (getErr, getData) {
        if (getErr || !getData || !getData.Content) {
          return pingCb(
            "Error waiting for credential report: " +
              (getErr ? getErr : "No data returned from AWS after 60 seconds.")
          );
        }
    ...
    async.retry(
      { times: 20, interval: 3000 },
      pingCredentialReport,
      function (reportErr, reportData) {
...
function addResult(results, status, message, region, resource, custom) {
  ...
  if (
    status == 3 &&
    region &&
    regions.optin.indexOf(region) > -1 &&
    message &&
    (message.indexOf(
...
function findOpenPorts(
  ...
  if (config.ec2_skip_unused_groups) {
    ...
    if (usedGroups && usedGroups.length && usedGroups[0] === "Error")
      return callback();
  }
  ...
  for (var g in groups) {
          ...
          for (var portIndex in ports[permission.IpProtocol]) {
            ...
            if (port.toString().indexOf("-") > -1) {
              ...
              if (permission.FromPort <= port && permission.ToPort >= port) {
                ...
                if (openV4Ports.indexOf(string) === -1)
                  openV4Ports.push(string);
              }
            }
          }
        ...
        if (rangeV6.CidrIpv6 === "::/0" && ports[permission.IpProtocol]) {
          for (var portIndexV6 in ports[permission.IpProtocol]) {
            ...
            if (portV6.toString().indexOf("-") > -1) {
              ...
              if (
                permission.FromPort <= portV6 &&
                permission.ToPort >= portV6
              ) {
                var stringV6 = `${permission.IpProtocol.toUpperCase()}:${portV6}`;
                if (openV6Ports.indexOf(stringV6) === -1)
                  openV6Ports.push(stringV6);
              }
            }
          }
        }
    ...
    if (openV4Ports.length || openV6Ports.length) {
      ...
      if (openV4Ports.length) {
        resultsString = `Security group: ${groups[g].GroupId} (${
          groups[g].GroupName
        }) has ${service}:${openV4Ports.join(" and ")} open to 0.0.0.0/0`;
      }

      if (openV6Ports.length) {
        if (resultsString.length) {
          resultsString = `${resultsString} and ${openV6Ports.join(
            " and "
          )} open to ::/0`;
        } else {
          resultsString = `Security group: ${groups[g].GroupId} (${
            groups[g].GroupName
          }) has ${service}:${openV6Ports.join(" and ")} open to ::/0`;
        }
      }

      if (
        config.ec2_skip_unused_groups &&
        groups[g].GroupId &&
        ...
        addResult(
          results,
          1,
          `Security Group: ${groups[g].GroupId} is not in use`,
          region,
          resource
        );
      } else {
      ...
      if (strings.length) {
        addResult(
          ...
          `Security group: ${groups[g].GroupId} (${
            groups[g].GroupName
          }) does not have ${strings.join(", ")} open to 0.0.0.0/0 or ::0`,
          region,
          resource
        );
      }
...
function normalizePolicyDocument(doc) {
  ...
  for (var s in doc.Statement) {
    ...
    if (
      !statement.Effect ||
      !statement.Effect.length ||
      !statement.Action ||
      !statement.Action.length
    ) {
      break;
    }
...
function userGlobalAccess(statement, restrictedPermissions) {
  if (
    ...
    statement.Action &&
    restrictedPermissions.some((permission) =>
      statement.Action.includes(permission)
    )
  ) {
    return true;
  }
...
function getDenyPermissionsMap(statements, excludeStatementId) {
  ...
  for (let statement of statements) {
    if (
      (statement.Sid && statement.Sid == excludeStatementId) ||
      (statement.Effect && statement.Effect.toUpperCase() !== "DENY")
    )
      continue;

    let principals = extractStatementPrincipals(statement);
    principals.forEach((principal) => {
      let permissionsObj = JSON.parse(
        JSON.stringify(getDenyActionResourceMap([statement]))
      );
      if (permissionsMap[principal])
        permissionsMap[principal] = {
          ...permissionsMap[principal],
          ...permissionsObj,
        };
...
function getDenyActionResourceMap(statements, excludeStatementId) {
  ...
  for (let statement of statements) {
    if (
      statement.Sid &&
      statement.Sid != excludeStatementId &&
      statement.Effect &&
      ...
      statement.Action.forEach((action) => {
        if (denyActionResourceMap[action])
          denyActionResourceMap[action].push.apply(
            denyActionResourceMap[action],
            statement.Resource
          );
  ...
  Object.keys(permissionsMap).forEach((key) => {
    if (matchKeys(key, principal)) {
      Object.keys(permissionsMap[key]).forEach((action) => {
        if (response[action])
          response[action].push.apply(
            response[action],
            permissionsMap[key][action]
          );
...
function isValidCondition(
  ...
  iamConditionOperators,
  fetchConditionPrincipals,
  accountId
) {
  if (statement.Condition && statement.Effect) {
    ...
    for (var operator of Object.keys(statement.Condition)) {
      ...
      for (var key of Object.keys(subCondition)) {
        ...
        if (
          !allowedConditionKeys.find(
            (conditionKey) => conditionKey.toLowerCase() == keyLower
          )
        )
          continue;
        ...
        if (
          iamConditionOperators.string[effect].includes(defaultOperator) ||
          iamConditionOperators.arn[effect].includes(defaultOperator)
        ) {
          if (
            keyLower === "kms:calleraccount" &&
            typeof value === "string" &&
            effect === "Allow" &&
            value === accountId
          ) {
          ...
          if ((effect === "Allow" && value) || (effect === "Deny" && !value))
            foundValid = true;
        } else if (
          iamConditionOperators.ipaddress[effect].includes(defaultOperator)
        ) {
    ...
    for (let key of Object.keys(denyActionResourceMap)) {
      if (matchKeys(key, action)) {
        ...
        for (let stmResource of statementActionResourceMap[action]) {
          if (
            denyActionResourceMap[key].find((deniedResource) =>
              matchKeys(deniedResource, stmResource)
            )
          )
            deniedResources.push(stmResource);
        }

        statementActionResourceMap[action] = statementActionResourceMap[
          action
        ].filter((resource) => !deniedResources.includes(resource));
      }
    }
...
function isEffectiveStatement(statement, denyPermissionsMap) {
  ...
  for (let principal of principals) {
    let denyActionResourceMap = filterDenyPermissionsByPrincipal(
      denyPermissionsMap,
      principal
    );
    if (isEffectivePolicyStatement(statement, denyActionResourceMap))
      return true;
  }
...
function matchKeys(first, second) {
  ...
  if (
    ...
    return matchKeys(first.slice(1), second.slice(1));

  if (first.length && first[0] == "*") {
    return (
      matchKeys(first.slice(1), second) || matchKeys(first, second.slice(1))
    );
  }
...
function getS3BucketLocation(cache, region, bucketName) {
  ...
  if (getBucketLocation && getBucketLocation.data) {
    if (
      ...
      regions.all.includes(getBucketLocation.data.LocationConstraint)
    )
...
function remediatePasswordPolicy(
  putCall,
  pluginName,
  remediation_file,
  passwordKey,
  ...
  if (
    (getAccountPasswordPolicy.err ||
      !getAccountPasswordPolicy.data ||
      !Object.keys(getAccountPasswordPolicy.data).length) &&
    settings.input &&
    settings.input[createPolicyInput]
  ) {
...
function remediateOpenPorts(
  ...
  if (!describeSecurityGroups.data || describeSecurityGroups.err)
    return cb(
      "Unable to query for security groups: " +
        helpers.addError(describeSecurityGroups)
    );

  if (!describeSecurityGroups.data.length)
    return cb("No security groups present");
  ...
  async.each(
    failingPermissions,
    function (failingPermission, fpCb) {
      ...
      if (failingPermission.IpRanges && failingPermission.IpRanges.length) {
        failingPermission.IpRanges.forEach((ipRange) => {
          if (ipRange.CidrIp && ipRange.CidrIp === "0.0.0.0/0") {
            ...
            ipRange.CidrIp &&
            settings.input &&
            settings.input[ipv4InputKey] &&
            ipRange.CidrIp === settings.input[ipv4InputKey]
          ) {
            localIpExists = true;
          } else {
      ...
      if (failingPermission.Ipv6Ranges && failingPermission.Ipv6Ranges.length) {
        failingPermission.Ipv6Ranges.forEach((ipv6Range) => {
          if (ipv6Range.CidrIpv6 && ipv6Range.CidrIpv6 === "::/0") {
            openIpv6Range = true;
            // Grabs the description when it identifies the failing rule
            ipv6Description = ipv6Range.Description
              ? ipv6Range.Description
              : null;
          } else if (
            ipv6Range.CidrIpv6 &&
            settings.input &&
            settings.input[ipv6InputKey] &&
            ipv6Range.CidrIpv6 === settings.input[ipv6InputKey]
          ) {
        ...
        IpPermissions: [
          {
            ...
            PrefixListIds: failingPermission.PrefixListIds.length
              ? failingPermission.PrefixListIds
              : null,
            UserIdGroupPairs: failingPermission.UserIdGroupPairs.length
              ? failingPermission.UserIdGroupPairs
              : null,
      ...
      remediation_file["pre_remediate"]["actions"][pluginName][resource].push(
        ...
        [
          function (rCb) {
            if (
              ...
              (openIpRange &&
                (!settings.input[ipv4InputKey] ||
                  !settings.input[ipv4InputKey].length) &&
                openIpv6Range &&
                (!settings.input[ipv6InputKey] ||
                  !settings.input[ipv6InputKey].length))
            )
              return rCb();
            ...
            if (openIpRange && !localIpExists && settings.input[ipv4InputKey]) {
              ...
              for (var newIpCidr of newIpCidrRange) {
                ...
                if (ipDescription && newIpRange)
                  newIpRange.Description = ipDescription;
                params.IpPermissions[0].IpRanges.push(newIpRange);
                finalIpRanges.push(newIpRange);
              }
            } else if (
              !openIpRange ||
              (openIpRange && localIpExists) ||
              !settings.input[ipv4InputKey] ||
              !settings.input[ipv4InputKey].length
            ) {
            ...
            if (
              ...
              for (var newIpv6Cidr of newIpv6CidrRange) {
                ...
                if (ipv6Description && newIpv6Range)
                  newIpv6Range.Description = ipv6Description;
                params.IpPermissions[0].Ipv6Ranges.push(newIpv6Range);
                finalIpv6Ranges.push(newIpv6Range);
              }
            } else if (
              !openIpv6Range ||
              (openIpv6Range && localIpV6Exists) ||
              !settings.input[ipv6InputKey] ||
              !settings.input[ipv6InputKey].length
            ) {
            ...
            remediatePlugin(config, putCall[0], params, function (err) {
              if (err) {
                ...
                if (openIpv6Range && !localIpV6Exists) {
                  remediation_file["remediate"]["actions"][pluginName][
                    resource
                  ]["steps"].push({
                    inboundRule: "::1/128",
                    action: "ADDED",
          ...
          function (rCb) {
            ...
            remediatePlugin(config, putCall[1], params, function (err) {
              ...
              params.IpPermissions[0].UserIdGroupPairs = failingPermission
...
function getDefaultKeyId(cache, region, defaultKeyDesc) {
  ...
  var defaultKey = listKeys.data.find((key) => {
    ...
    if (describeKey && describeKey.data && describeKey.data.KeyMetadata) {
      ...
      if (
        keyToAdd.KeyManager &&
        keyToAdd.KeyManager === "AWS" &&
        keyToAdd.Description &&
        keyToAdd.Description.indexOf(defaultKeyDesc) === 0 &&
        keyToAdd.Enabled &&
        keyToAdd.KeyState &&
        keyToAdd.KeyState === "Enabled"
      ) {
        return keyToAdd;
      }
    }
...
function getUsedSecurityGroups(cache, results, region) {
  ...
  if (
    !describeNetworkInterfaces ||
    describeNetworkInterfaces.err ||
    !describeNetworkInterfaces.data
  ) {
    helpers.addResult(
      results,
      3,
      "Unable to query for network interfaces: " +
        helpers.addError(describeNetworkInterfaces),
      region
    );
    return result["Error"];
  }
  ...
  if (!listFunctions || listFunctions.err || !listFunctions.data) {
    helpers.addResult(
      results,
      3,
      "Unable to list lambda functions: " + helpers.addError(listFunctions),
      region
    );
    return result["Error"];
  }
...
function getPrivateSubnets(subnetRTMap, subnets, routeTables) {
  ...
  subnets.forEach((subnet) => {
    if (
      subnet.SubnetId &&
      subnetRTMap[subnet.SubnetId] &&
      privateRouteTables.includes(subnetRTMap[subnet.SubnetId])
    )
      response.push(subnet.SubnetId);
  });
...
function getSubnetRTMap(subnets, routeTables) {
  ...
  routeTables.forEach((routeTable) => {
    if (
      ...
      routeTable.Associations.length
    ) {
      routeTable.Associations.forEach((association) => {
        if (association.SubnetId && !subnetRTMap[association.SubnetId])
          subnetRTMap[association.SubnetId] = routeTable.RouteTableId;
      });
    }
    if (
      routeTable.VpcId &&
      ...
      !vpcRTMap[routeTable.VpcId]
    )
      vpcRTMap[routeTable.VpcId] = routeTable.RouteTableId;
  });
  ...
  subnets.forEach((subnet) => {
    if (
      ...
      subnet.VpcId &&
      !subnetRTMap[subnet.SubnetId] &&
      vpcRTMap[subnet.VpcId]
    )
      subnetRTMap[subnet.SubnetId] = vpcRTMap[subnet.VpcId];
  });
...
function makeCustomCollectorCall(
  executor,
  callKey,
  params,
  retries,
  ...
  async.retry(
    {
      ...
      interval: function (retryCount) {
        ...
        let retry_temp = Math.min(
          apiRetryCap,
          apiRetryBackoff * (retryExponential + timestamp) ** retryCount
        );
        let retry_seconds = Math.round(
          retry_temp / retryLeveler + Math.random(0, retry_temp) * 5000
        );

        console.log(
          `Trying ${callKey} again in: ${retry_seconds / 1000} seconds`
        );
...
var debugApiCalls = function (call, service, debugMode, finished) {
  if (!debugMode) return;
  finished
    ? console.log(`[INFO] ${service}:${call} returned`)
    : console.log(`[INFO] ${service}:${call} invoked`);
};
...
var logError = function (
  ...
  if (err.code && !errorTypeSummaryLocal[err.code])
    errorTypeSummaryLocal[err.code] = {};
  ...
  if (err.code && !errorTypeSummaryLocal[err.code][service][call]) {
    ...
    errorTypeSummaryLocal[err.code][service][call].total =
      apiCallTypeErrorsLocal;
  ...
  if (debugMode) {
    ...
    if (err.code && !errorsLocal[service][call][err.code]) {
      ...
      if (err.requestId) {
        ...
        if (err.statusCode)
          errorsLocal[service][call][err.code][err.requestId].statusCode =
            err.statusCode;
        if (err.message)
          errorsLocal[service][call][err.code][err.requestId].message =
            err.message;
        if (err.time)
          errorsLocal[service][call][err.code][err.requestId].time = err.time;
        if (region)
          errorsLocal[service][call][err.code][err.requestId].region = region;
      }
    }
  }
};
...
var collectRateError = function (err, rateError) {
  ...
  if (
    ...
    err.statusCode &&
    rateError &&
    rateError.statusCode == err.statusCode
  ) {
    isError = true;
  } else if (
    err &&
...
var checkTags = function (cache, resourceName, resourceList, region, results) {
  ...
  if (!allResources || allResources.err || !allResources.data) {
    helpers.addResult(
      results,
      3,
      "Unable to query all resources from group tagging api:" +
        helpers.addError(allResources),
      region
    );
    return;
  }
  ...
  allResources.data.map((resource) => {
    if (
      resource.ResourceARN.startsWith(resourceARNPrefix) &&
  ...
  resourceList.map((arn) => {
    if (filteredResourceARN.includes(arn)) {
      ...
      helpers.addResult(
        results,
        2,
        `${resourceName} does not have any tags`,
        region,
        arn
      );
    }
