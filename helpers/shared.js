...
var daysBetween = function (date1, date2) {
  return Math.round(
    Math.abs((new Date(date1).getTime() - new Date(date2).getTime()) / ONE_DAY)
  );
};

var hoursBetween = function (date1, date2) {
  return Math.round(
    Math.abs((new Date(date1).getTime() - new Date(date2).getTime()) / ONE_HOUR)
  );
};
...
var minutesBetween = function (date1, date2) {
  return Math.round(
    Math.abs(
      (new Date(date1).getTime() - new Date(date2).getTime()) / (60 * 1000)
    )
  );
};

var processIntegration = function (
  serviceName,
  settings,
  collection,
  calls,
  ...
  localEvent.collection[serviceName.toLowerCase()] = collection[
    serviceName.toLowerCase()
  ]
    ? collection[serviceName.toLowerCase()]
    : {};
  localEvent.previousCollection[serviceName.toLowerCase()] =
    settings.previousCollection &&
    settings.previousCollection[serviceName.toLowerCase()]
      ? settings.previousCollection[serviceName.toLowerCase()]
      : {};
  ...
  processIntegrationAdditionalData(
    serviceName,
    settings,
    collection,
    calls,
...
var processIntegrationAdditionalData = function (
  serviceName,
  localSettings,
  localCollection,
  calls,
  ...
  if (
    callsMap &&
    callsMap.find((mycall) => mycall == "sendIntegration") &&
  ...
  if (
    ...
    if (foundData && Object.keys(foundData).length) {
  ...
  for (let postcall of postcalls) {
    ...
    if (
      postCallsMap.find((mycall) => mycall == "sendIntegration") &&
    ...
    if (
      ...
      foundData = integrationReliesOnData(
        postcall,
        localCollection,
        serviceName
      );
  ...
  if (callsMap.find((mycall) => mycall == "sendIntegration")) {
    if (
      ...
      for (let rc of calls[serviceName].sendIntegration.reliesOnCalls) {
        ...
        if (
          !(
            localCollection[svc.toLowerCase()] &&
...
var integrationReliesOnData = function (calls, localCollection, serviceName) {
  ...
  if (callsMap.find((mycall) => mycall == "sendIntegration")) {
    if (
      ...
      let serviceReliedOn = {};
      for (let serv of calls[serviceName].sendIntegration.integrationReliesOn
        .serviceName) {
        if (localCollection[serv.toLowerCase()]) {
          serviceReliedOn[serv.toLowerCase()] =
            localCollection[serv.toLowerCase()];
        }
      }
...
var callsCollected = function (serviceName, localCollection, calls, postcalls) {
  ...
  if (calls[serviceName]) {
    ...
    for (let checkCall of serviceCallMap) {
      if (
        callsMap.find(
          (mycall) => mycall != "sendIntegration" && mycall == checkCall
        )
      ) {
        if (reliesOnFound(calls, localCollection, serviceName) == false)
          return false;

        if (
          callsMap.find(
            (mycall) => mycall != "sendIntegration" && mycall == checkCall
          ) == serviceCallMap.find((mycall) => mycall == checkCall)
        ) {
  ...
  for (let postcall of postcalls) {
    ...
    for (let checkCall of serviceCallMap) {
      ...
      if (reliesOnFound(postcall, localCollection, serviceName) == false)
        return false;
...
module.exports = {
  ...
  addSource: function (cache, source, paths) {
    ...
    if (extra) {
      ...
      original =
        cache[service] && cache[service][call] && cache[service][call][region]
          ? cache[service][call][region]
          : null;
