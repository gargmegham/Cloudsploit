...
module.exports = {
  login: function (azureConfig, callback) {
    if (!azureConfig.ApplicationID)
      return callback("No ApplicationID provided");
    ...
    if (!azureConfig.SubscriptionID)
      return callback("No SubscriptionID provided");
    ...
    function performLogin(tokenAudience, cb) {
      msRestAzure.loginWithServicePrincipalSecret(
        ...
        function (err, credentials) {
          ...
          if (!credentials)
            return cb("Unable to log into Azure using provided credentials.");
          if (!credentials.environment)
            return cb("Unable to obtain environment from Azure application");
          if (
    ...
    performLogin(null, function (err, credentials) {
      ...
      performLogin(
        { tokenAudience: "graph" },
        function (graphErr, graphCredentials) {
          if (graphErr) return callback(graphErr);
          performLogin(
            { tokenAudience: "https://vault.azure.net" },
            function (vaultErr, vaultCredentials) {
        ...
        if (response && [200, 202].includes(response.statusCode) && body) {
          ...
          if (body) {
            try {
              ...
              return callback(
                `Error parsing error response from Azure API: ${e}`
              );
            }
            ...
            if (typeof body == "string") {
              ...
              try {
                ...
                return callback(
                  `Error parsing error response string from Azure API: ${e}`
                );
              }
            }
            ...
            if (
              ...
              if (body["odata.error"].requestId) {
                body[
                  "odata.error"
                ].message.value += ` RequestId: ${body["odata.error"].requestId}`;
              }
            ...
            console.log(
              `[ERROR] Unhandled error from Azure API: Body: ${JSON.stringify(
                body
              )}`
            );
          }
          ...
          console.log(
            `[ERROR] Unhandled error from Azure API: Error: ${error}`
          );
    ...
    locations[service].forEach(function (location) {
      ...
      if (err) {
        ...
        if (data.value && Array.isArray(data.value)) {
          collection[location.toLowerCase()].data = data.value.filter(function (
            dv
          ) {
            if (
              dv.location &&
              dv.location.toLowerCase().replace(/ /g, "") ==
                location.toLowerCase()
            ) {
              return true;
            } else if (
              location.toLowerCase() == "global" &&
              (!dv.location || obj.ignoreLocation)
            ) {
              return true;
            }
            return false;
