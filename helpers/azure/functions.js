...
function addResult(results, status, message, region, resource, custom) {
  ...
  if (status == 3 && message && typeof message == "string") {
    ...
    if (message.indexOf("The requested operation is not implemented") > -1) {
      overrideMsg =
        "Azure does not yet support this resource and method in this location.";
    } else if (message.indexOf("The specified account is disabled") > -1) {
      overrideMsg =
        "This resource is disabled for this functionality in this location.";
      ...
      overrideMsg =
        "Aqua does not have permission to list objects in this Key Vault. For help fixing this error, see: https://bit.ly/3cbpFyL";
      skipStatusChange = true;
    }
      ...
      if (sourceFound) {
        for (let protocol in protocols) {
          ...
          for (let port of ports) {
            if (
              securityRule.properties["access"] &&
              ...
              (securityRule.properties["protocol"].toUpperCase() === protocol ||
                securityRule.properties["protocol"].toUpperCase() === "*")
            ) {
              if (securityRule.properties["destinationPortRange"]) {
                if (
                  ...
                  let portRange =
                    securityRule.properties["destinationPortRange"].split("-");
                  let startPort = portRange[0];
                  let endPort = portRange[1];
                  if (
                    parseInt(startPort) <= port &&
                    parseInt(endPort) >= port
                  ) {
                    var string =
                      `Security Rule "` +
                      securityRule["name"] +
                  ...
                  parseInt(securityRule.properties["destinationPortRange"]) ===
                  port
                ) {
                  var string =
                    `Security Rule "` +
                    ...
                    (protocol === "*"
                      ? `All protocols`
                      : protocol.toUpperCase()) +
                    (ports === "*" ? ` and all ports` : ` port ` + ports) +
                    ` open to ` +
                    sourceFilter;
                  if (strings.indexOf(string) === -1) strings.push(string);
                } else if (
                  ...
                  var openAllstring =
                    ...
                    ` open to ` +
                    sourceFilter;
                  if (strings.indexOf(openAllstring) === -1)
                    strings.push(openAllstring);
                }
              } else if (securityRule.properties["destinationPortRanges"]) {
                if (
                  securityRule.properties["destinationPortRanges"].indexOf(
                    port.toString()
                  ) > -1
                ) {
                  var string =
                  ...
                  for (let portRange of securityRule.properties[
    ...
    if (strings.length) {
      ...
      if (strings.length) {
        addResult(
          results,
          0,
          `Security group:( ${sGroups.name}) does not have ${strings.join(
            ", "
          )} open *`,
          location,
          resource
...
function checkPolicyAssignment(
  policyAssignments,
  param,
  text,
  results,
  ...
  if (policyAssignments.err || !policyAssignments.data) {
    addResult(
      results,
      3,
      "Unable to query for Policy Assignments: " +
        shared.addError(policyAssignments),
      location
    );
    return;
  }
  ...
  if (!policyAssignment) {
    addResult(
      results,
      0,
      "There are no ASC Default Policy Assignments",
      location
    );
    return;
  }
  ...
  if (
    ...
    addResult(
      results,
      0,
      "There ASC Default Policy Assignment includes all plugins",
      location,
      policyAssignment.id
    );
    return;
  ...
  if (!policyAssignmentStatus.length) {
    addResult(
      results,
      0,
      text + " is no supported",
      location,
      policyAssignment.id
    );
  } else if (
    policyAssignmentStatus == "AuditIfNotExists" ||
    policyAssignmentStatus == "Audit"
  ) {
    addResult(results, 0, text + " is enabled", location, policyAssignment.id);
  } else {
    addResult(results, 2, text + " is disabled", location, policyAssignment.id);
  }
}
...
function checkLogAlerts(
  ...
  if (activityLogAlerts.err || !activityLogAlerts.data) {
    addResult(
      ...
      "Unable to query for Activity Alerts: " +
        shared.addError(activityLogAlerts),
      location
    );
    return;
  }
  ...
  for (let res in activityLogAlerts.data) {
    ...
    subscriptionId =
      "/subscriptions/" + activityLogAlertResource.id.split("/")[2];

    if (
      activityLogAlertResource.type &&
      activityLogAlertResource.type.toLowerCase() !==
        "Microsoft.Insights/ActivityLogAlerts".toLowerCase()
    )
      continue;

    const allConditions = activityLogAlertResource.condition;

    if (!allConditions || !allConditions.allOf || !allConditions.allOf.length)
      continue;

    var conditionOperation = allConditions.allOf.filter((d) => {
      return (
        (d.equals && d.equals.toLowerCase().indexOf(conditionResource) > -1) ||
        (parentConditionResource &&
          d.equals &&
          d.equals.toLowerCase().indexOf(parentConditionResource) > -1)
      );
    });

    if (conditionOperation && conditionOperation.length) {
      if (
        conditionResource.includes("microsoft.security") &&
        allConditions.allOf.every(
          (condition) =>
            condition.field &&
            condition.field == "category" &&
            condition.equals &&
            condition.equals.toLowerCase() == "security"
        )
      ) {
        alertCreateUpdateEnabled =
          !alertCreateUpdateEnabled && activityLogAlertResource.enabled
            ? true
            : alertCreateUpdateEnabled;
        break;
      }
      ...
      allConditions.allOf.forEach((condition) => {
        if (
          condition.field &&
          condition.field === "resourceType" &&
          condition.equals &&
          condition.equals.toLowerCase() === conditionResource
        ) {
          alertCreateDeleteEnabled =
            !alertCreateDeleteEnabled && activityLogAlertResource.enabled
              ? true
              : alertCreateDeleteEnabled;
        } else if (
          condition.equals &&
          condition.equals.toLowerCase().indexOf(conditionResource + "/write") >
            -1
        ) {
          alertCreateUpdateEnabled =
            !alertCreateUpdateEnabled && activityLogAlertResource.enabled
          ...
          condition.equals
            .toLowerCase()
            .indexOf(conditionResource + "/delete") > -1
        ) {
          alertDeleteEnabled =
            !alertDeleteEnabled && activityLogAlertResource.enabled
              ? true
              : alertDeleteEnabled;
        }
      });
  ...
  if (
    conditionResource == "microsoft.security/policies" &&
    alertCreateUpdateEnabled
  ) {
    addResult(
      results,
      0,
      `Log Alert for ${text} write/update is enabled`,
      location,
      subscriptionId
    );
  } else if (
    ...
    addResult(
      results,
      2,
      `Log Alert for ${text} write/update is not enabled`,
      location,
      subscriptionId
    );
  } else if (
    ...
    addResult(
      results,
      0,
      `Log Alert for ${text} write and delete is enabled`,
      location,
      subscriptionId
    );
  } else if (
    (alertCreateDeleteEnabled && alertDeleteEnabled) ||
    (alertDeleteEnabled &&
      !alertCreateUpdateEnabled &&
      !alertCreateDeleteEnabled)
  ) {
    addResult(
      results,
      0,
      `Log alert for ${text} delete is enabled`,
      location,
      subscriptionId
    );
    addResult(
      results,
      2,
      `Log alert for ${text} write is not enabled`,
      location,
      subscriptionId
    );
  } else if (
    (alertCreateDeleteEnabled && alertCreateUpdateEnabled) ||
    (alertCreateUpdateEnabled &&
      !alertCreateDeleteEnabled &&
      !alertDeleteEnabled)
  ) {
    addResult(
      results,
      0,
      `Log alert for ${text} write is enabled`,
      location,
      subscriptionId
    );
    addResult(
      results,
      2,
      `Log Alert for ${text} delete is not enabled`,
      location,
      subscriptionId
    );
  } else {
    addResult(
      results,
      2,
      `Log Alert for ${text} write and delete is not enabled`,
      location,
      subscriptionId
    );
  }
  ...
  if (!webConfigs || webConfigs.err || !webConfigs.data) {
    ...
    if (
      ...
      var version = parseFloat(
        webConfigs.data[0][checkProp].replace(/[^\d.-]/g, "")
      );
      ...
      if (Math.fround(version) >= Math.fround(allowedVersion)) {
        addResult(
          results,
          0,
          `The ${name} version (${webConfigs.data[0][checkProp]}) is the latest version`,
          location,
          webAppId,
          custom
        );
        ...
        addResult(
          results,
          2,
          `The ${name} version (${webConfigs.data[0][checkProp]}) is not the latest version`,
          location,
          webAppId,
          custom
        );
...
function checkServerConfigs(
  ...
  if (!servers.data.length) {
    addResult(
      results,
      0,
      "No existing " + serverType + " Servers found",
      location
    );
    return;
  }
    ...
    if (!configurations || configurations.err || !configurations.data) {
      addResult(
        ...
        "Unable to query for " +
          ...
          shared.addError(configurations),
        location,
        server.id
      );
    } else {
      var configuration = configurations.data.filter((config) => {
        return (
          config.name == configProperty && config.value.toLowerCase() == "on"
        );
      });
...
function remediateOpenPortsHelper(
  putCall,
  pluginName,
  protocols,
  ports,
          ...
          remediateOpenPorts(
            ...
            function (error, action) {
              if (error && (error.length || Object.keys(error).length)) {
                ...
                action &&
                (action.length || Object.keys(action).length)
              ) {
                actions.push(action);
              }
              cb();
        ...
        remediatePlugin(
          ...
          function (err) {
            ...
            if (errors && errors.length) {
              remediation_file["post_remediate"]["actions"][pluginName][
                "error"
              ] = errors.join(", ");
...
function remediateOpenPorts(
  ...
  if (!networkSecurityGroups.data || networkSecurityGroups.err)
    return cb(
      "Unable to query for network security groups: " +
        shared.addError(networkSecurityGroups)
    );

  if (!networkSecurityGroups.data.length)
    return cb("No network security groups present");
  ...
  if (!securityGroup)
    return cb("The target network security group was not found");

  if (
    !remediation_file["pre_remediate"]["actions"][pluginName][resource] ||
    !remediation_file["pre_remediate"]["actions"][pluginName][resource].length
  )
    remediation_file["pre_remediate"]["actions"][pluginName][resource] = [];
  if (
    !remediation_file["post_remediate"]["actions"][pluginName][resource] ||
    !remediation_file["post_remediate"]["actions"][pluginName][resource].length
  )
    remediation_file["post_remediate"]["actions"][pluginName][resource] = [];
  if (
  ...
  function findPortRange(portToCheck, protocolToCheck, rule) {
    if (portToCheck && portToCheck.toString().indexOf("-") > -1) {
      ...
      if (
        ...
        parseInt(endPort) >= port &&
        protocolToCheck &&
        (protocolToCheck === protocol || protocol === "*")
      ) {
  ...
  securityGroup.securityRules.forEach((rule) => {
    ...
    if (rule.properties["destinationPortRange"]) {
      ...
      rule.properties["destinationPortRanges"].length
    ) {
  ...
  if (passingSecurityRules.length) {
    for (var i in passingSecurityRules) {
      var foundPass = params.properties.securityRules.findIndex(
        (rule) => rule.name === passingSecurityRules[i].name
      );
      if (foundPass === -1) {
  ...
  async.each(
    ...
    function (failingPermission, fpCb) {
      ...
      if (
        failingPermission.properties &&
        (failingPermission.properties.sourceAddressPrefix ||
          failingPermission.properties.sourceAddressPrefixes)
      ) {
        ...
        remediation_file["pre_remediate"]["actions"][pluginName][resource].push(
          JSON.parse(JSON.stringify(failingPermission.properties))
        );

        if (failingPermission.properties.sourceAddressPrefixes) {
          sourceAddressArr = failingPermission.properties.sourceAddressPrefixes;
        }
        if (failingPermission.properties.sourceAddressPrefix)
          sourceAddressArr.push(
            failingPermission.properties.sourceAddressPrefix
          );
        ...
        function checkIp(inputKey, ipType, publicString) {
          if (sourceAddressArr.indexOf(publicString) > -1) {
            ...
            if (
              settings.input &&
              settings.input[inputKey] &&
              sourceAddressArr.indexOf(settings.input[inputKey]) === -1
            ) {
              sourceAddressArr.push(settings.input[inputKey]);
              sourceAddressArr.splice(
                sourceAddressArr.indexOf(publicString),
                1
              );
              ...
              (!settings.input || !settings.input[inputKey]) &&
              failingRulePortIndex[failingPermission.name] &&
              !spliced
            ) {
              spliced = true;
              failingPermission.properties["destinationPortRanges"].splice(
                [failingRulePortIndex[failingPermission.name]],
                1
              );
              if (
                failingPermission.properties["destinationPortRanges"].length ===
                0
              ) {
                sourceAddressArr = [];
              }
      ...
      if (!openIpv6Range && !openIpRange) {
        ...
        if (
          sourceAddressArr &&
          sourceAddressArr.length &&
          sourceAddressArr.length === 1
        ) {
          failingPermission.properties.sourceAddressPrefix =
            sourceAddressArr.join(", ");
          if (failingPermission.properties.sourceAddressPrefixes)
            delete failingPermission.properties.sourceAddressPrefixes;
        } else if (
          sourceAddressArr &&
          sourceAddressArr.length &&
          sourceAddressArr.length > 1
          ...
          if (failingPermission.properties.sourceAddressPrefix)
            delete failingPermission.properties.sourceAddressPrefix;
        }
        return fpCb();
      }
      ...
      if (openIpRange && openIpv6Range)
        return fpCb(
          "Invalid format, only IP or IPv6 can be remediated at one time"
        );
      ...
      var found = params.properties.securityRules.findIndex(
        (rule) => rule.name === failingPermission.name
      );
