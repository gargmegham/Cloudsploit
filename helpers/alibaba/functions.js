...
function findOpenPorts(cache, groups, ports, service, region, results) {
  ...
  for (var group of groups) {
    ...
    var accountId = helpers.addSource(cache, {}, [
      "sts",
      "GetCallerIdentity",
      defRegion,
      "data",
    ...
    if (
      ...
      for (var permission of describeSecurityGroupAttribute.data.Permissions
        .Permission) {
        ...
        if (permission.SourceCidrIp === "0.0.0.0/0" && ports[protocol]) {
          for (var port of ports[protocol]) {
            let fromPort = Number(permission.PortRange.split("/")[0])
              ? Number(permission.PortRange.split("/")[0])
              : Number(permission.PortRange);
            let toPort = Number(permission.PortRange.split("/")[1])
              ? Number(permission.PortRange.split("/")[1])
              : Number(permission.PortRange);
    ...
    if (openV4Ports.length) {
      var resultsString = "";
      if (openV4Ports.length) {
        resultsString = `Security group: ${
          group.SecurityGroupId
        } has ${service}:${openV4Ports.join(", ")} open to 0.0.0.0/0`;
      }
  ...
  if (kmsKey.Origin) {
    if (kmsKey.Origin === "Aliyun_KMS") {
      if (kmsKey.ProtectionLevel) {
        if (kmsKey.ProtectionLevel.toUpperCase() == "SOFTWARE")
          return encryptionLevels.indexOf("alibabacmk");
        if (kmsKey.ProtectionLevel.toUpperCase() == "HSM")
