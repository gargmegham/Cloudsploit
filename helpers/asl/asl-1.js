var parse = function (obj, path) {
  ...
  if (Array.isArray(path) && path.length && typeof obj === "object") {
    ...
    if (localPath.includes("[*]")) {
      ...
      if (
        obj[localPath] &&
        obj[localPath].length &&
        obj[localPath].length === 1
      ) {
        if (!path || !path.length) {
  ...
  if (condition.op) {
    if (condition.transform && condition.transform == "EACH" && condition) {
      ...
      if (condition.parsed == condition.value) {
        message.push(
          `${property}: ${condition.parsed} matched: ${condition.value}`
        );
        return 0;
      } else {
        message.push(
          `${property}: ${condition.parsed} did not match: ${condition.value}`
        );
        return 2;
      }
      ...
      if (condition.parsed > condition.value) {
        message.push(
          `${property}: count of ${condition.parsed} was greater than: ${condition.value}`
        );
      } else {
        conditionResult = 2;
        message.push(
          `${property}: count of ${condition.parsed} was not greater than: ${condition.value}`
        );
      }
    } else if (condition.op == "NE") {
      if (condition.parsed !== condition.value) {
        message.push(
          `${property}: ${condition.parsed} is not: ${condition.value}`
        );
      } else {
        conditionResult = 2;
      ...
      if (userRegex.test(condition.parsed)) {
        message.push(
          `${property}: ${condition.parsed} matches the regex: ${condition.value}`
        );
      } else {
        conditionResult = 2;
        message.push(
          `${property}: ${condition.parsed} does not match the regex: ${condition.value}`
        );
      }
      ...
      if (
        condition.parsed &&
        condition.parsed.length &&
        condition.parsed.includes(condition.value)
      ) {
        message.push(
          `${property}: ${condition.value} found in ${condition.parsed}`
        );
        return 0;
        ...
        message.push(
          `${condition.parsed} is not the right property type for this operation`
        );
        return 2;
      }
    }
...
var runValidation = function (
  ...
  if (condition.property) {
    ...
    if (
      (typeof condition.parsed !== "boolean" && !condition.parsed) ||
    ...
    if (property.includes("[*]")) {
      if (Array.isArray(condition.parsed)) {
        ...
        if (condition.op !== "CONTAINS") {
          condition.parsed.forEach((parsed) => {
            if (property.includes("[*]")) {
              ...
              let localConditionResult = validate(
                condition,
                conditionResult,
                inputResultsArr,
                message,
        ...
        if (nestedResultArr && nestedResultArr.length) {
          ...
          let resultObj;
          if (
            (condition.nested.toUpperCase() === "ONE" &&
              nestedResultArr.indexOf(0) > -1) ||
            (condition.nested.toUpperCase() === "ALL" &&
              nestedResultArr.indexOf(2) === 0)
          ) {
            resultObj = {
              status: 0,
              message: message.join(", "),
  ...
  localInput.conditions.forEach((condition) => {
    ...
    if (condition.property && condition.property.includes("[*]")) {
      if (conditionPropArr.length > 1 && conditionPropArr[1].includes("[*]")) {
        ...
        if (dataToValidate.length) {
          dataToValidate.forEach((newData) => {
            condition.validated = runValidation(
              newData,
              condition,
              inputResultsArr
            );
            ...
            if (
              typeof parsedResource !== "string" ||
              parsedResource === "not set"
            )
          ...
          if (condition.property.includes(".")) {
            condition.property =
              condition.property.split(".")[
                condition.property.split(".").length - 1
              ];
          }
          condition.validated = runValidation(
            newData,
      ...
      if (dataToValidate.length === 1) {
        ...
        if (typeof parsedResource !== "string" || parsedResource === "not set")
          parsedResource = resourceName;
      } else {
        newPath = dataToValidate[1];
        newData = dataToValidate[0];
...
var asl = function (source, input, resourceMap, callback) {
  ...
  if (!input.apis || !input.apis[0])
    return callback("No APIs provided for input");
  if (!input.conditions || !input.conditions.length)
    return callback("No conditions provided for input");

  let service = input.conditions[0].service;
  var subService = input.conditions[0].subservice
    ? input.conditions[0].subservice
    : null;
  let api = input.conditions[0].api;
  let resourcePath;
  if (resourceMap && resourceMap[service] && resourceMap[service][api]) {
    resourcePath = resourceMap[service][api];
  }

  if (!source[service])
    return callback(`Source data did not contain service: ${service}`);
  if (subService && !source[service][subService])
    return callback(
      `Source data did not contain service: ${service}:${subService}`
    );
  if (subService && !source[service][subService][api])
    return callback(`Source data did not contain API: ${api}`);
  if (!subService && !source[service][api])
    return callback(`Source data did not contain API: ${api}`);

  var results = [];
  let data = subService
    ? source[service][subService][api]
    : source[service][api];
  ...
  for (let region in data) {
    ...
    if (regionVal.err) {
      ...
      if (
        !Object.keys(regionVal).length ||
        (regionVal.data &&
          (!regionVal.data.length || !Object.keys(regionVal.data).length))
      ) {
        results.push({
          status: 0,
