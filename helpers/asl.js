  ...
  for (let i in conditions) {
    ...
    if (!condition.logical) {
      ...
      if (i == 0) {
        ...
        //err
        console.error(
          "No logical operator found and this is not the first element"
        );
      }
    } else {
      if (i == 0) {
    ...
    if (condition.parsed === "not set") {
      ...
      if (condition.op == "EQ") {
        if (condition.parsed == condition.value) {
          message.push(
            `${condition.property}: ${condition.parsed} matched: ${condition.value}`
          );
        } else {
          conditionResult = 2;
          message.push(
            `${condition.property}: ${condition.parsed} did not match: ${condition.value}`
          );
        }
      } else if (condition.op == "GT") {
        if (condition.parsed > condition.value) {
          message.push(
            `${condition.property}: count of ${condition.parsed} was greater than: ${condition.value}`
          );
        } else {
          conditionResult = 2;
          message.push(
            `${condition.property}: count of ${condition.parsed} was not greater than: ${condition.value}`
          );
        }
      } else if (condition.op == "NE") {
        if (condition.parsed !== condition.value) {
          message.push(
            `${condition.property}: ${condition.parsed} is not: ${condition.value}`
          );
        } else {
          conditionResult = 2;
          message.push(
            `${condition.property}: ${condition.parsed} is: ${condition.value}`
          );
        }
        ...
        if (userRegex.test(condition.parsed)) {
          message.push(
            `${condition.property}: ${condition.parsed} matches the regex: ${condition.value}`
          );
        } else {
          conditionResult = 2;
          message.push(
            `${condition.property}: ${condition.parsed} does not match the regex: ${condition.value}`
          );
        }
        ...
        if (condition.parsed.includes(condition.value)) {
          message.push(
            `${condition.property}: ${condition.value} found in ${condition.parsed}`
          );
        } else {
          conditionResult = 2;
    ...
    if (regionVal.err) {
      ...
      for (var resourceName in regionVal) {
        ...
        if (resourceObj.err) {
          ...
          var validated = evaluateConditions(
            resourceObj.data,
            input.conditions
          );
